...
class LargeSPIFlashROMRocketConfig
    extends Config(
      new chipyard.harness.WithSimSPIFlashModel(
        true
      ) ++ // add the SPI flash model in the harness (read-only)
        new chipyard.config.WithSPIFlash ++ // add the SPI flash controller
        new freechips.rocketchip.rocket.WithNHugeCores(1) ++
        new chipyard.config.AbstractConfig
    )

class SmallSPIFlashRocketConfig
    extends Config(
      new chipyard.harness.WithSimSPIFlashModel(
        false
      ) ++ // add the SPI flash model in the harness (writeable)
        new chipyard.config.WithSPIFlash(
          0x100000
        ) ++ // add the SPI flash controller (1 MiB)
        new freechips.rocketchip.rocket.WithNHugeCores(1) ++
        new chipyard.config.AbstractConfig
    )

class I2CRocketConfig
    extends Config(
...
class SimBlockDeviceRocketConfig
    extends Config(
...
class BlockDeviceModelRocketConfig
    extends Config(
...
class dmiCospikeCheckpointingRocketConfig
    extends Config(
      new chipyard.harness.WithSerialTLTiedOff ++ // don't attach anything to serial-tl
        new chipyard.config.WithDMIDTM ++ // have debug module expose a clocked DMI port
        new chipyard.harness.WithCospike ++ // attach spike-cosim
        new chipyard.config.WithTraceIO ++ // enable the traceio
        new chipyard.config.WithNPMPs(
          0
        ) ++ // remove PMPs (reduce non-core arch state)
        new freechips.rocketchip.rocket.WithDebugROB ++ // cospike needs wdata given by the unsynth. debug rom
        new freechips.rocketchip.rocket.WithCease(
          false
        ) ++ // remove xrocket ISA extension
        new freechips.rocketchip.rocket.WithNHugeCores(1) ++
        new chipyard.config.AbstractConfig
    )

class ManyPeripheralsRocketConfig
    extends Config(
      new chipyard.harness.WithI2CTiedOff ++ // Tie off the I2C port in the harness
        new chipyard.harness.WithSimSPIFlashModel(
          true
        ...
        new testchipip.iceblk.WithBlockDevice ++ // add block-device module to peripherybus
        new testchipip.soc.WithOffchipBusClient(
          MBUS
        ) ++ // OBUS provides backing memory to the MBUS
        new testchipip.soc.WithOffchipBus ++ // OBUS must exist for serial-tl to master off-chip memory
        new testchipip.serdes.WithSerialTLMem(isMainMemory =
          true
        ) ++ // set lbwif memory base to DRAM_BASE, use as main memory
        new chipyard.config.WithI2C ++ // Add I2C peripheral
