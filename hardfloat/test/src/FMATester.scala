...
trait FMATester extends AnyFlatSpec with Matchers with ParallelTestExecution {
  ...
  /** Run a FMA test. Before running, `softfloat_gen` should be accessible in
    * the $PATH environment.
    *
    * @param name
    *   is name of this test, which should corresponds to header's name in
    *   `includes` directory.
    * @param module
    *   function to generate DUT.
    * @param harness
    *   C++ harness name, which should corresponds to c++ hardness's name in
    *   `csrc` directory.
    * @param softfloatArgs
    *   arguments passed to `softfloat_gen` application. If has multiple command
    *   lines, multiple test will be executed.
    * @param dutArgs
    *   arguments passed to verilator dut executor, If set to [[None]], no
  ...
  def test(
    ...
    val testRunDir =
      os.pwd / "test_run_dir" / s"${this.getClass.getSimpleName}_$name" / s"${new SimpleDateFormat("yyyyMMddHHmmss")
    ...
    val verilatorCompile: Seq[String] = Seq(
      ...
      testRunDir.toString,
      "-CFLAGS",
      s"""-I${getClass.getResource("/includes/").getPath} -include ${getClass
          .getResource(s"/includes/$name.h")
          .getPath}""",
      "dut.sv",
      "--exe",
    ...
    /* Build C++ executor. */
    val verilatorBuild: Seq[String] =
      Seq("make", "-C", testRunDir.toString, "-j", "-f", s"dut.mk", "dut")
    ...
    def executeAndLog(
      ...
      val stdoutFile =
        testRunDir / s"${name}__${(softfloatArg ++ dutArg).mkString("_")}.txt"
      val vcdFile =
        testRunDir / s"${name}__${(softfloatArg ++ dutArg).mkString("_")}.vcd"
    ...
    (if (dutArgs.isDefined) {
       require(
         softfloatArgs.size == dutArgs.get.size,
         "size of softfloatArgs and dutArgs should be same."
       )
       (softfloatArgs zip dutArgs.get).par.map { case (s, d) =>
         executeAndLog(s, d)
       }
...
class DivSqrtRecFn_smallSpec extends FMATester {
  def test(f: Int, fn: String): Seq[String] = {
    def generator(options: Int) = fn match {
      case "div" =>
        () => new ValExec_DivSqrtRecFN_small_div(exp(f), sig(f), options)
      case "sqrt" =>
        () => new ValExec_DivSqrtRecFN_small_sqrt(exp(f), sig(f), options)
    }
    test(
