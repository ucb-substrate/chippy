...
case class CTCParams(
    translationParams: AddressTranslatorParams = OutwardAddressTranslatorParams(
    ...
    phyParams: Option[SerialPhyParams] = Some(
...
class CTCBridgeIO extends Bundle {
  val client_flit =
    new DecoupledFlitIO(CTC.INNER_WIDTH) // Driven by client/ctc2tl
  val manager_flit =
    new DecoupledFlitIO(CTC.INNER_WIDTH) // Driven by manager/tl2ctc
}
...
trait CanHavePeripheryCTC { this: BaseSubsystem =>
  ...
  val (ctc2tls, tl2ctcs, ctc_ios) = p(CTCKey).zipWithIndex.map {
    case (params, id) => {
      ...
      require(
        slave_bus.dtsFrequency.isDefined,
        s"Slave bus ${slave_bus.busName} must provide a frequency"
      )
      require(
        master_bus.dtsFrequency.isDefined,
        s"Master bus ${master_bus.busName} must provide a frequency"
      )
      require(
        slave_bus.dtsFrequency == master_bus.dtsFrequency,
      ...
      val tl2ctc = ctc_domain {
        LazyModule(new TileLinkToCTC(addrRegion = params.offchipRange)(p))
      }

      params.translationParams match {
        // Translate outgoing requests
        case OutwardAddressTranslatorParams(_, _, _) => {
          slave_bus.coupleTo(portName) {
            translator(tl2ctc.node) := TLBuffer() := _
          }
        ...
        case InwardAddressTranslatorParams(_, _, _) => {
          ...
          master_bus.coupleFrom(portName) {
            _ := TLBuffer() := translator(ctc2tl.node)
          }
        }
      }
      ...
      val ctc_clock_node = ctc_domain {
        ClockSinkNode(
          Seq(
            ClockSinkParameters(take = Some(ClockParameters(ctc_clock_freqMHz)))
          )
        )
      }
      ctc_clock_node := ClockGroup()(
        p,
        ValName(s"${ctc_name}_clock")
      ) := allClockGroupsNode

      val ctc_outer_io = ctc_domain {
        InModuleBody {
          val outer_io = params.phyParams
            .map(pP => IO(pP.genIO).suggestName(ctc_name))
            .getOrElse(IO(new CTCBridgeIO).suggestName(ctc_name))
          ...
          outer_io match {
            case io: CreditedSourceSyncPhitIO => {
              ...
              val outgoing_reset =
                ResetCatchAndSync(outgoing_clock, ctc2tl.module.reset.asBool)
              ...
              phy.io.outer_ser <> io.viewAsSupertype(
                new ValidPhitIO(params.phyParams.get.phitWidth)
              )
            }
            case io: DecoupledInternalSyncPhitIO => {
              ...
              phy.io.inner_ser(1).out <> ctc2tl.module.io.flit.out
              phy.io.outer_ser <> io.viewAsSupertype(
                new DecoupledPhitIO(params.phyParams.get.phitWidth)
              )
            }
...
class WithCTC(params: Seq[CTCParams] = Seq(CTCParams()))
    extends Config((site, here, up) => { case CTCKey =>
      params
    })

class WithCTCPhyParams(
    phyParams: SerialPhyParams = CreditedSourceSyncSerialPhyParams(
      phitWidth = CTC.OUTER_WIDTH,
      flitWidth = CTC.INNER_WIDTH,
      freqMHz = 100,
      flitBufferSz = 16
      ...
      up(CTCKey).map(p => p.copy(phyParams = Some(phyParams)))
    })

class WithCTCDecoupledPhy
    extends Config((site, here, up) => { case CTCKey =>
      up(CTCKey).map(p =>
        p.copy(phyParams =
          Some(
...
class WithNoCTCPhy
    extends Config((site, here, up) => { case CTCKey =>
      up(CTCKey).map(p => p.copy(phyParams = None))
    })
