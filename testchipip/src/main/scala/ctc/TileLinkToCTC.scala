...
class TileLinkToCTCModule(outer: TileLinkToCTC) extends LazyModuleImp(outer) {
  ...
  val (s_idle :: s_send_cmd :: s_send_addr :: s_recv_cmd :: s_recv_addr ::
    s_r_body :: s_r_ack :: s_w_req :: s_w_body :: s_w_ack :: s_reject :: Nil) =
    Enum(11)
  ...
  write_idx := Mux(
    cmd === CTCCommand.write_req && is_single_word,
    (tl_addr % beatBytes.U) >> log2Ceil(CTC.INNER_WIDTH / 8),
    idx
  )
  val shifted_body = Wire(UInt(dataBits.W))
  shifted_body := Mux(
    is_single_word,
    body(0) << (tl_addr % beatBytes.U * 8.U),
    body.asUInt
  )
  ...
  when(state === s_idle && mem.a.valid) {
    ...
    cmd := Mux(
      mem.a.bits.opcode === TLMessages.Get,
      CTCCommand.read_req,
      CTCCommand.write_req
    )
  ...
  when(state === s_recv_cmd && io.flit.in.valid) {
    assert(
      (io.flit.in.bits.flit(lenLen - 1, 0) === ctc_len - 1.U),
      "Mismatch in CTC length"
    )
    val ack_cmd = Mux(
      cmd === CTCCommand.read_req,
      CTCCommand.read_ack,
      CTCCommand.write_ack
    )
    assert(
      io.flit.in.bits.flit(cmdLen + lenLen - 1, lenLen) === ack_cmd,
      "Mismatch in CTC command"
    )
    state := s_recv_addr
  }
