    ...
    TLSlavePortParameters.v1(
      Seq(
        TLSlaveParameters.v1(
          ...
          regionType =
            if (cacheable) RegionType.UNCACHED
...
class TLSwitch(implicit p: Parameters) extends LazyModule {
  ...
  def unifyManagers(
      mgrs: Seq[Seq[TLSlaveParameters]]
  ): Seq[TLSlaveParameters] = {
    mgrs.flatten
      .groupBy(_.sortedAddress.head)
      .map { case (_, m) =>
        require(
          m.forall(_.address == m.head.address),
          "Require homogeneous address ranges"
        )
        require(
          m.forall(_.regionType == m.head.regionType),
          "Require homogeneous regionType"
        )
        require(
          m.forall(_.supports == m.head.supports),
          "Require homogeneous supported operations"
        )
        m.head
      }
      .toSeq
  ...
  val node = new TLNexusNode(
    ...
    managerFn = { m =>
      ...
      require(
        m.flatMap(_.responseFields).size == 0,
        "ResponseFields not supported in TLSwitch"
      )
      require(
        m.flatMap(_.requestKeys).size == 0,
        "RequestKeys not supported in TLSwitch"
      )
      require(
        m.forall(_.beatBytes == m.head.beatBytes),
        "Homogeneous beatBytes required"
      )
      TLSlavePortParameters.v1(
