...
class ClockMutexMux(val n: Int, depth: Int, genClockGate: () => ClockGate)
    extends RawModule {
  ...
  val andClocks = io.clocksIn.map(x =>
    ClockSignalNor2(ClockInverter(x), io.resetAsync.asBool)
  )

  val syncs = andClocks.map { c =>
    withClockAndReset(c, io.resetAsync) {
      Module(new AsyncResetSynchronizerShiftReg(1, sync = depth, init = 0))
    }
  }
  ...
  syncs.zipWithIndex.foreach { case (s, i) =>
    s.io.d := (io.sel === i.U) && !(syncs.zipWithIndex
      .filter(_._2 != i)
      .map(_._1.io.q.asBool)
      .reduce(_ || _))
  }
  ...
  def clockOrTree(in: Seq[Clock]): Seq[Clock] = {
    if (in.length == 1) {
      ...
      return clockOrTree(
        Seq
          .fill(in.length / 2)(Module(new ClockOr2))
          .zipWithIndex
          .map({ case (or, i) =>
...
object ClockMutexMux {
  ...
  def apply(clocks: Seq[Clock], genClockGate: () => ClockGate): ClockMutexMux =
    apply(clocks, genClockGate, defaultDepth)
  def apply(clocks: Seq[Clock])(implicit p: Parameters): ClockMutexMux =
    apply(clocks, defaultDepth)(p)
  def apply(clocks: Seq[Clock], depth: Int)(implicit
      p: Parameters
  ): ClockMutexMux = apply(clocks, p(ClockGateImpl), depth)
...
class ClockDivideOrPass(
  ...
  val io = IO(new Bundle {
    val clockIn = Input(Clock())
    val divisor =
      Input(UInt(width.W)) // this may not be synchronous with clockIn
    val resetAsync = Input(AsyncReset())
    val clockOut = Output(Clock())

  })
  val divider = withClockAndReset(io.clockIn, io.resetAsync) {
    Module(new ClockDivider(width, initDiv = 1))
  }
  divider.io.divisor := withClockAndReset(io.clockIn, io.resetAsync) {
    SynchronizerShiftReg(Mux(io.divisor === 0.U, 1.U, io.divisor))
  }
...
object withGatedClock {
  def apply[T](clock: Clock, enable: Bool, genClockGate: () => ClockGate)(
      block: => T
  ): T = {
    val g = Module(genClockGate())
    g.io.en := enable
  ...
  def apply[T](clock: Clock, enable: Bool)(
      block: => T
...
object withGatedClockAndReset {
  ...
  def apply[T](clock: Clock, enable: Bool, reset: Reset)(
      block: => T
  )(implicit p: Parameters): T = {
