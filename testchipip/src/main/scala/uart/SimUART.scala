...
/** Module to connect with a DUT UART and converts the UART signal to/from
  * DATA_WIDTH packets.
  *
  * @param uartno
  *   the uart number
  * @param div
  *   the divisor (equal to the clock frequency divided by the baud rate)
  */
class UARTAdapter(uartno: Int, div: Int, forcePty: Boolean) extends Module {
  val io = IO(new Bundle {
    val uart = Flipped(
      new UARTPortIO(UARTParams(address = 0))
    ) // We do not support the four wire variant
  })
  ...
  val txq = Module(
    new Queue(UInt(uartParams.dataBits.W), uartParams.nTxEntries)
  )
  val rxm = Module(new UARTTx(uartParams))
  val rxq = Module(
    new Queue(UInt(uartParams.dataBits.W), uartParams.nRxEntries)
  )
...
object UARTAdapter {
  ...
  def connect(
    ...
    UARTAdapter.connect(
      uart,
      baudrate,
      p(PeripheryBusKey).dtsFrequency.get,
      forcePty
    )
  }
  def connect(
...
/** Module to connect to a *.v blackbox that uses DPI calls to interact with the
  * DUT UART.
  *
  * @param uartno
  *   the uart number
  */
class SimUART(uartno: Int, forcePty: Boolean)
    extends BlackBox(
      Map(
