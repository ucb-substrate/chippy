...
class ClockMutexMuxTest(timeout: Int = 200000) extends UnitTest(timeout) {
  ...
  withClockAndReset(clocks(0), syncReset) {
    ...
    monitors.zipWithIndex.foreach { case (m, i) =>
      m.io.enable := (state === sOn) && (sel.value === i.U)
    }
...
class ClockDividerTest(timeout: Int = 200000) extends UnitTest(timeout) {
  ...
  val monitors = divs
    .map(x => if (x == 0) 0 else (x + 1) * base)
    .map(x => Module(new PeriodMonitor(x, Some(x))))

  val syncReset = ResetCatchAndSync(myClock, this.reset.asBool)

  val divider = withClock(myClock) {
    Module(new ClockDivider(log2Ceil(divs.max)))
  }
  ...
  withClockAndReset(myClock, syncReset) {
    ...
    monitors.zipWithIndex.foreach { case (m, i) =>
      m.io.enable := (state === sOn) && (div.value === i.U)
    }
...
class PeriodMonitor(minperiodps: Int, maxperiodps: Option[BigInt] = None)
    extends BlackBox(
      Map(
...
class ClockGenerator(periodps: Int)
    extends BlackBox(Map("periodps" -> IntParam(periodps))) {
...
object ClockUtilTests {
  ...
  def apply(): Seq[UnitTest] =
    Seq(Module(new ClockMutexMuxTest), Module(new ClockDividerTest))
