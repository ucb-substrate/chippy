...
object TSIHarness {
  def connectRAM(
      params: SerialTLParams,
      serdesser: TLSerdesser,
      port: DecoupledPhitIO,
      reset: Reset
  ): SerialRAM = {
...
object SerialTLROM {
  def apply(romParams: ManagerROMParams, beatBytes: Int)(implicit
      p: Parameters
  ): TLROM = {
    lazy val romContents = {
      val romData = romParams.contentFileName
        .map(n => Files.readAllBytes(Paths.get(n)))
        .getOrElse(
          Array(
            0x1b, 0x05, 0x10, 0x00, // 0010051b     addiw    a0,zero,1
            0x13, 0x15, 0xf5,
            0x01, // 01f51513     slli     a0,a0,0x1f (li a0, 0x8000_0000)
    ...
    val rom = LazyModule(
      new TLROM(romParams.address, romParams.size, romContents, true, beatBytes)
    )
    rom
  }
}

class SerialRAM(tl_serdesser: TLSerdesser, params: SerialTLParams)(implicit
    p: Parameters
) extends LazyModule {
  val managerParams = tl_serdesser.module.client_edge.map(
    _.slave
  ) // the managerParams are the chip-side clientParams
  val clientParams = tl_serdesser.module.manager_edge.map(
    _.master
  ) // The clientParams are the chip-side managerParams
  val serdesser = LazyModule(
    new TLSerdesser(
  ...
  serdesser.clientNode.foreach { clientNode =>
    ...
    val srams = memParams.map { memParams =>
      AddressSet.misaligned(memParams.address, memParams.size).map { aset =>
        LazyModule(new TLRAM(aset, beatBytes = beatBytes) {
          override lazy val desiredName = "SerialRAM_RAM"
        })
      }
    }.flatten
    srams.foreach { s =>
    ...
    val cohrams = cohParams.map { cohParams =>
      AddressSet.misaligned(cohParams.address, cohParams.size).map { aset =>
        LazyModule(new TLRAM(aset, beatBytes = beatBytes) {
          override lazy val desiredName = "SerialRAM_COH"
        })
      }
    }.flatten
    cohrams.foreach { s =>
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    require(
      serdesser.module.mergedParams == tl_serdesser.module.mergedParams,
