...
class TSIToTileLink(sourceIds: Int = 1)(implicit p: Parameters)
    extends LazyModule {
  val node = TLClientNode(
    Seq(
      TLMasterPortParameters.v1(
        Seq(
          TLClientParameters(name = "serial", sourceId = IdRange(0, sourceIds))
        )
      )
    )
  )
...
class TSIToTileLinkModule(outer: TSIToTileLink) extends LazyModuleImp(outer) {
  ...
  val put_acquire = edge
    .Put(
      0.U,
      beatAddr << byteAddrBits.U,
      log2Ceil(beatBytes).U,
      body.asUInt,
      wmask
    )
    ._2
  ...
  def shiftBits(bits: UInt, idx: UInt): UInt =
    if (nChunksPerWord > 1)
      bits << Cat(
        idx(log2Ceil(nChunksPerWord) - 1, 0),
        0.U(log2Up(TSI.WIDTH).W)
      )
    else bits

  def addrToIdx(addr: UInt): UInt =
    if (nChunksPerBeat > 1) addr(byteAddrBits - 1, log2Up(TSI.WIDTH / 8))
    else 0.U
