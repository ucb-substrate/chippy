...
class TLSourceAdjuster(maxClients: Int, maxInFlightPerClient: Int)(implicit
    p: Parameters
) extends LazyModule {
  ...
  val node = TLAdapterNode(
    clientFn = { cp =>
      {
        require(
          cp.clients.size <= maxClients,
          s"clients ${cp.clients.map(_.name)} > $maxClients"
        )
        val clients = cp.clients.padTo(
          maxClients,
          TLMasterParameters.v1(name = "TLSourceAdjusterNullClient")
        )
        TLMasterPortParameters.v1(
          clients = clients.zipWithIndex.map {
            case (c, i) => {
              require(c.sourceId.size <= maxInFlightPerClient)
              c.v1copy(sourceId =
                IdRange(
                  i * maxInFlightPerClient,
                  (i + 1) * maxInFlightPerClient
                )
              )
            }
          },
  ...
  class Impl extends LazyModuleImp(this) {
    node.in.zip(node.out).foreach { case ((in, edgeIn), (out, edgeOut)) =>
      val idOffsets = edgeIn.client.masters.zipWithIndex.map {
        case (m, i) => {
...
object TLSourceAdjuster {
  def apply(maxClients: Int, maxInFlightPerClient: Int)(implicit
      p: Parameters
  ): TLNode = {
    val adjuster = LazyModule(
      new TLSourceAdjuster(maxClients, maxInFlightPerClient)
    )
    adjuster.node
  }
}
