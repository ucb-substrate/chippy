...
class GenericSerializer[T <: Data](t: T, flitWidth: Int) extends Module {
  override def desiredName =
    s"GenericSerializer_${t.typeName}w${t.getWidth}_f${flitWidth}"
...
class GenericDeserializer[T <: Data](t: T, flitWidth: Int) extends Module {
  override def desiredName =
    s"GenericDeserializer_${t.typeName}w${t.getWidth}_f${flitWidth}"
  val io = IO(new Bundle {
...
class FlitToPhit(flitWidth: Int, phitWidth: Int) extends Module {
  ...
  io.in.ready := io.out.ready && beat === 0.U
  io.out.valid := io.in.valid || beat =/= 0.U
  io.out.bits.phit := (if (dataBeats == 1) io.in.bits.flit
                       else
                         Mux(beat === 0.U, io.in.bits.flit, data(beat - 1.U)))
...
class PhitToFlit(flitWidth: Int, phitWidth: Int) extends Module {
  ...
  io.in.ready := io.out.ready || beat =/= (dataBeats - 1).U
  io.out.valid := io.in.valid && beat === (dataBeats - 1).U
  io.out.bits.flit := (if (dataBeats == 1) io.in.bits.phit
                       else Cat(io.in.bits.phit, data.asUInt))
...
class PhitArbiter(phitWidth: Int, flitWidth: Int, channels: Int)
    extends Module {
  override def desiredName =
  ...
  if (channels == 1) {
    ...
    val header_idx =
      if (headerBeats == 1) 0.U else beat(log2Ceil(headerBeats) - 1, 0)

    io.out.valid := VecInit(io.in.map(_.valid))(chosen)
    io.out.bits.phit := Mux(
      beat < headerBeats.U,
    ...
    for (i <- 0 until channels) {
      io.in(i)
        .ready := io.out.ready && beat >= headerBeats.U && chosen_reg === i.U
    }
...
class PhitDemux(phitWidth: Int, flitWidth: Int, channels: Int) extends Module {
  override def desiredName =
    s"PhitDemux_p${phitWidth}_f${flitWidth}_n${channels}"
  val io = IO(new Bundle {
    val in = Flipped(Decoupled(new Phit(phitWidth)))
...
class DecoupledFlitToCreditedFlit(flitWidth: Int, bufferSz: Int)
    extends Module {
  override def desiredName =
    s"DecoupledFlitToCreditedFlit_f${flitWidth}_b${bufferSz}"
  ...
  when(credit_incr || credit_decr) {
    credits := credits + credit_incr - Mux(
      io.credit.valid,
      io.credit.bits.flit +& 1.U,
      0.U
    )
  }
...
class CreditedFlitToDecoupledFlit(flitWidth: Int, bufferSz: Int)
    extends Module {
  override def desiredName =
    s"CreditedFlitToDecoupledFlit_f${flitWidth}_b${bufferSz}"
  val io = IO(new Bundle {
    val in = Flipped(Decoupled(new Flit(flitWidth)))
    val out = Decoupled(new Flit(flitWidth))
