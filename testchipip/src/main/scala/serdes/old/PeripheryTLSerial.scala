...
trait CanHavePeripheryTLSerial { this: BaseSubsystem =>
  ...
  val (old_serdessers, old_serial_tls, old_serial_tl_debugs) =
    p(SerialTLKey).zipWithIndex.map { case (params, sid) =>
      ...
      serdesser.managerNode.foreach { managerNode =>
        manager_bus.get.coupleTo(s"port_named_${name}_out") {
          managerNode := TLSourceShrinker(
            1 << params.manager.get.idBits
          ) := TLWidthWidget(manager_bus.get.beatBytes) := _
        }
      }
        ...
        InModuleBody {
          ...
          inner_io match {
            case io: InternalSyncSerialIO => {
              ...
              val out_async = Module(
                new AsyncQueue(
                  UInt(params.phyParams.width.W),
                  AsyncQueueParams(depth = params.phyParams.asyncQueueSz)
                )
              )
            ...
            case io: SourceSyncSerialIO => {
              ...
              val out_async =
                Module(new AsyncQueue(UInt(params.phyParams.width.W)))
              out_async.io.enq_clock := serdesser.module.clock
              out_async.io.enq_reset := serdesser.module.reset
              ...
              when(io.in.valid) {
                assert(in_async.io.enq.ready, "Credited flow control broke")
              }

              // Sending credit out
              in_credits.io.enq.valid := in_async.io.deq.fire
              in_credits.io.enq.bits := DontCare
              when(in_async.io.deq.fire) {
                assert(in_credits.io.enq.ready, "Credited flow control broke")
              }
