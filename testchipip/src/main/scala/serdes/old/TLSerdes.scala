...
class TLMergedBundle(
  ...
  val param = UInt(
    Seq(
      TLAtomics.width,
      TLHints.width,
      TLPermissions.aWidth,
      TLPermissions.bdWidth,
      TLPermissions.cWidth
    ).max.W
  )
...
object TLMergedBundle {
  ...
  def apply(a: TLBundleA, hasCorruptDenied: Boolean): TLMergedBundle =
    apply(a, a.params, hasCorruptDenied)
  ...
  def apply(b: TLBundleB, hasCorruptDenied: Boolean): TLMergedBundle =
    apply(b, b.params, hasCorruptDenied)
  ...
  def apply(c: TLBundleC, hasCorruptDenied: Boolean): TLMergedBundle =
    apply(c, c.params, hasCorruptDenied)
  ...
  def apply(d: TLBundleD, hasCorruptDenied: Boolean): TLMergedBundle =
    apply(d, d.params, hasCorruptDenied)
  ...
  def apply(e: TLBundleE, hasCorruptDenied: Boolean): TLMergedBundle =
    apply(e, e.params, hasCorruptDenied)
  ...
  def apply(
      ...
      hasCorruptDenied: Boolean,
      last: DecoupledIO[TLChannel] => Bool
  ...
  def toA(chan: TLMergedBundle, hasCorruptDenied: Boolean): TLBundleA =
    toA(chan, chan.params, hasCorruptDenied)
  ...
  def toB(chan: TLMergedBundle, hasCorruptDenied: Boolean): TLBundleB =
    toB(chan, chan.params, hasCorruptDenied)
  ...
  def toC(chan: TLMergedBundle, hasCorruptDenied: Boolean): TLBundleC =
    toC(chan, chan.params, hasCorruptDenied)
  ...
  def toD(chan: TLMergedBundle, hasCorruptDenied: Boolean): TLBundleD =
    toD(chan, chan.params, hasCorruptDenied)
  ...
  def toE(chan: TLMergedBundle, hasCorruptDenied: Boolean): TLBundleE =
    toE(chan, chan.params, hasCorruptDenied)
...
class TLSerdes(
  ...
  val node = TLManagerNode(
    params.map(manager =>
      TLSlavePortParameters.v1(managers = Seq(manager), beatBytes = beatBytes)
    )
  )
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    node.in.zip(io.ser).zipWithIndex.foreach { case (((tl, edge), ser), i) =>
      ...
      val outChannels = Seq(tl.e, tl.c, tl.a)
        .map(TLMergedBundle(_, hasCorruptDenied, c => edge.last(c)))
      val outArb = Module(
        new HellaPeekingArbiter(
          mergeType,
          outChannels.size,
          (b: TLMergedBundle) => b.last
        )
      )
      ...
      inDes.io.out.ready := MuxLookup(inDes.io.out.bits.chanId, false.B)(
        Seq(
...
class TLDesser(
  ...
  val node = TLClientNode(
    params.map(client => TLMasterPortParameters.v1(Seq(client)))
  )
...
class TLSerdesser(
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    val client_tl = clientNode
      .map(_.out(0)._1)
      .getOrElse(0.U.asTypeOf(new TLBundle(bundleParams)))
    val client_edge = clientNode.map(_.out(0)._2)
    val manager_tl = managerNode
      .map(_.in(0)._1)
      .getOrElse(0.U.asTypeOf(new TLBundle(bundleParams)))
    ...
    require(
      ...
      "TLSerdesser does not support TileLink with echo fields"
    )
    require(
      mergedParams.requestFields.isEmpty,
      "TLSerdesser does not support TileLink with request fields"
    )
    require(
      mergedParams.responseFields.isEmpty,
      "TLSerdesser does not support TileLink with response fields"
    )
    require(
      mergedParams == bundleParams,
      s"TLSerdesser is misconfigured, the combined inwards/outwards parameters cannot be serialized using the provided bundle params\n$mergedParams > $bundleParams"
    )

    val mergeType = new TLMergedBundle(mergedParams, hasCorruptDenied)

    val outChannels =
      Seq(manager_tl.e, client_tl.d, manager_tl.c, client_tl.b, manager_tl.a)
    val outArb = Module(
      new HellaPeekingArbiter(
        mergeType,
        outChannels.size,
    ...
    val outSer = Module(new GenericSerializer(mergeType, w))
    outArb.io.in <> outChannels.map(o =>
      TLMergedBundle(
        o,
        mergedParams,
        hasCorruptDenied,
        c => client_edge.map(_.last(c)).getOrElse(false.B)
      )
    )
    outSer.io.in <> outArb.io.out
