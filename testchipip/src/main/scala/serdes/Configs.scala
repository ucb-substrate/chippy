...
class WithSerialTL(params: Seq[SerialTLParams] = Seq(SerialTLParams()))
    extends Config((site, here, up) => { case SerialTLKey =>
      params
    })

// Modify the width of all attached serial-TL ports
class WithSerialTLWidth(phitWidth: Int)
    extends Config((site, here, up) => { case SerialTLKey =>
      up(SerialTLKey).map(k =>
        k.copy(phyParams = k.phyParams match {
          case p: DecoupledInternalSyncSerialPhyParams =>
            p.copy(phitWidth = phitWidth)
          case p: DecoupledExternalSyncSerialPhyParams =>
            p.copy(phitWidth = phitWidth)
          case p: CreditedSourceSyncSerialPhyParams =>
            p.copy(phitWidth = phitWidth)
        })
      )
    })

class WithSerialTLPHYParams(
    phyParams: SerialPhyParams = DecoupledExternalSyncSerialPhyParams()
) extends Config((site, here, up) => { case SerialTLKey =>
      up(SerialTLKey).map(k => k.copy(phyParams = phyParams))
    })

// Set the bus which the serial-TL client will master on this system for all attached serial-TL ports
class WithSerialTLMasterLocation(masterWhere: TLBusWrapperLocation)
    extends Config((site, here, up) => { case SerialTLKey =>
      up(SerialTLKey).map(s =>
        s.copy(
...
class WithSerialTLSlaveLocation(slaveWhere: TLBusWrapperLocation)
    extends Config((site, here, up) => { case SerialTLKey =>
      up(SerialTLKey).map(s =>
        s.copy(
...
class WithSerialTLClient
    extends Config((site, here, up) => { case SerialTLKey =>
      up(SerialTLKey).map(s => s.copy(client = Some(SerialTLClientParams())))
    })

// Remove the client interface from all attached serial-TL ports on this system
class WithNoSerialTLClient
    extends Config((site, here, up) => { case SerialTLKey =>
      up(SerialTLKey).map(s => s.copy(client = None))
    })
...
class WithSerialTLMem(
    ...
    isMainMemory: Boolean = true
      ...
      case SerialTLKey => {
        ...
        up(SerialTLKey, site).map { k =>
          k.copy(
            manager = Some(
              k.manager
                .getOrElse(SerialTLManagerParams())
                .copy(
...
class WithSerialTLBundleParams(params: TLBundleParameters)
    extends Config((site, here, up) => { case SerialTLKey =>
      up(SerialTLKey).map(_.copy(bundleParams = params))
    })

// Enable the TL-C protoocl for all attached serial-TL ports on this system
class WithSerialTLBCE
    extends Config((site, here, up) => { case SerialTLKey =>
      up(SerialTLKey).map(s =>
        s.copy(
...
class WithSerialTLROM(base: BigInt = 0x20000, size: Int = 0x10000)
    extends Config((site, here, up) => { case SerialTLKey =>
      up(SerialTLKey, site).map { k =>
        k.copy(
          manager = k.manager.map { s =>
            s.copy(
...
class WithSerialTLROMFile(file: String)
    extends Config((site, here, up) => { case SerialTLKey =>
      up(SerialTLKey, site).map { k =>
        k.copy(
          manager = k.manager.map { s =>
...
class WithSerialTLCoherentMem(base: BigInt, size: BigInt)
    extends Config((site, here, up) => { case SerialTLKey =>
      up(SerialTLKey).map { k =>
        k.copy(
          manager = Some(k.manager.getOrElse(SerialTLManagerParams())).map(s =>
            s.copy(
...
class WithSerialTLClientIdBits(totalIdBits: Int)
    extends Config((site, here, up) => { case SerialTLKey =>
      up(SerialTLKey).map { k =>
        k.copy(
