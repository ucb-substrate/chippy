...
object DromajoHelper {
  def addArtefacts(
    ...
    p(PLICKey) map { pP =>
      ...
      dromajoParams += "\n" + "#define DROMAJO_PLIC_SIZE " + "\"" + "0x" + f"${PLICConsts
          .size(pP.maxHarts)}%X" + "\""
    }
    p(CLINTKey) map { cP =>
...
class SimDromajoBridge(traceType: TraceBundle) extends Module {
  ...
  dromajo.io.pc := Cat(
    traces
      .map(t => UIntToAugmentedUInt(t.iaddr).sextTo(DromajoConstants.xLen))
      .reverse
  )
  dromajo.io.inst := Cat(
    traces.map(t => t.insn.pad(DromajoConstants.instBits)).reverse
  )
  dromajo.io.wdata := Cat(
    traces
      .map(t => UIntToAugmentedUInt(t.wdata.get).sextTo(DromajoConstants.xLen))
      .reverse
  )
  ...
  dromajo.io.cause := traces(0).cause.pad(DromajoConstants.xLen) | (traces(
    0
  ).interrupt << DromajoConstants.xLen - 1)
}

/** Helper function to connect Dromajo bridge. Mirrors the Dromajo bridge in
  * FireSim.
  */
