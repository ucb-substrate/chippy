...
class ScratchpadBank(
  ...
  (0 until subBanks).map { sb =>
    val ram = LazyModule(
      new TLRAM(
        address = AddressSet(
          address.base + sb * p(CacheBlockBytes),
          address.mask - mask
        ),
        beatBytes = beatBytes,
    ...
    ram.node := TLFragmenter(
      beatBytes,
      p(CacheBlockBytes),
      nameSuffix = Some("ScratchpadBank")
    ) := TLBuffer(buffer) := xbar
  }
  override lazy val desiredName = "ScratchpadBank"
}

case object BankedScratchpadInjector
    extends SubsystemInjector((p, baseSubsystem) => {
      p(BankedScratchpadKey).zipWithIndex.foreach { case (params, si) =>
          ...
          override def describe(resources: ResourceBindings): Description = {
            Description(
              ...
              ListMap(
                ...
                "status" -> Seq(
                  ResourceString(if (params.dtsEnabled) "okay" else "disabled")
                )
              )
            )
          }
        ...
        def genBanks()(implicit p: Parameters) = (0 until banks).map { b =>
          ...
          bus.coupleTo(s"$name-$si-$b") {
            bank.xbar := bus { TLBuffer(params.outerBuffer) } := _
          }
        }

        if (params.disableMonitors) DisableMonitors({ implicit p =>
          genBanks()(p)
        })(p)
        else genBanks()(p)
      }
