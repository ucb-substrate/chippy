...
case object OffchipBusKey
    extends Field[OffchipBusParams](
      OffchipBusParams(1, 1)
    ) // default settings are non-sensical
...
case class OffchipBusTopologyConnectionParams(
    ...
    blockRange: Seq[AddressSet] =
      Nil, // offchip addresses which will not be accessible through this port
    replicationBase: Option[BigInt] =
      None // the offchip address region will be replicated above this address
) extends TLBusWrapperTopology(
      instantiations = Nil,
      connections = List(
        (
          location,
          OBUS,
          TLBusWrapperConnection(driveClockFromMaster = Some(true))(
            inject = (q: Parameters) => {
              implicit val p: Parameters = q
              val filter = if (blockRange.isEmpty) { TLTempNode() }
              else {
                TLFilter(TLFilter.mSubtract(blockRange))(p)
              }
              val replicator = replicationBase
                .map { base =>
                  val baseRegion = AddressSet(0, base - 1)
                  val offsetRegion = AddressSet(base, base - 1)
                  val replicator = LazyModule(
                    new RegionReplicator(
                      ReplicatedRegion(baseRegion, baseRegion.widen(base))
                    )
                  )
...
class OffchipBus(params: OffchipBusParams, name: String = "offchip_bus")(
    implicit p: Parameters
) extends TLBusWrapper(params, name) {
  private val offchip_bus_switch = LazyModule(new TLSwitch)
  val inwardNode: TLInwardNode =
    offchip_bus_switch.node :=* TLFIFOFixer(TLFIFOFixer.allVolatile)
  ...
  val io_sel = InModuleBody {
    val io_sel =
      offchip_bus_switch.module.io.sel.map(s => IO(Input(s.cloneType)))
...
trait CanHaveSwitchableOffchipBus { this: BaseSubsystem =>
  val io_obus_sel = InModuleBody {
    tlBusWrapperLocationMap
      .lift(OBUS)
      .map(_.asInstanceOf[OffchipBus].io_sel.getWrappedValue)
      .flatten
      .map(s => {
...
trait AddressTranslatorParams {
  ...
  def onchipRange: AddressSet =
    AddressSet(onchipBase, size) // This chip's onchip (local) address range
...
case class AddressTranslator(params: AddressTranslatorParams)(implicit
    p: Parameters
) extends LazyModule {
  ...
  val module_side = {
    val replicator = LazyModule(
      new RegionReplicator(
        ReplicatedRegion(
          params.onchipRange,
          params.onchipRange.widen(params.offchipBase)
        )
      )
    )
    val prefixSource = BundleBridgeSource[UInt](() => UInt(1.W))
  ...
  val bus_side = params match {
    ...
    case OutwardAddressTranslatorParams(_, _, _) =>
      TLFilter(TLFilter.mSubtract(params.onchipRange))(p)
    // Translate down
    case InwardAddressTranslatorParams(_, _, _) =>
      TLFilter(TLFilter.mSubtract(params.offchipRange))(p)
  }
