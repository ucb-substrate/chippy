...
class WithScratchpad(
    ...
    outerBuffer: BufferParams = BufferParams.default
      ...
      case BankedScratchpadKey =>
        up(BankedScratchpadKey) ++ (0 until partitions).map { pa =>
          BankedScratchpadParams(
      ...
      case SubsystemInjectorKey =>
        up(SubsystemInjectorKey) + BankedScratchpadInjector
    })

class WithMbusScratchpad(
    base: BigInt = 0x80000000L,
    size: BigInt = (4 << 20),
    banks: Int = 1,
    partitions: Int = 1,
    subBanks: Int = 1
) extends WithScratchpad(base, size, banks, partitions, MBUS, subBanks)

class WithSbusScratchpad(
    base: BigInt = 0x80000000L,
    size: BigInt = (4 << 20),
    banks: Int = 1,
    partitions: Int = 1,
    subBanks: Int = 1
) extends WithScratchpad(base, size, banks, partitions, SBUS, subBanks)

// Remove TL monitors from the Scratchpads. This is used to enable deduplication for VLSI flows
class WithNoScratchpadMonitors
    extends Config((site, here, up) => { case BankedScratchpadKey =>
      up(BankedScratchpadKey).map(_.copy(disableMonitors = true))
    })

// Remove all TL scratchpads from the system
class WithNoScratchpads
    extends Config((site, here, up) => { case BankedScratchpadKey =>
      Nil
    })
...
class WithOffchipBus
    extends Config((site, here, up) => {
      case TLNetworkTopologyLocated(InSubsystem) =>
        up(TLNetworkTopologyLocated(InSubsystem)) :+
          OffchipBusTopologyParams(site(OffchipBusKey))
      case OffchipBusKey =>
        up(OffchipBusKey)
          .copy(beatBytes = 8, blockBytes = site(CacheBlockBytes))
    })
...
class WithOffchipBusClient(
    ...
    replicationBase: Option[BigInt] = None
      ...
      case TLNetworkTopologyLocated(InSubsystem) =>
        up(TLNetworkTopologyLocated(InSubsystem)) :+
          OffchipBusTopologyConnectionParams(
            location,
            blockRange,
            replicationBase
          )
...
class WithChipIdPin(params: ChipIdPinParams = ChipIdPinParams())
    extends Config((site, here, up) => { case ChipIdPinKey =>
      Some(params)
    })

// Used for setting pin width
class WithChipIdPinWidth(width: Int)
    extends Config((site, here, up) => { case ChipIdPinKey =>
      up(ChipIdPinKey, site).map(p => p.copy(width = width))
    })
          ...
          topo match {
            case j: JustOneBusTopologyParams =>
              new TLBusWrapperTopology(
                j.instantiations.map(inst =>
                  inst match {
                    case (SBUS, sbus_params: SystemBusParams) =>
                      (SBUS, RingSystemBusParams(sbus_params, buffer))
                    case a => a
                  }
                ),
                j.connections
