...
class SPIChipIO(val csWidth: Int = 1) extends Bundle {
  ...
  val dq = Vec(
    4,
    Analog(1.W)
  ) // Not using Analog(4.W) because we can't connect these to IO cells
}

class SPIFlashIO extends SPIChipIO(1) {
  val reset = Output(
    Reset()
  ) // This is Output because we're going to flip it in the BlackBox
}
...
class SimSPIFlashModel(capacityBytes: BigInt, id: Int, rdOnly: Boolean = true)
    extends BlackBox(
      Map(
  ...
  require(
    capacityBytes < 0x100000000L,
    "SimSPIFlashModel only supports 32-bit addressing"
  )
...
class SPIFlashMemCtrl(addrBits: Int)
    extends BlackBox(Map("ADDR_BITS" -> IntParam(addrBits)))
    with HasBlackBoxResource {
