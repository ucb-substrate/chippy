...
case class CustomBootPinParams(
    ...
    masterWhere: TLBusWrapperLocation =
      CBUS // This needs to write to clint and bootaddrreg, which are on CBUS/PBUS
)
...
trait CanHavePeripheryCustomBootPin { this: BaseSubsystem =>
  val custom_boot_pin = p(CustomBootPinKey).map { params =>
    require(
      p(BootAddrRegKey).isDefined,
      "CustomBootPin relies on existence of BootAddrReg"
    )
    ...
    val inner_io = tlbus {
      ...
      InModuleBody {
        ...
        val inactive :: waiting_bootaddr_reg_a :: waiting_bootaddr_reg_d :: waiting_msip_a :: waiting_msip_d :: dead :: Nil =
          Enum(6)
        ...
        switch(state) {
          ...
          is(waiting_bootaddr_reg_d) {
            when(tl.d.fire) { state := waiting_msip_a }
          }
          is(waiting_msip_a) {
            tl.a.valid := true.B
            tl.a.bits := edge
              .Put(
                toAddress = (p(CLINTKey).get.baseAddress + CLINTConsts
                  .msipOffset(0)).U, // msip for hart0
                fromSource = 0.U,
