...
case class TileResetCtrlParams(
    initResetHarts: Seq[Int] = Nil,
    address: BigInt = 0x100000,
    slaveWhere: TLBusWrapperLocation = PBUS
)
case object TileResetCtrlKey
    extends Field[TileResetCtrlParams](TileResetCtrlParams())

object TLTileResetCtrl {
  def apply(
      sys: BaseSubsystem with InstantiatesHierarchicalElements
  )(implicit p: Parameters) = {
    val resetCtrlParams = p(TileResetCtrlKey)
    val tlbus = sys.locateTLBusWrapper(resetCtrlParams.slaveWhere)
    val domain = sys {
      tlbus.generateSynchronousDomain.suggestName("tile_reset_domain")
    }
...
class TLTileResetCtrl(
    w: Int,
    params: TileResetCtrlParams,
    element_prci_domains: Seq[HierarchicalElementPRCIDomain[_]]
)(implicit p: Parameters)
    extends LazyModule {
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    val r_tile_resets = (0 until nTiles).map({ i =>
      withReset(asyncResetSinkNode.in.head._1.reset) {
        Module(
          new AsyncResetRegVec(
            w = 1,
            init = (if (params.initResetHarts.contains(i)) 1 else 0)
          )
        )
      }
    })
    ...
    val tileMap = element_prci_domains.zipWithIndex.map({ case (d, i) =>
      d.element_reset_domain.clockNode.portParams(0).name.get -> r_tile_resets(
        i
      ).io.q
    })
    (tileResetProviderNode.out zip tileResetProviderNode.in).map {
      case ((o, _), (i, _)) =>
        (o.member.elements zip i.member.elements).foreach {
          case ((name, oD), (_, iD)) =>
            oD.clock := iD.clock
            oD.reset := iD.reset
            for ((n, r) <- tileMap) {
