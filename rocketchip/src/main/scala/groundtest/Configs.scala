...
class TraceGenBufferlessConfig
    extends Config(
...
class WithTraceGen(n: Int = 2, overrideMemOffset: Option[BigInt] = None)(
    ...
    wordBits: Int = 32
      ...
      case TilesLocated(InSubsystem) => {
        ...
        val memOffset: BigInt = overrideMemOffset
          .orElse(site(ExtMem).map(_.master.base))
          .getOrElse(0x0L)
                ...
                List
                  .tabulate(nWays) { i =>
                    Seq.tabulate(nBeats) { j =>
                      BigInt((j * 8) + ((i * nSets) << blockOffset))
                    }
                  }
                  .flatten
              },
