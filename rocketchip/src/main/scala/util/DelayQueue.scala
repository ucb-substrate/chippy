...
object DelayQueue {
  ...
  def apply[T <: Data](
    ...
    val delayQueue = Module(
      new DelayQueue(chiselTypeOf(source.bits), depth, timer.getWidth)
    )
  ...
  /** Helper to connect a delay queue and instantiate a timer to keep track of
    * cycle count.
    *
    * @param source
    *   decoupled queue input
    * @param timerWidth
    *   width of cycle count timer
    * @param delay
    *   cycle delay
    * @param depth
    *   queue size
  ...
  /** Helper to connect a delay queue that delays elements by a statically
    * defined cycle count.
    *
    * @param source
    *   decoupled queue input
  ...
  /** Helper to connect a delay queue that delays elements by a dynamically set
    * cycle count.
    *
    * @param source
    *   decoupled queue input
