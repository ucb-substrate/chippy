...
class LanePositionedDecoupledIO[T <: Data](
    private val gen: T,
    val maxValid: Int,
    val maxReady: Int
) extends Bundle {
  ...
  def driveWith(
      x: LanePositionedDecoupledIO[T],
      selfRotation: UInt = 0.U,
      xRotation: UInt = 0.U
  ): Unit = {
...
case class LanePositionedQueueArgs(
    ...
    free: Boolean =
      false, // split deq pointer from free pointer + enable free IO
    commit: Boolean =
      false, // split enq pointer from commit pointer + enable commit IO
    rewind: Boolean = false, // enable the 'rewind' feature
    abort: Boolean = false
) // enable the 'abort' feature
{
  require(!rewind || free)
  require(!abort || commit)
}

class LanePositionedQueueIO[T <: Data](
    private val gen: T,
    val args: LanePositionedQueueArgs
) extends Bundle {
  ...
  val free =
    if (args.free) Some(Flipped(Decoupled(UInt(depthBitsU.W)))) else None
  ...
  val commit =
    if (args.commit) Some(Flipped(Decoupled(UInt(depthBitsU.W)))) else None
...
trait LanePositionedQueue {
  def apply[T <: Data](
      gen: T,
      args: LanePositionedQueueArgs
  ): LanePositionedQueueModule[T]
  def apply[T <: Data](
      gen: T,
      lanes: Int,
      rows: Int,
...
class LanePositionedQueueBase[T <: Data](
    val gen: T,
    args: LanePositionedQueueArgs
) extends Module
    with LanePositionedQueueModule[T] {
  val LanePositionedQueueArgs(
    lanes,
    rows,
    flow,
    pipe,
  ...
  val committed =
    io.commit.map(x => Mux(x.fire, x.bits, 0.U)).getOrElse(io.enq.valid)
  ...
  val enq_set =
    (if (lanes == 1) enq_row else Cat(enq_row, enq_lane)) - abortSize
  when(doAbort) {
  ...
  val deq_set =
    (if (lanes == 1) deq_row else Cat(deq_row, deq_lane)) - rewindSize
  when(doRewind) {
  ...
  assert(
    nDeq + nCommit === diff_pos || (diff_pos === 0.U && nDeq + nCommit === capacity.U)
  )
  ...
  val enq_vmask =
    UIntToOH1(io.enq.valid +& enq_lane, 2 * lanes - 1).pad(2 * lanes)
  val enq_lmask =
    (if (lanes == 1) 0.U else UIntToOH1(enq_lane, lanes - 1)).pad(2 * lanes)
  val enq_mask = enq_vmask & ~enq_lmask

  val deq_rmask =
    UIntToOH1(io.deq.ready +& deq_lane, 2 * lanes - 1).pad(2 * lanes)
  val deq_lmask =
    (if (lanes == 1) 0.U else UIntToOH1(deq_lane, lanes - 1)).pad(2 * lanes)
...
class FloppedLanePositionedQueueModule[T <: Data](
    gen: T,
    args: LanePositionedQueueArgs
...
class OnePortLanePositionedQueueModule[T <: Data](ecc: Code)(
    gen: T,
    args: LanePositionedQueueArgs
) extends LanePositionedQueueBase(gen, args) {
      ...
      when(a) {
        when(
          enq_set(
            laneBits + rowBits - 1,
            laneBits + 1
          ) === (pre_enq_row >> 1) && nCommit_next < (2 * lanes).U
        ) {
  ...
  val ren =
    (deq_push && (!enq_refill_idle || !pre_gap2)) || (enq_refill_idle && (deq_refill_ren0 || deq_refill_ren1)) || enq_refill_ren0 || enq_refill_ren1
  ...
  val read_row =
    Mux(
      enq_refill_ren0 && !deq_push,
      enq_row_half,
      Mux(
        enq_refill_ren1 && !deq_push,
        enq_row_half - 1.U, // requires isPow2(rows)
        Mux(
          deq_refill_ren0,
          deq_row_half,
          Mux(
            deq_refill_ren1,
            deq_row_half + 1.U, // requires isPow2(rows)
            Mux(
              !isPow2(rows).B && deq_row_half === (rows / 2 - 1).U,
              1.U,
              Mux(
                !isPow2(rows).B && deq_row_half === (rows / 2 - 2).U,
                0.U,
                deq_row_half + 2.U
              )
            )
  ...
  val ram_o =
    ecc.decode(ram.read(read_row, ren)).corrected.asTypeOf(Vec(2 * lanes, gen))
  when(wen && !ren) { ram.write(write_row, ecc.encode(ram_i.asUInt)) }

  val abort_bypass = if (abort) { wen && write_row === read_row }
  else { false.B }
  val bypass = RegNext(
    (deq_push && pre_gap2 && enq_refill_idle) || (deq_refill_ren1 && pre_gap2) || (deq_refill_ren0 && pre_gap1) || abort_bypass
  )
  val dlatch0 = RegNext(
    (deq_push && !deq_row(1)) || (deq_refill_ren1 && deq_row(
      1
    )) || (deq_refill_ren0 && !deq_row(1))
  )
  val dlatch1 = RegNext(
    (deq_push && deq_row(1)) || (deq_refill_ren1 && !deq_row(
      1
  ...
  val elatch0 = RegNext(
    (enq_refill_ren1 && enq_row(1)) || (enq_refill_ren0 && !enq_row(1))
  )
  val elatch1 = RegNext(
    (enq_refill_ren1 && !enq_row(1)) || (enq_refill_ren0 && enq_row(1))
  )
  ...
  for (l <- 0 until lanes) {
    ...
    for (r <- 0 until 4) {
      ...
      when(
        (gap1 && enq_mask(l) && enq_row(1, 0) === r.U) ||
  ...
  val deq_buf1 =
    VecInit.tabulate(4) { i => deq_buffer((i + 1) % 4) }(deq_row(1, 0))
  for (l <- 0 until lanes) {
