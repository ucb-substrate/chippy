...
class Location[T](val name: String) extends Dynamic {
  def selectDynamic[A](portname: String): Location[A] =
    new Location[A](s"${name}_${portname}")
  def applyDynamic[A](portname: String)(args: A*): (Location[A], A) = {
    require(
      args.size == 1,
      "Location: can't support multiple things at one port yet"
    )
...
class LocationMap[T] private (val internalMap: Map[String, T])
    extends Map[Location[_], T] {
  ...
  def required[L <: T](key: Location[_]): L =
    internalMap(key.name).asInstanceOf[L]
  def optional[L <: T](key: Location[_]): Option[L] =
    internalMap.lift(key.name).map(_.asInstanceOf[L])
}
