...
@deprecated(
  "This will be removed in Rocket Chip 2020.08",
  "Rocket Chip 2020.05"
)
...
/** Case class for PlusArg information
  *
  * @tparam A
  *   scala type of the PlusArg value
  * @param default
  *   optional default value
  * @param docstring
  *   text to include in the help
  * @param doctype
  *   description of the Verilog type of the PlusArg value (e.g. STRING, INT)
  */
private case class PlusArgContainer[A](
    default: Option[A],
    docstring: String,
    doctype: String
)
...
object PlusArg {
  ...
  /** PlusArg("foo") will return 42.U if the simulation is run with +foo=42 Do
    * not use this as an initial register value. The value is set in an initial
    * block and thus accessing it from another initial is racey. Add a docstring
    * to document the arg, which can be dumped in an elaboration pass.
    */
  def apply(
      name: String,
      default: BigInt = 0,
  ...
  /** PlusArg.timeout(name, default, docstring)(count) will use chisel.assert to
    * kill the simulation when count exceeds the specified integer argument.
  ...
  def timeout(
    ...
    PlusArgArtefacts.append(name, Some(default), docstring)
    Module(
      new PlusArgTimeout(name + "=%d", default, docstring, width)
    ).io.count := count
  }
}
...
object PlusArgArtefacts {
  ...
  /** Add a new PlusArg
    ...
    *   scala type of the PlusArg value
    * @param name
    *   name for the PlusArg
    * @param default
    *   optional default value
    * @param docstring
    *   text to include in the help
    */
  def append[A: Doctypeable](
      name: String,
      default: Option[A],
      docstring: String
  ): Unit =
    artefacts = artefacts ++
      Map(
        name -> PlusArgContainer(
          default,
          docstring,
          implicitly[Doctypeable[A]].toDoctype(default)
        )
      )
