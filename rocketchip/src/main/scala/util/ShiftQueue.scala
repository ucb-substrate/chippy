...
/** Implements the same interface as chisel3.util.Queue, but uses a shift
  * register internally. It is less energy efficient whenever the queue has more
  * than one entry populated, but is faster on the dequeue side. It is efficient
  * for usually-empty flow-through queues.
  */
