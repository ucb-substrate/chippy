...
class MultiPortQueue[T <: Data](
    ...
    storage: LanePositionedQueue = FloppedLanePositionedQueue
) extends Module {
...
object MultiPortQueue {
  def gather[T <: Data](
      sparse: Seq[DecoupledIO[T]],
      dense: LanePositionedDecoupledIO[T],
      offset: UInt = 0.U
  ): Unit = {
    // Compute per-enq-port ready
    val enq_valid = DensePrefixSum(sparse.map(_.valid.asUInt))(_ +& _)
    val low_ready =
      if (dense.lanes == 1) 0.U else dense.ready(log2Ceil(dense.lanes) - 1, 0)
    ...
    (sparse zip (0.U +: enq_valid)) foreach { case (s, v) =>
      s.ready := v < ready
    }
    ...
    val enq_dense = Gather(
      ...
      Seq.fill(dense.lanes) { lowHoles } ++ DensePrefixSum(
        lowHoles +: highHoles
      )(_ + _).tail
    )
  ...
  def scatter[T <: Data](
    ...
    val low_valid =
      if (dense.lanes == 1) 0.U else dense.valid(log2Ceil(dense.lanes) - 1, 0)
    ...
    (sparse zip (0.U +: deq_ready)) foreach { case (s, r) =>
      s.valid := r < valid
    }
    ...
    sparse.zipWithIndex.foreach { case (s, i) =>
      s.bits := deq_sparse(i + dense.lanes)
    }
  }
}
...
class MultiPortQueueTest(
  ...
  val enq_bits = RipplePrefixSum(
    enq +: valid.asBools.map(x => WireInit(UInt(bits.W), x))
  )(_ + _)
  val deq_bits = RipplePrefixSum(
    deq +: ready.asBools.map(x => WireInit(UInt(bits.W), x))
  )(_ + _)
