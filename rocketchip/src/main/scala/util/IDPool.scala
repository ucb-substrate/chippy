...
class IDPool(
  ...
  val io = IO(new Bundle {
    ...
    val alloc =
      if (revocableSelect) Decoupled(UInt(idWidth.W))
      else Irrevocable(UInt(idWidth.W))
  })
  ...
  val valid1 = ((bitmap.orR && !((PopCount(
    bitmap
  ) === 1.U) && io.alloc.ready)) // bitmap not zero, and not allocating last bit
    || io.free.valid)
  ...
  if (!revocableSelect) {
    when(
      io.alloc.valid && RegNext(
        io.alloc.ready || (!io.alloc.valid && io.free.valid)
      )
    ) {
