...
abstract class Code {
  ...
  /** Encode x to a codeword suitable for decode. If poison is true, the decoded
    * value will report uncorrectable error despite uncorrected == corrected ==
    * x.
    */
  def encode(x: UInt, poison: Bool = false.B): UInt
  def decode(x: UInt): Decoding

  /** Copy the bits in x to the right bit positions in an encoded word, so that
    * x === decode(swizzle(x)).uncorrected; but don't generate the other code
    * bits, so decode(swizzle(x)).error might be true. For codes for which this
    * operation is not trivial, throw an UnsupportedOperationException.
    */
...
class IdentityCode extends Code {
  ...
  def encode(x: UInt, poison: Bool = false.B) = {
    require(
      poison.isLit && poison.litValue == 0,
      "IdentityCode can not be poisoned"
    )
    x
  }
...
class SECCode extends Code {
  ...
  def encode(x: UInt, poison: Bool = false.B) = {
    ...
    require(
      (poison.isLit && poison.litValue == 0) || poisonous(n),
      s"SEC code of length ${n} cannot be poisoned"
    )

    /* By setting the entire syndrome on poison, the corrected bit falls off the end of the code */
    val syndromeUInt = VecInit
      .tabulate(n - k) { j => (syndrome(j)(k - 1, 0) & x).xorR ^ poison }
      .asUInt
    Cat(syndromeUInt, x)
  }
  ...
  def decode(y: UInt) = new Decoding {
    ...
    val syndromeUInt =
      VecInit.tabulate(n - k) { j => (syndrome(j) & y).xorR }.asUInt

    val hammBadBitOH = UIntToOH(syndromeUInt, n + 1)
    val sysBadBitOH =
      VecInit.tabulate(k) { i => hammBadBitOH(sys2hamm(i)) }.asUInt
