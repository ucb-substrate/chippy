...
/** Transmission delay in credit-debit systems. debit delay is the number of
  * cycles it takes debit+'bits' to transit the link. credit delay is the number
  * of cycles it takes credits to be returned. round trip / total delay is the
  * sum of debit and credit delay. The system must have a positive total delay,
  * otherwise you have a combinational loop.
  */
...
/** CreditedIO provides credit-debit-based flow control for arbitrary Data. The
  * sender may only transmit Data when it has non-zero credits. Receivers
  * provide 0 or 1 credits to senders using the credit field. Senders consume 0
  * or 1 credits by setting the debit field. The bits Data field is DontCare
  * when debit=0. credit MAY depend combinationally on debit. debit MAY depend
  * combinationally on credit. WARNING: The user must ensure the round trip time
  * is > 0. Failure to comply will result in a combinational loop!
  */
final class CreditedIO[T <: Data](gen: T) extends Bundle {
  ...
  val credit = Input(
    Bool()
  ) // 1: a credit is given to the sender by the receiver
  val debit = Output(
    Bool()
  ) // 1: a credit is consumed by the sender to transfer 'bits'
  val bits = Output(genType)

  /** Provide a DecoupledIO interface for sending CreditedIO[Data]. Convert an
    * IrrevocableIO input to DecoupledIO via Decoupled(). depth controls the
    * maximum number of Data beats inflight. Sender powers on with
    * credits=depth, so sender and receiver must agree on depth. pipe=false
    * increases the receiver=>sender trip time by one cycle. pipe=true causes
    * debit to depend on credit.
    */
  ...
  /** Provide an IrrevocableIO interface for receiving CreditedIO[Data].
    * Conversion to DecoupledIO is done via application of Decoupled(). depth
    * controls the Queue depth and thus maximum number of elements inflight.
  ...
  /** Add register stages to the sender and receiver paths. Apply this method to
    * the producer/sender-facing bundle. The round-trip-time (RTT) is increased
    * by sender+receiver.
    */
