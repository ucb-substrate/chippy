...
/** Timer with a statically-specified period. Can take multiple inflight
  * start-stop events with ID Will continue to count down as long as at least
  * one event is inflight
  */
class Timer(initCount: Int, maxInflight: Int) extends Module {
  ...
  assert(
    !io.stop.valid || inflight(io.stop.bits),
    "Timer stop for transaction that's not inflight"
  )
}

/** Simplified Timer with a statically-specified period. Can be stopped
  * repeatedly, even when not active.
  */
