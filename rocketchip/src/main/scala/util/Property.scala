...
abstract class BasePropertyLibrary {
  def generateProperty(prop_param: BasePropertyParameters)(implicit
...
class DefaultPropertyLibrary extends BasePropertyLibrary {
  def generateProperty(
      prop_param: BasePropertyParameters
    ...
    crossProperties(cond)
      ...
      .map((c: CoverBoolean) => {
        new CoverPropertyParameters(
          cond = c.cond,
          label =
            c.labels.reduce((s1: String, s2: String) => { s1 + "_" + s2 }),
          message = message + " " + c.labels
            .map("<" + _ + ">")
            .reduce((s1: String, s2: String) => { s1 + " X " + s2 })
        )
      })
  }
...
object cover {
  ...
  def apply(cond: Bool, label: String)(implicit
      sourceInfo: SourceInfo
  ): Unit = {
    propLib.generateProperty(CoverPropertyParameters(cond, label))
  }
  def apply(cond: Bool, label: String, message: String)(implicit
      sourceInfo: SourceInfo
  ): Unit = {
    ...
    prop
      ...
      .foreach((pp: BasePropertyParameters) => {
        if (pp.pType == PropertyType.Cover) {
          propLib.generateProperty(
            CoverPropertyParameters(pp.cond, pp.label, pp.message)
          )
        }
      })
  }
  def apply[T <: Data](rv: ReadyValidIO[T], label: String, message: String)(
    ...
    apply(
      rv.valid && !rv.ready,
      label + "_STALL",
      message + ": valid and not ready"
    )
    apply(
      !rv.valid && rv.ready,
      label + "_IDLE",
      message + ": not valid and ready"
    )
    apply(
      !rv.valid && !rv.ready,
      label + "_FULL",
      message + ": not valid and not ready"
    )
  }
}
