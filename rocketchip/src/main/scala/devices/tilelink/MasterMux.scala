...
class MasterMuxNode(uFn: Seq[TLMasterPortParameters] => TLMasterPortParameters)(
  ...
  def resolveStar(
    ...
    require(
      iStars == 0 && oStars == 0,
      "MasterMux node does not support :=* or :*="
    )
  ...
  ): Seq[TLMasterPortParameters] = { Seq(uFn(p)) }
  def mapParamsU(
      n: Int,
      p: Seq[TLSlavePortParameters]
  ): Seq[TLSlavePortParameters] = { p ++ p }
}

class MuteMaster(name: String = "MuteMaster", maxProbe: Int = 0)(implicit
    p: Parameters
) extends LazyModule {
      ...
      TLMasterPortParameters.v1(clients =
        Seq(
          TLMasterParameters.v1(
            name = name,
            supportsProbe =
              if (maxProbe > 0) TransferSizes(1, maxProbe)
...
class MasterMux(uFn: Seq[TLMasterPortParameters] => TLMasterPortParameters)(
...
class TLMasterMuxTest(txns: Int = 5000, timeout: Int = 500000)(implicit
    p: Parameters
