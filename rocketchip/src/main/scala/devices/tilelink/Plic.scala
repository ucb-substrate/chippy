...
object PLICConsts {
  ...
  def size(maxHarts: Int): Int = {
    require(
      maxHarts > 0 && maxHarts <= maxMaxHarts,
      s"Must be: maxHarts=$maxHarts > 0 && maxHarts <= PLICConsts.maxMaxHarts=${PLICConsts.maxMaxHarts}"
    )
...
case class PLICParams(
    baseAddress: BigInt = 0xc000000,
    maxPriorities: Int = 7,
    intStages: Int = 0,
    maxHarts: Int = PLICConsts.maxMaxHarts
) {
...
class TLPLIC(params: PLICParams, beatBytes: Int)(implicit p: Parameters)
    extends LazyModule {
  // plic0 => max devices 1023
  val device: SimpleDevice =
    new SimpleDevice("interrupt-controller", Seq("riscv,plic0")) {
  ...
  val node: TLRegisterNode = TLRegisterNode(
    ...
    undefZero = true,
    concurrency = 1
  ) // limiting concurrency handles RAW hazards on claim registers
  ...
  def nPriorities =
    (1 << log2Ceil(minPriorities + 1)) - 1 // round up to next 2^n-1
  def nHarts = intnode.edges.out.map(_.source.num).sum

  // Assign all the devices unique ranges
  lazy val sources = intnode.edges.in.map(_.source)
  lazy val flatSources =
    (sources zip sources.map(_.num).scanLeft(0)(_ + _).init).map {
  ...
  ResourceBinding {
    flatSources.foreach { s =>
      s.resources.foreach { r =>
        ...
        (s.range.start until s.range.end).foreach { i =>
          r.bind(device, ResourceInt(i + 1))
        }
      }
    }
  }
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    val interrupts = intnode.in.map { case (i, e) =>
      i.take(e.source.num)
    }.flatten
    ...
    require(
      nDevices == interrupts.size,
      s"Must be: nDevices=$nDevices == interrupts.size=${interrupts.size}"
    )
    require(
      nHarts == harts.size,
      s"Must be: nHarts=$nHarts == harts.size=${harts.size}"
    )

    require(
      nDevices <= PLICConsts.maxDevices,
      s"Must be: nDevices=$nDevices <= PLICConsts.maxDevices=${PLICConsts.maxDevices}"
    )
    require(
      nHarts > 0 && nHarts <= params.maxHarts,
      s"Must be: nHarts=$nHarts > 0 && nHarts <= PLICParams.maxHarts=${params.maxHarts}"
    )
    ...
    if (nDevices > 0) {
      for (hart <- 0 until nHarts) {
        ...
        harts(hart) := ShiftRegister(
          RegNext(fanin.io.max) > threshold(hart),
          params.intStages
        )
      }
    }
    ...
    def pendingRegDesc(i: Int) =
      RegFieldDesc(
        name = s"pending_$i",
        desc =
          s"Set to 1 if interrupt source $i is pending, regardless of its enable or priority setting.",
        group = Some("pending"),
    ...
    def enableRegDesc(i: Int, j: Int, wide: Int) = {
      ...
      RegFieldDesc(
        name = s"enables_${j}",
        desc =
          s"Targets ${low}-${high}. Set bits to 1 if interrupt should be enabled.",
        group = Some(s"enables_${i}"),
        groupDesc = Some(
          s"Enable bits for each interrupt source for target $i. 1 bit for each interrupt source."
        )
      )
    }
    ...
    val pendingRegFields = Seq(
      PLICConsts.pendingBase ->
        (RegField(1) +: pending.zipWithIndex.map { case (b, i) =>
          RegField.r(1, b, pendingRegDesc(i + 1))
        })
    )
    val enableRegFields = enables.zipWithIndex.map { case (e, i) =>
      PLICConsts.enableBase(i) -> (RegField(1) +: e.zipWithIndex.map {
        case (x, j) =>
    ...
    val claiming = Seq
      .tabulate(nHarts) { i => Mux(claimer(i), maxDevs(i), 0.U) }
      .reduceLeft(_ | _)
    ...
    val completedDevs =
      Mux(completer.reduce(_ || _), UIntToOH(completerDev, nDevices + 1), 0.U)
    ...
    def thresholdRegDesc(i: Int) =
      RegFieldDesc(
        name = s"threshold_$i",
        desc =
          s"Interrupt & claim threshold for target $i. Maximum value is ${nPriorities}.",
        reset = if (nPriorities > 0) None else Some(1)
      )
    ...
    if (nDevices >= 2) {
      ...
      property.cover(
        claimed && RegEnable(claimed, false.B, claimed || completed),
        "TWO_CLAIMS",
        "two claims with no intervening complete"
      )
      property.cover(
        completed && RegEnable(completed, false.B, claimed || completed),
        "TWO_COMPLETES",
        "two completes with no intervening claim"
      )
      ...
      property.cover(
        (diff & (diff - 1.U)) =/= 0.U,
        "TWO_INTS_PENDING",
        "two enabled interrupts became pending on same cycle"
      )

      if (nPriorities > 0)
        ccover(
          maxDevs(0) > (1.U << priority(0).getWidth) && maxDevs(0) <= Cat(
            1.U,
            threshold(0)
          ),
          "THRESHOLD",
          "interrupt pending but less than threshold"
        )
    }
...
class PLICFanIn(nDevices: Int, prioBits: Int) extends Module {
  ...
  val effectivePriority = (1.U << prioBits) +: (io.ip.asBools zip io.prio).map {
    case (p, x) => Cat(p, x)
  }
...
trait CanHavePeripheryPLIC { this: BaseSubsystem =>
  val (plicOpt, plicDomainOpt) = p(PLICKey).map { params =>
    ...
    val plicDomainWrapper =
      tlbus.generateSynchronousDomain("PLIC").suggestName("plic_domain")
