...
class DevicePMP(val params: DevicePMPParams) extends Bundle {
  ...
  def fields(blockAddress: Bool, initial: PMPInitialValue): Seq[RegField] = {
    val initialInts = DevicePMP.getInitialValueInts(
      params.addressBits,
      params.pageBits,
      Some(initial)
    )

    val lDesc = RegFieldDesc(
      "l",
    ...
    val aDesc = RegFieldDesc(
      "a",
      "Address match mode. When clear, this PMP does not match any address. When set, Top-of-Range (TOR) matching is applied by this PMP. Cannot be modified if lock bit is set.",
      reset = Some(initialInts.a),
      wrType = Some(RegFieldWrType.MODIFY)
    )

    val rDesc = RegFieldDesc(
      "r",
      "Read bit. When set grants read access to the matching address range. Cannot be modified if lock bit is set.",
      reset = Some(initialInts.r),
      wrType = Some(RegFieldWrType.MODIFY)
    )

    val wDesc = RegFieldDesc(
      "w",
      "Write bit. When set grants write access to the matching address range. Cannot be modified if lock bit is set.",
      reset = Some(initialInts.w),
      wrType = Some(RegFieldWrType.MODIFY)
    )

    val addrHiDesc = RegFieldDesc(
      "addr_hi",
      "Page address. Specifies top-of-range page address for this PMP and bottom-of-range address for following PMP. Cannot be modified if lock bit is set, or if address match mode is TOR and lock bit is set on the subsequent PMP.",
    ...
    Seq(
      ...
      field(
        params.addressBits - params.pageBits,
        addr_hi,
        addrHiDesc,
        l(0) || blockAddress
      ),
...
object DevicePMP {
  ...
  /** Create DevicePMP bundle with appropriate initial value intended for use in
    * RegInit
    *
    * @param addressBits
    *   width of addresses
    ...
    * @param initial
    *   optional initial value. 0 is used for all fields if not specified.
    */

  def apply(
      addressBits: Int,
      pageBits: Int,
      initial: Option[PMPInitialValue] = None
  ): DevicePMP = {
  ...
  /** Helper to convert from Booleans in the optional PMPInitialValue to
    * Int/BigInt values needed to create the UInts in a DevicePMP Bundle
    */
  def getInitialValueInts(
      addressBits: Int,
      pageBits: Int,
      initial: Option[PMPInitialValue]
  ): PMPInitialValueInt = {

    val addr_hi = initial
      .map { i =>
        require(
          (i.address >> addressBits) == 0,
          s"Device PMP Initial value address must be 0 for bits past ${addressBits}, not ${i.address}"
        )
        require(
          (i.address >> pageBits) << pageBits == i.address,
    ...
    def get(f: PMPInitialValue => Boolean): Int =
      initial.map { i => if (f(i)) 1 else 0 }.getOrElse(0)
    PMPInitialValueInt(
      addr_hi = addr_hi,
      l = get(_.l),
      a = get(_.a),
...
case class PhysicalFilterParams(
  ...
  require(
    controlAddress % size == 0,
    s"PhysicalFilter Control address must be aligned to its size, not size ${size} at ${controlAddress}"
  )
  require(
    controlBeatBytes > 0 && isPow2(controlBeatBytes),
    s"PhyiscalFilter control beat bytes must be a positive power of two, not ${controlBeatBytes}"
  )
}

/** The PhyisicalFilter provides physical memory protection for Tile Link bus
  * traffic, granting or denying accesses based on address and access type.
  *
  * The PhysicalFilter uses a set of DevicePMP registers to control whether
  * accesses of certain types are allowed or denied. For transactions in flight,
  * the PhysicalFilter will only prevent acquisition of NEW permissions; it will
  * not shoot down permissions acquired previously.
  *
  * The blocking behavior is controlled by a series of PMP registers which are
  * accessible via memory mapped reads and writes. The list is a priority allow
  * list. If no PMP matches the transaction will be denied. Otherwise the first
  * PMP which is active and address matches is compared against the requested
  * read and/or write permissions. When an access is denied the Phyiscal Filter
  * crafts and responds with a Tile Link Denied response message.
  *
  * When a device PMP register's `a` bit is set, it is enabled and Top of Range
  * (TOR) matching is applied. For a given PMP register, the associated address
  * register forms the top of the address range, and the preceding PMP address
  * register forms the bottom of the address range. If PMP[i]'s `a` field is set
  * to TOR, the entry matches any address `y` such that `PMP[i-1].address <= y <
  * PMP[i].address` . If PMP[0].a is set (TOR is applied), zero is used for the
  * lower bound, and so it matches any address `y < PMP[0].address`. Note that
  * the addresses bits stored in the PMP registers are addr_hi, or page address.
  *
  * Accesses which need write access are anything but Get, AcquireBlock, or
  * NtoB. Accesses which need read access are anything but PutFull data or
  * PutPartial Data.
  *
  * Setting a PMP's r or w bit set grants read or write access respectively.
  *
  * PMP registers are protected by a lock bit. Once lock bit is set the PMP
  * register can no longer be modified. In addition, if the following PMP access
  * is set to TOR, the PMP's address cannot be modified, even if its own lock
  * bit is not set.
  *
  * @param params
  *   sets the base address and width of the control registers, the number of
  *   PMP registers and their initial values.
  */

class PhysicalFilter(params: PhysicalFilterParams)(implicit p: Parameters)
    extends LazyModule {
  val node = TLAdapterNode(managerFn = { mp =>
    mp.v1copy(
      managers = mp.managers.map(_.v1copy(alwaysGrantsT = false)),
      endSinkId = if (mp.endSinkId == 0) { 0 }
      else { mp.endSinkId + 1 },
  ...
  val device =
    new SimpleDevice("physical-filter", Seq("sifive,physical-filter-v0"))
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    val addressBits = log2Ceil(
      node.edges.out.map(_.manager.maxAddress).max + 1 + 1
    )
    val pmps = RegInit(VecInit(params.pmpRegisters.map { ival =>
      DevicePMP(addressBits, params.pageBits, Some(ival))
    }))
    val blocks = pmps.tail.map(_.blockPriorAddress) :+ false.B
    controlNode.regmap(
      0 -> ((pmps zip blocks) zip params.pmpRegisters).zipWithIndex
        .map { case (((p, b), init), i) =>
          RegFieldGroup(
            s"devicepmp${i}",
            Some(s"Physical Filter Device PMP Register ${i}"),
            p.fields(b, init)
          )
        }
        .toList
    ...
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      val sel = (pmps.map(_.a) zip (lt.init zip lt.tail)) map {
        case (a, (l, r)) => a(0) && !l && r
      }
      val ok = pmps.map(p => (p.r(0) || !needR) && (p.w(0) || !needW))
      // If PMP[i] matches the address and is active, apply PMP[i].r/w permissions.
      val allowFirst =
        PriorityMux(sel :+ true.B, ok :+ false.B) // deny if no match
      val allow =
        allowFirst holdUnless a_first // don't change our mind mid-transaction

      // Track the progress of transactions from A => D
      val d_rack =
        edgeIn.manager.anySupportAcquireB.B && in.d.bits.opcode === TLMessages.ReleaseAck
      val flight = RegInit(
        0.U(log2Ceil(edgeIn.client.endSourceId + 1 + 1).W)
      ) // +1 for inclusive range +1 for a_first vs. d_last
      ...
      val d_grant =
        edgeIn.manager.anySupportAcquireB.B && deny.opcode === TLMessages.Grant
      ...
      if (edgeIn.manager.anySupportAcquireB) {
        ...
        val bypass =
          (edgeIn.manager.minLatency == 0).B && in.a.valid && in.a.bits.source === in.d.bits.source
        val d_grant =
          in.d.bits.opcode === TLMessages.Grant || in.d.bits.opcode === TLMessages.GrantData
