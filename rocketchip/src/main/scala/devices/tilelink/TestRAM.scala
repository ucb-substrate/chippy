...
class TLTestRAM(
  ...
  class Impl extends LazyModuleImp(this) {
    def bigBits(
        x: BigInt,
        tail: List[Boolean] = List.empty[Boolean]
    ): List[Boolean] =
    ...
    val addrBits =
      (mask zip edge.addr_hi(in.a.bits).asBools).filter(_._1).map(_._2)
    ...
    val wdata = VecInit(Seq.tabulate(beatBytes) { i =>
      in.a.bits.data(8 * (i + 1) - 1, 8 * i)
    })
...
class TLRAMZeroDelay(ramBeatBytes: Int, txns: Int)(implicit p: Parameters)
    extends LazyModule {
  val fuzz = LazyModule(new TLFuzzer(txns))
  val model = LazyModule(new TLRAMModel("ZeroDelay"))
  val ram = LazyModule(
    new TLTestRAM(AddressSet(0x0, 0x3ff), beatBytes = ramBeatBytes)
  )
