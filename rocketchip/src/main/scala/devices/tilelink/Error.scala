...
class TLError(
    ...
    extends DevNullDevice(
      ...
      beatBytes,
      new SimpleDevice("error-device", Seq("sifive,error0"))
    ) {
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    assert(
      idle || da_first
    ) // we only send Grant, never GrantData => simplified flow control below
