...
class TLMaskROM(c: MaskROMParams)(implicit p: Parameters) extends LazyModule {
      ...
      TLSlavePortParameters.v1(
        Seq(
          TLSlaveParameters.v1(
            ...
            resources =
              new SimpleDevice("rom", Seq("sifive,maskrom0")).reg("mem"),
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    rom.io.clock := clock
    rom.io.address := edge
      .addr_hi(in.a.bits.address - c.address.U)(log2Ceil(c.depth) - 1, 0)
...
case class MaskROMLocated(loc: HierarchicalLocation)
    extends Field[Seq[MaskROMParams]](Nil)

object MaskROM {
  def attach(
      params: MaskROMParams,
      subsystem: Attachable,
    ...
    maskROM.node := bus.coupleTo("MaskROM") {
      TLFragmenter(maskROM.beatBytes, bus.blockBytes) :*= TLWidthWidget(
        bus
      ) := _
    }
