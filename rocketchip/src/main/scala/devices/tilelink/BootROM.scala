...
case class BootROMParams(
    ...
    size: Int = 0x10000,
    hang: BigInt =
      0x10040, // The hang parameter is used as the power-on reset vector
...
class TLROM(
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    val words = (contents ++ Seq.fill(wrapSize - contents.size)(0.toByte))
      .grouped(beatBytes)
      .toSeq
    val bigs = words.map(_.foldRight(BigInt(0)) { case (x, y) =>
      (x.toInt & 0xff) | y << 8
    })
    ...
    val high =
      if (wrapSize == size) 0.U
      else in.a.bits.address(log2Ceil(size) - 1, log2Ceil(wrapSize))
...
case class BootROMLocated(loc: HierarchicalLocation)
    extends Field[Seq[BootROMParams]](Nil)

object BootROM {

  /** BootROM.attach not only instantiates a TLROM and attaches it to the
    * tilelink interconnect at a configurable location, but also drives the
    * tiles' reset vectors to point at its 'hang' address parameter value.
    */
  def attach(
    ...
    val bootROMDomainWrapper = tlbus
      .generateSynchronousDomain(params.name)
      .suggestName(s"${params.name}_domain")
    ...
    if (params.driveResetVector) {
      ...
      InModuleBody {
        ...
        require(
          reset_vector_source.getWidth >= params.hang.bitLength,
