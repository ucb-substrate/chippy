...
case class DevNullParams(
  ...
  require(
    maxAtomic <= maxTransfer,
    s"Atomic transfer size must be <= max transfer (but $maxAtomic > $maxTransfer)"
  )
  require(
    maxTransfer <= 4096,
    s"Max transfer size must be <= 4096 (was $maxTransfer)"
  )
  def acquire: Boolean = region == RegionType.TRACKED
}

/** DevNullDevices don't obey standard memory operation semantics. They may
  * discard writes, refuse to respond to requests, issue error responses, or
  * otherwise violate 'expected' memory behavior.
  */
abstract class DevNullDevice(
    params: DevNullParams,
    minLatency: Int,
    beatBytes: Int,
    protected val device: SimpleDevice
)(implicit p: Parameters)
    extends LazyModule
    with HasClockDomainCrossing {
  val xfer =
    if (params.maxTransfer > 0) TransferSizes(1, params.maxTransfer)
    else TransferSizes.none
  val atom =
    if (params.maxAtomic > 0) TransferSizes(1, params.maxAtomic)
    else TransferSizes.none
