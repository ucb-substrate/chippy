...
class JtagDTMKeyDefault
    extends JtagDTMConfig(
      idcodeVersion = 0,
      idcodePartNum = 0,
      idcodeManufId = 0,
      debugIdleCycles = 5
    ) // Reasonable guess for synchronization.
...
class DebugTransportModuleJTAG(debugAddrBits: Int, c: JtagDTMConfig)(implicit
    val p: Parameters
) extends RawModule {
  ...
  val io = IO(new Bundle {
    ...
    val jtag_reset =
      Input(Reset()) // This is internally converted to AsyncReset.
    // We'd prefer to call this AsyncReset, but that's a fairly
    // invasive API change.
    val dmi = new DMIIO()(p)
    val jtag =
      Flipped(new JTAGIO(hasTRSTn = false)) // TODO: re-use SystemJTAGIO here?
  ...
  withClockAndReset(io.jtag_clock, io.jtag_reset.asAsyncReset) {
    ...
    val dmiAccessChain = Module(
      CaptureUpdateChain(
        genCapture = new DMIAccessCapture(debugAddrBits),
    ...
    property.cover(
      !nonzeroResp,
      "Should see a non-zero response (e.g. when accessing most DM registers when dmactive=0)"
    )
    property.cover(
      !stickyNonzeroRespReg,
      "Should see a sticky non-zero response (e.g. when accessing most DM registers when dmactive=0)"
    )

    busyResp.addr := 0.U
    busyResp.resp := ~(0.U(
      DMIConsts.dmiRespSize.W
    )) // Generalizing busy to 'all-F'
    busyResp.data := 0.U
    ...
    dmiAccessChain.io.capture.bits := Mux(
      busy,
      busyResp,
      Mux(io.dmi.resp.valid, dmiResp, nopResp)
    )
    ...
    assert(
      !(dmiReqValidCheck && io.dmi.req.fire),
      "Conflicting updates for dmiReqValidReg, should not happen."
    );
    ...
    when(dmiAccessChain.io.update.valid) {
      when(stickyBusyReg) {
        ...
        downgradeOpReg || (dmiAccessChain.io.update.bits.op === DMIConsts.dmi_OP_NONE)
      ) {
        // Do Nothing
        dmiReqReg.addr := 0.U
        dmiReqReg.data := 0.U
    ...
    property.cover(
      dmiReqReg.op === DMIConsts.dmi_OP_WRITE & dmiAccessChain.io.capture.capture & busy,
      "Not enough Idle after DMI Write"
    );
    // correct operation - enough time was spent in JTAG Idle state after DMI Write
    property.cover(
      dmiReqReg.op === DMIConsts.dmi_OP_WRITE & dmiAccessChain.io.capture.capture & !busy,
      "Enough Idle after DMI Write"
    );

    // incorrect operation - not enough time was spent in JTAG Idle state after DMI Read
    property.cover(
      dmiReqReg.op === DMIConsts.dmi_OP_READ & dmiAccessChain.io.capture.capture & busy,
      "Not enough Idle after DMI Read"
    );
    // correct operation - enough time was spent in JTAG Idle state after DMI Read
    property.cover(
      dmiReqReg.op === DMIConsts.dmi_OP_READ & dmiAccessChain.io.capture.capture & !busy,
      "Enough Idle after DMI Read"
    );
