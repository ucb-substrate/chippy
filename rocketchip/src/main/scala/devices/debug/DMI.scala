...
/** Structure to define the top-level DMI interface of DebugModule. DebugModule
  * is the consumer of this interface. Therefore it has the 'flipped' version of
  * this.
  */
...
/** This includes the clock and reset as these are passed through the hierarchy
  * until the Debug Module is actually instantiated.
  */

class ClockedDMIIO(implicit val p: Parameters)
...
/** Convert DMI to TL. Avoids using special DMI synchronizers and register
  * accesses
  */
...
class DMIToTL(implicit p: Parameters) extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    val (_, nbits) = edge.Put(
      src,
      toAddress = (DMI_RegAddrs.DMI_HARTINFO << 2).U,
      size,
      data = 0.U
    )
    ...
    io.dmi.resp.bits.resp := Mux(
      tl.d.bits.corrupt || tl.d.bits.denied,
