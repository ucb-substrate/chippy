...
import freechips.rocketchip.amba.apb.{
  APBBundle,
  APBBundleParameters,
  APBMasterNode,
  APBMasterParameters,
...
class DebugIO(implicit val p: Parameters) extends Bundle {
  ...
  val apb = p(ExportDebug).apb.option(
    Flipped(
      new ClockedAPBBundle(APBBundleParameters(addrBits = 12, dataBits = 32))
    )
  )
  // ------------------------------
...
class PSDIO(implicit val p: Parameters)
    extends Bundle
    with CanHavePSDTestModeIO {}

class ResetCtrlIO(val nComponents: Int)(implicit val p: Parameters)
    extends Bundle {
  val hartResetReq = (p(DebugModuleKey)
    .exists(x => x.hasHartResets))
    .option(Output(Vec(nComponents, Bool())))
  val hartIsInReset = Input(Vec(nComponents, Bool()))
}

/** Either adds a JTAG DTM to system, and exports a JTAG interface, or exports
  * the Debug Module Interface (DMI), or exports and hooks up APB, based on a
  * global parameter.
  */
...
trait HasPeripheryDebug { this: BaseSubsystem =>
  ...
  lazy val debugCustomXbarOpt = p(DebugModuleKey).map(params =>
    LazyModule(new DebugCustomXbar(outputRequiresInput = false))
  )
  lazy val apbDebugNodeOpt = p(ExportDebug).apb.option(
    APBMasterNode(
      Seq(APBMasterPortParameters(Seq(APBMasterParameters("debugAPB"))))
    )
  )
  ...
  val resetctrl = InModuleBody {
    debugOpt.map { debug =>
      ...
      val resetctrl =
        IO(new ResetCtrlIO(debug.dmOuter.dmOuter.intnode.edges.out.size))
      debug.module.io.hartIsInReset := resetctrl.hartIsInReset
      resetctrl.hartResetReq.foreach { rcio =>
        debug.module.io.hartResetReq.foreach { rcdm => rcio := rcdm }
      }
      resetctrl
    }
  }
  ...
  val debug = InModuleBody {
    noPrefix(debugOpt.map { debugmod =>
      ...
      require(
        !(debug.clockeddmi.isDefined && debug.systemjtag.isDefined),
        "You cannot have both DMI and JTAG interface in HasPeripheryDebug"
      )

      require(
        !(debug.clockeddmi.isDefined && debug.apb.isDefined),
        "You cannot have both DMI and APB interface in HasPeripheryDebug"
      )

      require(
        !(debug.systemjtag.isDefined && debug.apb.isDefined),
      ...
      (debug.apb
        ...
        zip debugmod.module.io.apb_reset).foreach { case (((io, apb), c), r) =>
        apb.out(0)._1 <> io
        c := io.clock
        r := io.reset
      }
      ...
      debug.extTrigger.foreach { x =>
        debugmod.module.io.extTrigger.foreach { y => x <> y }
      }
  ...
  def instantiateJtagDTM(sj: SystemJTAGIO): DebugTransportModuleJTAG = {
    ...
    debug.map(_.disableDebug.foreach { x =>
      dtm.io.jtag.TMS := sj.jtag.TMS | x
    }) // force TMS high when debug is disabled
...
class SimJTAG(tickDelay: Int = 50)
    extends BlackBox(Map("TICK_DELAY" -> IntParam(tickDelay)))
...
object Debug {
  def connectDebug(
    ...
    debugOpt.map { debug =>
      ...
      debug.apb.foreach { apb =>
        require(
          false,
          "No support for connectDebug for an APB debug connection."
        )
      }
  ...
  def connectDebugClockAndReset(
    ...
    debugOpt.foreach { debug =>
      val dmi_reset =
        debug.clockeddmi.map(_.dmiReset.asBool).getOrElse(false.B) |
  ...
  def connectDebugClockHelper(
    ...
    withClockAndReset(c, dmi_reset) {
      val debug_reset_syncd =
        if (sync)
          ~AsyncResetSynchronizerShiftReg(
            in = true.B,
            sync = 3,
            name = Some("debug_reset_sync")
          )
        else dmi_reset
    ...
    withClockAndReset(c, debug_reset.asAsyncReset) {
      ...
      debug.reset := (if (p(SubsystemResetSchemeKey) == ResetSynchronous)
                        debug_reset
                      else debug_reset.asAsyncReset)
  ...
  def tieoffDebug(
      debugOpt: Option[DebugIO],
      resetctrlOpt: Option[ResetCtrlIO] = None,
      psdio: Option[PSDIO] = None
