...
case class DebugCustomParams(
  ...
  require(
    width % 8 == 0,
    s"Currently only support custom debug widths which are multiples of 8, not ${width}"
  )
}
...
class DebugCustomImp
  ...
  def edge(
      pd: DebugCustomParams,
      pu: DebugCustomNull,
      p: Parameters,
      sourceInfo: SourceInfo
  ) = pd
  def bundle(e: DebugCustomParams) = new DebugCustomBundle(e)
  def render(e: DebugCustomParams) =
    RenderedEdge(colour = "#20B2AA" /* lightseagreen */ )

}

// The Sink is the Debug Module
case class DebugCustomSink()(implicit valName: ValName)
    extends SinkNode(new DebugCustomImp)(pi = Seq(DebugCustomNull()))

// The Source is the sources of data you want to snoop
case class DebugCustomSource(snoopable: DebugCustomParams)(implicit
    valName: ValName
) extends SourceNode(new DebugCustomImp)(Seq(snoopable))
...
case class DebugCustomNexusNode(
    ...
    extends NexusNode(new DebugCustomImp)(
      sourceFn,
      sinkFn,
      inputRequiresOutput,
      outputRequiresInput
...
class DebugCustomXbar(
  ...
  val node = DebugCustomNexusNode(
    sourceFn = { seq =>
      if (seq.size == 0) { // Allow no sources
        ...
        require(
          all_addrs.size == all_addrs.distinct.size,
          "Different Custom sources can't use the same addresses."
        )
        val max_width = seq.foldLeft(0) { (result, current) =>
          result max current.width
        }
  ...
  class Impl extends LazyRawModuleImp(this) {
    ...
    require(
      node.out.size == 1,
      "Must have exactly one sink node, not ${node.out.size}"
    )
    // send address to all sources
    val (sink, sinkParam) = node.out.head
    val (sources, sourceParams) = node.in.unzip
    val decoded = sourceParams.map { x =>
      x.addrs.foldLeft(false.B) { (result, current) =>
        result || current.U === sink.addr
      }
    }
    ...
    sink.ready := (decoded zip sources).foldLeft(false.B) {
      case (result, (d, i)) => result || (d & i.ready)
    }
    sink.data := (decoded zip sources).foldLeft(0.U) { case (result, (d, i)) =>
      result | Mux(i.ready, i.data, 0.U)
    }
