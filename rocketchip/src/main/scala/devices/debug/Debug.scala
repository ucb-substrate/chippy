...
import freechips.rocketchip.devices.debug.systembusaccess.{
  SBToTL,
  SystemBusAccessModule
}
...
object DsbRegAddrs {
  ...
  def ABSTRACT(cfg: DebugModuleParams) =
    PROGBUF(cfg) - (cfg.nAbstractInstructions * 4)
...
/** Enumerations used both in the hardware and in the configuration
  * specification.
  */
...
/** Parameters exposed to the top-level design, set based on external
  * requirements, etc.
  *
  * This object checks that the parameters conform to the full specification.
  * The implementation which receives this object can perform more checks on
  * what that implementation actually supports.
  * @param nComponents
  ...
  *   Include "safe" logic in Async Crossings so that only one side needs to be
  *   reset.
  */
...
case class DebugModuleParams(
  ...
  require(
    (nDMIAddrSize >= 7) && (nDMIAddrSize <= 32),
    s"Legal DMIAddrSize is 7-32, not ${nDMIAddrSize}"
  )
  ...
  require(
    nExtTriggers <= 16,
    s"Legal nExtTriggers is 0-16, not ${nExtTriggers}"
  )
  ...
  /** The number of generated instructions
    *
    * When the base address is not zero, we need more instruction also, more
    * dscratch registers) to load/store memory mapped data register because they
    * may no longer be directly addressible with x0 + 12-bit imm
    */
...
object DefaultDebugModuleParams {
  ...
  def apply(
      xlen: Int /*TODO , val configStringAddr: Int*/
  ): DebugModuleParams = {
...
/** Functional parameters exposed to the design configuration.
  *
  * hartIdToHartSel: For systems where hart ids are not 1:1 with hartsel,
  * provide the mapping. hartSelToHartId: Provide inverse mapping of the above
  */
...
class DebugExtTriggerIO()(implicit val p: Parameters)
...
class DebugAuthenticationIO()(implicit val p: Parameters)
    extends ParameterizedBundle()(p) {
...
/** Control signals for Inner, generated in Outer
  ...
  * }}}
  */
class DebugInternalBundle(val nComponents: Int)(implicit val p: Parameters)
    extends ParameterizedBundle()(p) {
...
/** structure for top-level Debug Module signals which aren't the bus
  * interfaces.
  */
class DebugCtrlBundle(nComponents: Int)(implicit val p: Parameters)
    extends ParameterizedBundle()(p) {
  ...
  /** reset signal
    *
    * for every part of the hardware platform, including every hart, except for
    * the DM and any logic required to access the DM
    */
...
/** Parameterized version of the Debug Module defined in the RISC-V Debug
  * Specification
  *
  * DebugModule is a slave to two asynchronous masters: The Debug Bus (DMI) --
  * This is driven by an external debugger
  *
  * The System Bus -- This services requests from the cores. Generally this
  * interface should only be active at the request of the debugger, but the
  * Debug Module may also provide the default MTVEC since it is mapped to
  * address 0x0.
  *
  * DebugModule is responsible for control registers and RAM, and Debug ROM. It
  * runs partially off of the dmiClk (e.g. TCK) and the TL clock. Therefore, it
  * is divided into "Outer" portion (running off dmiClock and dmiReset) and
  * "Inner" (running off tl_clock and tl_reset). This allows DMCONTROL.haltreq,
  * hartsel, hasel, hawindowsel, hawindow, dmactive, and ndreset to be modified
  * even while the Core is in reset or not being clocked. Not all reads from the
  * Debugger to the Debug Module will actually complete in these scenarios
  * either, they will just block until tl_clock and tl_reset allow them to
  * complete. This is not strictly necessary for proper debugger functionality.
  */
...
object WNotifyWire {
  def apply(
    ...
    RegField(
      ...
      Some(
        RegFieldDesc(name = name, desc = desc, access = RegFieldAccessType.W)
      )
    )
  }
}
...
class TLDebugModuleOuter(device: Device)(implicit p: Parameters)
    extends LazyModule {
  ...
  val intnode = IntNexusNode(
    sourceFn = { _ =>
      IntSourcePortParameters(
        Seq(IntSourceParameters(1, Seq(Resource(device, "int"))))
      )
    },
  ...
  class Impl extends LazyModuleImp(this) {
    require(
      ...
      "Debug Module does not accept interrupts"
    )

    val nComponents = intnode.out.size
    def getNComponents = () => nComponents

    val supportHartArray =
      cfg.supportHartArray && (nComponents > 1) // no hart array if only one hart
    ...
    val io = IO(new Bundle {
      ...
      val hartResetReq =
        cfg.hasHartResets.option(Output(Vec(nComponents, Bool())))
    ...
    val omRegMap = withReset(reset.asAsyncReset) {
      ...
      val dmAuthenticated = io.dmAuthenticated
        .map(dma =>
          ResetSynchronizerShiftReg(
            in = dma,
            sync = 3,
            name = Some("dmAuthenticated_sync")
          )
        )
        .getOrElse(true.B)
      ...
      val DMCONTROLReg =
        RegNext(next = DMCONTROLNxt, init = 0.U.asTypeOf(DMCONTROLNxt))
          .suggestName("DMCONTROLReg")

      val hartsel_mask =
        if (nComponents > 1) ((1 << p(MaxHartIdBits)) - 1).U else 0.U
      ...
      def hartSelected(hart: Int): Bool = {
        ...
        (if (supportHartArray)
           io.innerCtrl.bits.hasel && io.innerCtrl.bits.hamask(hart)
         else false.B))
      }

      val HAWINDOWSELNxt = WireInit(0.U.asTypeOf(new HAWINDOWSELFields()))
      val HAWINDOWSELReg =
        RegNext(next = HAWINDOWSELNxt, init = 0.U.asTypeOf(HAWINDOWSELNxt))
        ...
        when(~dmactive || ~dmAuthenticated) {
          ...
          when(HAWINDOWSELWrEn) {
            ...
            if (nComponents > haWindowSize) {
              HAWINDOWSELNxt.hawindowsel := HAWINDOWSELWrData.hawindowsel & ((1 << (log2Up(
                nComponents
              ) - 5)) - 1).U
            } else {
      ...
      for (component <- 0 until nComponents) {
        when(~dmactive || ~dmAuthenticated) {
          ...
          clrresethaltreqWrEn && DMCONTROLWrData.clrresethaltreq && hartSelected(
            component
          )
        ) {
          hrmaskNxt(component) := false.B
        }.elsewhen(
          setresethaltreqWrEn && DMCONTROLWrData.setresethaltreq && hartSelected(
            component
          )
        ) {
        ...
        Seq(
          RegField.r(
            ...
            RegFieldDesc(
              "dataaddr",
              "data address",
              reset = Some(if (cfg.atzero) DsbRegAddrs.DATA else 0)
            )
          ),
          RegField.r(
            4,
          ...
          RegField.r(
            ...
            RegFieldDesc(
              "dataaccess",
              "data access type",
              reset = Some(if (cfg.atzero) 1 else 0)
            )
          ),
          RegField(3),
          RegField.r(
            ...
            RegFieldDesc(
              ...
              reset = Some(if (cfg.atzero) cfg.nScratch else 0)
            )
          )
        )
      )
      ...
      val debugIntRegs =
        RegNext(next = debugIntNxt, init = 0.U.asTypeOf(debugIntNxt))
          .suggestName("debugIntRegs")

      debugIntNxt := debugIntRegs
      val (intnode_out, _) = intnode.out.unzip
      for (component <- 0 until nComponents) {
        intnode_out(component)(0) := debugIntRegs(component) | io.hgDebugInt(
          component
        )
      }
        ...
        when(~dmactive || ~dmAuthenticated) {
          ...
          when(
            haltreqWrEn && ((DMCONTROLWrData.hartsello === component.U)
              || (if (supportHartArray)
                    DMCONTROLWrData.hasel && hamask(component)
                  else false.B))
          ) {
      ...
      val innerCtrlResumeReqReg =
        RegInit(false.B).suggestName("innerCtrlResumeReqReg")
      val innerCtrlAckHaveResetReg =
        RegInit(false.B).suggestName("innerCtrlAckHaveResetReg")
      ...
      if (cfg.hasHartResets) {
        ...
        val hartResetReg =
          RegNext(next = hartResetNxt, init = 0.U.asTypeOf(hartResetNxt))

        for (component <- 0 until nComponents) {
          hartResetNxt(component) := DMCONTROLNxt.hartreset & hartSelected(
            component
          )
...
class TLDebugModuleOuterAsync(device: Device)(implicit p: Parameters)
    extends LazyModule {
  ...
  val apbNodeOpt = p(ExportDebug).apb.option({
    ...
    val tlErrorParams = DevNullParams(
      AddressSet
        .misaligned(dmTopAddr, APBDebugConsts.apbDebugRegBase - dmTopAddr),
      maxAtomic = 0,
      maxTransfer = 4
    )
  ...
  intnode :*= IntSyncCrossingSource(alreadyRegistered =
    true
  ) :*= dmOuter.intnode

  val dmiBypass = LazyModule(
    new TLBusBypass(
      beatBytes = 4,
      bufferError = false,
      maxAtomic = 0,
  ...
  class Impl extends LazyRawModuleImp(this) {
    ...
    val io = IO(new Bundle {
      ...
      val innerCtrl = new AsyncBundle(
        new DebugInternalBundle(nComponents),
        AsyncQueueParams.singleton(safe = cfg.crossingHasSafeReset)
      )
      ...
      /** hart reset request to core */
      val hartResetReq = p(DebugModuleKey).get.hasHartResets
        .option(Output(Vec(nComponents, Bool())))

      /** Authentication signal from core */
      val dmAuthenticated =
        p(DebugModuleKey).get.hasAuthentication.option(Input(Bool()))
    })
    ...
    withClockAndReset(childClock, childReset) {
      ...
      val dmactiveAck = AsyncResetSynchronizerShiftReg(
        in = io.ctrl.dmactiveAck,
        sync = 3,
        name = Some("dmactiveAckSync")
      )
      ...
      io.innerCtrl <> ToAsyncBundle(
        dmOuter.module.io.innerCtrl,
        AsyncQueueParams.singleton(safe = cfg.crossingHasSafeReset)
      )
      dmOuter.module.io.hgDebugInt := io.hgDebugInt
      io.hartResetReq.foreach { x =>
        dmOuter.module.io.hartResetReq.foreach { y => x := y }
      }
      io.dmAuthenticated.foreach { x =>
        dmOuter.module.io.dmAuthenticated.foreach { y => y := x }
      }
    }
  }
}
...
class TLDebugModuleInner(
  ...
  val dmiNode = TLRegisterNode(
    ...
    address = AddressSet.misaligned(0, DMI_DMCONTROL << 2) ++
      AddressSet.misaligned(
        (DMI_DMCONTROL + 1) << 2,
        ((DMI_HARTINFO << 2) - ((DMI_DMCONTROL + 1) << 2))
      ) ++
      AddressSet.misaligned(
        (DMI_HARTINFO + 1) << 2,
        ((DMI_HAWINDOWSEL << 2) - ((DMI_HARTINFO + 1) << 2))
      ) ++
      AddressSet.misaligned(
        (DMI_HAWINDOW + 1) << 2,
        (dmTopAddr - ((DMI_HAWINDOW + 1) << 2))
      ),
    device = device,
    beatBytes = 4,
    executable = false
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    val hartSelFuncs =
      if (getNComponents() > 1) p(DebugModuleHartSelKey)
      else
    ...
    val io = IO(new Bundle {
      ...
      /** conrol signals for Inner
        ...
        */
      val innerCtrl =
        Flipped(new DecoupledIO(new DebugInternalBundle(nComponents)))
    ...
    require(
      (nHaltGroups > 0) || (nExtTriggers == 0),
      "External triggers require at least 1 halt group"
    )
    ...
    val dmiProgramBufferRdEn = WireInit(
      VecInit(Seq.fill(cfg.nProgramBufferWords * 4) { false.B })
    )
    val dmiProgramBufferAccessLegal = WireInit(false.B)
    val dmiProgramBufferWrEnMaybe = WireInit(
      VecInit(Seq.fill(cfg.nProgramBufferWords * 4) { false.B })
    )

    val dmiAbstractDataRdEn = WireInit(
      VecInit(Seq.fill(cfg.nAbstractDataWords * 4) { false.B })
    )
    val dmiAbstractDataAccessLegal = WireInit(false.B)
    val dmiAbstractDataWrEnMaybe = WireInit(
      VecInit(Seq.fill(cfg.nAbstractDataWords * 4) { false.B })
    )
    ...
    if (supportHartArray) {
      ...
      when(~io.dmactive || ~dmAuthenticated) {
        ...
        hamaskReg := Mux(
          io.innerCtrl.bits.hasel,
          io.innerCtrl.bits.hamask,
          hamaskZero
        )
      }
    ...
    for (component <- 0 until nComponents) {
      hamaskWrSel(component) := ((io.innerCtrl.bits.hartsel === component.U) ||
        (if (supportHartArray)
           io.innerCtrl.bits.hasel && io.innerCtrl.bits.hamask(component)
         else false.B))
    }
    ...
    for (component <- 0 until nComponents) {
      hartIsInResetSync(component) := AsyncResetSynchronizerShiftReg(
        io.hartIsInReset(component),
        3,
    ...
    when(dmAuthenticated) {
      ...
      DMSTATUSRdData.allnonexistent := (selectedHartReg >= nComponents.U) & (~hamaskFull
        .reduce(_ | _))

      when(~DMSTATUSRdData.allnonexistent) { // if no existent harts selected, all other status is false
        DMSTATUSRdData.anyunavail := (io.debugUnavail & hamaskFull).reduce(
          _ | _
        )
        DMSTATUSRdData.anyhalted := ((~io.debugUnavail & (haltedBitRegs.asBools)) & hamaskFull)
          .reduce(_ | _)
        DMSTATUSRdData.anyrunning := ((~io.debugUnavail & ~(haltedBitRegs.asBools)) & hamaskFull)
          .reduce(_ | _)
        DMSTATUSRdData.anyhavereset := (haveResetBitRegs.asBools & hamaskFull)
          .reduce(_ | _)
        DMSTATUSRdData.anyresumeack := (resumeAcks.asBools & hamaskFull).reduce(
          _ | _
        )
        when(~DMSTATUSRdData.anynonexistent) { // if one hart is nonexistent, no 'all' status is set
          DMSTATUSRdData.allunavail := (io.debugUnavail | ~hamaskFull).reduce(
            _ & _
          )
          DMSTATUSRdData.allhalted := ((~io.debugUnavail & (haltedBitRegs.asBools)) | ~hamaskFull)
            .reduce(_ & _)
          DMSTATUSRdData.allrunning := ((~io.debugUnavail & ~(haltedBitRegs.asBools)) | ~hamaskFull)
            .reduce(_ & _)
          DMSTATUSRdData.allhavereset := (haveResetBitRegs.asBools | ~hamaskFull)
            .reduce(_ & _)
          DMSTATUSRdData.allresumeack := (resumeAcks.asBools | ~hamaskFull)
            .reduce(_ & _)
        }
      }
    ...
    if (nHaltGroups > 0)
      withReset(reset.asAsyncReset) { // async reset ensures triggers don't falsely fire during startup
        ...
        val hgParticipateHart =
          RegInit(VecInit(Seq.fill(nComponents)(0.U(hgBits.W))))
        val hgParticipateTrig =
          if (nExtTriggers > 0)
            RegInit(VecInit(Seq.fill(nExtTriggers)(0.U(hgBits.W))))
          else Nil
        ...
        DMCS2RdData.haltgroup := hgParticipateHart(
          if (nComponents == 1) 0.U(0.W) else selectedHartReg
        )
        ...
        io.extTrigger.foreach { extTrigger =>
          ...
          val trigInReq = ResetSynchronizerShiftReg(
            in = extTriggerInReq,
            sync = 3,
            name = Some("dm_extTriggerInReqSync")
          )
          val trigOutAck = ResetSynchronizerShiftReg(
            in = extTriggerOutAck,
            sync = 3,
            name = Some("dm_extTriggerOutAckSync")
          )
          for (hg <- 1 to nHaltGroups) {
            hgTrigFiring(hg) := (trigInReq & ~RegNext(
              trigInReq
            ) & hgParticipateTrig.map(_ === hg.U)).reduce(_ | _)
            hgTrigsAllAcked(hg) := (trigOutAck | hgParticipateTrig.map(
              _ =/= hg.U
            )).reduce(_ & _)
          }
        ...
        for (hg <- 1 to nHaltGroups) {
          hgHartFiring(hg) := hartHaltedWrEn & ~haltedBitRegs(
            hartSelFuncs.hartIdToHartSel(hartHaltedId)
          ) & (hgParticipateHart(
            hartSelFuncs.hartIdToHartSel(hartHaltedId)
          ) === hg.U)
          hgHartsAllHalted(hg) := (haltedBitRegs.asBools | hgParticipateHart
            .map(_ =/= hg.U)).reduce(_ & _)

          when(~io.dmactive || ~dmAuthenticated) {
            hgFired(hg) := false.B
          }.elsewhen(
            ~hgFired(hg) & ~hgHartsAllHalted(hg) & (hgHartFiring(
              hg
            ) | hgTrigFiring(hg))
          ) {
            hgFired(hg) := true.B
    ...
    val selectedHaltedStatus = Mux(
      (selectedHartReg >> 5) > numHaltedStatus.U,
      0.U,
    ...
    val dmiAbstractDataAccessVec = WireInit(
      VecInit(Seq.fill(cfg.nAbstractDataWords * 4) { false.B })
    )
    dmiAbstractDataAccessVec := (dmiAbstractDataWrEnMaybe zip dmiAbstractDataRdEn)
      .map { case (r, w) => r | w }
    // Program Buffer access vector(byte-addressable)
    val dmiProgramBufferAccessVec = WireInit(
      VecInit(Seq.fill(cfg.nProgramBufferWords * 4) { false.B })
    )
    dmiProgramBufferAccessVec := (dmiProgramBufferWrEnMaybe zip dmiProgramBufferRdEn)
      .map { case (r, w) => r | w }
    ...
    val autoexecProg = WireInit(VecInit(Seq.fill(cfg.nProgramBufferWords) {
      false.B
    }))(autoexecData zip ABSTRACTAUTOReg.autoexecdata.asBools).zipWithIndex
      .foreach { case (t, i) =>
        t._1 := dmiAbstractDataAccessVec(i * 4) && t._2
      }(autoexecProg zip ABSTRACTAUTOReg.autoexecprogbuf.asBools)
      .zipWithIndex
      .foreach { case (t, i) =>
    ...
    when(~io.dmactive || ~dmAuthenticated) {
      ...
      val hartHaltedIdIndex =
        UIntToOH(hartSelFuncs.hartIdToHartSel(hartHaltedId))
      val hartResumingIdIndex =
        UIntToOH(hartSelFuncs.hartIdToHartSel(hartResumingId))
    ...
    val dmstatusRegFields = RegFieldGroup(
      ...
      Seq(
        RegField.r(
          4,
          DMSTATUSRdData.version,
          RegFieldDesc("version", "version", reset = Some(2))
        ),
        RegField.r(
          1,
          DMSTATUSRdData.confstrptrvalid,
          RegFieldDesc("confstrptrvalid", "confstrptrvalid", reset = Some(0))
        ),
        RegField.r(
          1,
          DMSTATUSRdData.hasresethaltreq,
          RegFieldDesc("hasresethaltreq", "hasresethaltreq", reset = Some(1))
        ),
        RegField.r(
          1,
          DMSTATUSRdData.authbusy,
          RegFieldDesc("authbusy", "authbusy", reset = Some(0))
        ),
        RegField.r(
          1,
          DMSTATUSRdData.authenticated,
          RegFieldDesc("authenticated", "authenticated", reset = Some(1))
        ),
        RegField.r(
          1,
          DMSTATUSRdData.anyhalted,
          RegFieldDesc("anyhalted", "anyhalted", reset = Some(0))
        ),
        RegField.r(
          1,
          DMSTATUSRdData.allhalted,
          RegFieldDesc("allhalted", "allhalted", reset = Some(0))
        ),
        RegField.r(
          1,
          DMSTATUSRdData.anyrunning,
          RegFieldDesc("anyrunning", "anyrunning", reset = Some(1))
        ),
        RegField.r(
          1,
          DMSTATUSRdData.allrunning,
          RegFieldDesc("allrunning", "allrunning", reset = Some(1))
        ),
        RegField.r(
          1,
          DMSTATUSRdData.anyunavail,
          RegFieldDesc("anyunavail", "anyunavail", reset = Some(0))
        ),
        RegField.r(
          1,
          DMSTATUSRdData.allunavail,
          RegFieldDesc("allunavail", "allunavail", reset = Some(0))
        ),
        RegField.r(
          1,
          DMSTATUSRdData.anynonexistent,
          RegFieldDesc("anynonexistent", "anynonexistent", reset = Some(0))
        ),
        RegField.r(
          1,
          DMSTATUSRdData.allnonexistent,
          RegFieldDesc("allnonexistent", "allnonexistent", reset = Some(0))
        ),
        RegField.r(
          1,
          DMSTATUSRdData.anyresumeack,
          RegFieldDesc("anyresumeack", "anyresumeack", reset = Some(1))
        ),
        RegField.r(
          1,
          DMSTATUSRdData.allresumeack,
          RegFieldDesc("allresumeack", "allresumeack", reset = Some(1))
        ),
        RegField.r(
          1,
          DMSTATUSRdData.anyhavereset,
          RegFieldDesc("anyhavereset", "anyhavereset", reset = Some(0))
        ),
        RegField.r(
          1,
          DMSTATUSRdData.allhavereset,
          RegFieldDesc("allhavereset", "allhavereset", reset = Some(0))
        ),
        RegField(2),
        RegField.r(
          1,
          ...
          RegFieldDesc(
            "impebreak",
            "impebreak",
            reset = Some(if (cfg.hasImplicitEbreak) 1 else 0)
          )
        )
      )
    )
    ...
    val dmcs2RegFields = RegFieldGroup(
      ...
      Seq(
        WNotifyVal(
          ...
          RegFieldDesc(
            "hgselect",
            "select halt groups or external triggers",
            reset = Some(0),
            volatile = true
          )
        ),
      ...
      Seq(
        RegField.r(
          ...
          RegFieldDesc(
            "datacount",
            "number of DATA registers",
            reset = Some(cfg.nAbstractDataWords)
          )
        ),
        RegField(4),
        WNotifyVal(
          ...
          RegFieldDesc(
            ...
            "command error",
            reset = Some(0),
            wrType = Some(RegFieldWrType.ONE_TO_CLEAR)
          )
        ),
        RegField(1),
        RegField.r(
          1,
          ABSTRACTCSRdData.busy,
          RegFieldDesc("busy", "busy", reset = Some(0))
        ),
        RegField(11),
        RegField.r(
          5,
          ...
          RegFieldDesc(
            "progbufsize",
            "number of PROGBUF registers",
            reset = Some(cfg.nProgramBufferWords)
          )
        )
      )
    )
    ...
    val (sbcsFields, sbAddrFields, sbDataFields)
        : (Seq[RegField], Seq[Seq[RegField]], Seq[Seq[RegField]]) = sb2tlOpt
      .map { sb2tl =>
    ...
    val omRegMap = dmiNode.regmap(
      ...
      (DMI_HALTSUM0 << 2) -> RegFieldGroup(
        ...
        Seq(
          RegField.r(
            32,
            HALTSUM0RdData.asUInt,
            RegFieldDesc("dmi_haltsum0", "halt summary 0")
          )
        )
      ),
      (DMI_HALTSUM1 << 2) -> RegFieldGroup(
        ...
        Seq(
          RegField.r(
            32,
            HALTSUM1RdData.asUInt,
            RegFieldDesc("dmi_haltsum1", "halt summary 1")
          )
        )
      ),
      ...
      (DMI_ABSTRACTAUTO << 2) -> RegFieldGroup(
        ...
        Seq(
          WNotifyVal(
            ...
            RegFieldDesc(
              "autoexecdata",
              "abstract command data autoexec",
              reset = Some(0)
            )
          ),
          ...
          WNotifyVal(
            ...
            RegFieldDesc(
              "autoexecprogbuf",
              "abstract command progbuf autoexec",
              reset = Some(0)
            )
          )
        )
        ...
        abstractDataMem.zipWithIndex.map { case (x, i) =>
          RWNotify(
            ...
            Some(
              RegFieldDesc(
                s"dmi_data_$i",
                s"abstract command data register $i",
                reset = Some(0),
                volatile = true
              )
            )
        ...
        programBufferMem.zipWithIndex.map { case (x, i) =>
          RWNotify(
            ...
            Some(
              RegFieldDesc(
                s"dmi_progbuf_$i",
                s"abstract command progbuf register $i",
                reset = Some(0)
              )
            )
          )
      ...
      (DMI_AUTHDATA << 2) -> (if (cfg.hasAuthentication)
                                RegFieldGroup(
                                  ...
                                  Seq(
                                    RWNotify(
                                      32,
                                      io.auth.get.dmAuthRdata,
                                      io.auth.get.dmAuthWdata,
                                      authRdEnMaybe,
                                      authWrEnMaybe,
                                      Some(
                                        RegFieldDesc(
    ...
    abstractDataMem.zipWithIndex.foreach { case (x, i) =>
      when(
        dmAuthenticated && dmiAbstractDataWrEnMaybe(
          i
    ...
    if (needCustom) {
      ...
      require(
        customP.width % 8 == 0,
        s"Debug Custom width must be divisible by 8, not ${customP.width}"
      )
    ...
    programBufferMem.zipWithIndex.foreach { case (x, i) =>
      when(
        dmAuthenticated && dmiProgramBufferWrEnMaybe(
          i
        ) && dmiProgramBufferAccessLegal
      ) {
    ...
    val jalAbstract = WireInit(
      Instructions.JAL.value.U.asTypeOf(new GeneratedUJ())
    )
    ...
    when(~io.dmactive) {
      ...
      when(goAbstract) {
        ...
        assert(
          hartGoingId === 0.U,
          "Unexpected 'GOING' hart."
        ) // Chisel3 #540 %x, expected %x", hartGoingId, 0.U)
        goReg := false.B
      }
    }
    ...
    for (component <- 0 until nComponents) {
      ...
      flags(hartSelFuncs.hartSelToHartId(componentSel)).resume := resumeReqRegs(
        component
      )
    }
    ...
    val accessRegisterCommandWr = WireInit(
      COMMANDWrData.asUInt.asTypeOf(new ACCESS_REGISTERFields())
    )

    /** real COMMAND */
    val accessRegisterCommandReg = WireInit(
      COMMANDReg.asUInt.asTypeOf(new ACCESS_REGISTERFields())
    )
    ...
    def abstractGeneratedI(cfg: DebugModuleParams): UInt = {
      ...
      val base =
        if (cfg.atzero) 0.U
        else Mux(accessRegisterCommandReg.regno(0), 8.U, 9.U)
    ...
    def abstractGeneratedCSR: UInt = {
      ...
      val base = Mux(
        accessRegisterCommandReg.regno(0),
        8.U,
        9.U
      ) // use s0 as base for odd regs, s1 as base for even regs
      ...
      if (cfg.nAbstractInstructions == 2) {
        ...
        abstractGeneratedMem(0) := Mux(
          ...
          Mux(
            ...
            abstractGeneratedI(cfg),
            abstractGeneratedS(cfg)
          ),
          nop.asUInt
        )
    ...
    tlNode.regmap(
      ...
      DATA -> RegFieldGroup(
        ...
        abstractDataMem.zipWithIndex.map { case (x, i) =>
          RegField(8, x, RegFieldDesc(s"debug_data_$i", ""))
        }
      ),
      PROGBUF(cfg) -> RegFieldGroup(
        ...
        programBufferMem.zipWithIndex.map { case (x, i) =>
          RegField(8, x, RegFieldDesc(s"debug_progbuf_$i", ""))
        }
      ),
        ...
        if (cfg.hasImplicitEbreak)
          Seq(
            RegField.r(
              ...
              RegFieldDesc(
                "debug_impebreak",
                "Debug Implicit EBREAK",
                reset = Some(Instructions.EBREAK.value)
              )
            )
          )
        else Nil
      ...
      ABSTRACT(cfg) -> RegFieldGroup(
        ...
        abstractGeneratedMem.zipWithIndex.map { case (x, i) =>
          RegField
            .r(32, x, RegFieldDesc(s"debug_abstract_$i", "", volatile = true))
        }
      ),
      FLAGS -> RegFieldGroup(
        "debug_flags",
        ...
        if (nComponents == 1) {
          Seq.tabulate(1024) { i =>
            RegField.r(
              8,
              flags(0).asUInt,
              RegFieldDesc(s"debug_flags_$i", "", volatile = true)
            )
          }
        } else {
      ...
      ROMBASE -> RegFieldGroup(
        "debug_rom",
        Some("Debug ROM"),
        (if (cfg.atzero) DebugRomContents()
         else DebugRomNonzeroContents()).toIndexedSeq.zipWithIndex.map {
          case (x, i) =>
            RegField.r(
              8,
              (x & 0xff).U(8.W),
    ...
    val commandWrIsAccessRegister =
      (COMMANDWrData.cmdtype === DebugAbstractCommandType.AccessRegister.id.U)
    val commandRegIsAccessRegister =
      (COMMANDReg.cmdtype === DebugAbstractCommandType.AccessRegister.id.U)
    ...
    val accessRegIsLegalSize =
      (accessRegisterCommandReg.size === 2.U) || (accessRegisterCommandReg.size === 3.U)
    val accessRegIsGPR =
    ...
    when(commandRegIsAccessRegister) {
      when(
        accessRegIsCustom && accessRegisterCommandReg.transfer && accessRegisterCommandReg.write === false.B
      ) {
    ...
    val wrAccessRegisterCommand =
      COMMANDWrEn && commandWrIsAccessRegister && (ABSTRACTCSReg.cmderr === 0.U)
    val regAccessRegisterCommand =
      autoexec && commandRegIsAccessRegister && (ABSTRACTCSReg.cmderr === 0.U)
    ...
    when(ctrlStateReg === CtrlState(Waiting)) {
      ...
      when(
        goReg === false.B && hartHaltedWrEn && (hartSelFuncs
          .hartIdToHartSel(hartHaltedId) === selectedHartReg)
      ) {
        ctrlStateNxt := CtrlState(Waiting)
      }
...
class TLDebugModuleInnerAsync(
  ...
  val cfg = p(DebugModuleKey).get
  val dmInner = LazyModule(
    new TLDebugModuleInner(device, getNComponents, beatBytes)
  )
  val dmiXing = LazyModule(
    new TLAsyncCrossingSink(
      AsyncQueueParams.singleton(safe = cfg.crossingHasSafeReset)
    )
  )
  ...
  class Impl extends LazyRawModuleImp(this) {
    ...
    val io = IO(new Bundle {
      ...
      /** conrol signals for Inner
        ...
        * generated in Outer
        */
      val innerCtrl = Flipped(
        new AsyncBundle(
          new DebugInternalBundle(getNComponents()),
          AsyncQueueParams.singleton(safe = cfg.crossingHasSafeReset)
        )
      )
      ...
      val extTrigger =
        (p(DebugModuleKey).get.nExtTriggers > 0).option(new DebugExtTriggerIO())
      ...
      /** Debug Authentication signals from core */
      val auth = p(DebugModuleKey).get.hasAuthentication
        .option(new DebugAuthenticationIO())
    })
    val rf_reset = IO(Input(Reset())) // RF transform
    ...
    val dmactive_synced = withClockAndReset(childClock, childReset) {
      val dmactive_synced = AsyncResetSynchronizerShiftReg(
        in = io.dmactive,
        sync = 3,
        name = Some("dmactiveSync")
      )
      ...
      io.extTrigger.foreach { x =>
        dmInner.module.io.extTrigger.foreach { y => x <> y }
      }
...
/** Create a version of the TLDebugModule which includes a synchronization
  * interface internally for the DMI. This is no longer optional outside of this
  * module because the Clock must run when tl_clock isn't running or tl_reset is
  * asserted.
  */
...
class TLDebugModule(beatBytes: Int)(implicit p: Parameters) extends LazyModule {
  ...
  val device = new SimpleDevice(
    "debug-controller",
    Seq("sifive,debug-013", "riscv,debug-013")
  ) {
    override val alwaysExtended = true
    override def describe(resources: ResourceBindings): Description = {
      val Description(name, mapping) = super.describe(resources)
      val attach = Map(
        "debug-attach" -> ((if (p(ExportDebug).apb) Seq(ResourceString("apb"))
                            else Seq()) ++
  ...
  val dmOuter: TLDebugModuleOuterAsync = LazyModule(
    new TLDebugModuleOuterAsync(device)(p)
  )
  val dmInner: TLDebugModuleInnerAsync = LazyModule(
    new TLDebugModuleInnerAsync(
      device,
  ...
  class Impl extends LazyRawModuleImp(this) {
    ...
    io.auth.foreach { x =>
      dmOuter.module.io.dmAuthenticated.get := x.dmAuthenticated
    }
    io.auth.foreach { x => dmInner.module.io.auth.foreach { y => x <> y } }
  }
}
