...
class TLAtomicAutomata(
  ...
  val node = TLAdapterNode(managerFn = { case mp =>
    mp.v1copy(managers = mp.managers.map { m =>
      val ourSupport = TransferSizes(1, mp.beatBytes)
      def widen(x: TransferSizes) = if (
        passthrough && x.min <= 2 * mp.beatBytes
      ) TransferSizes(1, max(mp.beatBytes, x.max))
      else ourSupport
      val canDoit =
        m.supportsPutFull.contains(ourSupport) && m.supportsGet.contains(
          ourSupport
        )
      ...
      m.v1copy(
        supportsArithmetic =
          if (!arithmetic || !canDoit) m.supportsArithmetic
          else widen(m.supportsArithmetic),
        supportsLogical =
          if (!logical || !canDoit) m.supportsLogical
          else widen(m.supportsLogical),
  ...
  class Impl extends LazyModuleImp(this) {
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      val violations = managersNeedingHelp.flatMap(_.findTreeViolation()).map {
        node => (node.name, node.inputs.map(_._1.name))
      }
      require(
        violations.isEmpty,
      ...
      def camFifoId(m: TLSlaveParameters) =
        m.fifoId.map(id => max(0, domainsNeedingHelp.indexOf(id))).getOrElse(0)
      ...
      val params =
        TLAtomicAutomata.CAMParams(out.a.bits.params, domainsNeedingHelp.size)
      ...
      if (camSize > 0) {
        ...
        val cam_abusy =
          cam_s.map(e => e.state === GET || e.state === AMO) // A is blocked
        val cam_dmatch =
          cam_s.map(e => e.state =/= FREE) // D should inspect these entries

        // Can the manager already handle this message?
        val a_address = edgeIn.address(in.a.bits)
        val a_size = edgeIn.size(in.a.bits)
        val a_canLogical = passthrough.B && edgeOut.manager.supportsLogicalFast(
          a_address,
          a_size
        )
        val a_canArithmetic = passthrough.B && edgeOut.manager
          .supportsArithmeticFast(a_address, a_size)
        val a_isLogical = in.a.bits.opcode === TLMessages.LogicalData
        val a_isArithmetic = in.a.bits.opcode === TLMessages.ArithmeticData
        val a_isSupported = Mux(
          a_isLogical,
          a_canLogical,
          Mux(a_isArithmetic, a_canArithmetic, true.B)
        )

        // Must we do a Put?
        val a_cam_any_put = cam_amo.reduce(_ || _)
        val a_cam_por_put = cam_amo.scanLeft(false.B)(_ || _).init
        val a_cam_sel_put = (cam_amo zip a_cam_por_put) map { case (a, b) =>
          a && !b
        }
        ...
        val a_fifoId =
          edgeOut.manager.fastProperty(a_address, camFifoId _, (i: Int) => i.U)
        val a_cam_busy = (cam_abusy zip cam_a.map(_.fifoId === a_fifoId)) map {
          case (a, b) => a && b
        } reduce (_ || _)
        ...
        val a_cam_sel_free = (cam_free zip a_cam_por_free) map { case (a, b) =>
          a && !b
        }

        // Logical AMO
        val indexes = Seq.tabulate(beatBytes * 8) { i =>
          Cat(a_a(i, i), a_d(i, i))
        }
        ...
        val signbits_a = Cat(
          Seq.tabulate(beatBytes) { i => a_a(8 * i + 7, 8 * i + 7) }.reverse
        )
        val signbits_d = Cat(
          Seq.tabulate(beatBytes) { i => a_d(8 * i + 7, 8 * i + 7) }.reverse
        )
        ...
        val a_bigger_uneq =
          unsigned === a_a_ext(h) // result if high bits are unequal
        val a_bigger =
          Mux(a_a_ext(h) === a_d_ext(h), !adder_out(h), a_bigger_uneq)
        ...
        TLArbiter(TLArbiter.lowestIndexFirst)(
          out.a,
          (0.U, source_c),
          (edgeOut.numBeats1(in.a.bits), source_i)
        )
        ...
        when(source_i.fire && !a_isSupported) {
          (a_cam_sel_free zip cam_a) foreach { case (en, r) =>
            when(en) {
              r.fifoId := a_fifoId
              r.bits := in.a.bits
              r.lut := MuxLookup(in.a.bits.param(1, 0), 0.U(4.W))(
                Seq(
                  TLAtomics.AND -> 0x8.U,
                  TLAtomics.OR -> 0xe.U,
                  TLAtomics.XOR -> 0x6.U,
        ...
        val d_cam_sel_match = (d_cam_sel_raw zip cam_dmatch) map {
          case (a, b) => a && b
        }
        ...
        val d_cam_sel_bypass =
          if (edgeOut.manager.minLatency > 0) false.B
          else
            out.d.bits.source === in.a.bits.source && in.a.valid && !a_isSupported
        val d_cam_sel = (a_cam_sel_free zip d_cam_sel_match) map {
          case (a, d) => Mux(d_cam_sel_bypass, a, d)
        }
...
object TLAtomicAutomata {
  def apply(
    ...
    val atomics = LazyModule(
      new TLAtomicAutomata(logical, arithmetic, concurrency, passthrough) {
        override lazy val desiredName =
          (Seq("TLAtomicAutomata") ++ nameSuffix).mkString("_")
      }
    )
    atomics.node
  }
...
class TLRAMAtomicAutomata(txns: Int)(implicit p: Parameters)
    extends LazyModule {
  ...
  val test = new RequestPattern({ a: TLBundleA =>
    ...
    val doesW =
      a.opcode === PutFullData || a.opcode === PutPartialData || doesA
...
class TLRAMAtomicAutomataTest(txns: Int = 5000, timeout: Int = 500000)(implicit
    p: Parameters
