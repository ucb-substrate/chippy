...
class TLRegisterRouterExtraBundle(val sourceBits: Int, val sizeBits: Int)
    extends Bundle {
  val source = UInt((sourceBits max 1).W)
  val size = UInt((sizeBits max 1).W)
}

case object TLRegisterRouterExtra
    extends ControlKey[TLRegisterRouterExtraBundle]("tlrr_extra")
case class TLRegisterRouterExtraField(sourceBits: Int, sizeBits: Int)
    extends BundleField[TLRegisterRouterExtraBundle](
      TLRegisterRouterExtra,
      Output(new TLRegisterRouterExtraBundle(sourceBits, sizeBits)),
      x => {
        x.size := 0.U
        x.source := 0.U
      }
...
/** TLRegisterNode is a specialized TL SinkNode that encapsulates MMIO
  * registers. It provides functionality for describing and outputting metdata
  * about the registers in several formats. It also provides a concrete
  * implementation of a regmap function that will be used to wire a map of
  * internal registers associated with this node to the node's interconnect
  * port.
  */
case class TLRegisterNode(
  ...
  def regmap(mapping: RegField.Map*) = {
    ...
    val fields = TLRegisterRouterExtraField(
      edge.bundle.sourceBits,
      edge.bundle.sizeBits
    ) +: a.bits.params.echoFields
...
/** Mix HasTLControlRegMap into any subclass of RegisterRouter to gain helper
  * functions for attaching a device control register map to TileLink.
  *   - The intended use case is that controlNode will diplomatically publish a
  *     SW-visible device's memory-mapped control registers.
  *   - Use the clock crossing helper controlXing to externally connect
  *     controlNode to a TileLink interconnect.
  *   - Use the mapping helper function regmap to internally fill out the space
  *     of device control registers.
  */
