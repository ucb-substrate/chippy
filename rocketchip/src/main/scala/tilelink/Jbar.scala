...
class TLJbar(policy: TLArbiter.Policy = TLArbiter.roundRobin)(implicit
    p: Parameters
) extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) {
    node.inoutGrouped.foreach { case (in, out) =>
      TLXbar.circuit(policy, in, out)
    }
  }
}
...
class TLJbarTestImp(nClients: Int, nManagers: Int, txns: Int)(implicit
    p: Parameters
) extends LazyModule {
  ...
  for (n <- 0 until nManagers) {
    TLRAM(AddressSet(0x0 + 0x400 * n, 0x3ff)) := TLFragmenter(
      4,
      256
    ) := TLDelayer(0.1) := jbar.node
  }
...
class TLJbarTest(
  ...
  val dut = Module(
    LazyModule(new TLJbarTestImp(nClients, nManagers, txns)).module
  )
