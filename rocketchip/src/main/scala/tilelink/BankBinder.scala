...
case class BankBinderNode(mask: BigInt)(implicit valName: ValName)
    extends TLCustomNode {
  private val bit = mask & -mask
  val maxXfer =
    TransferSizes(1, if (bit == 0 || bit > 4096) 4096 else bit.toInt)
  ...
  ): Seq[TLMasterPortParameters] =
    (p zip ids) map { case (cp, id) =>
      cp.v1copy(clients = cp.clients.map { c =>
        c.v1copy(
          visibility = c.visibility.flatMap { a =>
  ...
  def mapParamsU(
    ...
    (p zip ids) map { case (mp, id) =>
      mp.v1copy(managers = mp.managers.flatMap { m =>
        val addresses =
          m.address.flatMap(a => a.intersect(AddressSet(id, ~mask)))
