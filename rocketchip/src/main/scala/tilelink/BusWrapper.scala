...
import freechips.rocketchip.subsystem.{
  ...
  CanConnectWithinContextThatHasTileLinkLocations,
...
trait HasTLBusParams {
  ...
  def fixedClockOpt =
    dtsFrequency.map(f => ClockParameters(freqMHz = f.toDouble / 1000000.0))

  require(isPow2(beatBytes))
  require(isPow2(blockBytes))
}

abstract class TLBusWrapper(params: HasTLBusParams, val busName: String)(
    implicit p: Parameters
) extends ClockDomain
    with HasTLBusParams
    with CanHaveBuiltInDevices {
  private val clockGroupAggregator =
    LazyModule(new ClockGroupAggregator(busName) {
  ...
  val clockGroupNode =
    clockGroupAggregator.node // other bus clock groups attach here
  val clockNode = clockGroup.node
  val fixedClockNode = FixedClockBroadcast(
    fixedClockOpt
  ) // device clocks attach here
  private val clockSinkNode = ClockSinkNode(
    List(ClockSinkParameters(take = fixedClockOpt))
  )
  ...
  InModuleBody {
    ...
    (clockGroup.node.edges.in zip clockGroupAggregator.node.edges.out).zipWithIndex map {
      case ((in: ClockGroupEdgeParameters, out: ClockGroupEdgeParameters), i) =>
        require(
          in.members.keys == out.members.keys,
          s"clockGroup := clockGroupAggregator not working as you expect for index ${i}, becuase clockGroup has ${in.members.keys} and clockGroupAggregator has ${out.members.keys}"
        )
    }
  }
  ...
  val dtsClk =
    fixedClockNode.fixedClockResources(s"${busName}_clock").flatten.headOption
  ...
  def generateSynchronousDomain(domainName: String): ClockSinkDomain = {
    val domain = LazyModule(
      new ClockSinkDomain(take = fixedClockOpt, name = Some(domainName))
    )
  ...
  protected val addressPrefixNexusNode =
    BundleBroadcast[UInt](registered = false, default = Some(() => 0.U(1.W)))

  def to[T](name: String)(body: => T): T = {
    this {
      LazyScope(
        s"coupler_to_${name}",
        s"TLInterconnectCoupler_${busName}_to_${name}"
      ) { body }
    }
  }
  ...
  def from[T](name: String)(body: => T): T = {
    this {
      LazyScope(
        s"coupler_from_${name}",
        s"TLInterconnectCoupler_${busName}_from_${name}"
      ) { body }
    }
  }
  ...
  def crossToBus(
      ...
      allClockGroupNode: ClockGroupEphemeralNode
  ): NoHandle = {
    bus.clockGroupNode := asyncMux(
      xType,
      allClockGroupNode,
      this.clockGroupNode
    )
...
trait TLBusWrapperConnectionLike {
  ...
  def connect(
      ...
      master: Location[TLBusWrapper],
      slave: Location[TLBusWrapper]
  )(implicit p: Parameters): Unit
}

object TLBusWrapperConnection {

  /** Backwards compatibility factory for master driving clock and slave setting
    * cardinality
    */
  def crossTo(
      xType: ClockCrossingType,
  ...
  /** Backwards compatibility factory for slave driving clock and master setting
    * cardinality
    */
  def crossFrom(
      xType: ClockCrossingType,
...
/** TLBusWrapperConnection is a parameterization of a connection between two
  * TLBusWrappers. It has the following serializable parameters:
  *   - xType: What type of TL clock crossing adapter to insert between the
  *     buses. The appropriate half of the crossing adapter ends up inside each
  *     bus.
  *   - driveClockFromMaster: if None, don't bind the bus's diplomatic
  *     clockGroupNode, otherwise have either the master or the slave bus bind
  *     the other one's clockGroupNode, assuming the inserted crossing type is
  *     not asynchronous.
  *   - nodeBinding: fine-grained control of multi-edge cardinality resolution
  *     for diplomatic bindings within the connection.
  *   - flipRendering: fine-grained control of the graphML rendering of the
  *     connection.
  * If has the following non-serializable parameters:
  *   - slaveNodeView: programmatic control of the specific attachment point
  *     within the slave bus.
  *   - masterNodeView: programmatic control of the specific attachment point
  *     within the master bus.
  *   - injectNode: programmatic injection of additional nodes into the middle
  *     of the connection.
  * The connect method applies all these parameters to create a diplomatic
  * connection between two Location[TLBusWrapper]s.
  */
class TLBusWrapperConnection(
  ...
  def connect(
    ...
    def bindClocks(implicit p: Parameters) = driveClockFromMaster match {
      case Some(true) =>
        slaveTLBus.clockGroupNode := asyncMux(
          xType,
          context.allClockGroupsNode,
          masterTLBus.clockGroupNode
        )
      case Some(false) =>
        masterTLBus.clockGroupNode := asyncMux(
          xType,
          context.allClockGroupsNode,
          slaveTLBus.clockGroupNode
        )
      case None =>
    }
    def bindTLNodes(implicit p: Parameters) = nodeBinding match {
      case BIND_ONCE =>
        slaveNodeView(slaveTLBus, p) := TLWidthWidget(
          masterTLBus.beatBytes
        ) := inject(p) := masterNodeView(masterTLBus, p)
      case BIND_QUERY =>
        slaveNodeView(slaveTLBus, p) :=* TLWidthWidget(
          masterTLBus.beatBytes
        ) :=* inject(p) :=* masterNodeView(masterTLBus, p)
...
class TLBusWrapperTopology(
    val instantiations: Seq[
      (Location[TLBusWrapper], TLBusWrapperInstantiationLike)
    ],
    val connections: Seq[
      (
          Location[TLBusWrapper],
          Location[TLBusWrapper],
          TLBusWrapperConnectionLike
      )
    ]
  ...
  def instantiate(
    ...
    instantiations.foreach { case (loc, params) =>
      context { params.instantiate(context, loc) }
    }
  }
  def connect(context: HasTileLinkLocations)(implicit p: Parameters): Unit = {
    connections.foreach { case (master, slave, params) =>
      context { params.connect(context, master, slave) }
    }
  }
}

trait HasTLXbarPhy { this: TLBusWrapper =>
  private val xbar = LazyModule(new TLXbar(nameSuffix = Some(busName)))
    .suggestName(busName + "_xbar")
...
case class AddressAdjusterWrapperParams(
  ...
  def instantiate(context: HasTileLinkLocations, loc: Location[TLBusWrapper])(
    ...
    val aaWrapper = LazyModule(
      new AddressAdjusterWrapper(this, context.busContextName + "_" + loc.name)
    )
...
class AddressAdjusterWrapper(
    params: AddressAdjusterWrapperParams,
    name: String
)(implicit p: Parameters)
  ...
  private val address_adjuster = params.replication.map { r =>
    LazyModule(
      new AddressAdjuster(
        r,
        params.forceLocal,
        params.localBaseAddressDefault,
        params.ordered
      )
    )
  }
  private val viewNode = TLIdentityNode()
  val inwardNode: TLInwardNode = address_adjuster
    .map(_.node :*=* TLFIFOFixer(params.policy) :*=* viewNode)
    .getOrElse(viewNode)
  def outwardNode: TLOutwardNode =
    address_adjuster.map(_.node).getOrElse(viewNode)
...
case class TLJBarWrapperParams(
  ...
  def instantiate(context: HasTileLinkLocations, loc: Location[TLBusWrapper])(
    ...
    val jbarWrapper = LazyModule(
      new TLJBarWrapper(this, context.busContextName + "_" + loc.name)
    )
    jbarWrapper.suggestName(
      context.busContextName + "_" + loc.name + "_wrapper"
    )
...
class TLJBarWrapper(params: TLJBarWrapperParams, name: String)(implicit
    p: Parameters
) extends TLBusWrapper(params, name) {
