...
class ProbePicker(implicit p: Parameters) extends LazyModule {
  val node = TLAdapterNode(
    clientFn = { p =>
      ...
      def combine(
          next: TLMasterParameters,
          pair: (TLMasterParameters, Seq[TLMasterParameters])
      ) = {
        val (head, output) = pair
        if (
          head.visibility.exists(x => next.visibility.exists(_.overlaps(x)))
        ) {
          (
            next,
            head +: output
          ) // pair is not banked, push head without merging
        } else {
          def redact(x: TLMasterParameters) = x.v1copy(
            sourceId = IdRange(0, 1),
            nodePath = Nil,
            visibility = Seq(AddressSet(0, ~0))
          )
          require(
            redact(next) == redact(head),
            s"${redact(next)} != ${redact(head)}"
          )
      ...
      val (head, output) =
        p.clients.init.foldRight((p.clients.last, myNil))(combine)
      ...
      if (edgeIn.client.clients.size != edgeOut.client.clients.size) {
        in.b.bits.source := Mux1H(
          ...
          edgeOut.client.clients.map { c =>
            val banks =
              edgeIn.client.clients.filter(c.sourceId contains _.sourceId)
            if (banks.size == 1) {
              out.b.bits.source // allow sharing the value between single-bank cases
            } else {
              Mux1H(
                banks.map(
                  _.visibility.map(_ contains out.b.bits.address).reduce(_ || _)
                ),
