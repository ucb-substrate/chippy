...
/** Stores the client-side coherence information, such as permissions on the
  * data and whether the data is dirty. Its API can be used to make TileLink
  * messages in response to memory operations, cache control oeprations, or
  * Probe messages.
  */
class ClientMetadata extends Bundle {
  ...
  def onSecondaryAccess(
      first_cmd: UInt,
      second_cmd: UInt
  ): (Bool, Bool, UInt, ClientMetadata, UInt) = {
    import MemoryOpCategories._
    val r1 = growStarter(first_cmd)
    val r2 = growStarter(second_cmd)
    val needs_second_acq =
  ...
  def onGrant(cmd: UInt, param: UInt): ClientMetadata = ClientMetadata(
    growFinisher(cmd, param)
  )
    ...
    MuxTLookup(
      ...
      Seq(
        ...
        Cat(
          toB,
          Trunk
        ) -> (false.B, TtoB, Branch), // Policy: Don't notify on clean downgrade
        ...
        Cat(
          toN,
          Trunk
        ) -> (false.B, TtoN, Nothing), // Policy: Don't notify on clean downgrade
        Cat(
          toN,
          Branch
        ) -> (false.B, BtoN, Nothing), // Policy: Don't notify on clean downgrade
