...
import freechips.rocketchip.amba.ahb.{
  AHBImpMaster,
  AHBParameters,
  AHBMasterParameters,
...
case class TLToAHBNode(supportHints: Boolean)(implicit valName: ValName)
    extends MixedAdapterNode(TLImp, AHBImpMaster)(
      dFn = { cp =>
        AHBMasterPortParameters(
          masters = cp.clients.map { case c =>
            AHBMasterParameters(name = c.name, nodePath = c.nodePath)
          },
          requestFields =
            cp.requestFields.filter(!_.isInstanceOf[AMBAProtField]),
...
class TLToAHB(
    val aFlow: Boolean = false,
    val supportHints: Boolean = true,
  ...
  class Impl extends LazyModuleImp(this) {
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      when(send.send && !a_flow) /* retry AHB */ {
        ...
        step.first := false.B
        step.last := (if (lgBytes + 1 >= lgMax) true.B
                      else
                        !((UIntToOH1(
                          send.size,
                          lgMax
                        ) & ~send.addr) >> (lgBytes + 1)).orR)
        step.addr := Cat(
          send.addr(edgeIn.bundle.addressBits - 1, lgMax),
          send.addr(lgMax - 1, 0) + beatBytes.U
        )
      }.otherwise /* new burst */ {
        step.full := false.B
        step.send := false.B
      ...
      val a_singleBeat =
        a_hint || (lgBytes >= lgMax).B || a_sizeDelta(edgeIn.bundle.sizeBits)
      ...
      val rebuild =
        RegInit(false.B) // rewrite as NSEQ       (for next-beat  EBT)
      val increment =
        RegInit(false.B) // rewrite as BURST_INCR (for same-burst EBT)
      ...
      d.bits.opcode := Mux(
        d_hint,
        TLMessages.HintAck,
        Mux(d_write, TLMessages.AccessAck, TLMessages.AccessAckData)
      )
      ...
      if (edgeOut.slave.lite) {
        ...
        assert(
          !d_flow || !out.hresp(1),
          "TLToAHB not configured with support for SPLIT/RETRY responses"
        )
        retry := false.B
      } else {
