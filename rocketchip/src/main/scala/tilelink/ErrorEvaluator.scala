...
object RequestPattern {
  ...
  implicit def apply(pattern: Seq[AddressSet]): RequestPattern =
    new RequestPattern(overlaps(pattern) _)
}
...
class TLErrorEvaluator(
  ...
  val node = TLAdapterNode(managerFn = { mp =>
    mp.v1copy(managers = mp.managers.map { m =>
      m.v1copy(mayDenyPut = true, mayDenyGet = deny || m.mayDenyGet)
    })
  })
  ...
  class Impl extends LazyModuleImp(this) {
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      val bypass =
        (edgeOut.manager.minLatency == 0).B && in.a.fire && in.a.bits.source === in.d.bits.source
      val d_inject = Mux(
        bypass,
        inject_now,
        inject_map.read(in.d.bits.source)
      ) holdUnless d_first
      ...
      val d_detect =
        (!d_first && r_detect) || ((!deny).B && out.d.bits.corrupt) || out.d.bits.denied
      when(out.d.fire) { r_detect := d_detect }

      val d_hint =
        out.d.bits.opcode === TLMessages.HintAck // even illegal hints can succeed
      assert(
        (!testOn).B || !out.d.fire || !d_last || !d_inject || d_detect || d_hint,
        "Denied/Corrupt flag was not set!"
      )
      assert(
        (!testOff).B || !out.d.fire || !d_last || d_inject || !d_detect,
