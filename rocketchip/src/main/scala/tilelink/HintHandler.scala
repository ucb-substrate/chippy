...
class TLHintHandler(passthrough: Boolean = true)(implicit p: Parameters)
    extends LazyModule {
  val node = TLAdapterNode(
    clientFn = { cp =>
      cp.v1copy(clients = cp.clients.map { c =>
        c.v1copy(sourceId = IdRange(c.sourceId.start * 2, c.sourceId.end * 2))
      })
    },
    managerFn = { mp =>
  ...
  class Impl extends LazyModuleImp(this) {
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      // What about Get?
      val mapGet = isHint && edgeOut.manager.fastProperty(
        in.a.bits.address,
        useGet,
        (b: Boolean) => b.B
      )
      ...
      out.a.bits.opcode := Mux(
        mapPP,
        TLMessages.PutPartialData,
        Mux(mapGet, TLMessages.Get, a.bits.opcode)
      )
...
class TLRAMHintHandlerTest(txns: Int = 5000, timeout: Int = 500000)(implicit
    p: Parameters
