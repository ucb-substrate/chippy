...
class TLRAMModel(
    log: String = "",
    ignoreCorruptData: Boolean = false,
    ignoreDeniedData: Boolean = true
  ...
  class Impl extends LazyModuleImp(this) {
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      val inc_trees = Seq.tabulate(decTrees) { i =>
        Mem(endAddressHi >> (i + 1), UInt(countBits.W))
      }
      val dec_trees = Seq.tabulate(decTrees) { i =>
        Mem(endAddressHi >> (i + 1), UInt(countBits.W))
      }
      ...
      val bypass =
        if (edge.manager.minLatency > 0) false.B
        else in.a.valid && in.a.bits.source === out.d.bits.source
      val d_flight = RegEnable(
        Mux(bypass, a_flight, flight(out.d.bits.source)),
        edge.first(out.d)
      )
      ...
      val a_fifo =
        edge.manager.hasFifoIdFast(a_base) && edge.client.requestFifo(a.source)
      ...
      val a_inc_trees = inc_trees.zipWithIndex.map { case (m, i) =>
        m.read(a_addr_hi >> (i + 1))
      }
      val a_dec_trees = dec_trees.zipWithIndex.map { case (m, i) =>
        m.read(a_addr_hi >> (i + 1))
      }
      ...
      when(a_fire) {
        ...
        assert(
          a.opcode =/= TLMessages.AcquireBlock && a.opcode =/= TLMessages.AcquirePerm
        )
        ...
        when(
          a_first && a.opcode =/= TLMessages.Hint && a.opcode =/= TLMessages.Get
        ) {
          when(a_size <= shift.U) {
        ...
        when(a.opcode === TLMessages.Get) {
          printf(
            log + " G  0x%x - 0x%x\n",
            a_base,
            a_base | UIntToOH1(a_size, addressBits)
          )
        }
      }
      ...
      val a_crc_new = Cat(a_shadow.zipWithIndex.map { case (z, i) =>
        Mux(a_mask(i), z.value, 0.U)
      }.reverse)
      ...
      for (i <- 0 until beatBytes) {
        ...
        data.valid := Mux(
          wipe,
          false.B,
          (!busy || a_fifo) && (!amo || (a_known_old && beat_amo))
        )
      ...
      val d_fifo =
        edge.manager.hasFifoIdFast(d_flight.base) && edge.client.requestFifo(
          d.source
        )
      ...
      val d_inc_trees = inc_trees.zipWithIndex.map { case (m, i) =>
        m.read(d_addr_hi >> (i + 1))
      }
      val d_dec_trees = dec_trees.zipWithIndex.map { case (m, i) =>
        m.read(d_addr_hi >> (i + 1))
      }
      ...
      val crc_bypass =
        if (edge.manager.minLatency > 0) false.B
        else a_fire && a.source === d.source
      val d_crc_valid = Mux(
        crc_bypass,
        a_crc_valid,
        crc_valid.read(d.source)
      ) holdUnless d_first
      val d_crc_check =
        Mux(crc_bypass, a_crc, crc.read(d.source)) holdUnless d_first
      ...
      when(d_fire) {
        ...
        when(
          d_last && d_flight.opcode =/= TLMessages.Hint && d_flight.opcode =/= TLMessages.Get
        ) {
          when(d_size <= shift.U) {
        ...
        when(
          d_flight.opcode === TLMessages.PutFullData || d_flight.opcode === TLMessages.PutPartialData
        ) {
          ...
          printf(
            " 0x%x - 0x%x\n",
            d_base,
            d_base | UIntToOH1(d_size, addressBits)
          )
        }
        ...
        when(
          ...
          for (i <- 0 until beatBytes) {
            ...
            when(d_mask(i)) {
              ...
              when(!shadow.valid) {
                printf(
                  ", undefined (uninitialized or prior overlapping puts)\n"
                )
        ...
        when(
          d_flight.opcode === TLMessages.ArithmeticData || d_flight.opcode === TLMessages.LogicalData
        ) {
          ...
          when(d_last) {
            ...
            val corrupt =
              (ignoreCorruptData.B && d.corrupt) || (ignoreDeniedData.B && d.denied)
...
object TLRAMModel {
  def apply(
    ...
    val model = LazyModule(
      new TLRAMModel(log, ignoreCorruptData, ignoreDeniedData)
    )
    model.node
  }
