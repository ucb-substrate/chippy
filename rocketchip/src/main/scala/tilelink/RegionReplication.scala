...
case class ReplicatedRegion(local: AddressSet, region: AddressSet) {
  ...
  require(
    (local.base & replicationMask) == 0,
    s"Local region ${local} not aligned within ${region}"
  )
...
class RegionReplicator(val params: ReplicatedRegion)(implicit p: Parameters)
    extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) {
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      val totalContainment =
        edgeIn.slave.slaves.forall(_.address.forall(params.region contains _))
