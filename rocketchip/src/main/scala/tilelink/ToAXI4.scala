...
case class AXI4TLStateField(sourceBits: Int)
    extends BundleField[AXI4TLStateBundle](
      AXI4TLState,
      Output(new AXI4TLStateBundle(sourceBits)),
      x => {
        x.size := 0.U
        x.source := 0.U
      }
...
/** TLtoAXI4IdMap serves as a record for the translation performed between id
  * spaces.
  *
  * Its member [axi4Masters] is used as the new AXI4MasterParameters in
  * diplomacy. Its member [mapping] is used as the template for the circuit
  * generated in TLToAXI4Node.module.
  */
class TLtoAXI4IdMap(tlPort: TLMasterPortParameters)
    extends IdMap[TLToAXI4IdMapEntry] {
  val tlMasters =
    tlPort.masters.sortBy(_.sourceId).sortWith(TLToAXI4.sortByType)
  private val axi4IdSize = tlMasters.map { tl =>
    if (tl.requestFifo) 1 else tl.sourceId.size
  }
  private val axi4IdStart = axi4IdSize.scanLeft(0)(_ + _).init
  val axi4Masters =
    axi4IdStart.zip(axi4IdSize).zip(tlMasters).map { case ((start, size), tl) =>
      AXI4MasterParameters(
        name = tl.name,
  ...
  protected val fmt =
    s"\t[%${axiDigits}d, %${axiDigits}d) <= [%${tlDigits}d, %${tlDigits}d) %s%s%s"

  val mapping: Seq[TLToAXI4IdMapEntry] =
    tlMasters.zip(axi4Masters).map { case (tl, axi) =>
      TLToAXI4IdMapEntry(
        axi.id,
        tl.sourceId,
        tl.name,
...
case class TLToAXI4Node(wcorrupt: Boolean = true)(implicit valName: ValName)
    extends MixedAdapterNode(TLImp, AXI4Imp)(
      dFn = { p =>
        AXI4MasterPortParameters(
          masters = (new TLtoAXI4IdMap(p)).axi4Masters,
          requestFields = (if (wcorrupt) Seq(AMBACorruptField())
                           else Seq()) ++ p.requestFields
            .filter(!_.isInstanceOf[AMBAProtField]),
          echoFields =
            AXI4TLStateField(log2Ceil(p.endSourceId)) +: p.echoFields,
...
class TLToAXI4(
  ...
  class Impl extends LazyModuleImp(this) {
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      slaves.foreach { s =>
        require(s.interleavedId == slaves(0).interleavedId)
      }

      // Construct the source=>ID mapping table
      val map = new TLtoAXI4IdMap(edgeIn.client)
      val sourceStall =
        WireDefault(VecInit.fill(edgeIn.client.endSourceId)(false.B))
      val sourceTable = WireDefault(
        VecInit.fill(edgeIn.client.endSourceId)(0.U.asTypeOf(out.aw.bits.id))
      )
      ...
      adapterName.foreach { n =>
        ...
        ElaborationArtefacts.add(
          s"$n.axi4.json",
          s"""{"mapping":[${map.mapping.mkString(",")}]}"""
        )
      }
      ...
      val queue_arw =
        Queue.irrevocable(out_arw, entries = depth, flow = combinational)
      ...
      arw.len := UIntToOH1(
        a_size,
        AXI4Parameters.lenBits + log2Ceil(beatBytes)
      ) >> log2Ceil(beatBytes)
      ...
      in.a.ready := !stall && Mux(
        a_isPut,
        (doneAW || out_arw.ready) && out_w.ready,
        out_arw.ready
      )
      out_arw.valid := !stall && in.a.valid && Mux(
        a_isPut,
        !doneAW && out_w.ready,
        true.B
      )
      ...
      val r_denied =
        out.r.bits.resp === AXI4Parameters.RESP_DECERR holdUnless r_first
      val r_corrupt = out.r.bits.resp =/= AXI4Parameters.RESP_OKAY
      val b_denied = out.b.bits.resp =/= AXI4Parameters.RESP_OKAY

      val r_d = edgeIn.AccessAck(
        r_source,
        r_size,
        0.U,
        denied = r_denied,
        corrupt = r_corrupt || r_denied
      )
      ...
      val d_sel = UIntToOH(
        Mux(r_wins, out.r.bits.id, out.b.bits.id),
        edgeOut.master.endId
      ).asBools
...
object TLToAXI4 {
  def apply(
    ...
    val tl2axi4 = LazyModule(
      new TLToAXI4(combinational, adapterName, stripBits, wcorrupt)
    )
    tl2axi4.node
  }
