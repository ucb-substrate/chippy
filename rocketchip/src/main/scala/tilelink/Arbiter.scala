...
object TLArbiter {
  ...
  val lowestIndexFirst: Policy = (width, valids, select) =>
    ~(leftOR(valids) << 1)(width - 1, 0)

  val highestIndexFirst: Policy = (width, valids, select) =>
    ~((rightOR(valids) >> 1).pad(width))
  ...
  def lowestFromSeq[T <: TLChannel](
      edge: TLEdge,
      sink: DecoupledIO[T],
      sources: Seq[DecoupledIO[T]]
  ): Unit = {
    apply(lowestIndexFirst)(
      sink,
      sources.map(s => (edge.numBeats1(s.bits), s)): _*
    )
  }

  def lowest[T <: TLChannel](
      edge: TLEdge,
      sink: DecoupledIO[T],
    ...
    apply(lowestIndexFirst)(
      sink,
      sources.toList.map(s => (edge.numBeats1(s.bits), s)): _*
    )
  }

  def highest[T <: TLChannel](
      edge: TLEdge,
      sink: DecoupledIO[T],
      sources: DecoupledIO[T]*
  ): Unit = {
    apply(highestIndexFirst)(
      sink,
      sources.toList.map(s => (edge.numBeats1(s.bits), s)): _*
    )
  }

  def robin[T <: TLChannel](
      edge: TLEdge,
      sink: DecoupledIO[T],
      sources: DecoupledIO[T]*
  ): Unit = {
  ...
  def apply[T <: Data](
      policy: Policy
  )(sink: DecoupledIO[T], sources: (UInt, DecoupledIO[T])*): Unit = {
    if (sources.isEmpty) {
      sink.bits := DontCare
      ...
      val readys = VecInit(
        policy(valids.size, Cat(valids.reverse), latch).asBools
      )
      ...
      val maskedBeats = (winner zip beatsIn) map { case (w, b) =>
        Mux(w, b, 0.U)
      }
...
abstract class DecoupledArbiterTest(
  ...
  TLArbiter(policy)(
    sink,
    sources.zipWithIndex.map { case (z, i) =>
      (beatsLeftFromIdx(i), z)
    }: _*
  )
...
/** This tests that when a specific pattern of source valids are driven, a new
  * index from amongst that pattern is always selected, unless one of those
  * sources takes multiple beats, in which case the same index should be
  * selected until the arbiter goes idle.
  */
class TLDecoupledArbiterRobinTest(
    txns: Int = 128,
    timeout: Int = 500000,
    print: Boolean = false
)(implicit p: Parameters)
    extends DecoupledArbiterTest(
      TLArbiter.roundRobin,
      txns,
      timeout,
      6,
      i => i.U
  ...
  sources.zipWithIndex
    .map { // pattern: every even-indexed valid is driven the same random way
  ...
  when(sink.fire) {
    ...
    when(beatsLeft === 0.U) {
      assert(
        ...
        "Round robin did not pick a new idx despite one being valid."
      )
      lastWinner := sink.bits
      beatsLeft := sink.bits
    }.otherwise {
      assert(
        lastWinner === sink.bits,
        "Round robin did not pick the same index over multiple beats"
      )
...
/** This tests that the lowest index is always selected across random single
  * cycle transactions.
  */
class TLDecoupledArbiterLowestTest(txns: Int = 128, timeout: Int = 500000)(
    implicit p: Parameters
) extends DecoupledArbiterTest(
  ...
  def assertLowest(id: Int): Unit = {
    when(sources(id).valid) {
      assert(
        (numSources - 1 until id by -1)
          .map(!sources(_).fire)
          .foldLeft(true.B)(_ && _),
        s"$id was valid but a higher valid source was granted ready."
      )
    }
  }
...
/** This tests that the highest index is always selected across random single
  * cycle transactions.
  */
class TLDecoupledArbiterHighestTest(txns: Int = 128, timeout: Int = 500000)(
    implicit p: Parameters
) extends DecoupledArbiterTest(
      TLArbiter.highestIndexFirst,
      txns,
      timeout,
      15,
      _ => 0.U
  ...
  def assertHighest(id: Int): Unit = {
    when(sources(id).valid) {
      assert(
        (0 until id).map(!sources(_).fire).foldLeft(true.B)(_ && _),
        s"$id was valid but a lower valid source was granted ready."
      )
    }
  }
