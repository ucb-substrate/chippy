...
class TLEdge(
  ...
  def staticHasData(bundle: TLChannel): Option[Boolean] = {
    bundle match {
      case _: TLBundleA => {
        ...
        val aDataNo =
          manager.anySupportAcquireB || manager.anySupportGet || manager.anySupportHint
      ...
      case _: TLBundleB => {
        ...
        val bDataNo =
          client.anySupportProbe || client.anySupportGet || client.anySupportHint
        // Statically optimize the case where hasData is a constant
  ...
  def numBeats(x: TLChannel): UInt = {
    x match {
      ...
      case bundle: TLDataChannel => {
        ...
        val small =
          if (manager.maxTransfer <= manager.beatBytes) true.B
          else size <= (cutoff).U
    ...
    x match {
      ...
      case bundle: TLDataChannel => {
        if (maxLgSize == 0) {
          ...
          val decode =
            UIntToOH1(size(bundle), maxLgSize) >> log2Ceil(manager.beatBytes)
  ...
  def firstlast(x: DecoupledIO[TLChannel]): (Bool, Bool, Bool) =
    firstlast(x.bits, x.fire)
  def firstlast(x: ValidIO[TLChannel]): (Bool, Bool, Bool) =
    firstlast(x.bits, x.valid)

  def count(bits: TLChannel, fire: Bool): (Bool, Bool, Bool, UInt) = {
    val r = firstlastHelper(bits, fire)
    (r._1, r._2, r._3, r._4)
  }
  def count(x: DecoupledIO[TLChannel]): (Bool, Bool, Bool, UInt) =
    count(x.bits, x.fire)
  def count(x: ValidIO[TLChannel]): (Bool, Bool, Bool, UInt) =
    count(x.bits, x.valid)

  def addr_inc(bits: TLChannel, fire: Bool): (Bool, Bool, Bool, UInt) = {
    val r = firstlastHelper(bits, fire)
    (r._1, r._2, r._3, r._4 << log2Ceil(manager.beatBytes))
  }
  def addr_inc(x: DecoupledIO[TLChannel]): (Bool, Bool, Bool, UInt) =
    addr_inc(x.bits, x.fire)
  def addr_inc(x: ValidIO[TLChannel]): (Bool, Bool, Bool, UInt) =
    addr_inc(x.bits, x.valid)

  // Does the request need T permissions to be executed?
  def needT(a: TLBundleA): Bool = {
    val acq_needT = MuxLookup(a.param, WireDefault(Bool(), DontCare))(
      Seq(
    ...
    MuxLookup(a.opcode, WireDefault(Bool(), DontCare))(
      Seq(
        ...
        TLMessages.Hint -> MuxLookup(a.param, WireDefault(Bool(), DontCare))(
          Seq(
  ...
  def inFlight(x: TLBundle): (UInt, UInt) = {
    ...
    val inc = Cat(
      Seq(a_inc, d_inc) ++ (if (bce) Seq(b_inc, c_inc, e_inc) else Nil)
    )
    ...
    val dec = Cat(
      Seq(a_dec, d_dec) ++ (if (bce) Seq(b_dec, c_dec, e_dec) else Nil)
    )
...
class TLEdgeOut(
  ...
  def AccessAck(b: TLBundleB): TLBundleC =
    AccessAck(b.source, address(b), b.size)
  ...
  def AccessAck(b: TLBundleB, data: UInt): TLBundleC =
    AccessAck(b.source, address(b), b.size, data)
  ...
  def AccessAck(
      ...
      lgSize: UInt,
      data: UInt
  ): TLBundleC = AccessAck(fromSource, toAddress, lgSize, data, false.B)
  def AccessAck(
      fromSource: UInt,
      toAddress: UInt,
      lgSize: UInt,
...
class TLEdgeIn(
  ...
  def Grant(
      ...
      lgSize: UInt,
      capPermissions: UInt
  ): TLBundleD = Grant(fromSink, toSource, lgSize, capPermissions, false.B)
  def Grant(
      fromSink: UInt,
      toSource: UInt,
      lgSize: UInt,
  ...
  def Grant(
    ...
    Grant(fromSink, toSource, lgSize, capPermissions, data, false.B, false.B)
  def Grant(
      fromSink: UInt,
      toSource: UInt,
      lgSize: UInt,
  ...
  def ReleaseAck(c: TLBundleC): TLBundleD =
    ReleaseAck(c.source, c.size, false.B)
  ...
  def AccessAck(a: TLBundleA, denied: Bool): TLBundleD =
    AccessAck(a.source, a.size, denied)
  def AccessAck(toSource: UInt, lgSize: UInt): TLBundleD =
    AccessAck(toSource, lgSize, false.B)
  ...
  def AccessAck(a: TLBundleA, data: UInt): TLBundleD =
    AccessAck(a.source, a.size, data)
  def AccessAck(
      a: TLBundleA,
      data: UInt,
      denied: Bool,
      corrupt: Bool
  ): TLBundleD = AccessAck(a.source, a.size, data, denied, corrupt)
  def AccessAck(toSource: UInt, lgSize: UInt, data: UInt): TLBundleD =
    AccessAck(toSource, lgSize, data, false.B, false.B)
  def AccessAck(
      toSource: UInt,
      lgSize: UInt,
      data: UInt,
  ...
  def HintAck(a: TLBundleA, denied: Bool): TLBundleD =
    HintAck(a.source, a.size, denied)
  def HintAck(toSource: UInt, lgSize: UInt): TLBundleD =
    HintAck(toSource, lgSize, false.B)
