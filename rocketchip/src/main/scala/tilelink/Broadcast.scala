...
case class TLBroadcastControlParams(address: AddressSet, beatBytes: Int)
...
class TLBroadcast(params: TLBroadcastParams)(implicit p: Parameters)
    extends LazyModule {
      ...
      mp.v1copy(
        ...
        managers = mp.managers.map { m =>
          ...
          if (m.regionType == RegionType.UNCACHED) {
            ...
            require(
              !m.supportsPutFull || m.supportsPutFull.contains(
                params.lineBytes
              ),
              s"${m.name} only supports PutFull(${m.supportsPutFull}), which does not include ${params.lineBytes}"
            )
            require(
              !m.supportsGet || m.supportsGet.contains(params.lineBytes),
              s"${m.name} only supports Get(${m.supportsGet}), which does not include ${params.lineBytes}"
            )
            m.v1copy(
              ...
              supportsAcquireT =
                if (m.supportsPutFull)
                  TransferSizes(lowerBound, params.lineBytes)
              ...
              supportsPutFull = TransferSizes(
                m.supportsPutFull.min,
                min(m.supportsPutFull.max, params.lineBytes)
              ),
              supportsPutPartial = TransferSizes(
                m.supportsPutPartial.min,
                min(m.supportsPutPartial.max, params.lineBytes)
              ),
              supportsGet = TransferSizes(
                m.supportsGet.min,
                min(m.supportsGet.max, params.lineBytes)
              ),
              supportsHint = TransferSizes(
                m.supportsHint.min,
                min(m.supportsHint.max, params.lineBytes)
              ),
              supportsArithmetic = TransferSizes(
                m.supportsArithmetic.min,
                min(m.supportsArithmetic.max, params.lineBytes)
              ),
              supportsLogical = TransferSizes(
                m.supportsLogical.min,
                min(m.supportsLogical.max, params.lineBytes)
              ),
  ...
  val intNode =
    if (params.control.isEmpty) None
    else Some(IntSourceNode(IntSourcePortSimple(num = 1)))
  val controlNode = params.control.map(x =>
    TLRegisterNode(
  ...
  class Impl extends LazyModuleImp(this) {
    val (ints, fields) = node.in
      .zip(node.out)
      .zipWithIndex
      .map { case (((in, edgeIn), (out, edgeOut)), bankIndex) =>
        ...
        val flatAddresses =
          AddressSet.unify(edgeOut.manager.managers.flatMap(_.address))
        val addressMask = AddressDecoder(
          flatAddresses.map(Seq(_)),
          flatAddresses.map(_.mask).reduce(_ | _)
        )
        val filter = Module(
          params.filterFactory(
            ProbeFilterParams(
        ...
        val d_trackerOH = VecInit(trackers.map { t =>
          t.need_d && t.source === d_normal.bits.source
        }).asUInt holdUnless d_first

        assert(
          !out.d.valid || !d_drop || out.d.bits.opcode === TLMessages.AccessAck
        )
        ...
        when(d_what(1)) { // TRANSFORM_*
          d_normal.bits.opcode := Mux(
            d_hasData,
            TLMessages.GrantData,
            TLMessages.ReleaseAck
          )
          d_normal.bits.param := Mux(
            d_hasData,
            Mux(d_what(0), TLPermissions.toT, TLPermissions.toB),
            0.U
          )
        }
        ...
        assert(
          !d_normal.valid || (d_trackerOH.orR || d_normal.bits.opcode === TLMessages.ReleaseAck)
        )
        ...
        filter.io.update.bits.cacheOH := Mux1H(
          d_trackerOH,
          trackers.map(_.cacheOH)
        )
        ...
        (trackers zip c_trackerOH) foreach { case (tracker, select) =>
          ...
          tracker.probesack := in.c.fire && select && (c_probeack || c_probeackdata) && (in.c.bits.param === TLPermissions.TtoB ||
        ...
        putfull.bits := edgeOut
          .Put(
            Cat(put_what, put_who),
            in.c.bits.address,
            in.c.bits.size,
            in.c.bits.data
          )
        ...
        val probe_target =
          if (caches.size == 0) 0.U else Mux1H(probe_next, cache_targets)
        ...
        val a_cache =
          if (caches.size == 0) 0.U
          else VecInit(caches.map(_.contains(in.a.bits.source))).asUInt
        ...
        val matchTrackers = VecInit(trackers.map { t =>
          t.line === in.a.bits.address >> lineShift
        }).asUInt
        ...
        val leaveB =
          !filter.io.response.bits.needT && !filter.io.response.bits.gaveT
        val others =
          filter.io.response.bits.cacheOH & ~filter.io.response.bits.allocOH
        ...
        val responseMSHR =
          UIntToOH(filter.io.response.bits.mshr, params.numTrackers).asBools
...
case class ProbeFilterParams(
  ...
  def maskBits(address: UInt) = Cat(
    (addressMaskBitList zip address.asBools).filter(_._1).map(_._2).reverse
  )
}
...
class ProbeFilterResponse(params: ProbeFilterParams)
    extends ProbeFilterRequest(params) {
  val gaveT = Bool()
...
object BroadcastFilter {
  def factory: TLBroadcast.ProbeFilterFactory = params =>
    new BroadcastFilter(params)
}
...
class TLBroadcastTracker(
  ...
  val o_data = Queue(
    i_data,
    if (bufferless) 1 else (lineBytes / edgeIn.manager.beatBytes),
    bufferless
  )
  ...
  val acquire =
    opcode === TLMessages.AcquireBlock || opcode === TLMessages.AcquirePerm
