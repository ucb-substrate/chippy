...
class TLRAMErrors(val params: ECCParams, val addrBits: Int)
    extends Bundle
    with CanHaveErrors {
  val correctable = (params.code.canCorrect && params.notifyErrors)
    .option(Valid(UInt(addrBits.W)))
  val uncorrectable = (params.code.canDetect && params.notifyErrors)
    .option(Valid(UInt(addrBits.W)))
}
...
class TLRAM(
    ...
    sramReg: Boolean =
      false, // drive SRAM data output directly into a register => 1 cycle longer response
        ...
        Seq(
          TLSlaveParameters
            .v1(
              ...
              supportsArithmetic =
                if (atomics) TransferSizes(1, beatBytes)
  ...
  val notifyNode = ecc.notifyErrors.option(
    BundleBridgeSource(() =>
      new TLRAMErrors(ecc, log2Ceil(address.max)).cloneType
    )
  )
  ...
  class Impl extends LazyModuleImp(this) with HasJustOneSeqMem {
    ...
    val d_lanes = Cat(
      Seq
        .tabulate(lanes) { i =>
          d_mask(eccBytes * (i + 1) - 1, eccBytes * i).orR
        }
        .reverse
    )
    ...
    val r_lanes = Cat(
      Seq
        .tabulate(lanes) { i =>
          r_mask(eccBytes * (i + 1) - 1, eccBytes * i).orR
        }
        .reverse
    )
        ...
        Seq
          .tabulate(beatBytes) { i =>
            ...
            val fix = d_corrected(
              8 * (i + 1) - 1,
              8 * i
            ) // safe to use, because D-stage write-back always wins arbitration
            Mux(upd, rmw, fix)
          }
          .reverse
      )
    ...
    val mem_active_valid = Seq(
      property.CoverBoolean(in.d.valid, Seq("mem_active"))
    )
    val data_error = Seq(
      property.CoverBoolean(!d_need_fix && !d_error, Seq("no_data_error")),
      property.CoverBoolean(
        d_need_fix && !in.d.bits.corrupt,
        Seq("data_correctable_error_not_reported")
      ),
      property.CoverBoolean(
        d_error && in.d.bits.corrupt,
        Seq("data_uncorrectable_error_reported")
      )
    )

    val error_cross_covers = new property.CrossProperty(
      Seq(mem_active_valid, data_error),
      Seq(),
      "Ecc Covers"
    )
    ...
    val r_ready =
      !d_wb && !r_replay && (!d_full || d_ready) && (!r_respond || (!d_win && in.d.ready))
    ...
    val a_data = VecInit(Seq.tabulate(lanes) { i =>
      in.a.bits.data(eccBytes * 8 * (i + 1) - 1, eccBytes * 8 * i)
    })
    val r_data = VecInit(Seq.tabulate(lanes) { i =>
      r_rmw_data(eccBytes * 8 * (i + 1) - 1, eccBytes * 8 * i)
    })
    val d_data = VecInit(Seq.tabulate(lanes) { i =>
    ...
    val d_poisonv = VecInit(Seq.tabulate(lanes) { i =>
      ...
      (!upd.andR && d_uncorrectable(
        i
      )) || d_poison // sub-lane writes should not correct uncorrectable
    })

    val a_lanes = Cat(
      Seq
        .tabulate(lanes) { i =>
          in.a.bits.mask(eccBytes * (i + 1) - 1, eccBytes * i).orR
        }
        .reverse
    )
    ...
    val index = Cat(
      mask
        .zip((addr >> log2Ceil(beatBytes)).asBools)
        .filter(_._1)
        .map(_._2)
        .reverse
    )
...
class TLRAMSimple(ramBeatBytes: Int, sramReg: Boolean, txns: Int)(implicit
    p: Parameters
) extends LazyModule {
...
class TLRAMSimpleTest(
  ...
  val dut = Module(
    LazyModule(new TLRAMSimple(ramBeatBytes, sramReg, txns)).module
  )
  dut.io.start := io.start
  io.finished := dut.io.finished
}

class TLRAMECC(ramBeatBytes: Int, eccBytes: Int, sramReg: Boolean, txns: Int)(
    implicit p: Parameters
) extends LazyModule {
  val fuzz = LazyModule(new TLFuzzer(txns))
  val model = LazyModule(new TLRAMModel("SRAMSimple"))
...
class TLRAMECCTest(
    ...
    extends UnitTest(timeout) {
  val dut = Module(
    LazyModule(new TLRAMECC(ramBeatBytes, eccBytes, sramReg, txns)).module
  )
  dut.io.start := io.start
  io.finished := dut.io.finished
}
