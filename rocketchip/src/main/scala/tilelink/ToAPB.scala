...
case class TLToAPBNode()(implicit valName: ValName)
    extends MixedAdapterNode(TLImp, APBImp)(
      dFn = { cp =>
        APBMasterPortParameters(
          masters = cp.clients.map { case c =>
            APBMasterParameters(name = c.name, nodePath = c.nodePath)
          },
          requestFields =
      ...
      uFn = { sp =>
        val managers = sp.slaves.map { case s =>
          TLSlaveParameters.v1(
            ...
            nodePath = s.nodePath,
            supportsGet =
              if (s.supportsRead) TransferSizes(1, sp.beatBytes)
              else TransferSizes.none,
            supportsPutPartial =
              if (s.supportsWrite) TransferSizes(1, sp.beatBytes)
              else TransferSizes.none,
            supportsPutFull =
              if (s.supportsWrite) TransferSizes(1, sp.beatBytes)
              else TransferSizes.none,
...
class TLToAPB(val aFlow: Boolean = true)(implicit p: Parameters)
    extends LazyModule {
