  ...
  val node = new TLAdapterNode(
    ...
    managerFn = { mp =>
      val managers = mp.managers.flatMap { m =>
        ...
        out.map { o => // Confirm the filter only REMOVES capability
          o.address.foreach { a =>
            require(m.address.map(_.contains(a)).reduce(_ || _))
          }
...
object TLFilter {
  ...
  def mSubtract(excepts: Seq[AddressSet]): ManagerFilter = { m =>
    val filtered = excepts.foldLeft(m.address) { (a, e) =>
      a.flatMap(_.subtract(e))
    }
    val alignment: BigInt =
      if (filtered.isEmpty) 0 else filtered.map(_.alignment).min
    transferSizeHelper(m, filtered, alignment)
  }
