...
class TestGenerator(gen: LazyModule => Seq[LazyUnitTest]) {
  ...
  def ++(other: TestGenerator) =
    new TestGenerator(gen = lm => gen(lm) ++ other(lm))
}

object TestGenerator {
  def apply(
      matcher: PartialFunction[LazyModule, Seq[LazyUnitTest]]
  ): TestGenerator =
  ...
  def recurse(other: TestGenerator): TestGenerator = {
    def helper(lm: LazyModule, tail: Seq[LazyUnitTest]): Seq[LazyUnitTest] =
      lm.getChildren.foldLeft(other(lm) ++ tail) { case (tail, child) =>
        helper(child, tail)
      }
