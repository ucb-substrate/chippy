...
class WithTestDuration(x: Int)
    extends Config((site, here, up) => { case TestDurationMultiplier =>
      x
    })

class WithAMBAUnitTests
    extends Config((site, here, up) => { case UnitTests =>
      (q: Parameters) => {
        implicit val p = q
        ...
        Seq(
          ...
          Module(
            new AXI4LiteUserBitsFuzzRAMTest(txns = 6 * txns, timeout = timeout)
          ),
          ...
          Module(
            new AXI4RAMAsyncCrossingTest(txns = 3 * txns, timeout = timeout)
          ),
          Module(
            new AXI4RAMCreditedCrossingTest(txns = 3 * txns, timeout = timeout)
          )
        )
      }
    })

class WithTLSimpleUnitTests
    extends Config((site, here, up) => { case UnitTests =>
      (q: Parameters) => {
        implicit val p = q
        val txns = 100 * site(TestDurationMultiplier)
        ...
        Seq(
          Module(
            new TLRAMSimpleTest(1, true, txns = 15 * txns, timeout = timeout)
          ),
          Module(
            new TLRAMSimpleTest(4, false, txns = 15 * txns, timeout = timeout)
          ),
          Module(
            new TLRAMSimpleTest(16, true, txns = 15 * txns, timeout = timeout)
          ),
          Module(
            new TLRAMZeroDelayTest(4, txns = 15 * txns, timeout = timeout)
          ),
          ...
          Module(
            new TLDecoupledArbiterLowestTest(txns = 3 * txns, timeout = timeout)
          ),
          Module(
            new TLDecoupledArbiterHighestTest(
              txns = 3 * txns,
              timeout = timeout
            )
          ),
          Module(
            new TLDecoupledArbiterRobinTest(txns = 3 * txns, timeout = timeout)
          ),
          Module(
            new TLRAMRationalCrossingTest(txns = 3 * txns, timeout = timeout)
          ),
          Module(
            new TLRAMAsyncCrossingTest(txns = 5 * txns, timeout = timeout)
          ),
          Module(
            new TLRAMCreditedCrossingTest(txns = 5 * txns, timeout = timeout)
          ),
          Module(
            new TLRAMAtomicAutomataTest(txns = 10 * txns, timeout = timeout)
          ),
          Module(
            new TLRAMECCTest(8, 4, true, txns = 15 * txns, timeout = timeout)
          ),
          Module(
            new TLRAMECCTest(4, 1, true, txns = 15 * txns, timeout = timeout)
          ),
          Module(
            new TLRAMECCTest(1, 1, true, txns = 15 * txns, timeout = timeout)
          ),
          Module(
            new TLRAMECCTest(8, 4, false, txns = 15 * txns, timeout = timeout)
          ),
          Module(
            new TLRAMECCTest(4, 1, false, txns = 15 * txns, timeout = timeout)
          ),
          Module(
            new TLRAMECCTest(1, 1, false, txns = 15 * txns, timeout = timeout)
          )
        )
      }
    })

class WithTLWidthUnitTests
    extends Config((site, here, up) => { case UnitTests =>
      (q: Parameters) => {
        implicit val p = q
        val txns = 100 * site(TestDurationMultiplier)
        ...
        Seq(
          Module(
            new TLRAMFragmenterTest(4, 256, txns = 5 * txns, timeout = timeout)
          ),
          Module(
            new TLRAMFragmenterTest(16, 64, txns = 15 * txns, timeout = timeout)
          ),
          Module(
            new TLRAMFragmenterTest(4, 16, txns = 15 * txns, timeout = timeout)
          ),
          Module(
            new TLRAMWidthWidgetTest(1, 1, txns = 1 * txns, timeout = timeout)
          ),
          Module(
            new TLRAMWidthWidgetTest(4, 64, txns = 4 * txns, timeout = timeout)
          ),
          Module(
            new TLRAMWidthWidgetTest(64, 4, txns = 5 * txns, timeout = timeout)
          )
        )
      }
    })

class WithTLXbarUnitTests
    extends Config((site, here, up) => { case UnitTests =>
      (q: Parameters) => {
        implicit val p = q
        val txns = 100 * site(TestDurationMultiplier)
        ...
        Seq(
          ...
          Module(
            new TLMulticlientXbarTest(4, 4, txns = 2 * txns, timeout = timeout)
          ),
...
class WithPowerQueueTests
    extends Config((site, here, up) => { case UnitTests =>
      (q: Parameters) => {
        Seq(
          Module(
            new PositionedQueueTest(
              FloppedLanePositionedQueue,
              1,
              2,
              false,
              false,
          ...
          Module(
            new PositionedQueueTest(
              OnePortLanePositionedQueue(new IdentityCode),
              4,
              12,
              false,
              false,
...
class AMBAUnitTestConfig
    extends Config(
      new WithAMBAUnitTests ++ new WithTestDuration(
        10
      ) ++ new BaseSubsystemConfig
    )
class TLSimpleUnitTestConfig
    extends Config(
      new WithTLSimpleUnitTests ++ new WithTestDuration(
        10
      ) ++ new BaseSubsystemConfig
    )
class TLWidthUnitTestConfig
    extends Config(
      new WithTLWidthUnitTests ++ new WithTestDuration(
        10
      ) ++ new BaseSubsystemConfig
    )
class TLXbarUnitTestConfig
    extends Config(
      new WithTLXbarUnitTests ++ new WithTestDuration(
        10
      ) ++ new BaseSubsystemConfig
    )
