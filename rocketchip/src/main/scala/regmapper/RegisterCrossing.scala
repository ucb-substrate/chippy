...
class RegisterWriteCrossingIO[T <: Data](gen: T) extends Bundle {
  ...
  val slave_valid = Output(
    Bool()
  ) // is high on 1st cycle slave_register has a new value
}
...
/** Wrapper to create an asynchronously reset slave register which can be both
  * read and written using crossing FIFOs. The reset and allow assertion &
  * de-assertion should be synchronous to their respective domains.
  */
...
object AsyncRWSlaveRegField {
  ...
  def apply(
    ...
    (
      async_slave_reg.io.q,
      RegField(
        width,
        rd_crossing.io.master_port,
        wr_crossing.io.master_port,
        desc
      )
    )
  }
