...
object RegFieldWrType extends scala.Enumeration {
  ...
  val ONE_TO_CLEAR, ONE_TO_SET, ONE_TO_TOGGLE, ZERO_TO_CLEAR, ZERO_TO_SET,
      ZERO_TO_TOGGLE, CLEAR, SET, MODIFY = Value
}
...
object RegFieldDesc {
  def reserved: RegFieldDesc =
    RegFieldDesc("reserved", "", access = RegFieldAccessType.R, reset = Some(0))
...
object RegFieldAddressBlock {
  ...
  def apply(
      addressBlockInfo: AddressBlockInfo,
    ...
    regmap.toList.map { regmapEntry =>
      ...
      val offsetIncrement =
        if (addAddressOffset) addressBlockInfo.addressOffset.toInt else 0
