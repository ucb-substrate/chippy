...
case class RegReadFn private (
    combinational: Boolean,
    fn: (Bool, Bool) => (Bool, Bool, UInt)
)
object RegReadFn {
...
case class RegWriteFn private (
    combinational: Boolean,
    fn: (Bool, Bool, UInt) => (Bool, Bool)
)
object RegWriteFn {
  ...
  implicit def apply(x: DecoupledIO[UInt]): RegWriteFn =
    RegWriteFn((valid, data) => { x.valid := valid; x.bits := data; x.ready })
  // updates a register (or adds a mux to a wire)
  implicit def apply(x: UInt): RegWriteFn = RegWriteFn((valid, data) => {
    when(valid) { x := data }; true.B
  })
  // noop
...
case class RegField(
  ...
  def toJson(byteOffset: Int, bitOffset: Int): JValue = {
    (("byteOffset" -> s"0x${byteOffset.toHexString}") ~
      ...
      ("description" -> desc.map { d =>
        if (d.desc == "") None else Some(d.desc)
      }) ~
...
object RegField {
  ...
  def apply(n: Int, r: RegReadFn, w: RegWriteFn): RegField =
    apply(n, r, w, None)
  def apply(n: Int, r: RegReadFn, w: RegWriteFn, desc: RegFieldDesc): RegField =
    apply(n, r, w, Some(desc))
  def apply(n: Int, rw: UInt): RegField = apply(n, rw, rw, None)
  def apply(n: Int, rw: UInt, desc: RegFieldDesc): RegField =
    apply(n, rw, rw, Some(desc))
  ...
  def bytes(
    ...
    val oldBytes = VecInit.tabulate(numBytes) { i =>
      pad(8 * (i + 1) - 1, 8 * i)
    }
  ...
  def bytes(reg: UInt, desc: Option[RegFieldDesc]): Seq[RegField] = {
    ...
    require(
      width % 8 == 0,
      s"RegField.bytes must be called on byte-sized reg, not ${width} bits"
    )
    bytes(reg, width / 8, desc)
  }

  def bytes(reg: UInt, numBytes: Int): Seq[RegField] =
    bytes(reg, numBytes, None)
