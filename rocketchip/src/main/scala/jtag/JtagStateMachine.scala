...
object JtagState {
  ...
  case object TestLogicReset
      extends State(
        15
      ) // no effect on system logic, entered when TMS high for 5 TCK rising edges
  case object RunTestIdle
      extends State(12) // runs active instruction (which can be idle)
  case object SelectDRScan extends State(7)
  case object CaptureDR
      extends State(
        6
      ) // parallel-load DR shifter when exiting this state (if required)
  case object ShiftDR
      extends State(
        2
  ...
  case object UpdateDR
      extends State(
        5
      ) // parallel-load output from DR shifter on TCK falling edge while in this state (not a rule?)
...
class JtagStateMachine(implicit val p: Parameters) extends Module() {
  ...
  switch(currState) {
    is(JtagState.TestLogicReset.U) {
      nextState := Mux(
        io.tms,
        JtagState.TestLogicReset.U,
        JtagState.RunTestIdle.U
      )
    }
    is(JtagState.RunTestIdle.U) {
      nextState := Mux(
        io.tms,
        JtagState.SelectDRScan.U,
        JtagState.RunTestIdle.U
      )
    }
  ...
  JtagState.State.all.foreach { s =>
    property.cover(
      currState === s.U && io.tms === true.B,
      s"${s.toString}_tms_1",
      s"JTAG; ${s.toString} with TMS = 1; State Transition from ${s.toString} with TMS = 1"
    )
    property.cover(
      currState === s.U && io.tms === false.B,
      s"${s.toString}_tms_0",
      s"JTAG; ${s.toString} with TMS = 0; State Transition from ${s.toString} with TMS = 0"
    )
    property.cover(
      currState === s.U && reset.asBool === true.B,
      s"${s.toString}_reset",
      s"JTAG; ${s.toString} with reset; JTAG Reset asserted during ${s.toString}"
    )
