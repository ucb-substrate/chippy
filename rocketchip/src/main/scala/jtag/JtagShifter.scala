...
/** Base JTAG shifter IO, viewed from input to shift register chain. Can be
  * chained together.
  */
class ShifterIO extends Bundle {
  ...
  val capture =
    Bool() // high in the CaptureIR/DR state when this chain is selected
  val update =
    Bool() // high in the UpdateIR/DR state when this chain is selected

  /** Sets a output shifter IO's control signals from a input shifter IO's
    * control signals.
    */
...
class Capture[+T <: Data](gen: T) extends Bundle {
  ...
  val capture = Output(
    Bool()
  ) // will be high in capture state (single cycle), captured on following rising edge
}
...
/** Trait that all JTAG chains (data and instruction registers) must extend,
  * providing basic chain IO.
  */
...
class JtagBypassChain(implicit val p: Parameters) extends Chain {
  ...
  property.cover(
    io.chainIn.capture,
    "bypass_chain_capture",
    "JTAG; bypass_chain_capture; This Bypass Chain captured data"
  )
...
/** Simple shift register with parallel capture only, for read-only data
  * registers.
  *
  * Number of stages is the number of bits in gen, which must have a known
  * width.
  *
  * Useful notes: 7.2.1c shifter shifts on TCK rising edge 4.3.2a TDI captured
  * on TCK rising edge, 6.1.2.1b assumed changes on TCK falling edge
  */
class CaptureChain[+T <: Data](gen: T)(implicit val p: Parameters)
    extends Chain {
  ...
  val n = DataMirror.widthOf(gen) match {
    case KnownWidth(x) => x
    case _ =>
      require(false, s"can't generate chain for unknown width data type $gen");
  ...
  property.cover(
    io.chainIn.capture,
    "chain_capture",
    "JTAG; chain_capture; This Chain captured data"
  )
...
/** Simple shift register with parallel capture and update. Useful for general
  * instruction and data scan registers.
  *
  * Number of stages is the max number of bits in genCapture and genUpdate, both
  * of which must have known widths. If there is a width mismatch, the unused
  * most significant bits will be zero.
  *
  * Useful notes: 7.2.1c shifter shifts on TCK rising edge 4.3.2a TDI captured
  * on TCK rising edge, 6.1.2.1b assumed changes on TCK falling edge
  */
class CaptureUpdateChain[+T <: Data, +V <: Data](genCapture: T, genUpdate: V)(
    implicit val p: Parameters
) extends Chain {
  override def desiredName =
    s"CaptureUpdateChain_${genCapture.typeName}_To_${genUpdate.typeName}"
  class ModIO extends ChainIO {
    val capture = Capture(genCapture)
    val update = Valid(
      genUpdate
    ) // valid high when in update state (single cycle), contents may change any time after
  }
  val io = IO(new ModIO)
  io.chainOut chainControlFrom io.chainIn
  ...
  val captureWidth = DataMirror.widthOf(genCapture) match {
    ...
    case _ =>
      require(
        false,
        s"can't generate chain for unknown width data type $genCapture"
      ); -1 // TODO: remove -1 type hack
  }
  val updateWidth = DataMirror.widthOf(genUpdate) match {
    ...
    case _ =>
      require(
        false,
        s"can't generate chain for unknown width data type $genUpdate"
      ); -1 // TODO: remove -1 type hack
  }
  ...
  property.cover(
    io.chainIn.capture,
    "chain_update",
    "JTAG;chain_update; This Chain updated data"
  )
...
object CaptureUpdateChain {
  ...
  /** Capture-update chain with matching capture and update types.
    */
  def apply[T <: Data](gen: T)(implicit p: Parameters) =
    new CaptureUpdateChain(gen, gen)
  def apply[T <: Data, V <: Data](genCapture: T, genUpdate: V)(implicit
      p: Parameters
  ) =
