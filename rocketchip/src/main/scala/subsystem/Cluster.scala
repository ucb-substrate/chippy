...
case class ClustersLocated(loc: HierarchicalLocation)
    extends Field[Seq[CanAttachCluster]](Nil)
...
abstract class InstantiableClusterParams[ClusterType <: Cluster]
  ...
  )(implicit p: Parameters): ClusterType
}
...
class Cluster(
    ...
    extends BaseHierarchicalElement(crossing)(p)
    with Attachable
  ...
  val csbus = tlBusWrapperLocationMap(
    CSBUS(clusterId)
  ) // like the sbus in the base subsystem
  val ccbus = tlBusWrapperLocationMap(
    CCBUS(clusterId)
  ) // like the cbus in the base subsystem
  ...
  def hasSupervisorTileIdList: Seq[Int] =
    totalTiles.filter(_._2.tileParams.core.hasSupervisorMode).keys.toSeq.sorted
  lazy val msipNodes =
    totalTileIdList.map { i => (i, IntIdentityNode()) }.to(SortedMap)
  lazy val meipNodes =
    totalTileIdList.map { i => (i, IntIdentityNode()) }.to(SortedMap)
  lazy val seipNodes =
    hasSupervisorTileIdList.map { i => (i, IntIdentityNode()) }.to(SortedMap)
  lazy val tileToPlicNodes =
    totalTileIdList.map { i => (i, IntIdentityNode()) }.to(SortedMap)
  lazy val debugNodes =
    totalTileIdList.map { i => (i, IntSyncIdentityNode()) }.to(SortedMap)
  lazy val nmiNodes = totalTiles
    .filter { case (i, t) => t.tileParams.core.useNMI }
    .view
    .mapValues(_ => BundleBridgeIdentityNode[NMI]())
    .to(SortedMap)
  lazy val tileHartIdNodes = totalTileIdList
    .map { i => (i, BundleBridgeIdentityNode[UInt]()) }
    .to(SortedMap)
  lazy val tileResetVectorNodes = totalTileIdList
    .map { i => (i, BundleBridgeIdentityNode[UInt]()) }
    .to(SortedMap)
  lazy val traceCoreNodes = totalTileIdList
    .map { i => (i, BundleBridgeIdentityNode[TraceCoreInterface]()) }
    .to(SortedMap)
  lazy val traceNodes = totalTileIdList
    .map { i => (i, BundleBridgeIdentityNode[TraceBundle]()) }
    .to(SortedMap)
...
class ClusterModuleImp(outer: Cluster)
    extends BaseHierarchicalElementModuleImp[Cluster](outer)
...
abstract class ClusterPRCIDomain[ClusterType <: Cluster](
    ...
    extends HierarchicalElementPRCIDomain[ClusterType](
      clockSinkParams,
      crossingParams
    ) {
...
trait CanAttachCluster {
  ...
  def instantiate(
      allClusterParams: Seq[BaseClusterParams],
      instantiatedClusters: SortedMap[Int, ClusterPRCIDomain[_]]
  )(implicit p: Parameters): ClusterPRCIDomain[ClusterType] = {
    val clockSinkParams =
      clusterParams.clockSinkParams.copy(name = Some(clusterParams.uniqueName))
    val cluster_prci_domain = LazyModule(
      new ClusterPRCIDomain[ClusterType](
        clockSinkParams,
        crossingParams,
        clusterParams,
        PriorityMuxClusterIdFromSeq(allClusterParams)
      ) {}
    )
    cluster_prci_domain
  }
  ...
  def connect(
      domain: ClusterPRCIDomain[ClusterType],
      context: ClusterContextType
  ): Unit = {
  ...
  def connectInterrupts(
    ...
    domain.element.msipNodes.foreach { case (hartid, node) =>
      context.msipDomain {
        domain.crossIntIn(crossingParams.crossingType, node) := context
          .msipNodes(hartid)
      }
    }
    ...
    domain.element.tileToPlicNodes.foreach { case (hartid, node) =>
      FlipRendering { implicit p =>
        context.tileToPlicNodes(hartid) :=* domain.crossIntOut(
          crossingParams.crossingType,
          node
        )
      }
    }
    context.ibus.fromSync :=* domain.crossIntOut(
      crossingParams.crossingType,
      domain.element.ibus.toPLIC
    )
  ...
  def connectOutputNotifications(
    ...
    context.tileHaltXbarNode :=* domain.crossIntOut(
      NoCrossing,
      domain.element.tileHaltXbarNode
    )
    context.tileWFIXbarNode :=* domain.crossIntOut(
      NoCrossing,
      domain.element.tileWFIXbarNode
    )
    context.tileCeaseXbarNode :=* domain.crossIntOut(
      NoCrossing,
      domain.element.tileCeaseXbarNode
    )
