...
import freechips.rocketchip.devices.tilelink.{
  ...
  BuiltInZeroDeviceParams
}
import freechips.rocketchip.tilelink.{
  ReplicatedRegion,
  HasTLBusParams,
  HasRegionReplicatorParams,
  TLBusWrapper,
...
/** Wrapper for creating TL nodes from a bus connected to the back of each mem
  * channel
  */
class MemoryBus(params: MemoryBusParams, name: String = "memory_bus")(implicit
    p: Parameters
) extends TLBusWrapper(params, name)(p) {
  private val replicator =
    params.replication.map(r => LazyModule(new RegionReplicator(r)))
  val prefixNode = replicator.map { r =>
    r.prefix := addressPrefixNexusNode
    addressPrefixNexusNode
  }

  private val xbar = LazyModule(new TLXbar(nameSuffix = Some(name)))
  ...
  val builtInDevices: BuiltInDevices =
    BuiltInDevices.attach(params, outwardNode)
}
