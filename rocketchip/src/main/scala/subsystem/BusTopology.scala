...
case class CSBUS(clusterId: Int)
    extends TLBusWrapperLocation(s"csbus$clusterId")
case class CMBUS(clusterId: Int)
    extends TLBusWrapperLocation(s"cmbus$clusterId")
case class CCBUS(clusterId: Int)
    extends TLBusWrapperLocation(s"ccbus$clusterId")
case class CCOH(clusterId: Int) extends TLBusWrapperLocation(s"ccoh$clusterId")

/** Parameterizes the subsystem in terms of optional clock-crossings that are
  * insertable between some of the five traditional tilelink bus wrappers. This
  * class exists for backwards compatiblity reasons but could eventually be
  * retired in favor of manually filling in crossing types within each custom
  * TLBusWrapperTopology.
  */
...
/** Keys to parameterize the most common crossings between the five traditional
  * TL buses. Used to populated [[SubsystemCrossingParams]].
  */
...
case object CbusToPbusXTypeKey
    extends Field[ClockCrossingType](SynchronousCrossing())
case object FbusToSbusXTypeKey
    extends Field[ClockCrossingType](SynchronousCrossing())

/** By default, ClockSources for the five traditional TL buses are provided from
  * connections that originate at the SBUS. Setting this to false removes these
  * connections and permits supplying diplomatic clocks to each bus
  * independently.
  */
...
/** Parameterization of a topology containing a single bus named
  * "subsystem_sbus".
  */
...
/** Parameterization of a topology containing three additional, optional buses
  * for attaching MMIO devices.
  */
case class HierarchicalBusTopologyParams(
    ...
    driveClocksFromSBus: Boolean = true
      ...
      instantiations = List((PBUS, pbus), (FBUS, fbus), (CBUS, cbus)),
      connections = List(
        (
          SBUS,
          CBUS,
          TLBusWrapperConnection.crossTo(
            xTypes.sbusToCbusXType,
            if (driveClocksFromSBus) Some(true) else None
          )
        ),
        (
          CBUS,
        ...
        (
          FBUS,
          SBUS,
          TLBusWrapperConnection.crossFrom(
            xTypes.fbusToSbusXType,
            if (driveClocksFromSBus) Some(false) else None
          )
        )
      )
    )

/** Parameterization of a topology containing a banked coherence manager and a
  * bus for attaching memory devices.
  */
case class CoherentBusTopologyParams(
    ...
    driveMBusClockFromSBus: Boolean = true
      ...
      instantiations =
        (if (coherence.nBanks == 0) Nil
         else
           List(
             (MBUS, mbus),
             (
               COH,
               CoherenceManagerWrapperParams(
                 mbus.blockBytes,
                 mbus.beatBytes,
                 coherence.nBanks,
                 COH.name
            ...
            (
              ...
              TLBusWrapperConnection.crossTo(
                ...
                driveClockFromMaster =
                  if (driveMBusClockFromSBus) Some(true) else None,
