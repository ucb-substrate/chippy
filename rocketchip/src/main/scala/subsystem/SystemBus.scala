...
class SystemBus(params: SystemBusParams, name: String = "system_bus")(implicit
    p: Parameters
  ...
  private val system_bus_xbar = LazyModule(
    new TLXbar(policy = params.policy, nameSuffix = Some(name))
  )
  val inwardNode: TLInwardNode = system_bus_xbar.node :=* TLFIFOFixer(
