...
class PeripheryBus(params: PeripheryBusParams, name: String)(implicit
    p: Parameters
  ...
  private val node: TLNode = params.atomics
    .map { pa =>
      ...
      (out_xbar.node
        ...
        :*= (pa.widenBytes
          .filter(_ > beatBytes)
          .map { w =>
            TLWidthWidget(w) :*= TLAtomicAutomata(
              arithmetic = pa.arithmetic,
