...
/** A wrapper containing all logic within a managed reset domain for a element.
  ...
  */
class HierarchicalElementResetDomain(
...
/** A wrapper containing all logic necessary to safely place a tile inside of a
  * particular Power/Reset/Clock/Interrupt domain.
  *
  * This adds a layer to the module hierarchy which is a parent of the tile and
  * should contain all logic related to clock crossings, isolation cells,
  * hierarchical P&R boundary buffers, core-local interrupt handling, and any
  * other IOs related to PRCI control.
  */
abstract class HierarchicalElementPRCIDomain[T <: BaseHierarchicalElement](
    ...
    extends ClockDomain {
  val element: T
  val element_reset_domain = LazyModule(
    new HierarchicalElementResetDomain(
      clockSinkParams,
      crossingParams.resetCrossingType
    )
  )
  ...
  /** External code looking to connect and clock-cross the interrupts driven
    * into this tile can call this.
    */
  def crossIntIn(
      crossingType: ClockCrossingType,
      tileNode: IntInwardNode
  ): IntInwardNode = {
  ...
  /** External code looking to connect and clock/reset-cross
    ...
    * Takes crossingType as an argument because some interrupts are supposed to
    * be synchronous Takes tileNode as an argument because tiles might have
    * multiple outbound interrupt nodes
    */
  def crossIntOut(
      crossingType: ClockCrossingType,
      tileNode: IntOutwardNode
  ): IntOutwardNode = {
  ...
  /** External code looking to connect the ports where this tile is slaved to an
    * interconnect (while also crossing clock domains) can call this.
    */
  ...
  /** External code looking to connect the ports where this tile masters an
    * interconnect (while also crossing clock domains) can call this.
    */
