...
case class TLNetworkTopologyLocated(where: HierarchicalLocation)
    extends Field[Seq[
      CanInstantiateWithinContextThatHasTileLinkLocations
        with CanConnectWithinContextThatHasTileLinkLocations
    ]]
case class TLManagerViewpointLocated(where: HierarchicalLocation)
    extends Field[Location[TLBusWrapper]](SBUS)

class HierarchicalLocation(override val name: String)
    extends Location[LazyScope](name)
...
abstract class BareSubsystemModuleImp[+L <: BareSubsystem](_outer: L)
...
case object SubsystemResetSchemeKey
    extends Field[SubsystemResetScheme](ResetSynchronous)

/** Concrete attachment points for PRCI-related signals. These aren't actually
  * very configurable, yet.
  */
...
/** Look up the topology configuration for the TL buses located within this
...
trait HasConfigurableTLNetworkTopology { this: HasTileLinkLocations =>
  ...
  def viewpointBus: TLBusWrapper = tlBusWrapperLocationMap(
    p(TLManagerViewpointLocated(location))
  )
...
abstract class BaseSubsystemModuleImp[+L <: BaseSubsystem](_outer: L)
    extends BareSubsystemModuleImp(_outer)
    with HasDTSImp[L] {
  def dtsLM: L = _outer
  private val mapping: Seq[AddressMapEntry] = {
    dtsLM.collectResourceAddresses
      .groupBy(_._2)
      .toList
      .flatMap { case (key, seq) =>
        AddressRange.fromSets(key.address).map { r =>
          AddressMapEntry(r, key.permissions, seq.map(_._1))
        }
      }
      .sortBy(_.range)
  }
  ...
  mapping.foreach(entry =>
    println(
      entry.toString(
        (dtsLM
          .tlBusWrapperLocationMap(p(TLManagerViewpointLocated(dtsLM.location)))
          .busView
          .bundle
          .addressBits - 1) / 4 + 1
      )
  ...
  ElaborationArtefacts.add(
    "memmap.json",
    s"""{"mapping":[${mapping.map(_.toJSON).mkString(",")}]}"""
  )

  // Confirm that all of memory was described by DTS
  private val dtsRanges = AddressRange.unify(mapping.map(_.range))
  private val allRanges = AddressRange.unify(dtsLM.topManagers.flatMap { m =>
    AddressRange.fromSets(m.address)
  })
