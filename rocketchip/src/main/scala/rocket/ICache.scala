...
/** Parameter of [[ICache]].
  ...
  *   TLB Superpages
  * @param tagECC
  *   tag ECC, will be parsed to [[freechips.rocketchip.util.Code]].
  * @param dataECC
  *   data ECC, will be parsed to [[freechips.rocketchip.util.Code]].
  * @param itimAddr
  *   optional base ITIM address, if None, ITIM won't be generated, if Some,
  *   ITIM will be generated, with itimAddr as ITIM base address.
  * @param prefetch
  *   if set, will send next-line[[TLEdgeOut.Hint]] to manger.
  * @param blockBytes
  *   size of a cacheline, calculates in byte.
  * @param latency
  *   latency of a instruction fetch, 1 or 2 are available
  * @param fetchBytes
  *   byte size fetched by CPU for each cycle.
  */
...
trait HasL1ICacheParameters
    extends HasL1CacheParameters
    with HasCoreParameters {
  val cacheParams = tileParams.icache.get
}

class ICacheReq(implicit p: Parameters)
    extends CoreBundle()(p)
    with HasL1ICacheParameters {
  val addr = UInt(vaddrBits.W)
}

class ICacheErrors(implicit p: Parameters)
    extends CoreBundle()(p)
    with HasL1ICacheParameters
    with CanHaveErrors {
  val correctable =
  ...
  val uncorrectable =
    (cacheParams.itimAddr.nonEmpty && cacheParams.dataCode.canDetect)
      .option(Valid(UInt(paddrBits.W)))
...
/** [[ICache]] is a set associated cache I$(Instruction Cache) of Rocket.
  ...
  *                    │    line       │
  * }}}
  * if `way` == b11 (last way), deallocate if write to ITIM all I$ will be
  * invalidate
  *
  * The optional dynamic configurable ITIM sharing SRAM with I$ is set by
  * [[icacheParams.itimAddr]]. if PutFullData/PutPartialData to the ITIM
  * address, it will dynamically allocate base address to the address of this
  * accessing from SRAM. if access to last way of ITIM, it set will change back
  * to I$.
  *
  * If ITIM is configured: set: if address to access is not to be configured to
  * ITIM yet, a memory accessing to ITIM address range will modify
  * `scratchpadMax`, from ITIM base to `scratchpadMax` will be used as ITIM.
  * unset: @todo
  *
  * There will always be one way(the last way) used for I$, which cannot be
  * allocated to ITIM.
  *
  * @param icacheParams
  *   parameter to this I$.
  * @param staticIdForMetadataUseOnly
  *   metadata used for hart id.
  */
class ICache(
  ...
  val hartIdSinkNodeOpt =
    icacheParams.itimAddr.map(_ => BundleBridgeSink[UInt]())

  /** @todo base address offset for ITIM? */
  val mmioAddressPrefixSinkNodeOpt =
    icacheParams.itimAddr.map(_ => BundleBridgeSink[UInt]())

  /** Rocket configuration has virtual memory.
    *
    * This only affect [[masterNode]] AMBA ports only: AMBA privileged, secure
    * will be set as true while others set as false. see
    * [[freechips.rocketchip.amba.AMBAProt]] for more informations.
    */
  val useVM = p(TileKey).core.useVM

  /** [[TLClientNode]] of I$.
    *
    * source Id range: 0: use [[TLEdgeOut.Get]] to get instruction. 1: use
    * [[TLEdgeOut.Hint]] to hint next level memory device fetching next cache
    * line, if configured [[icacheParams.prefetch]].
    *
    * @todo
    *   why if no [[useVM]], will have AMBAProtField in requestFields?
    */
  ...
  /** last way will be configured to control offest, access it will deallocate
    * an entire set to I$.
    */
  ...
  val device = new SimpleDevice("itim", Seq("sifive,itim0")) {
    override def describe(resources: ResourceBindings): Description = {
      ...
      val Seq(Binding(_, ResourceAddress(address, perms))) = resources(
        "reg/mem"
      )
      val base_address = address.head.base
      val mem_part = AddressSet.misaligned(base_address, itim_control_offset)
      val control_part = AddressSet.misaligned(
        base_address + itim_control_offset,
        size - itim_control_offset
      )
      val extra = Map(
        "reg-names" -> Seq(ResourceString("mem"), ResourceString("control")),
        "reg" -> Seq(
          ResourceAddress(mem_part, perms),
  ...
  def itimProperty: Option[Seq[ResourceValue]] =
    icacheParams.itimAddr.map(_ => device.asProperty)

  /** @todo
    *   why [[wordBytes]] is defined by [[icacheParams.fetchBytes]], rather than
    *   32 directly?
    */
...
class ICacheResp(outer: ICache) extends Bundle {
  ...
  /** access exception: indicate CPU an tag ECC error happened. if
    * [[outer.icacheParams.latency]] is 1, tie 0.
    */
  val ae = Bool()
...
class ICacheBundle(val outer: ICache) extends CoreBundle()(outer.p) {
  ...
  /** I$ has error, notify to bus. TODO: send to BPU.
    */
  ...
  /** I$ miss or ITIM access will still enable clock even [[ICache]] is asked to
    * be gated.
    */
  val keep_clock_enabled = Output(Bool())
}

class ICacheModule(outer: ICache)
  ...
  /** TileLink port as ITIM memory. if [[outer.slaveNode]] is not connected
    * [[outer.slaveNode.in]] will be empty.
    *
  ...
  require(
    !usingVM || outer.icacheParams.itimAddr.isEmpty || pgIdxBits >= untagBits,
  ...
  /** a cut point to SRAM, indicates which SRAM will be used as SRAM or Cache.
    */
  val scratchpadMax =
    tl_in.map(tl => Reg(UInt(log2Ceil(nSets * (nWays - 1)).W)))

  /** Check if a line is in the scratchpad.
    *
    * line is a minimal granularity accessing to SRAM, calculated by
    * [[scratchpadLine]]
    */
  def lineInScratchpad(line: UInt) =
    scratchpadMax.map(scratchpadOn && line <= _).getOrElse(false.B)

  /** scratchpad base address, if exist [[ICacheParams.itimAddr]], add
    * [[ReplicatedRegion]] to base.
    * @todo
    *   seem [[io_hartid]] is not connected? maybe when implementing itim,
    *   LookupByHartId should be changed to [[]]?
    */
  val scratchpadBase = outer.icacheParams.itimAddr.map { dummy =>
    p(LookupByHartId)(
      _.icache.flatMap(_.itimAddr.map(_.U)),
      io_hartid.get
    ) | io_mmio_address_prefix.get
  }

  /** check an address in the scratchpad address range. */
  def addrMaybeInScratchpad(addr: UInt) = scratchpadBase
    .map(base => addr >= base && addr < base + outer.size.U)
    .getOrElse(false.B)

  /** check property this address(paddr) exists in scratchpad.
    * @todo
    *   seems duplicated in `addrMaybeInScratchpad(addr)` between
    *   `lineInScratchpad(addr(untagBits+log2Ceil(nWays)-1, blockOffBits))`?
    */
  def addrInScratchpad(addr: UInt) = addrMaybeInScratchpad(
    addr
  ) && lineInScratchpad(addr(untagBits + log2Ceil(nWays) - 1, blockOffBits))
  ...
  /** return the way which will be used as scratchpad for accessing address
    ...
    *   address to be found.
    */
  def scratchpadWay(addr: UInt) =
    addr.extract(untagBits + log2Ceil(nWays) - 1, untagBits)

  /** check if the selected way is legal. note: the last way should be reserved
    * to ICache.
    */
  def scratchpadWayValid(way: UInt) = way < (nWays - 1).U

  /** return the cacheline which will be used as scratchpad for accessing
    * address
    * {{{
    * │          tag         │    set    │offset│
    *                    ├way┘                    → indicate way location
    ...
    */
  def scratchpadLine(addr: UInt) =
    addr(untagBits + log2Ceil(nWays) - 1, blockOffBits)
  ...
  /** CPU I$ Hit in stage 1.
    *
    * @note
    *   for logic in `Mux(s1_slaveValid, true.B,
    *   addrMaybeInScratchpad(io.s1_paddr))`, there are two different types
    *   based on latency:
    *
    * if latency is 1: `s1_slaveValid === false.B` and
    * `addrMaybeInScratchpad(io.s1_paddr) === false.B` , since in this case,
    * ITIM must be empty.
    *
    * if latency is 2: if `s1_slaveValid` is true, this SRAM accessing is coming
    * from [[tl_in]], so it will hit. if `s1_slaveValid` is false, but CPU is
    * accessing memory range in scratchpad address, it will hit by default.
    * Hardware won't guarantee this access will access to a data which have been
    * written in ITIM.
    *
    * @todo
    *   seem CPU access are both processed by `s1_tag_hit` and
    *   `Mux(s1_slaveValid, true.B, addrMaybeInScratchpad(io.s1_paddr))`?
    */
  val s1_hit = s1_tag_hit.reduce(_ || _) || Mux(
    s1_slaveValid,
    true.B,
  ...
  /** register to indicate [[tl_out]] is performing a hint. prefetch only
    * happens after refilling
    */
  ...
  /** real refill signal, stage 2 miss, and was permit to refill in stage 1.
    * Since a miss will trigger burst. miss under miss won't trigger another
    * burst.
    */
  ...
  /** way to be replaced, implemented with a hardcoded random replacement
    * algorithm
    */
  val repl_way =
    if (isDM) 0.U
  ...
  /** Tag SRAM, indexed with virtual memory, content with `refillError ##
    * tag[19:0]` after ECC
    */
  ...
  val refillError =
    tl_out.d.bits.corrupt || (refill_cnt > 0.U && accruedRefillError)
  when(refill_done) {
    ...
    tag_array.write(
      refill_idx,
      VecInit(Seq.fill(nWays) { enc_tag }),
  ...
  /** true indicate this cacheline is valid, indexed by (wayIndex ## setIndex)
  ...
  when(refill_one_beat) {
    ...
    vb_array := vb_array.bitSet(
      Cat(repl_way, refill_idx),
  ...
  /** wire indicates that tag is correctable or uncorrectable. will trigger CPU
    * to replay and I$ invalidating, if correctable.
    */
  val s1_tag_disparity = Wire(Vec(nWays, Bool()))

  /** wire indicates that bus has an uncorrectable error. respond to CPU
    * [[io.resp.bits.ae]], cause [[Causes.fetch_access]].
    */
  ...
  for (i <- 0 until nWays) {
    ...
    val scratchpadHit = scratchpadWayValid(i.U) &&
      Mux(
        ...
        lineInScratchpad(scratchpadLine(s1s3_slaveAddr)) && scratchpadWay(
          s1s3_slaveAddr
        ) === i.U,
    ...
    val s1_vb =
      vb_array(Cat(i.U, s1_idx).pad(log2Ceil(nSets * nWays))) && !s1_slaveValid
    ...
    /** if tag matched but ecc checking failed, this access will trigger
      * [[Causes.fetch_access]] exception.
      */
    s1_tl_error(i) := tagMatch && tl_error.asBool
    s1_tag_hit(i) := tagMatch || scratchpadHit
  }
  assert(
    !(s1_valid || s1_slaveValid) || PopCount(
      s1_tag_hit zip s1_tag_disparity map { case (h, d) => h && !d }
    ) <= 1.U
  )
  ...
  /** Data SRAM
    *
    * banked with TileLink beat bytes / CPU fetch bytes, indexed with [[index]]
    * and multi-beats cycle, content with `eccError ## wordBits` after ECC.
    * {{{
    * │                          │xx│xxxxxx│xxx│x│xx│
    *                                            ↑word
    *                                          ↑bank
    ...
    *   - vaddr[11:3]->row,
    *   - vaddr[2]->bank=i
    *   - Cache line size = refillCycels(8) * bank(2) * datasize(4 bytes) = 64
    *     bytes
    *   - data width = 32
    *
    * read: read happens in stage 0
    *
    * write: It takes 8 beats to refill 16 instruction in each refilling cycle.
    * Data_array receives data[63:0](2 instructions) at once,they will be
    * allocated in deferent bank according to vaddr[2]
    */
  ...
  for ((data_array, i) <- data_arrays.zipWithIndex) {
    ...
    /** bank match (vaddr[2]) */
    def wordMatch(addr: UInt) = addr.extract(
      log2Ceil(tl_out.d.bits.data.getWidth / 8) - 1,
      log2Ceil(wordBits / 8)
    ) === i.U
    def row(addr: UInt) =
    ...
    /** read_enable signal */
    val s0_ren = (s0_valid && wordMatch(
      s0_vaddr
    )) || (s0_slaveValid && wordMatch(s0_slaveAddr))

    /** write_enable signal refill from [[tl_out]] or ITIM write.
      */
    val wen = (refill_one_beat && !invalidated) || (s3_slaveValid && wordMatch(
      s1s3_slaveAddr
    ))
    ...
    when(wen) {
      ...
      data_array.write(
        mem_idx,
        VecInit(Seq.fill(nWays) { dECC.encode(data) }),
  ...
  /** When writing full words to ITIM, ECC errors are correctable. When writing
    * a full scratchpad word, suppress the read so Xs don't leak out
    */
  val s1s2_full_word_write = WireDefault(false.B)
  val s1_dont_read = s1_slaveValid && s1s2_full_word_write

  /** clock gate signal for [[s2_tag_hit]], [[s2_dout]], [[s2_tag_disparity]],
    * [[s2_tl_error]], [[s2_scratchpad_hit]].
    */
  val s1_clk_en = s1_valid || s1_slaveValid
  val s2_tag_hit = RegEnable(
    Mux(s1_dont_read, 0.U.asTypeOf(s1_tag_hit), s1_tag_hit),
    s1_clk_en
  )
  ...
  val s2_scratchpad_word_addr = Cat(
    s2_hit_way,
    Mux(s2_slaveValid, s1s3_slaveAddr, io.s2_vaddr)(
      untagBits - 1,
      log2Ceil(wordBits / 8)
    ),
  ...
  /** access hit in ITIM, if [[s1_slaveValid]], this access is from [[tl_in]],
    * else from CPU [[io]].
    */
  val s1_scratchpad_hit = Mux(
    s1_slaveValid,
  ...
  /** ITIM uncorrectable read. `s2_scratchpad_hit`: processing a scratchpad
    * read(from [[tl_in]] or [[io]]) `s2_data_decoded.uncorrectable`: read a
    * uncorrectable data. `s2_valid`: [[io]] non-canceled read. `(s2_slaveValid
    * && !s2_full_word_write)`: [[tl_in]] read or write a word with wormhole. if
    * write a full word, even stage 2 read uncorrectable. stage 3 full word
    * write will recovery this.
    */
  ...
  val s2_error_addr = scratchpadBase
    .map(base => Mux(s2_scratchpad_hit, base + s2_scratchpad_word_addr, 0.U))
    .getOrElse(0.U)
      ...
      tl_in.map { tl =>
        ...
        when(s0_slaveValid) {
          ...
          when(edge_in.get.hasData(a)) {
            ...
            val itim_increase = scratchpadOn && enable && scratchpadLine(
              a.address
            ) > scratchpadMax.get
            val refilling = refill_valid && refill_cnt > 0.U
            ccover(itim_allocated, "ITIM_ALLOCATE", "ITIM allocated")
            ccover(
              itim_allocated && refilling,
              "ITIM_ALLOCATE_WHILE_REFILL",
              "ITIM allocated while I$ refill"
            )
            ccover(itim_deallocated, "ITIM_DEALLOCATE", "ITIM deallocated")
            ccover(
              itim_deallocated && refilling,
              "ITIM_DEALLOCATE_WHILE_REFILL",
              "ITIM deallocated while I$ refill"
            )
            ccover(itim_increase, "ITIM_SIZE_INCREASE", "ITIM size increased")
            ccover(
              itim_increase && refilling,
              "ITIM_SIZE_INCREASE_WHILE_REFILL",
              "ITIM size increased while I$ refill"
            )
          }
        }
        ...
        when(
          ...
          s1s3_slaveAddr := s2_scratchpad_word_addr | s1s3_slaveAddr(
            log2Ceil(wordBits / 8) - 1,
            0
          )
        }
        ...
        val respError = RegEnable(
          s2_scratchpad_hit && s2_data_decoded.uncorrectable && !s1s2_full_word_write,
          s2_slaveValid
        )
        when(s2_slaveValid) {
          ...
          /** data not masked by the TileLink PutData/PutPartialData. means data
            * is stored at [[s1s3_slaveData]] which was read at stage 1.
            */
          ...
          s1s3_slaveData := (0 until wordBits / 8)
            .map(i =>
              Mux(byteEn(i), s2_data_decoded.corrected, s1s3_slaveData)(
                8 * (i + 1) - 1,
                8 * i
              )
            )
        ...
        tl.d.bits := Mux(
          ...
          // Get -> AccessAckData
          edge_in.get
            .AccessAck(s1_a, 0.U, denied = false.B, corrupt = respError)
        )
        ...
        ccover(
          s0_valid && s1_slaveValid,
          "CONCURRENT_ITIM_ACCESS_1",
          "ITIM accessed, then I$ accessed next cycle"
        )
        ccover(
        ...
        ccover(
          ...
          "ITIM_D_STALL",
          "ITIM response blocked by D-channel"
        )
        ccover(
          tl_out.d.valid && !tl_out.d.ready,
          "ITIM_BLOCK_D",
          "D-channel blocked by ITIM access"
        )
      }
  }
  ...
  if (cacheParams.prefetch) {
    ...
    /** [[crosses_page]] indicate if there is a crosses page access
      ...
      */
    val (crosses_page, next_block) = Split(
      refill_paddr(pgIdxBits - 1, blockOffBits) +& 1.U,
    ...
    when(send_hint) {
      ...
      tl_out.a.bits := edge_out
        .Hint(
          fromSource = 1.U,
          toAddress =
            Cat(refill_paddr >> pgIdxBits, next_block) << blockOffBits,
          lgSize = lgCacheBlockBytes.U,
    ...
    ccover(
      send_hint && !tl_out.a.ready,
      "PREFETCH_A_STALL",
      "I$ prefetch blocked by A-channel"
    )
    ccover(
    ...
    ccover(
      tl_out.a.fire && hint_outstanding,
  ...
  io.keep_clock_enabled :=
    tl_in
      .map(tl =>
        tl.a.valid || tl.d.valid || s1_slaveValid || s2_slaveValid || s3_slaveValid
      )
  ...
  /** index to access [[data_arrays]] and [[tag_array]].
    ...
    *   }}}
    *
    * else use paddr directly. Note: if [[untagBits]] > [[pgIdxBits]], there
    * will be a alias issue which isn't addressend by the icache yet.
    */
  def index(vaddr: UInt, paddr: UInt) = {
    ...
    /** if [[untagBits]] > [[pgIdxBits]], append [[vaddr]] to higher bits of
      * index as [[msbs]].
      */
    val msbs = (idxBits + blockOffBits > pgUntagBits).option(
      vaddr(idxBits + blockOffBits - 1, pgUntagBits)
    )
    msbs ## lsbs
  }

  ccover(
    !send_hint && (tl_out.a.valid && !tl_out.a.ready),
    "MISS_A_STALL",
    "I$ miss blocked by A-channel"
  )
  ccover(
    invalidate && refill_valid,
    "FLUSH_DURING_MISS",
    "I$ flushed during miss"
  )
  ...
  val data_error = Seq(
    property.CoverBoolean(
      !s2_data_decoded.correctable && !s2_data_decoded.uncorrectable,
      Seq("no_data_error")
    ),
    property
      .CoverBoolean(s2_data_decoded.correctable, Seq("data_correctable_error")),
    property.CoverBoolean(
      s2_data_decoded.uncorrectable,
