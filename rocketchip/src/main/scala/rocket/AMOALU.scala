...
class StoreGen(typ: UInt, addr: UInt, dat: UInt, maxSize: Int) {
  ...
  def mask = {
    ...
    for (i <- 0 until log2Up(maxSize)) {
      val upper = Mux(addr(i), res, 0.U) | Mux(
        size >= (i + 1).U,
        ((BigInt(1) << (1 << i)) - 1).U,
        0.U
      )
  ...
  protected def genData(i: Int): UInt =
    if (i >= log2Up(maxSize)) dat_padded
      ...
      Mux(
        size === i.U,
        Fill(1 << (log2Up(maxSize) - i), dat_padded((8 << i) - 1, 0)),
        genData(i + 1)
      )
  ...
  private def genData(logMinSize: Int): UInt = {
    ...
    for (i <- log2Up(maxSize) - 1 to logMinSize by -1) {
      ...
      res = Cat(
        Mux(
          size === i.U || doZero,
          Fill(8 * maxSize - pos, signed && zeroed(pos - 1)),
          res(8 * maxSize - 1, pos)
        ),
        zeroed
      )
...
class AMOALU(operandBits: Int)(implicit p: Parameters) extends Module {
  ...
  val adder_out = {
    ...
    val mask = ~(0.U(operandBits.W) +: widths.init.map(w =>
      !io.mask(w / 8 - 1) << (w - 1)
    )).reduce(_ | _)
  ...
  val less = {
    ...
    def isLessUnsigned(x: UInt, y: UInt, n: Int): Bool = {
      if (n == minXLen) x(n - 1, 0) < y(n - 1, 0)
        ...
        x(n - 1, n / 2) < y(n - 1, n / 2) || x(n - 1, n / 2) === y(
          n - 1,
          n / 2
        ) && isLessUnsigned(x, y, n / 2)
    }
    ...
    def isLess(x: UInt, y: UInt, n: Int): Bool = {
      ...
      Mux(
        x(n - 1) === y(n - 1),
        isLessUnsigned(x, y, n),
        Mux(signed, x(n - 1), y(n - 1))
      )
    }

    PriorityMux(
      widths.reverse.map(w => (io.mask(w / 8 / 2), isLess(io.lhs, io.rhs, w)))
    )
  }
