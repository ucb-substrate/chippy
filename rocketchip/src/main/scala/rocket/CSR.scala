...
class PTBR(implicit p: Parameters) extends CoreBundle()(p) {
  def additionalPgLevels =
    mode.extract(log2Ceil(pgLevels - minPgLevels + 1) - 1, 0)
...
object CSR {
  ...
  def rnmiIntCause =
    13 // NMI: Higher numbers = higher priority, must not reuse debugIntCause
  def rnmiBEUCause = 12
...
class PerfCounterIO(implicit p: Parameters)
    extends CoreBundle
    with HasCoreParameters {
...
class CSRFileIO(hasBeu: Boolean)(implicit p: Parameters)
    extends CoreBundle
    with HasCoreParameters {
  ...
  val rw_stall = Output(
    Bool()
  ) // stall rw, rw will have no effect while rw_stall
...
object VType {
  def fromUInt(that: UInt, ignore_vill: Boolean = false)(implicit
      p: Parameters
  ): VType = {
    val res = WireInit(0.U.asTypeOf(new VType))
    val in = that.asTypeOf(new VType)
    val vill =
      (in.max_vsew.U < in.vsew) || !in.lmul_ok || in.reserved =/= 0.U || in.vill
  ...
  def computeVL(
    ...
    VType
      .fromUInt(vtype, true)
      .vl(avl, currentVL, useCurrentVL, useMax, useZero)
}
...
class VType(implicit p: Parameters) extends CoreBundle {
  ...
  def lmul_ok: Bool = Mux(
    this.vlmul_sign,
    this.vlmul_mag =/= 0.U && ~this.vlmul_mag < max_vsew.U - this.vsew,
    true.B
  )

  def minVLMax: Int =
    ((maxVLMax / eLen) >> ((1 << vlmul_mag.getWidth) - 1)) max 1

  def vlMax: UInt =
    (maxVLMax.U >> (this.vsew +& Cat(this.vlmul_sign, ~this.vlmul_mag)))
      .andNot((minVLMax - 1).U)
  ...
  def vl(
    ...
    val atLeastMaxVLMax =
      useMax || Mux(useCurrentVL, currentVL >= maxVLMax.U, avl >= maxVLMax.U)
    val avl_lsbs = Mux(useCurrentVL, currentVL, avl)(maxVLMax.log2 - 1, 0)

    val atLeastVLMax =
      atLeastMaxVLMax || (avl_lsbs & (-maxVLMax.S >> (this.vsew +& Cat(
        this.vlmul_sign,
        ~this.vlmul_mag
      ))).asUInt.andNot((minVLMax - 1).U)).orR
    val isZero = vill || useZero
    Mux(!isZero && atLeastVLMax, vlMax, 0.U) | Mux(
      !isZero && !atLeastVLMax,
      avl_lsbs,
      0.U
    )
...
class CSRFile(
  ...
  val (supported_interrupts, delegable_interrupts) = {
    ...
    val supported_high_interrupts =
      if (io.interrupts.buserror.nonEmpty && !usingNMI)
        (BigInt(1) << CSR.busErrorIntCause).U
      else 0.U
  ...
  val reg_dscratch1 = (p(DebugModuleKey).map(_.nDscratch).getOrElse(1) > 1)
    .option(Reg(UInt(xLen.W)))
  val reg_singleStepped = Reg(Bool())

  val reg_mcontext = (coreParams.mcontextWidth > 0).option(
    RegInit(0.U(coreParams.mcontextWidth.W))
  )
  val reg_scontext = (coreParams.scontextWidth > 0).option(
    RegInit(0.U(coreParams.scontextWidth.W))
  )
  ...
  val (reg_mideleg, read_mideleg) = {
    ...
    (
      ...
      Mux(
        usingSupervisor.B,
        reg & delegable_interrupts | mideleg_always_hs,
        0.U
      )
    )
  }
  ...
  val reg_stvec = Reg(
    UInt((if (usingHypervisor) vaddrBitsExtended else vaddrBits).W)
  )
  ...
  // XLEN=32: 0x00002000
  // XLEN=64: 0x00003000
  val Seq(read_mtinst, read_htinst) =
    Seq(reg_mtinst_read_pseudo, reg_htinst_read_pseudo).map(r =>
      Cat(r, (xLen == 32).option(0.U).getOrElse(r), 0.U(12.W))
    )
  ...
  val reg_cycle =
    if (enableCommitLog)
      WideCounter(64, io.retire, inhibit = reg_mcountinhibit(0))
    else
      withClock(io.ungated_clock) {
        WideCounter(64, !io.csr_stall, inhibit = reg_mcountinhibit(0))
      }
  val reg_hpmevent = io.counters
    .map(c => RegInit(0.U(xLen.W)))(io.counters zip reg_hpmevent) foreach {
    case (c, e) => c.eventSel := e
  }
  ...
  val high_interrupts =
    (if (usingNMI) 0.U
     else io.interrupts.buserror.map(_ << CSR.busErrorIntCause).getOrElse(0.U))

  val pending_interrupts = high_interrupts | (read_mip & reg_mie)
  val d_interrupts = io.interrupts.debug << CSR.debugIntCause
  val (nmi_interrupts, nmiFlag) = io.interrupts.nmi
    .map(nmi =>
      (
        ((nmi.rnmi && reg_rnmie) << CSR.rnmiIntCause) |
          io.interrupts.buserror.map(_ << CSR.rnmiBEUCause).getOrElse(0.U),
        !io.interrupts.debug && nmi.rnmi && reg_rnmie
      )
    )
    .getOrElse(0.U, false.B)
  val m_interrupts = Mux(
    nmie && (reg_mstatus.prv <= PRV.S.U || reg_mstatus.mie),
    ~(~pending_interrupts | read_mideleg),
    0.U
  )
  ...
  val isaString = (if (coreParams.useRVE) "E" else "I") +
    ...
    (if (usingUser) "U" else "")
  val isaMax =
    (BigInt(log2Ceil(xLen) - 4) << (xLen - 2)) | isaStringToMask(isaString)
  ...
  if (coreParams.haveBasicCounters) {
    ...
    for (
      ((e, c), i) <- (reg_hpmevent.padTo(CSR.nHPM, 0.U)
  ...
  if (reg_pmp.nonEmpty) {
    ...
    for (i <- 0 until read_pmp.size by pmpCfgPerCSR)
      read_mapping += (CSRs.pmpcfg0 + pmpCfgIndex(i)) -> read_pmp
        .map(_.cfg)
        .slice(i, i + pmpCfgPerCSR)
        .asUInt
  ...
  def generateCustomCSR(csr: CustomCSR, csr_io: CustomCSRIO) = {
    ...
    val reg =
      csr.init.map(init => RegInit(init.U(xLen.W))).getOrElse(Reg(UInt(xLen.W)))
  ...
  val reg_custom =
    customCSRs.zip(io.customCSRs).map(t => generateCustomCSR(t._1, t._2))
  val reg_rocc =
    roccCSRs.zip(io.roccCSRs).map(t => generateCustomCSR(t._1, t._2))
  ...
  Seq(CSRs.mimpid, CSRs.marchid, CSRs.mvendorid, CSRs.mconfigptr).foreach(id =>
    read_mapping.getOrElseUpdate(id, 0.U)
  )
  ...
  val decoded_addr = {
    ...
    val pats =
      for (((k, _), i) <- read_mapping.zipWithIndex)
        yield (
          BitPat(k.U),
          (0 until read_mapping.size).map(j => BitPat((i == j).B))
        )
    val decoded = DecodeLogic(addr, Seq.fill(read_mapping.size)(X), pats)
    val unvirtualized_mapping =
      (for (((k, _), v) <- read_mapping zip decoded) yield k -> v.asBool).toMap
    ...
    for ((k, v) <- unvirtualized_mapping) yield k -> {
      val alt: Option[Bool] = CSR.mode(k) match {
        ...
        case PRV.S =>
          unvirtualized_mapping.contains(k + (1 << CSR.modeLSB)).option(false.B)
        case _ => None
      }
  ...
  val decode_table = Seq(
    ...
    coreParams.haveCFlush.option(
      CFLUSH_D_L1 -> List(N, N, N, N, N, N, N, N, N)
    ) ++
    ...
    (if (usingHypervisor) hlsv.map(_ -> List(N, N, N, N, N, N, N, N, Y))
     else Seq())
  val insn_call :: insn_break :: insn_ret :: insn_cease :: insn_wfi :: _ :: _ :: _ :: _ :: Nil = {
    val insn = ECALL.value.U | (io.rw.addr << 20)
    DecodeLogic(insn, decode_table(0)._2.map(x => X), decode_table)
      .map(system_insn && _.asBool)
  }

  for (io_dec <- io.decode) {
    val addr = io_dec.inst(31, 20)

    def decodeAny(m: LinkedHashMap[Int, Bits]): Bool =
      m.map { case (k: Int, _: Bits) => addr === k.U }.reduce(_ || _)
    def decodeFast(s: Seq[Int]): Bool = DecodeLogic(
      addr,
      s.map(_.U),
      (read_mapping.toMap -- s).keys.toList.map(_.U)
    )

    val _ :: is_break :: is_ret :: _ :: is_wfi :: is_sfence :: is_hfence_vvma :: is_hfence_gvma :: is_hlsv :: Nil =
      DecodeLogic(io_dec.inst, decode_table(0)._2.map(x => X), decode_table)
        .map(_.asBool)
    val is_counter =
      (addr.inRange(CSR.firstCtr.U, (CSR.firstCtr + CSR.nCtr).U) || addr
        .inRange(CSR.firstCtrH.U, (CSR.firstCtrH + CSR.nCtr).U))

    val allow_wfi =
      (!usingSupervisor).B || reg_mstatus.prv > PRV.S.U || !reg_mstatus.tw && (!reg_mstatus.v || !reg_hstatus.vtw)
    val allow_sfence_vma = (!usingVM).B || reg_mstatus.prv > PRV.S.U || !Mux(
      reg_mstatus.v,
      reg_hstatus.vtvm,
      reg_mstatus.tvm
    )
    val allow_hfence_vvma =
      (!usingHypervisor).B || !reg_mstatus.v && (reg_mstatus.prv >= PRV.S.U)
    val allow_hlsv =
      (!usingHypervisor).B || !reg_mstatus.v && (reg_mstatus.prv >= PRV.S.U || reg_hstatus.hu)
    val allow_sret = (!usingSupervisor).B || reg_mstatus.prv > PRV.S.U || !Mux(
      reg_mstatus.v,
      reg_hstatus.vtsr,
      reg_mstatus.tsr
    )
    ...
    val allow_counter =
      (reg_mstatus.prv > PRV.S.U || read_mcounteren(counter_addr)) &&
        (!usingSupervisor.B || reg_mstatus.prv >= PRV.S.U || read_scounteren(
          counter_addr
        )) &&
        (!usingHypervisor.B || !reg_mstatus.v || read_hcounteren(counter_addr))
    io_dec.fp_illegal := io.status.fs === 0.U || reg_mstatus.v && reg_vsstatus.fs === 0.U || !reg_misa(
      'f' - 'a'
    )
    ...
    io_dec.rocc_illegal := io.status.xs === 0.U || reg_mstatus.v && reg_vsstatus.xs === 0.U || !reg_misa(
      'x' - 'a'
    )
    val csr_addr_legal = reg_mstatus.prv >= CSR.mode(addr) ||
      usingHypervisor.B && !reg_mstatus.v && reg_mstatus.prv === PRV.S.U && CSR
        .mode(addr) === PRV.H.U
    ...
    io_dec.virtual_access_illegal := reg_mstatus.v && csr_exists && (CSR.mode(
      addr
    ) === PRV.H.U ||
      is_counter && read_mcounteren(counter_addr) && (!read_hcounteren(
        counter_addr
      ) || !reg_mstatus.prv(0) && !read_scounteren(counter_addr)) ||
      CSR.mode(addr) === PRV.S.U && !reg_mstatus.prv(0) ||
      addr === CSRs.satp.U && reg_mstatus.prv(0) && reg_hstatus.vtvm)

    io_dec.virtual_system_illegal := reg_mstatus.v && (is_hfence_vvma ||
      is_hfence_gvma ||
      is_hlsv ||
      is_wfi && (!reg_mstatus.prv(0) || !reg_mstatus.tw && reg_hstatus.vtw) ||
      is_ret && CSR
        .mode(addr) === PRV.S.U && (!reg_mstatus.prv(0) || reg_hstatus.vtsr) ||
  ...
  val cause =
    Mux(
      insn_call,
      Causes.user_ecall.U + Mux(
        reg_mstatus.prv(0) && reg_mstatus.v,
        PRV.H.U,
        reg_mstatus.prv
      ),
  ...
  val causeIsDebugTrigger =
    !cause(xLen - 1) && cause_lsbs === CSR.debugTriggerCause.U
  val causeIsDebugBreak = !cause(xLen - 1) && insn_break && Cat(
    reg_dcsr.ebreakm,
    reg_dcsr.ebreakh,
    reg_dcsr.ebreaks,
    reg_dcsr.ebreaku
  ...
  val debugException =
    p(DebugModuleKey).map(_.debugException).getOrElse(BigInt(0x808))
  val debugTVec = Mux(
    reg_debug,
    Mux(insn_break, debugEntry.U, debugException.U),
    debugEntry.U
  )
  val delegate = usingSupervisor.B && reg_mstatus.prv <= PRV.S.U && Mux(
    cause(xLen - 1),
    read_mideleg(cause_deleg_lsbs),
    read_medeleg(cause_deleg_lsbs)
  )
  val delegateVS = reg_mstatus.v && delegate && Mux(
    cause(xLen - 1),
    read_hideleg(cause_deleg_lsbs),
    read_hedeleg(cause_deleg_lsbs)
  )
  ...
  val notDebugTVec = {
    ...
    val interruptVec =
      Cat(base >> (mtvecInterruptAlign + mtvecBaseAlign), interruptOffset)
    val doVector = base(0) && cause(
      cause.getWidth - 1
    ) && (cause_lsbs >> mtvecInterruptAlign) === 0.U
    Mux(doVector, interruptVec, base >> mtvecBaseAlign << mtvecBaseAlign)
  }

  val causeIsRnmiInt = cause(xLen - 1) && cause(
    xLen - 2
  ) && (cause_lsbs === CSR.rnmiIntCause.U || cause_lsbs === CSR.rnmiBEUCause.U)
  val causeIsRnmiBEU =
    cause(xLen - 1) && cause(xLen - 2) && cause_lsbs === CSR.rnmiBEUCause.U
  val causeIsNmi = causeIsRnmiInt
  val nmiTVecInt =
    io.interrupts.nmi.map(nmi => nmi.rnmi_interrupt_vector).getOrElse(0.U)
  val nmiTVecXcpt =
    io.interrupts.nmi.map(nmi => nmi.rnmi_exception_vector).getOrElse(0.U)
  ...
  io.status.dprv := Mux(
    reg_mstatus.mprv && !reg_debug,
    reg_mstatus.mpp,
    reg_mstatus.prv
  )
  io.status.dv := reg_mstatus.v || Mux(
    reg_mstatus.mprv && !reg_debug,
    reg_mstatus.mpv,
    false.B
  )
  ...
  when(exception) {
    when(trapToDebug) {
      ...
      when(reg_rnmie) {
        ...
        reg_rnmie := false.B
        reg_mnepc := epc
        reg_mncause := (BigInt(1) << (xLen - 1)).U | Mux(
          causeIsRnmiBEU,
          3.U,
          2.U
        )
  ...
  for (i <- 0 until supported_interrupts.getWidth) {
    val en = exception && (supported_interrupts & (BigInt(
      1
    ) << i).U) =/= 0.U && cause === (BigInt(1) << (xLen - 1)).U + i.U
  ...
  val set_vs_dirty = WireDefault(
    io.vector.map(_.set_vs_dirty).getOrElse(false.B)
  )
  ...
  io.csrw_counter := Mux(
    coreParams.haveBasicCounters.B && csr_wen && (io.rw.addr.inRange(
      CSRs.mcycle.U,
      (CSRs.mcycle + CSR.nCtr).U
    ) || io.rw.addr.inRange(CSRs.mcycleh.U, (CSRs.mcycleh + CSR.nCtr).U)),
    UIntToOH(io.rw.addr(log2Ceil(CSR.nCtr + nPerfCounters) - 1, 0)),
    0.U
  )
  when(csr_wen) {
    val scause_mask =
      ((BigInt(1) << (xLen - 1)) + 31).U /* only implement 5 LSBs and MSB */
    val satp_valid_modes =
      0 +: (minPgLevels to pgLevels).map(new PTBR().pgLevelsToMode(_))
    ...
    when(decoded_addr(CSRs.mip)) {
      ...
      val new_mip = readModifyWriteCSR(io.rw.cmd, reg_mip.asUInt, io.rw.wdata)
        .asTypeOf(new MIP)
      if (usingSupervisor) {
    ...
    when(decoded_addr(CSRs.mcause)) {
      reg_mcause := wdata & ((BigInt(1) << (xLen - 1)) + (BigInt(
        1
      ) << whichInterrupt.getWidth) - 1).U
    }
    ...
    if (usingSupervisor) {
      ...
      when(decoded_addr(CSRs.sip)) {
        val new_sip = ((read_mip & ~read_mideleg) | (wdata & read_mideleg))
          .asTypeOf(new MIP())
    ...
    if (usingHypervisor) {
      ...
      when(decoded_addr(CSRs.hgatp)) {
        ...
        val valid_modes =
          0 +: (minPgLevels to pgLevels).map(new_hgatp.pgLevelsToMode(_))
      ...
      when(decoded_addr(CSRs.hip)) {
        val new_hip =
          ((read_mip & ~hs_delegable_interrupts) | (wdata & hs_delegable_interrupts))
            .asTypeOf(new MIP())
      ...
      val write_mhtinst_read_pseudo =
        wdata(13) && (xLen == 32).option(true.B).getOrElse(wdata(12))
      ...
      when(decoded_addr(CSRs.vsip)) {
        val new_vsip =
          ((read_hip & ~read_hideleg) | ((wdata << 1) & read_hideleg))
            .asTypeOf(new MIP())
      ...
      when(decoded_addr(CSRs.vsatp)) {
        ...
        when(mode_ok || !reg_mstatus.v) {
          reg_vsatp.ppn := new_vsatp.ppn(
            vpnBits.min(new_vsatp.ppn.getWidth) - 1,
            0
          )
      ...
      for ((bp, i) <- reg_bp.zipWithIndex) {
        when(i.U === reg_tselect && (!bp.control.dmode || reg_debug)) {
          ...
          when(decoded_addr(CSRs.tdata1)) {
            ...
            val nextChain =
              if (i >= nBreakpoints - 1) true.B else reg_bp(i + 1).control.chain
            val nextDMode =
              if (i >= nBreakpoints - 1) true.B else reg_bp(i + 1).control.dmode
            val newBPC =
              readModifyWriteCSR(io.rw.cmd, bp.control.asUInt, io.rw.wdata)
                .asTypeOf(bp.control)
            ...
            }.otherwise { bp.control.action := 0.U }
    ...
    reg_mcontext.foreach { r =>
      when(decoded_addr(CSRs.mcontext)) { r := wdata }
    }
    reg_scontext.foreach { r =>
      when(decoded_addr(CSRs.scontext)) { r := wdata }
    }
    if (reg_pmp.nonEmpty)
      for (
        ((pmp, next), i) <-
          (reg_pmp zip (reg_pmp.tail :+ reg_pmp.last)).zipWithIndex
      ) {
        require(xLen % pmp.cfg.getWidth == 0)
        when(decoded_addr(CSRs.pmpcfg0 + pmpCfgIndex(i)) && !pmp.cfgLocked) {
          val newCfg =
            (wdata >> ((i * pmp.cfg.getWidth) % xLen)).asTypeOf(new PMPConfig())
          pmp.cfg := newCfg
    ...
    for (
      (io, csr, reg) <- io.customCSRs.lazyZip(customCSRs).lazyZip(reg_custom)
    ) {
  ...
  def chooseInterrupt(masksIn: Seq[UInt]): (Bool, UInt) = {
    ...
    val any = masks
      .flatMap(m => priority.filter(_ < m.getWidth).map(i => m(i)))
      .reduce(_ || _)
    val which = PriorityMux(
      masks.flatMap(m => priority.filter(_ < m.getWidth).map(i => (m(i), i.U)))
    )
    (any, which)
  }
  ...
  def formTVec(x: UInt) = x andNot Mux(
    x(0),
    ((((BigInt(1) << mtvecInterruptAlign) - 1) << mtvecBaseAlign) | 2).U,
    2.U
  )
