...
class TLBResp(lgMaxSize: Int = 3)(implicit p: Parameters)
...
class TLBEntry(
  ...
  def getData(vpn: UInt) = OptimizationBarrier(
    data(sectorIdx(vpn)).asTypeOf(new TLBEntryData)
  )

  /** returns whether a sector hits */
  def sectorHit(vpn: UInt, virtual: Bool) =
    valid.orR && sectorTagMatch(vpn, virtual)

  /** returns whether tag matches vpn */
  def sectorTagMatch(vpn: UInt, virtual: Bool) =
    (((tag_vpn ^ vpn) >> nSectors.log2) === 0.U) && (tag_v === virtual)
  ...
  def hit(vpn: UInt, virtual: Bool): Bool = {
    if (superpage && usingVM) {
      ...
      for (j <- 0 until pgLevels) {
        ...
        tagMatch =
          tagMatch && (ignore || (tag_vpn ^ vpn)(base + n - 1, base) === 0.U)
      }
      tagMatch
    } else {
...
/** =Overview=
  ...
  *
  * TLB caches PTE and accelerates the address translation process. When tlb
  * miss happens, ask PTW(L2TLB) for Page Table Walk. Perform PMP and PMA check
  * during the translation and throw exception if there were any.
  *
  * ==Cache Structure==
  *   - Sectored Entry (PTE)
...
class TLB(instruction: Boolean, lgMaxSize: Int, cfg: TLBConfig)(implicit
  ...
  val memIdx =
    vpn.extract(cfg.nSectors.log2 + cfg.nSets.log2 - 1, cfg.nSectors.log2)

  /** TLB Entry */
  val sectored_entries = Reg(
    Vec(
      cfg.nSets,
      Vec(cfg.nWays / cfg.nSectors, new TLBEntry(cfg.nSectors, false, false))
    )
  )

  /** Superpage Entry */
  val superpage_entries = Reg(
    Vec(cfg.nSuperpageEntries, new TLBEntry(1, true, true))
  )
  ...
  /** Special Entry
    ...
    */
  val special_entry =
    (!pageGranularityPMPs).option(Reg(new TLBEntry(1, true, false)))
  def ordinary_entries = sectored_entries(memIdx) ++ superpage_entries
  def all_entries = ordinary_entries ++ special_entry
  def all_real_entries =
    sectored_entries.flatten ++ superpage_entries ++ special_entry
  ...
  val vstage1_en = usingHypervisor.B && priv_v && io.ptw.vsatp.mode(
    io.ptw.vsatp.mode.getWidth - 1
  )

  /** G-stage translation enable */
  val stage2_en = usingHypervisor.B && priv_v && io.ptw.hgatp.mode(
    io.ptw.hgatp.mode.getWidth - 1
  )
  ...
  /** Enable Virtual Memory when:
    ...
    *   Register
    * @see
    *   RV-priv spec 8.2.18 Virtual Supervisor Address Translation and
    *   Protection Register (vsatp)
    */
  val vm_enabled =
    (stage1_en || stage2_en) && priv_uses_vm && !io.req.bits.passthrough

  // flush guest entries on vsatp.MODE Bare <-> SvXX transitions
  val v_entries_use_stage1 = RegInit(false.B)
  val vsatp_mode_mismatch =
    priv_v && (vstage1_en =/= v_entries_use_stage1) && !io.req.bits.passthrough

  // share a single physical memory attribute checker (unshare if critical path)
  val refill_ppn =
    if (usingVM) io.ptw.resp.bits.pte.ppn(ppnBits - 1, 0) else 0.U

  /** refill signal */
  val do_refill = usingVM.B && io.ptw.resp.valid

  /** sfence invalidate refill */
  val invalidate_refill = state.isOneOf(
    s_request /* don't care */,
    s_wait_invalidate
  ) || io.sfence.valid
  // PMP
  val mpu_ppn = Mux(
  ...
  val mpu_priv = Mux[UInt](
    usingVM.B && (do_refill || io.req.bits.passthrough /* PTW */ ),
    PRV.S.U,
    Cat(io.ptw.status.debug, priv)
  )
  ...
  val cacheable =
    pma.io.resp.cacheable && (instruction || !usingDataScratchpad).B
  val homogeneous = TLBPageLookup(
    edge.manager.managers,
    xLen,
    p(CacheBlockBytes),
    BigInt(1) << pgIdxBits,
    1 << lgMaxSize
  )(mpu_physaddr).homogeneous
  // In M mode, if access DM address(debug module program buffer)
  val deny_access_to_debug = mpu_priv <= PRV.M.U && p(DebugModuleKey)
    .map(dmp => dmp.address.contains(mpu_physaddr))
    .getOrElse(false.B)
  ...
  when(do_refill) {
    ...
    when(special_entry.nonEmpty.B && !io.ptw.resp.bits.homogeneous) {
      special_entry.foreach(
        _.insert(r_refill_tag, refill_v, io.ptw.resp.bits.level, newEntry)
      )
    }.elsewhen(io.ptw.resp.bits.level < (pgLevels - 1).U) {
      val waddr = Mux(
        r_superpage_hit.valid && usingHypervisor.B,
        r_superpage_hit.bits,
        r_superpage_repl_addr
      )
      for ((e, i) <- superpage_entries.zipWithIndex)
        when(r_superpage_repl_addr === i.U) {
          e.insert(r_refill_tag, refill_v, io.ptw.resp.bits.level, newEntry)
          when(invalidate_refill) { e.invalidate() }
        }
      // refill sectored_hit
    }.otherwise {
      val r_memIdx = r_refill_tag.extract(
        cfg.nSectors.log2 + cfg.nSets.log2 - 1,
        cfg.nSectors.log2
      )
      val waddr =
        Mux(r_sectored_hit.valid, r_sectored_hit.bits, r_sectored_repl_addr)
      for ((e, i) <- sectored_entries(r_memIdx).zipWithIndex)
        when(waddr === i.U) {
  ...
  val ppn = Mux1H(
    hitsVec :+ !vm_enabled,
    (all_entries zip entries).map { case (entry, data) =>
      entry.ppn(vpn, data)
    } :+ vpn(ppnBits - 1, 0)
  )
  ...
  val priv_rw_ok = Mux(!priv_s || sum, entries.map(_.u).asUInt, 0.U) | Mux(
    priv_s,
  ...
  val stage1_bypass = Fill(entries.size, usingHypervisor.B) & (Fill(
    entries.size,
    !stage1_en
  ) | entries.map(_.ae_stage2).asUInt)
  ...
  val r_array = Cat(
    ...
    (priv_rw_ok & (entries
      .map(_.sr)
      .asUInt | Mux(mxr, entries.map(_.sx).asUInt, 0.U))) | stage1_bypass
  )
  val w_array =
    Cat(true.B, (priv_rw_ok & entries.map(_.sw).asUInt) | stage1_bypass)
  val x_array =
    Cat(true.B, (priv_x_ok & entries.map(_.sx).asUInt) | stage1_bypass)
  val stage2_bypass = Fill(entries.size, !stage2_en)
  val hr_array = Cat(
    true.B,
    entries.map(_.hr).asUInt | Mux(
      io.ptw.status.mxr,
      entries.map(_.hx).asUInt,
      0.U
    ) | stage2_bypass
  )
  ...
  val pr_array = Cat(
    Fill(nPhysicalEntries, prot_r),
    normal_entries.map(_.pr).asUInt
  ) & ~(ptw_ae_array | final_ae_array)
  // user mode can write: PMA OK, TLB OK, AE OK
  val pw_array = Cat(
    Fill(nPhysicalEntries, prot_w),
    normal_entries.map(_.pw).asUInt
  ) & ~(ptw_ae_array | final_ae_array)
  // user mode can write: PMA OK, TLB OK, AE OK
  val px_array = Cat(
    Fill(nPhysicalEntries, prot_x),
    normal_entries.map(_.px).asUInt
  ) & ~(ptw_ae_array | final_ae_array)
  // put effect
  val eff_array =
    Cat(Fill(nPhysicalEntries, prot_eff), normal_entries.map(_.eff).asUInt)
  // cacheable
  val c_array =
    Cat(Fill(nPhysicalEntries, cacheable), normal_entries.map(_.c).asUInt)
  // put partial
  val ppp_array =
    Cat(Fill(nPhysicalEntries, prot_pp), normal_entries.map(_.ppp).asUInt)
  // atomic arithmetic
  val paa_array =
    Cat(Fill(nPhysicalEntries, prot_aa), normal_entries.map(_.paa).asUInt)
  // atomic logic
  val pal_array =
    Cat(Fill(nPhysicalEntries, prot_al), normal_entries.map(_.pal).asUInt)
  val ppp_array_if_cached = ppp_array | c_array
  val paa_array_if_cached =
    paa_array | (if (usingAtomicsInCache) c_array else 0.U)
  val pal_array_if_cached =
    pal_array | (if (usingAtomicsInCache) c_array else 0.U)
  val prefetchable_array = Cat(
    (cacheable && homogeneous) << (nPhysicalEntries - 1),
  ...
  def badVA(guestPA: Boolean): Bool = {
    val additionalPgLevels =
      (if (guestPA) io.ptw.hgatp else satp).additionalPgLevels
    ...
    (for (i <- 0 until nPgLevelChoices) yield {
      val mask = ((BigInt(1) << vaddrBitsExtended) - (BigInt(
        1
      ) << (minVAddrBits + i * pgLevelBits - signed.toInt))).U
  ...
  val bad_va =
    if (!usingVM || (minPgLevels == pgLevels && vaddrBits == vaddrBitsExtended))
      false.B
  ...
  val cmd_write_perms =
    cmd_write ||
      io.req.bits.cmd
        .isOneOf(M_FLUSH_ALL, M_WOK) // not a write, but needs write permissions

  val lrscAllowed =
    Mux((usingDataScratchpad || usingAtomicsOnlyForIO).B, 0.U, c_array)
  val ae_array =
  ...
  val ae_ld_array = Mux(
    cmd_read,
    ae_array | ~Mux(cmd_readx, pr_array & px_array, pr_array),
    0.U
  )
  val ae_st_array =
  ...
  val pf_ld_array = Mux(
    ...
    ((~Mux(
      ...
      x_array,
      r_array
    ) & ~ptw_ae_array) | ptw_pf_array) & ~ptw_gf_array,
    0.U
  )
  val pf_st_array = Mux(
    cmd_write_perms,
    ((~w_array & ~ptw_ae_array) | ptw_pf_array) & ~ptw_gf_array,
    0.U
  )
  val pf_inst_array =
    ((~x_array & ~ptw_ae_array) | ptw_pf_array) & ~ptw_gf_array
  val gf_ld_array = Mux(
    priv_v && cmd_read,
    (~Mux(cmd_readx, hx_array, hr_array) | ptw_gf_array) & ~ptw_ae_array,
    0.U
  )
  val gf_st_array = Mux(
    priv_v && cmd_write_perms,
    (~hw_array | ptw_gf_array) & ~ptw_ae_array,
    0.U
  )
  val gf_inst_array =
    Mux(priv_v, (~hx_array | ptw_gf_array) & ~ptw_ae_array, 0.U)

  val gpa_hits = {
    val need_gpa_mask =
      if (instruction) gf_inst_array else gf_ld_array | gf_st_array
    val hit_mask = Fill(
      ordinary_entries.size,
      r_gpa_valid && r_gpa_vpn === vpn
    ) | Fill(all_entries.size, !vstage1_en)
  ...
  val sectored_plru =
    new SetAssocLRU(cfg.nSets, sectored_entries.head.size, "plru")
  ...
  io.resp.gpa := {
    ...
    val offset = Mux(
      io.resp.gpa_is_pte,
      r_gpa(pgIdxBits - 1, 0),
      io.req.bits.vaddr(pgIdxBits - 1, 0)
    )
    Cat(page, offset)
  }
  ...
  if (usingVM) {
    ...
    when(io.req.fire && tlb_miss) {
      ...
      r_superpage_repl_addr := replacementEntry(
        superpage_entries,
        superpage_plru.way
      )
      r_sectored_repl_addr := replacementEntry(
        sectored_entries(memIdx),
    ...
    ccover(
      io.ptw.req.valid && !io.ptw.req.ready,
      "PTW_STALL",
      "TLB miss, but PTW busy"
    )
    ccover(
      state === s_wait_invalidate,
      "SFENCE_DURING_REFILL",
      "flush TLB during TLB refill"
    )
    ccover(
      sfence && !io.sfence.bits.rs1 && !io.sfence.bits.rs2,
      "SFENCE_ALL",
      "flush TLB"
    )
    ccover(
  ...
  def ccover(cond: Bool, label: String, desc: String)(implicit
    ...
    property.cover(
      cond,
      s"${if (instruction) "I" else "D"}TLB_$label",
      "MemorySystem;;" + desc
    )
