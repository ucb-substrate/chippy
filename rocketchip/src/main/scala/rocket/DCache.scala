...
trait InlineInstance { self: chisel3.experimental.BaseModule =>
  chisel3.experimental.annotate(self)(
    Seq(firrtl.passes.InlineAnnotation(self.toNamed))
  )
}

class DCacheErrors(implicit p: Parameters)
    extends L1HellaCacheBundle()(p)
    with CanHaveErrors {
  val correctable =
    (cacheParams.tagCode.canCorrect || cacheParams.dataCode.canCorrect)
      .option(Valid(UInt(paddrBits.W)))
  val uncorrectable =
    (cacheParams.tagCode.canDetect || cacheParams.dataCode.canDetect)
      .option(Valid(UInt(paddrBits.W)))
...
class DCacheDataArray(implicit p: Parameters) extends L1HellaCacheModule()(p) {
  ...
  require(
    rowBits % subWordBits == 0,
    "rowBits must be a multiple of subWordBits"
  )
  val eccMask =
    if (eccBits == subWordBits) Seq(true.B) else io.req.bits.eccMask.asBools
  val wMask =
    if (nWays == 1) eccMask
    else (0 until nWays).flatMap(i => eccMask.map(_ && io.req.bits.way_en(i)))
  val wWords = io.req.bits.wdata.grouped(encBits * (subWordBits / eccBits))
  val addr = io.req.bits.addr >> rowOffBits
  val data_arrays = Seq.tabulate(rowBits / subWordBits) { i =>
    DescribedSRAM(
  ...
  val rdata = for ((array, i) <- data_arrays.zipWithIndex) yield {
    val valid =
      io.req.valid && ((data_arrays.size == 1).B || io.req.bits.wordMask(i))
    when(valid && io.req.bits.write) {
      val wMaskSlice = (0 until wMask.size)
        .filter(j =>
          i % (wordBits / subWordBits) == (j % (wordBytes / eccBytes)) / (subWordBytes / eccBytes)
        )
        .map(wMask(_))
      val wData = wWords(i).grouped(encBits)
      array.write(
        addr,
        VecInit((0 until nWays).flatMap(i => wData)),
        wMaskSlice
      )
    }
  ...
  (io.resp zip rdata.transpose).foreach { case (resp, data) =>
    resp := data.asUInt
  }
}

class DCacheMetadataReq(implicit p: Parameters)
    extends L1HellaCacheBundle()(p) {
  val write = Bool()
...
class DCache(staticIdForMetadataUseOnly: Int, val crossing: ClockCrossingType)(
...
class DCacheModule(outer: DCache) extends HellaCacheModule(outer) {
  ...
  require(
    subWordBits % eccBits == 0,
    "subWordBits must be a multiple of eccBits"
  )
  require(eccBytes == 1 || !dECC.isInstanceOf[IdentityCode])
  require(
    cacheParams.silentDrop || cacheParams.acquireBeforeRelease,
    "!silentDrop requires acquireBeforeRelease"
  )
  val usingRMW = eccBytes > 1 || usingAtomicsInCache
  val mmioOffset = outer.firstMMIO
  edge.manager.requireFifo(
    TLFIFOFixer.allVolatile
  ) // TileLink pipelining MMIO requests
  ...
  class DCacheModuleImpl { // entering gated-clock domain
    ...
    val tlb = Module(
      new TLB(
        false,
        log2Ceil(coreDataBytes),
        TLBConfig(
          nTLBSets,
          nTLBWays,
          cacheParams.nTLBBasePageSectors,
          cacheParams.nTLBSuperpages
        )
      )
    )
    val pma_checker = Module(
      new TLB(
        false,
        log2Ceil(coreDataBytes),
        TLBConfig(
    ...
    // tags
    val replacer =
      ReplacementPolicy.fromString(cacheParams.replacementPolicy, nWays)

    /** Metadata Arbiter: 0: Tag update on reset 1: Tag update on ECC error 2:
      * Tag update on hit 3: Tag update on refill 4: Tag update on release 5:
      * Tag update on flush 6: Tag update on probe 7: Tag update on CPU request
      */
    val metaArb = Module(
      new Arbiter(new DCacheMetadataReq, 8) with InlineInstance
    )
    ...
    /** Data Arbiter 0: data from pending store buffer 1: data from TL-D refill
      * 2: release to TL-A 3: hit path to CPU
      */
    val dataArb = Module(new Arbiter(new DCacheDataReq, 4) with InlineInstance)
    dataArb.io.in.tail.foreach(
      _.bits.wdata := dataArb.io.in.head.bits.wdata
    ) // tie off write ports by default
    ...
    tl_out.a <> {
      val a_queue_depth = outer.crossing match {
        case RationalCrossing(
              _
            ) => // TODO make this depend on the actual ratio?
          if (cacheParams.separateUncachedResp) (maxUncachedInFlight + 1) / 2
          else 2 min maxUncachedInFlight - 1
        case SynchronousCrossing(BufferParams.none) =>
          1 // Need some buffering to guarantee livelock freedom
        case SynchronousCrossing(_) =>
          0 // Adequate buffering within the crossing
    ...
    val (tl_out_c, release_queue_empty) =
      if (cacheParams.acquireBeforeRelease) {
        val q = Module(
          new Queue(chiselTypeOf(tl_out.c.bits), cacheDataBeats, flow = true)
        )
        tl_out.c <> q.io.deq
    ...
    val probe_bits =
      RegEnable(tl_out.b.bits, tl_out.b.fire) // TODO has data now :(
    ...
    s0_req.addr := Cat(
      metaArb.io.out.bits.addr >> blockOffBits,
      io.cpu.req.bits.addr(blockOffBits - 1, 0)
    )
    s0_req.idx.foreach(
      _ := Cat(metaArb.io.out.bits.idx, s0_req.addr(blockOffBits - 1, 0))
    )
    when(!metaArb.io.in(7).ready) { s0_req.phys := true.B }
    val s1_req = RegEnable(s0_req, s0_clk_en)
    val s1_vaddr = Cat(
      s1_req.idx.getOrElse(s1_req.addr) >> tagLSB,
      s1_req.addr(tagLSB - 1, 0)
    )
    ...
    val flushCounter = RegInit(
      (nSets * (nWays - 1)).U(log2Ceil(nSets * nWays).W)
    )
    ...
    val inWriteback =
      release_state.isOneOf(s_voluntary_writeback, s_probe_rep_dirty)
    val releaseWay = Wire(UInt())
    io.cpu.req.ready := (release_state === s_ready) && !cached_grant_wait && !s1_nack

    // I/O MSHRs
    val uncachedInFlight = RegInit(
      VecInit(Seq.fill(maxUncachedInFlight)(false.B))
    )
    ...
    dataArb.io.in(3).valid := io.cpu.req.valid && likelyNeedsRead(
      io.cpu.req.bits
    )
    ...
    dataArb.io.in(3).bits.wordMask := {
      val mask = (subWordBytes.log2 until rowOffBits).foldLeft(1.U) {
        case (in, i) =>
          val upper_mask = Mux(
            (i >= wordBytes.log2).B || io.cpu.req.bits.size <= i.U,
            0.U,
    ...
    val s1_did_read = RegEnable(
      dataArb.io.in(3).ready && (io.cpu.req.valid && needsRead(
        io.cpu.req.bits
      )),
      s0_clk_en
    )
    ...
    val (s1_hit_way, s1_hit_state, s1_meta) =
      if (usingDataScratchpad) {
        val baseAddr = p(LookupByHartId)(
          _.dcache.flatMap(_.scratch.map(_.U)),
          io_hartid.get
        ) | io_mmio_address_prefix.get
        val inScratchpad =
          s1_paddr >= baseAddr && s1_paddr < baseAddr + (nSets * cacheBlockBytes).U
        val hitState =
          Mux(inScratchpad, ClientMetadata.maximum, ClientMetadata.onReset)
        ...
        when(metaReq.valid && metaReq.bits.write) {
          val wmask =
            if (nWays == 1) Seq(true.B) else metaReq.bits.way_en.asBools
          tag_array.write(
            metaIdx,
            VecInit(Seq.fill(nWays)(metaReq.bits.data)),
            wmask
          )
        }
        val s1_meta =
          tag_array.read(metaIdx, metaReq.valid && !metaReq.bits.write)
        val s1_meta_uncorrected =
          s1_meta.map(tECC.decode(_).uncorrected.asTypeOf(new L1Metadata))
        val s1_tag = s1_paddr >> tagLSB
        val s1_meta_hit_way = s1_meta_uncorrected
          .map(r => r.coh.isValid() && r.tag === s1_tag)
          .asUInt
    ...
    val s1_data_way = WireDefault(
      if (nWays == 1) 1.U else Mux(inWriteback, releaseWay, s1_hit_way)
    )
    val tl_d_data_encoded = Wire(
      chiselTypeOf(encodeData(tl_out.d.bits.data, false.B))
    )
    val s1_all_data_ways = VecInit(
      data.io.resp ++ (!cacheParams.separateUncachedResp).option(
        tl_d_data_encoded
      )
    )
    val s1_mask_xwr =
      new StoreGen(s1_req.size, s1_req.addr, 0.U, wordBytes).mask
    ...
    assert(
      !(s1_valid_masked && s1_req.cmd === M_PWR) || (s1_mask_xwr | ~io.cpu.s1_data.mask).andR
    )
    ...
    val s2_uncached_resp_addr = Reg(
      chiselTypeOf(s2_req.addr)
    ) // should be DCE'd in synthesis
    ...
    val s2_vaddr = Cat(
      RegEnable(s1_vaddr, s1_valid_not_nacked || s1_flush_valid) >> tagLSB,
      s2_req.addr(tagLSB - 1, 0)
    )
    ...
    val s2_meta_correctable_errors =
      s1_meta_decoded.map(m => RegEnable(m.correctable, s1_meta_clk_en)).asUInt
    val s2_meta_uncorrectable_errors = s1_meta_decoded
      .map(m => RegEnable(m.uncorrectable, s1_meta_clk_en))
      .asUInt
    val s2_meta_error_uncorrectable = s2_meta_uncorrectable_errors.orR
    val s2_meta_corrected = s1_meta_decoded.map(m =>
      RegEnable(m.corrected, s1_meta_clk_en).asTypeOf(new L1Metadata)
    )
    val s2_meta_error =
      (s2_meta_uncorrectable_errors | s2_meta_correctable_errors).orR
    ...
    val s2_data = {
      ...
      val word_en = Mux(
        inWriteback,
        Fill(wordsPerRow, 1.U),
        Mux(s1_did_read, s1_read_mask, 0.U)
      )
      val s1_way_words = s1_all_data_ways.map(
        _.grouped(dECC.width(eccBits) * (subWordBits / eccBits))
      )
      if (cacheParams.pipelineWayMux) {
        ...
        (for (i <- 0 until wordsPerRow) yield {
          ...
          val s2_way_words = (0 until nWays).map(j =>
            RegEnable(s1_way_words(j)(i), en && word_en(i))
          )
          (0 until nWays)
            .map(j => Mux(s2_way_en(j), s2_way_words(j), 0.U))
            .reduce(_ | _)
        }).asUInt
      } else {
        val s1_word_en = Mux(
          ...
          UIntToOH(
            uncachedResp.addr
              .extract(log2Up(rowBits / 8) - 1, log2Up(wordBytes)),
            wordsPerRow
          )
        )
        (for (i <- 0 until wordsPerRow) yield {
          RegEnable(
            Mux1H(Mux(s1_word_en(i), s1_data_way, 0.U), s1_way_words.map(_(i))),
            en
          )
        }).asUInt
      }
    ...
    val s2_hit_state =
      RegEnable(s1_hit_state, s1_valid_not_nacked || s1_flush_valid)
    ...
    val (s2_hit, s2_grow_param, s2_new_hit_state) =
      s2_hit_state.onAccess(s2_req.cmd)
    val s2_data_decoded = decodeData(s2_data)
    val s2_word_idx =
      s2_req.addr.extract(log2Up(rowBits / 8) - 1, log2Up(wordBytes))
    ...
    val s2_data_uncorrected =
      (s2_data_decoded.map(_.uncorrected): Seq[UInt]).asUInt
    val s2_valid_hit_maybe_flush_pre_data_ecc_and_waw =
    ...
    val s2_no_alloc_hazard =
      if (!usingVM || pgIdxBits >= untagBits) false.B
        ...
        val s1_need_check =
          any_no_alloc_in_flight || s2_valid && s2_req.no_alloc

        val concerns =
          (uncachedInFlight zip uncachedReqs) :+ (s2_valid && s2_req.no_alloc, s2_req)
        val s1_uncached_hits = concerns.map { c =>
          val concern_wmask =
            new StoreGen(c._2.size, c._2.addr, 0.U, wordBytes).mask
          val addr_match = (c._2.addr ^ s1_paddr)(
            pgIdxBits + pgLevelBits - 1,
            wordBytes.log2
          ) === 0.U
          val mask_match =
            (concern_wmask & s1_mask_xwr).orR || c._2.cmd === M_PWR || s1_req.cmd === M_PWR
          val cmd_match = isWrite(c._2.cmd) || isWrite(s1_req.cmd)
          c._1 && s1_need_check && cmd_match && addr_match && mask_match
        }

        val s2_uncached_hits =
          RegEnable(s1_uncached_hits.asUInt, s1_valid_not_nacked)
    ...
    val s2_valid_flush_line =
      s2_valid_hit_maybe_flush_pre_data_ecc_and_waw && s2_cmd_flush_line
    val s2_valid_hit_pre_data_ecc =
      s2_valid_hit_pre_data_ecc_and_waw && (!s2_waw_hazard || s2_store_merge)
    ...
    val s2_valid_miss =
      s2_valid_masked && s2_readwrite && !s2_meta_error && !s2_hit
    val s2_uncached =
      !s2_pma.cacheable || s2_req.no_alloc && !s2_pma.must_alloc && !s2_hit_valid
    val s2_valid_cached_miss =
      s2_valid_miss && !s2_uncached && !uncachedInFlight.asUInt.orR
    ...
    val s2_valid_uncached_pending =
      s2_valid_miss && s2_uncached && !uncachedInFlight.asUInt.andR
    val s2_victim_way = UIntToOH(
      RegEnable(s1_victim_way, s1_valid_not_nacked || s1_flush_valid)
    )
    ...
    val s2_victim_state = Mux(
      s2_hit_valid,
      s2_hit_state,
      Mux1H(s2_victim_way, s2_meta_corrected).coh
    )

    val (s2_prb_ack_data, s2_report_param, probeNewCoh) =
      s2_probe_state.onProbe(probe_bits.param)
    ...
    when(
      io.cpu.s2_nack || (s2_valid_hit_pre_data_ecc_and_waw && s2_update_meta)
    ) { s1_nack := true.B }

    // tag updates on ECC errors
    val s2_first_meta_corrected =
      PriorityMux(s2_meta_correctable_errors, s2_meta_corrected)
    metaArb.io
      .in(1)
      .valid := s2_meta_error && (s2_valid_masked || s2_flush_valid_pre_tag_ecc || s2_probe)
    ...
    metaArb.io.in(1).bits.idx := Mux(
      s2_probe,
      probeIdx(probe_bits),
      s2_vaddr(idxMSB, idxLSB)
    )
    metaArb.io.in(1).bits.addr := Cat(
      io.cpu.req.bits.addr >> untagBits,
      metaArb.io.in(1).bits.idx << blockOffBits
    )
    ...
    metaArb.io
      .in(2)
      .valid := s2_valid_hit_pre_data_ecc_and_waw && s2_update_meta
    ...
    when(
      (s2_valid_hit && s2_lr && !cached_grant_wait || s2_valid_cached_miss) && !io.cpu.s2_kill
    ) {
    ...
    val s2_correct = s2_data_error && !any_pstore_valid && !RegNext(
      any_pstore_valid || s2_valid
    ) && usingDataScratchpad.B
    // pending store buffer
    val s2_valid_correct =
      s2_valid_hit_pre_data_ecc_and_waw && s2_correct && !io.cpu.s2_kill
    ...
    val pstore1_data =
      RegEnable(io.cpu.s1_data.data, s1_valid_not_nacked && s1_write)
    ...
    val pstore1_rmw = usingRMW.B && RegEnable(
      needsRead(s1_req),
      s1_valid_not_nacked && s1_write
    )
    val pstore1_merge_likely =
    ...
    val pstore_drain_opportunistic = !(io.cpu.req.valid && likelyNeedsRead(
      io.cpu.req.bits
    ...
    def pstore1_valid_not_rmw(s2_kill: Bool) =
      s2_valid_hit_pre_data_ecc && s2_write && !s2_kill || pstore1_held
    val pstore1_valid = s2_store_valid || pstore1_held
    any_pstore_valid := pstore1_held || pstore2_valid
    val pstore_drain_structural =
      pstore1_valid_likely && pstore2_valid && ((s1_valid && s1_write) || pstore1_rmw)
    assert(
      pstore1_rmw || pstore1_valid_not_rmw(io.cpu.s2_kill) === pstore1_valid
    )
    ccover(
      pstore_drain_structural,
      "STORE_STRUCTURAL_HAZARD",
      "D$ read-modify-write structural hazard"
    )
    ccover(
      pstore1_valid && pstore_drain_on_miss,
      "STORE_DRAIN_ON_MISS",
      "D$ store buffer drain on miss"
    )
    ccover(
      s1_valid_not_nacked && s1_waw_hazard,
      "WAW_HAZARD",
    ...
    val advance_pstore1 =
      (pstore1_valid || s2_valid_correct) && (pstore2_valid === pstore_drain)
    pstore2_valid := pstore2_valid && !pstore_drain || advance_pstore1
    val pstore2_addr =
      RegEnable(Mux(s2_correct, s2_vaddr, pstore1_addr), advance_pstore1)
    val pstore2_way =
      RegEnable(Mux(s2_correct, s2_hit_way, pstore1_way), advance_pstore1)
    ...
    s2_store_merge := (if (eccBytes == 1) false.B
                         ...
                         // WAW hazards
                         val wordMatch = (eccMask(
                           pstore2_storegen_mask
                         ) | ~eccMask(pstore1_mask)).andR
                         val idxMatch = s2_vaddr(
                           untagBits - 1,
                           log2Ceil(wordBytes)
                         ) === pstore2_addr(untagBits - 1, log2Ceil(wordBytes))
    ...
    dataArb.io.in(0).bits.wordMask := {
      val eccMask = dataArb.io
        .in(0)
        .bits
        .eccMask
        .asBools
        .grouped(subWordBytes / eccBytes)
        .map(_.orR)
        .toSeq
        .asUInt
      val wordMask = UIntToOH(
        Mux(pstore2_valid, pstore2_addr, pstore1_addr)
          .extract(rowOffBits - 1, wordBytes.log2)
      )
      FillInterleaved(wordBytes / subWordBytes, wordMask) & Fill(
        rowBytes / wordBytes,
        eccMask
      )
    }
    dataArb.io.in(0).bits.eccMask := eccMask(
      Mux(pstore2_valid, pstore2_storegen_mask, pstore1_mask)
    )
    ...
    def s1Depends(addr: UInt, mask: UInt) =
      addr(idxMSB, wordOffBits) === s1_vaddr(idxMSB, wordOffBits) &&
        Mux(
          s1_write,
          (eccByteMask(mask) & eccByteMask(s1_mask_xwr)).orR,
          (mask & s1_mask_xwr).orR
        )
    val s1_hazard =
    ...
    io.cpu.s2_nack_cause_raw := RegNext(
      s1_raw_hazard
    ) || !(!s2_waw_hazard || s2_store_merge)

    // Prepare a TileLink request message that initiates a transaction
    val a_source = PriorityEncoder(
      ~uncachedInFlight.asUInt << mmioOffset
    ) // skip the MSHR
    ...
    val a_mask = pstore1_mask << (access_address.extract(
      beatBytes.log2 - 1,
      wordBytes.log2
    ) << 3)
    ...
    val putpartial =
      edge.Put(a_source, access_address, a_size, a_data, a_mask)._2
    val atomics = if (edge.manager.anySupportLogical) {
      MuxLookup(
        ...
        Seq(
          M_XA_SWAP -> edge
            .Logical(a_source, access_address, a_size, a_data, TLAtomics.SWAP)
            ._2,
          M_XA_XOR -> edge
            .Logical(a_source, access_address, a_size, a_data, TLAtomics.XOR)
            ._2,
          M_XA_OR -> edge
            .Logical(a_source, access_address, a_size, a_data, TLAtomics.OR)
            ._2,
          M_XA_AND -> edge
            .Logical(a_source, access_address, a_size, a_data, TLAtomics.AND)
            ._2,
          M_XA_ADD -> edge
            .Arithmetic(a_source, access_address, a_size, a_data, TLAtomics.ADD)
            ._2,
          M_XA_MIN -> edge
            .Arithmetic(a_source, access_address, a_size, a_data, TLAtomics.MIN)
            ._2,
          M_XA_MAX -> edge
            .Arithmetic(a_source, access_address, a_size, a_data, TLAtomics.MAX)
            ._2,
          M_XA_MINU -> edge
            .Arithmetic(
              a_source,
    ...
    tl_out_a.bits := Mux(
      !s2_uncached,
      acquire(s2_vaddr, s2_req.addr, s2_grow_param),
      Mux(
        !s2_write,
        get,
        Mux(s2_req.cmd === M_PWR, putpartial, Mux(!s2_read, put, atomics))
      )
    )
    ...
    val a_sel =
      UIntToOH(a_source, maxUncachedInFlight + mmioOffset) >> mmioOffset
    when(tl_out_a.fire) {
      when(s2_uncached) {
        (a_sel.asBools zip (uncachedInFlight zip uncachedReqs)) foreach {
          case (s, (f, r)) =>
            when(s) {
              ...
              r.cmd := Mux(
                s2_write,
                Mux(s2_req.cmd === M_PWR, M_PWR, M_XWR),
                M_XRD
              )
            }
        }
    ...
    val (d_opc, grantIsUncached, grantIsUncachedData) = {
      ...
      val uncachedGrantOpcodes =
        uncachedGrantOpcodesWithData ++ uncachedGrantOpcodesSansData
      ...
      if (usingDataScratchpad) {
        ...
        val data = DecodeLogic(
          opc,
          uncachedGrantOpcodesWithData,
          uncachedGrantOpcodesSansData
        )
        (opc, true.B, data)
      } else {
        (
          whole_opc,
          whole_opc.isOneOf(uncachedGrantOpcodes),
          whole_opc.isOneOf(uncachedGrantOpcodesWithData)
        )
      }
    }
    ...
    val grantIsVoluntary =
      d_opc === ReleaseAck // Clears a different pending bit
    ...
    val canAcceptCachedGrant = !release_state.isOneOf(
    ...
    tl_out.d.ready := Mux(
      grantIsCached,
      (!d_first || tl_out.e.ready) && canAcceptCachedGrant,
      true.B
    )
    val uncachedRespIdxOH = UIntToOH(
      tl_out.d.bits.source,
      maxUncachedInFlight + mmioOffset
    ) >> mmioOffset
        ...
        (uncachedRespIdxOH.asBools zip uncachedInFlight) foreach {
          case (s, f) =>
            when(s && d_last) {
              assert(
                f,
                "An AccessAck was unexpected by the dcache."
    ...
    dataArb.io
      .in(1)
      .valid := tl_out.d.valid && grantIsRefill && canAcceptCachedGrant
    ...
    if (!usingDataScratchpad) {
      ...
      dataArb.io
        .in(1)
        .bits
        .addr := (s2_vaddr >> idxLSB) << idxLSB | d_address_inc
    ...
    metaArb.io.in(3).bits.data := tECC.encode(
      L1Metadata(
        s2_req.addr >> tagLSB,
        s2_hit_state.onGrant(s2_req.cmd, tl_out.d.bits.param)
      ).asUInt
    )
    ...
    val block_probe_for_core_progress =
      blockProbeAfterGrantCount > 0.U || lrscValid
    val block_probe_for_pending_release_ack =
      release_ack_wait && (tl_out.b.bits.address ^ release_ack_addr)(
        ((pgIdxBits + pgLevelBits) min paddrBits) - 1,
        idxLSB
      ) === 0.U
    val block_probe_for_ordering =
      releaseInFlight || block_probe_for_pending_release_ack || grantInProgress
    metaArb.io
      .in(6)
      .valid := tl_out.b.valid && (!block_probe_for_core_progress || lrscBackingOff)
    ...
    metaArb.io.in(6).bits.addr := Cat(
      io.cpu.req.bits.addr >> paddrBits,
    ...
    s1_victim_way := (if (replacer.perSet && nWays > 1) {
                        ...
                        val s1_repl_idx =
                          s1_req.addr(idxBits + blockOffBits - 1, blockOffBits)
                        val s2_repl_idx =
                          s2_vaddr(idxBits + blockOffBits - 1, blockOffBits)
                        val s2_repl_state = Reg(UInt(replacer.nBits.W))
                        val s2_new_repl_state = replacer.get_next_state(
                          s2_repl_state,
                          OHToUInt(s2_hit_way)
                        )
                        val s2_repl_wen =
                          s2_valid_masked && s2_hit_way.orR && s2_repl_state =/= s2_new_repl_state
                        val s1_repl_state = Mux(
                          s2_repl_wen && s2_repl_idx === s1_repl_idx,
                          s2_new_repl_state,
                          repl_array(s1_repl_idx)
                        )
    ...
    val s2_release_data_valid = RegNext(
      s1_release_data_valid && !releaseRejected
    )
    releaseRejected := s2_release_data_valid && !tl_out_c.fire
    val releaseDataBeat = Cat(0.U, c_count) + Mux(
      releaseRejected,
      0.U,
      s1_release_data_valid + Cat(0.U, s2_release_data_valid)
    )

    val nackResponseMessage =
      edge.ProbeAck(b = probe_bits, reportPermissions = TLPermissions.NtoN)
    val cleanReleaseMessage =
      edge.ProbeAck(b = probe_bits, reportPermissions = s2_report_param)
    val dirtyReleaseMessage = edge.ProbeAck(
      b = probe_bits,
    ...
    if (!usingDataScratchpad) {
      when(s2_victimize) {
        ...
        val discard_line = s2_valid_flush_line && s2_req.size(
          1
        ) || s2_flush_valid && flushing_req.size(1)
        release_state := Mux(
          s2_victim_dirty && !discard_line,
          s_voluntary_writeback,
          Mux(
            !cacheParams.silentDrop.B && !release_ack_wait && release_queue_empty && s2_victim_state
              .isValid() && (s2_valid_flush_line || s2_flush_valid || s2_readwrite && !s2_hit_valid),
            s_voluntary_release,
            s_voluntary_write_meta
          )
        )
        probe_bits := addressToProbe(
          s2_vaddr,
          Cat(s2_victim_tag, s2_req.addr(tagLSB - 1, idxLSB)) << idxLSB
        )
      }
      when(s2_probe) {
    ...
    dataArb.io.in(2).bits.addr := (probeIdx(
      probe_bits
    ) << blockOffBits) | (releaseDataBeat(
      log2Up(refillCycles) - 1,
      0
    ) << rowOffBits)
    ...
    metaArb.io.in(4).valid := release_state.isOneOf(
      s_voluntary_write_meta,
      s_probe_write_meta
    )
    ...
    metaArb.io.in(4).bits.addr := Cat(
      io.cpu.req.bits.addr >> untagBits,
      probe_bits.address(idxMSB, 0)
    )
    metaArb.io.in(4).bits.data := tECC.encode(
      L1Metadata(tl_out_c.bits.address >> tagLSB, newCoh).asUInt
    )
    ...
    io.cpu.store_pending := (cached_grant_wait && isWrite(
      s2_req.cmd
    )) || uncachedInFlight.asUInt.orR

    val s1_xcpt_valid = tlb.io.req.valid && !s1_isSlavePortAccess && !s1_nack
    io.cpu.s2_xcpt := Mux(
      RegNext(s1_xcpt_valid),
      s2_tlb_xcpt,
      0.U.asTypeOf(s2_tlb_xcpt)
    )
    ...
    val s1_uncached_data_word = {
      val word_idx =
        uncachedResp.addr.extract(log2Up(rowBits / 8) - 1, log2Up(wordBytes))
      val words = tl_out.d.bits.data.grouped(wordBits)
      words(word_idx)
    }
    val s2_uncached_data_word =
      RegEnable(s1_uncached_data_word, io.cpu.replay_next)
    ...
    val s2_data_word = (0 until rowBits by wordBits)
      .map(i => s2_data_uncorrected(wordBits + i - 1, i))
      .reduce(_ | _)
    val s2_data_word_corrected = (0 until rowBits by wordBits)
      .map(i => s2_data_corrected(wordBits + i - 1, i))
      .reduce(_ | _)
    val s2_data_word_possibly_uncached = Mux(
      cacheParams.pipelineWayMux.B && doUncachedResp,
    ...
    val loadgen = new LoadGen(
      ...
      s2_req.signed,
      s2_req.addr,
      s2_data_word_possibly_uncached,
      s2_sc,
      wordBytes
    )
    ...
    if (usingRMW) {
      ...
      pstore1_storegen_data := (if (!usingDataScratchpad)
                                  amoalus.map(_.io.out).asUInt
                                else {
                                  val mask = FillInterleaved(
                                    8,
                                    Mux(s2_correct, 0.U, pstore1_mask)
                                  )
                                  amoalus
                                    .map(_.io.out_unmasked)
                                    .asUInt & mask | s2_data_word_corrected & ~mask
                                })
    } else if (!usingAtomics) {
      assert(
        !(s1_valid_masked && s1_read && s1_write),
    ...
    if (coreParams.useVector) {
      edge.manager.managers.foreach { m =>
        ...
        require(
          !m.supportsAcquireB || m.supportsGet,
          "With a vector unit, cacheable memory must support Get"
        )
        require(
          !m.supportsAcquireT || m.supportsPutPartial,
          "With a vector unit, cacheable memory must support PutPartial"
        )
      }
    }
    ...
    metaArb.io.in(5).bits.addr := Cat(
      io.cpu.req.bits.addr >> untagBits,
      metaArb.io.in(5).bits.idx << blockOffBits
    )
    ...
    if (supports_flush) {
      when(s2_valid_masked && s2_cmd_flush_all) {
        when(
          !flushed && !io.cpu.s2_kill && !release_ack_wait && !uncachedInFlight.asUInt.orR
        ) {
          flushing := true.B
          flushing_req := s2_req
        }
    ...
    metaArb.io.in(0).bits.data := tECC.encode(
      L1Metadata(0.U, ClientMetadata.onReset).asUInt
    )
    when(resetting) {
      flushCounter := flushCounterNext
      when(flushDone) {
    ...
    io.cpu.perf.blocked := {
      ...
      val beatsBeforeEnd = outer.crossing match {
        ...
        case RationalCrossing(_) =>
          1 // assumes 1 < ratio <= 2; need more bookkeeping for optimal handling of >2
        case _: AsynchronousCrossing => 1 // likewise
        case _: CreditedCrossing     => 1 // likewise
      }
      val near_end_of_refill =
        if (cacheBlockBytes / beatBytes <= beatsBeforeEnd) tl_out.d.valid
        else {
    ...
    val (data_error, data_error_uncorrectable, data_error_addr) =
      if (usingDataScratchpad)
        (s2_valid_data_error, s2_data_error_uncorrectable, s2_req.addr)
      else {
        (
      ...
      }
    {
      val error_addr =
        Mux(
          ...
          Cat(
            s2_first_meta_corrected.tag,
            metaArb.io.in(1).bits.addr(tagLSB - 1, idxLSB)
          ),
      ...
      io.errors.uncorrectable.foreach { u =>
        u.valid := metaArb.io
          .in(1)
          .valid && s2_meta_error_uncorrectable || data_error && data_error_uncorrectable
        u.bits := error_addr
      }
      ...
      ccoverNotScratchpad(
        ...
        "D_ERROR_CACHED",
        "D$ D-channel error, cached"
      )
      ccover(
        io.errors.bus.valid && !grantIsCached,
        "D_ERROR_UNCACHED",
        "D$ D-channel error, uncached"
      )
    }
    ...
    if (usingDataScratchpad) {
      ...
      val data_error_type = Seq(
        ...
        property.CoverBoolean(
          s2_valid_data_error && !s2_data_error_uncorrectable,
          Seq("data_correctable_error")
        ),
        property.CoverBoolean(
          s2_valid_data_error && s2_data_error_uncorrectable,
      ...
      val tag_error_cover = Seq(
        ...
        property.CoverBoolean(
          s2_meta_error && !s2_meta_error_uncorrectable,
          Seq("tag_correctable_error")
        ),
        property.CoverBoolean(
          s2_meta_error && s2_meta_error_uncorrectable,
          Seq("tag_uncorrectable_error")
        )
      )
      property.cover(
        new property.CrossProperty(
  ...
  def encodeData(x: UInt, poison: Bool) = x
    .grouped(eccBits)
    .map(dECC.encode(_, if (dECC.canDetect) poison else false.B))
    .asUInt
  ...
  def needsRead(req: HellaCacheReq) =
    isRead(req.cmd) ||
      (isWrite(req.cmd) && (req.cmd === M_PWR || req.size < log2Ceil(
        eccBytes
      ).U))

  def ccover(cond: Bool, label: String, desc: String)(implicit
      sourceInfo: SourceInfo
  ) =
    property.cover(cond, s"DCACHE_$label", "MemorySystem;;" + desc)
  def ccoverNotScratchpad(cond: Bool, label: String, desc: String)(implicit
      sourceInfo: SourceInfo
  ) =
  ...
  def acquire(vaddr: UInt, paddr: UInt, param: UInt): TLBundleA = {
    if (!edge.manager.anySupportAcquireB)
      ...
      edge
        .AcquireBlock(
          0.U,
          paddr >> lgCacheBlockBytes << lgCacheBlockBytes,
          lgCacheBlockBytes.U,
          param
        )
        ._2
