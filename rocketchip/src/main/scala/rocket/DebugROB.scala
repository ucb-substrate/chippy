...
class DebugROBPushTrace(implicit val p: Parameters)
    extends BlackBox
...
class DebugROBPushWb(implicit val p: Parameters)
    extends BlackBox
    with HasBlackBoxResource
    with HasCoreParameters {
  val io = IO(new Bundle {
...
class DebugROBPopTrace(implicit val p: Parameters)
    extends BlackBox
    with HasBlackBoxResource
    with HasCoreParameters {
  val io = IO(new Bundle {
...
object DebugROB {
  ...
  def popTrace(clock: Clock, reset: Reset, hartid: UInt)(implicit
      p: Parameters
  ): TracedInstruction = {
...
class TaggedInstruction(val nXPR: Int)(implicit val p: Parameters)
    extends Bundle {
...
class TaggedWbData(implicit val p: Parameters)
    extends Bundle
    with HasCoreParameters {
  val valid = Bool()
...
class HardDebugROB(val traceRatio: Int, val nXPR: Int)(implicit
    val p: Parameters
) extends Module
    with HasCoreParameters {
  val io = IO(new Bundle {
  ...
  val iq = Module(
    new Queue(new TaggedInstruction(nXPR), traceRatio * nXPR, flow = true)
  )
  ...
  val fireTrace = DecoupledHelper(iq.io.deq.valid, maybeFire)
