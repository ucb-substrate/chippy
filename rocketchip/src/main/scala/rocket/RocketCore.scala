...
case class RocketCoreParams(
  ...
  val traceHasWdata: Boolean =
    debugROB.isDefined // ooo wb, so no wdata in trace
  ...
  override val customIsaExt =
    Option.when(haveCease)("xrocket") // CEASE instruction
...
trait HasRocketCoreParameters extends HasCoreParameters {
  lazy val rocketParams: RocketCoreParams =
    tileParams.core.asInstanceOf[RocketCoreParams]

  val fastLoadWord = rocketParams.fastLoadWord
  val fastLoadByte = rocketParams.fastLoadByte

  val mulDivParams =
    rocketParams.mulDiv.getOrElse(MulDivParams()) // TODO ask andrew about this

  require(!fastLoadByte || fastLoadWord)
  require(
    !rocketParams.haveFSDirty,
    "rocket doesn't support setting fs dirty from outside, please disable haveFSDirty"
  )
}

class RocketCustomCSRs(implicit p: Parameters)
    extends CustomCSRs
    with HasRocketCoreParameters {
  override def bpmCSR = {
    rocketParams.branchPredictionModeCSR.option(
      CustomCSR(bpmCSRId, BigInt(1), Some(BigInt(0)))
    )
  }
  ...
  def mvendorid =
    CustomCSR.constant(CSRs.mvendorid, BigInt(rocketParams.mvendorid))
...
class CoreInterrupts(val hasBeu: Boolean)(implicit p: Parameters)
    extends TileInterrupts()(p) {
...
trait HasRocketCoreIO extends HasRocketCoreParameters {
  ...
  def traceIngressParams = freechips.rocketchip.trace.TraceCoreParams(
    nGroups = 1,
    iretireWidth = coreParams.retireWidth,
    xlen = coreParams.xLen,
    iaddrWidth = coreParams.xLen
  )
  val io = IO(new CoreBundle()(p) {
    ...
    val interrupts = Input(
      new CoreInterrupts(
        tileParams.asInstanceOf[RocketTileParams].beuAddr.isDefined
      )
    )
    ...
    val bpwatch =
      Output(Vec(coreParams.nBreakpoints, new BPWatch(coreParams.retireWidth)))
    val cease = Output(Bool())
    val wfi = Output(Bool())
    val traceStall = Input(Bool())
    val vector = if (usingVector) Some(Flipped(new VectorCoreIO)) else None
    val trace_core_ingress =
      if (rocketParams.enableTraceCoreIngress)
        Some(
          Output(
            new freechips.rocketchip.trace.TraceCoreInterface(
              traceIngressParams
            )
          )
        )
...
class Rocket(tile: RocketTile)(implicit p: Parameters)
  ...
  class RocketImpl { // entering gated-clock domain
        ...
        new EventSet(
          (mask, hits) =>
            Mux(
              wb_xcpt,
              mask(0),
              wb_valid && pipelineIDToWB((mask & hits).orR)
            ),
          Seq(
            ("exception", () => false.B),
            (
            ...
            (
              "store",
              () => id_ctrl.mem && id_ctrl.mem_cmd === M_XWR && !id_ctrl.fp
            ),
            (
              "amo",
              () =>
                usingAtomics.B && id_ctrl.mem && (isAMO(
                  id_ctrl.mem_cmd
            ...
            ++ (if (!usingMulDiv) Seq()
                  ...
                  Seq(
                    (
                      "mul",
                      () =>
                        if (pipelinedMul) id_ctrl.mul
                        else id_ctrl.div && (id_ctrl.alu_fn & FN_DIV) =/= FN_DIV
                    ),
                    (
                      "div",
                      () =>
                        if (pipelinedMul) id_ctrl.div
                        else id_ctrl.div && (id_ctrl.alu_fn & FN_DIV) === FN_DIV
                    )
                  ))
            ++ (if (!usingFPU) Seq()
                  ...
                  Seq(
                    (
                      "fp load",
                      () => id_ctrl.fp && io.fpu.dec.ldst && io.fpu.dec.wen
                    ),
                    (
                      "fp store",
                      () => id_ctrl.fp && io.fpu.dec.ldst && !io.fpu.dec.wen
                    ...
                    (
                      "fp add",
                      () => id_ctrl.fp && io.fpu.dec.fma && io.fpu.dec.swap23
                    ),
                    (
                      "fp mul",
                      () =>
                        id_ctrl.fp && io.fpu.dec.fma && !io.fpu.dec.swap23 && !io.fpu.dec.ren3
                    ),
                    (
                      "fp mul-add",
                      () => id_ctrl.fp && io.fpu.dec.fma && io.fpu.dec.ren3
                    ),
                    (
                      "fp div/sqrt",
                      () => id_ctrl.fp && (io.fpu.dec.div || io.fpu.dec.sqrt)
                    ),
                    (
                      "fp other",
                      () =>
    ...
    val decode_table = {
      (if (usingMulDiv)
         new MDecode(pipelinedMul) +: (xLen > 32)
           .option(new M64Decode(pipelinedMul))
           .toSeq
       else Nil) ++:
        (if (usingAtomics)
           new ADecode +: (xLen > 32).option(new A64Decode).toSeq
         else Nil) ++:
        (if (fLen >= 32) new FDecode +: (xLen > 32).option(new F64Decode).toSeq
         else Nil) ++:
        (if (fLen >= 64) new DDecode +: (xLen > 32).option(new D64Decode).toSeq
         else Nil) ++:
        (if (minFLen == 16)
           new HDecode +: (xLen > 32)
             .option(new H64Decode)
             .toSeq ++: (fLen >= 64).option(new HDDecode).toSeq
         else Nil) ++:
        ...
        coreParams.haveCFlush.option(
          new CFlushDecode(tile.dcache.canSupportCFlushLine)
        ) ++:
        rocketParams.haveCease.option(new CeaseDecode) ++:
        usingVector.option(new VCFGDecode) ++:
        (if (coreParams.useZba)
           new ZbaDecode +: (xLen > 32).option(new Zba64Decode).toSeq
         else Nil) ++:
        (if (coreParams.useZbb)
           Seq(
             new ZbbDecode,
             if (xLen == 32) new Zbb32Decode else new Zbb64Decode
           )
         else Nil) ++:
    ...
    require(
      !(coreParams.useRVE && coreParams.fpu.nonEmpty),
      "Can't select both RVE and floating-point"
    )
    require(
      !(coreParams.useRVE && coreParams.useHypervisor),
      "Can't select both RVE and Hypervisor"
    )
    ...
    def decodeReg(x: UInt) =
      (x.extract(x.getWidth - 1, lgNXRegs).asBool, x(lgNXRegs - 1, 0))
    ...
    val id_csr_ren =
      id_ctrl.csr.isOneOf(CSR.S, CSR.C) && id_expanded_inst(0).rs1 === 0.U
    val id_csr = Mux(
      id_system_insn && id_ctrl.mem,
      CSR.N,
      Mux(id_csr_ren, CSR.R, id_ctrl.csr)
    )
    val id_csr_flush = id_system_insn || (id_csr_en && !id_csr_ren && csr.io
      .decode(0)
      .write_flush)
    val id_set_vconfig = Seq(
      Instructions.VSETVLI,
      Instructions.VSETIVLI,
      Instructions.VSETVL
    ).map(_ === id_inst(0)).orR && usingVector.B
    ...
    val id_illegal_insn = !id_ctrl.legal ||
      ...
      id_ctrl.fp && (csr.io
        .decode(0)
        .fp_illegal || (io.fpu.illegal_rm && !id_ctrl.vec)) ||
      ...
      id_csr_en && (csr.io
        .decode(0)
        .read_illegal || !id_csr_ren && csr.io.decode(0).write_illegal) ||
      !ibuf.io.inst(0).bits.rvc && (id_system_insn && csr.io
        .decode(0)
        .system_illegal)
    val id_virtual_insn = id_ctrl.legal &&
      ((id_csr_en && !(!id_csr_ren && csr.io.decode(0).write_illegal) && csr.io
        .decode(0)
        .virtual_access_illegal) ||
        (!ibuf.io.inst(0).bits.rvc && id_system_insn && csr.io
          .decode(0)
          .virtual_system_illegal))
    ...
    val id_csr_rocc_write = tile.roccCSRs.flatten
      .map(_.id.U === id_inst(0)(31, 20))
      .orR && id_csr_en && !id_csr_ren
    val id_vec_busy =
      io.vector.map(v => v.backend_busy || v.trap_check_busy).getOrElse(false.B)
    val id_do_fence = WireDefault(
      id_rocc_busy && (id_ctrl.fence || id_csr_rocc_write) ||
    ...
    val id_bypass_src =
      id_raddr.map(raddr => bypass_sources.map(s => s._1 && s._2 === raddr))

    // execute stage
    val bypass_mux = bypass_sources.map(_._3)
    val ex_reg_rs_bypass = Reg(Vec(id_raddr.size, Bool()))
    val ex_reg_rs_lsb = Reg(
      Vec(id_raddr.size, UInt(log2Ceil(bypass_sources.size).W))
    )
    ...
    val ex_rs1shl =
      Mux(ex_reg_inst(3), ex_rs(0)(31, 0), ex_rs(0)) << ex_reg_inst(14, 13)
    val ex_op1 = MuxLookup(ex_ctrl.sel_alu1, 0.S)(
      Seq(
    ...
    val ex_op2_oh = UIntToOH(
      Mux(ex_ctrl.sel_alu2(0), (ex_reg_inst >> 20).asUInt, ex_rs(1))(
        log2Ceil(xLen) - 1,
        0
      )
    ).asSInt
    val ex_op2 = MuxLookup(ex_ctrl.sel_alu2, 0.S)(
      Seq(
    ...
    val (ex_new_vl, ex_new_vconfig) = if (usingVector) {
      val ex_new_vtype = VType.fromUInt(
        MuxCase(
          ex_rs(1),
          Seq(
    ...
    ex_reg_replay := !take_pc && ibuf.io
      .inst(0)
      .valid && ibuf.io.inst(0).bits.replay
    ex_reg_xcpt := !ctrl_killd && id_xcpt
    ex_reg_xcpt_interrupt := !take_pc && ibuf.io
      .inst(0)
      .valid && csr.io.interrupt
    ...
    when(!ctrl_killd) {
      ...
      ex_reg_hls := usingHypervisor.B && id_system_insn && id_ctrl.mem_cmd
        .isOneOf(M_XRD, M_XWR, M_HLVX)
      ex_reg_mem_size := Mux(
        usingHypervisor.B && id_system_insn,
        id_inst(0)(27, 26),
        id_inst(0)(13, 12)
      )
      when(
        id_ctrl.mem_cmd.isOneOf(M_SFENCE, M_HFENCEV, M_HFENCEG, M_FLUSH_ALL)
      ) {
      ...
      when(id_illegal_insn || id_virtual_insn) {
        val inst =
          Mux(ibuf.io.inst(0).bits.rvc, id_raw_inst(0)(15, 0), id_raw_inst(0))
    ...
    val replay_ex =
      ex_reg_replay || (ex_reg_valid && (replay_ex_structural || replay_ex_load_use))
    ...
    val (ex_xcpt, ex_cause) = checkExceptions(
      List((ex_reg_xcpt_interrupt || ex_reg_xcpt, ex_reg_cause))
    )
    ...
    val mem_wrong_npc =
      Mux(
        ...
        mem_npc =/= ex_reg_pc,
        Mux(
          ibuf.io.inst(0).valid || ibuf.io.imem.valid,
          mem_npc =/= ibuf.io.pc,
          true.B
        )
      )
    val mem_npc_misaligned =
      !csr.io.status.isa('c' - 'a') && mem_npc(1) && !mem_reg_sfence
    val mem_int_wdata = Mux(
      !mem_reg_xcpt && (mem_ctrl.jalr ^ mem_npc_misaligned),
      mem_br_target,
      mem_reg_wdata.asSInt
    ).asUInt
    val mem_cfi = mem_ctrl.branch || mem_ctrl.jalr || mem_ctrl.jal
    val mem_cfi_taken =
      (mem_ctrl.branch && mem_br_taken) || mem_ctrl.jalr || mem_ctrl.jal
    val mem_direction_misprediction =
      mem_ctrl.branch && mem_br_taken =/= (usingBTB.B && mem_reg_btb_resp.taken)
    ...
    val mem_breakpoint =
      (mem_reg_load && bpu.io.xcpt_ld) || (mem_reg_store && bpu.io.xcpt_st)
    val mem_debug_breakpoint =
      (mem_reg_load && bpu.io.debug_ld) || (mem_reg_store && bpu.io.debug_st)
    ...
    val dcache_kill_mem =
      mem_reg_valid && mem_ctrl.wxd && io.dmem.replay_next // structural hazard on writeback port
    val fpu_kill_mem = mem_reg_valid && mem_ctrl.fp && io.fpu.nack_mem
    val vec_kill_mem = mem_reg_valid && mem_ctrl.mem && io.vector
      .map(_.mem.block_mem)
      .getOrElse(false.B)
    val vec_kill_all =
      mem_reg_valid && io.vector.map(_.mem.block_all).getOrElse(false.B)
    val replay_mem =
      dcache_kill_mem || mem_reg_replay || fpu_kill_mem || vec_kill_mem || vec_kill_all
    val killm_common =
    ...
    wb_reg_xcpt := mem_xcpt && !take_pc_wb && !io.vector
      .map(_.mem.block_all)
      .getOrElse(false.B)
    ...
    when(mem_pc_valid) {
      ...
      wb_reg_wdata := Mux(
        !mem_reg_xcpt && mem_ctrl.fp && mem_ctrl.wxd,
        io.fpu.toint_data,
        mem_int_wdata
      )
    ...
    val (wb_xcpt, wb_cause) = checkExceptions(
      List(
        ...
        (
          wb_reg_valid && wb_ctrl.mem && io.dmem.s2_xcpt.pf.st,
          Causes.store_page_fault.U
        ),
        (
          wb_reg_valid && wb_ctrl.mem && io.dmem.s2_xcpt.pf.ld,
          Causes.load_page_fault.U
        ),
        (
          wb_reg_valid && wb_ctrl.mem && io.dmem.s2_xcpt.gf.st,
          Causes.store_guest_page_fault.U
        ),
        (
          wb_reg_valid && wb_ctrl.mem && io.dmem.s2_xcpt.gf.ld,
          Causes.load_guest_page_fault.U
        ),
        (
          wb_reg_valid && wb_ctrl.mem && io.dmem.s2_xcpt.ae.st,
          Causes.store_access.U
        ),
        (
          wb_reg_valid && wb_ctrl.mem && io.dmem.s2_xcpt.ae.ld,
          Causes.load_access.U
        ),
        (
          wb_reg_valid && wb_ctrl.mem && io.dmem.s2_xcpt.ma.st,
          Causes.misaligned_store.U
        ),
        (
          wb_reg_valid && wb_ctrl.mem && io.dmem.s2_xcpt.ma.ld,
    ...
    val wb_set_sboard =
      wb_ctrl.div || wb_dcache_miss || wb_ctrl.rocc || wb_ctrl.vec
    val replay_wb_common = io.dmem.s2_nack || wb_reg_replay
    val replay_wb_rocc = wb_reg_valid && wb_ctrl.rocc && !io.rocc.cmd.ready
    val replay_wb_csr: Bool = wb_reg_valid && csr.io.rw_stall
    val replay_wb_vec =
      wb_reg_valid && io.vector.map(_.wb.replay).getOrElse(false.B)
    val replay_wb =
      replay_wb_common || replay_wb_rocc || replay_wb_csr || replay_wb_vec
    ...
    io.vector.map { v =>
      v.resp.ready := Mux(
        v.resp.bits.fp,
        !(dmem_resp_valid && dmem_resp_fpu),
        ll_arb.io.in(2).ready
      )
    ...
    if (rocketParams.enableTraceCoreIngress) {
      ...
      trace_ingress.io.in.is_compressed := !wb_reg_raw_inst(
        1,
        0
      ).andR // 2'b11 is uncompressed, everything else is compressed
      trace_ingress.io.in.interrupt := csr.io.trace(0).interrupt && csr.io
        .trace(0)
        .exception
      trace_ingress.io.in.exception := !csr.io.trace(0).interrupt && csr.io
        .trace(0)
        .exception
    ...
    csr.io
      .inst(0) := (if (usingCompressed)
                     Cat(
                       Mux(wb_reg_raw_inst(1, 0).andR, wb_reg_inst >> 16, 0.U),
                       wb_reg_raw_inst(15, 0)
                     )
                   else wb_reg_inst)
    ...
    val vector_fcsr_flags_valid =
      io.vector.map(_.set_fflags.valid).getOrElse(false.B)
    csr.io.fcsr_flags.valid := io.fpu.fcsr_flags.valid | vector_fcsr_flags_valid
    csr.io.fcsr_flags.bits := (io.fpu.fcsr_flags.bits & Fill(
      5,
      io.fpu.fcsr_flags.valid
    )) | (vector_fcsr_flags & Fill(5, vector_fcsr_flags_valid))
    ...
    val (htval, mhtinst_read_pseudo) = {
      val htval_valid_imem =
        wb_reg_xcpt && wb_reg_cause === Causes.fetch_guest_page_fault.U
      ...
      val mhtinst_read_pseudo =
        (io.imem.gpa_is_pte && htval_valid_imem) || (io.dmem.s2_gpa_is_pte && htval_valid_dmem)
    ...
    if (rocketParams.debugROB.isDefined) {
      ...
      if (sz < 1) { // use unsynthesizable ROB
        ...
        val should_wb = WireInit(
          (wb_ctrl.wfd || (wb_ctrl.wxd && wb_waddr =/= 0.U)) && !csr.io
            .trace(0)
            .exception
        )
        ...
        io.vector.foreach { v =>
          when(v.wb.retire) {
            ...
            has_wb := false.B
            wb_addr := Cat(
              v.wb.rob_should_wb_fp,
              csr_trace_with_wdata.insn(11, 7)
            )
          }
        }
        ...
        require(
          !usingVector,
          "Synthesizable ROB does not support vector implementations"
        )
        val csr_trace_with_wdata = WireInit(csr.io.trace(0))
        csr_trace_with_wdata.wdata.get := rf_wdata
    ...
    def id_sboard_clear_bypass(r: UInt) = {
      ...
      if (!tileParams.dcache.get.dataECC.isDefined) ll_wen && ll_waddr === r
        ...
        div.io.resp.fire && div.io.resp.bits.tag === r || dmem_resp_replay && dmem_resp_xpu && dmem_resp_waddr === r
    }
    val id_sboard_hazard = checkHazards(
      hazard_targets,
      rd => sboard.read(rd) && !id_sboard_clear_bypass(rd)
    )
    ...
    val data_hazard_ex =
      ex_ctrl.wxd && checkHazards(hazard_targets, _ === ex_waddr)
    val fp_data_hazard_ex = id_ctrl.fp && ex_ctrl.wfd && checkHazards(
      fp_hazard_targets,
      _ === ex_waddr
    )
    val id_ex_hazard =
    ...
    val data_hazard_mem =
      mem_ctrl.wxd && checkHazards(hazard_targets, _ === mem_waddr)
    val fp_data_hazard_mem = id_ctrl.fp && mem_ctrl.wfd && checkHazards(
      fp_hazard_targets,
      _ === mem_waddr
    )
    val id_mem_hazard =
      mem_reg_valid && (data_hazard_mem && mem_cannot_bypass || fp_data_hazard_mem)
    id_load_use := mem_reg_valid && data_hazard_mem && mem_ctrl.mem
    val id_vconfig_hazard =
      id_ctrl.vec && ((ex_reg_valid && ex_reg_set_vconfig) ||
        (mem_reg_valid && mem_reg_set_vconfig) ||
        (wb_reg_valid && wb_reg_set_vconfig))

    // stall for RAW/WAW hazards on load/AMO misses and mul/div in writeback.
    val data_hazard_wb =
      wb_ctrl.wxd && checkHazards(hazard_targets, _ === wb_waddr)
    val fp_data_hazard_wb = id_ctrl.fp && wb_ctrl.wfd && checkHazards(
      fp_hazard_targets,
      _ === wb_waddr
    )
    val id_wb_hazard =
      wb_reg_valid && (data_hazard_wb && wb_set_sboard || fp_data_hazard_wb)

    val id_stall_fpu = if (usingFPU) {
      val fp_sboard = new Scoreboard(32)
      fp_sboard.set(
        ((wb_dcache_miss || wb_ctrl.vec) && wb_ctrl.wfd || io.fpu.sboard_set) && wb_valid,
        wb_waddr
      )
      val v_ll =
        io.vector.map(v => v.resp.fire && v.resp.bits.fp).getOrElse(false.B)
      fp_sboard.clear(
        (dmem_resp_replay && dmem_resp_fpu) || v_ll,
        io.fpu.ll_resp_tag
      )
    ...
    val ctrl_stalld =
      id_ex_hazard || id_mem_hazard || id_wb_hazard || id_sboard_hazard ||
        ...
        id_reg_pause ||
        io.traceStall
    ctrl_killd := !ibuf.io.inst(0).valid || ibuf.io
      .inst(0)
      .bits
      .replay || take_pc_mem_wb || ctrl_stalld || csr.io.interrupt
    ...
    io.imem.might_request := {
      imem_might_request_reg := ex_pc_valid || mem_pc_valid || io.ptw.customCSRs.disableICacheClockGate || io.vector
        .map(_.trap_check_busy)
        .getOrElse(false.B)
    ...
    io.imem.btb_update.bits.cfiType :=
      Mux(
        ...
        CFIType.call,
        Mux(
          mem_ctrl.jalr && (mem_reg_inst(19, 15) & regAddrMask.U) === BitPat(
            "b00?01"
          ),
          CFIType.ret,
    ...
    io.imem.btb_update.bits.br_pc := (if (usingCompressed)
                                        mem_reg_pc + Mux(mem_reg_rvc, 0.U, 2.U)
                                      else mem_reg_pc)
    ...
    io.fpu.ll_resp_data := (if (minFLen == 32)
                              io.dmem.resp.bits.data_word_bypass
                            else io.dmem.resp.bits.data)
    ...
    io.vector.foreach { v =>
      ...
      v.ex.vstart := Mux(
        mem_reg_valid && mem_ctrl.vec || wb_reg_valid && wb_ctrl.vec,
        0.U,
        csr.io.vector.get.vstart
      )
      v.ex.rs1 := ex_rs(0)
      v.ex.rs2 := ex_rs(1)
      v.ex.pc := ex_reg_pc
    ...
    io.dmem.req.bits.dprv := Mux(
      ex_reg_hls,
      csr.io.hstatus.spvp,
      csr.io.status.dprv
    )
    io.dmem.req.bits.dv := ex_reg_hls || csr.io.status.dv
    io.dmem.req.bits.no_resp := !isRead(
      ex_ctrl.mem_cmd
    ) || (!ex_ctrl.fp && ex_waddr === 0.U)
    ...
    io.dmem.s1_data.data := (if (fLen == 0) mem_reg_rs2
                               ...
                               Mux(
                                 mem_ctrl.fp,
                                 Fill(coreDataBits / fLen, io.fpu.store_data),
                                 mem_reg_rs2
                               ))
    ...
    io.dmem.keep_clock_enabled := ibuf.io
      .inst(0)
      .valid && id_ctrl.mem && !csr.io.csr_stall
    ...
    // gate the clock
    val unpause = csr.io.time(
      rocketParams.lgPauseCycles - 1,
      0
    ) === 0.U || csr.io.inhibit_cycle || io.dmem.perf.release || take_pc
    ...
    if (rocketParams.clockGate) {
      ...
      clock_en_reg :=
        ex_pc_valid || mem_pc_valid || wb_pc_valid || // instruction in flight
          ...
          (!long_latency_stall && (ibuf.io
            .inst(0)
            .valid || io.imem.resp.valid)) // instruction pending
    ...
    coreMonitorBundle.valid := csr.io.trace(0).valid && !csr.io
      .trace(0)
      .exception
    coreMonitorBundle.pc := csr.io
      .trace(0)
      .iaddr(vaddrBitsExtended - 1, 0)
      .sextTo(xLen)
    ...
    if (enableCommitLog) {
      ...
      when(t.valid && !t.exception) {
        when(wfd) {
          printf(
            "%d 0x%x (0x%x) f%d p%d 0xXXXXXXXXXXXXXXXX\n",
            t.priv,
            t.iaddr,
            t.insn,
            rd,
          ...
          .elsewhen(wxd && rd =/= 0.U && !has_data) {
            printf(
              "%d 0x%x (0x%x) x%d p%d 0xXXXXXXXXXXXXXXXX\n",
              t.priv,
              t.iaddr,
              t.insn,
              rd,
    ...
    xrfWriteBundle.wrenx := rf_wen && !(csr.io
      .trace(0)
      .valid && wb_wen && (wb_waddr === rf_waddr))
  ...
  def coverExceptions(
    ...
    for ((coverCause, label) <- coverCausesLabels) {
      property.cover(
        exceptionValid && (cause === coverCause.U),
        s"${labelPrefix}_${label}"
      )
    }
  }
  ...
  def encodeVirtualAddress(a0: UInt, ea: UInt) = if (
    vaddrBitsExtended == vaddrBits
  ) ea
  else {
