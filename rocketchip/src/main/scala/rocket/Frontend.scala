...
class Frontend(val icacheParams: ICacheParams, tileId: Int)(implicit
    p: Parameters
) extends LazyModule {
  ...
  val resetVectorSinkNode = BundleBridgeSink[UInt](
    Some(() => UInt(masterNode.edges.out.head.bundle.addressBits.W))
  )
}
...
class FrontendModule(outer: Frontend)
  ...
  withClock(gated_clock) { // entering gated-clock domain
    ...
    val tlb = Module(
      new TLB(
        ...
        TLBConfig(
          nTLBSets,
          nTLBWays,
          outer.icacheParams.nTLBBasePageSectors,
          outer.icacheParams.nTLBSuperpages
        )
      )
    )
    ...
    val s2_xcpt =
      s2_tlb_resp.ae.inst || s2_tlb_resp.pf.inst || s2_tlb_resp.gf.inst
    ...
    s2_replay := (s2_valid && !fq.io.enq.fire) || RegNext(
      s2_replay && !s0_valid,
      true.B
    )
    ...
    fq.io.enq.bits.mask := ((1 << fetchWidth) - 1).U << s2_pc.extract(
    ...
    if (usingBTB) {
      ...
      def scanInsns(
        ...
        val rviReturn =
          rviJALR && !rviBits(7) && BitPat("b00?01") === rviBits(19, 15)
        val rviCall = (rviJALR || rviJump) && rviBits(7)
        val rvcBranch =
          bits === Instructions.C_BEQZ || bits === Instructions.C_BNEZ
        val rvcJAL = (xLen == 32).B && bits === Instructions32.C_JAL
        val rvcJump = bits === Instructions.C_J || rvcJAL
        val rvcImm = Mux(
          bits(14),
          new RVCDecoder(bits, xLen, fLen).bImm.asSInt,
          new RVCDecoder(bits, xLen, fLen).jImm.asSInt
        )
        ...
        val rviImm =
          Mux(rviBits(3), ImmGen(IMM_UJ, rviBits), ImmGen(IMM_SB, rviBits))
        ...
        val predictReturn =
          btb.io.ras_head.valid && (prevRVI && rviReturn || valid && rvcReturn)
        val predictJump = prevRVI && rviJump || valid && rvcJump
        val predictBranch =
          predict_taken && (prevRVI && rviBranch || valid && rvcBranch)

        when(
          s2_valid && s2_btb_resp_valid && s2_btb_resp_bits.bridx === idx.U && valid && !rvc
        ) {
        ...
        when(!prevTaken) {
          ...
          btb.io.ras_update.bits.cfiType := Mux(
            ...
            Mux(
              ...
              CFIType.call,
              Mux(
                Mux(prevRVI, rviBranch, rvcBranch) && !force_taken,
                CFIType.branch,
                CFIType.jump
              )
            )
          ...
          when(!s2_btb_taken) {
            when(
              fq.io.enq.fire && taken && !predictBranch && !predictJump && !predictReturn
            ) {
              wrong_path := true.B
            }
            ...
            when(s2_valid && (predictBranch || predictJump)) {
              ...
              val npc =
                if (idx == 0) pc.asSInt + Mux(prevRVI, rviImm -& 2.S, rvcImm)
                  ...
                  Mux(prevRVI, pc - coreInstBytes.U, pc).asSInt + Mux(
                    prevRVI,
                    rviImm,
                    rvcImm
                  )
      ...
      when(!io.cpu.btb_update.valid) {
        ...
        btb.io.btb_update.bits.br_pc := s2_base_pc | (taken_idx << log2Ceil(
          coreInstBytes
        ))
        btb.io.btb_update.bits.pc := s2_base_pc
      }

      btb.io.ras_update.bits.returnAddr := s2_base_pc + (after_idx << log2Ceil(
        coreInstBytes
      ))
...
trait HasICacheFrontend extends CanHavePTW { this: BaseTile =>
  ...
  tlMasterXbar.node := TLWidthWidget(
    tileParams.icache.get.rowBits / 8
  ) := frontend.masterNode
  ...
  private val deviceOpt =
    if (tileParams.icache.get.itimAddr.isDefined) Some(frontend.icache.device)
    else None
}
