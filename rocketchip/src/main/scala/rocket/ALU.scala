...
class ALU(implicit p: Parameters) extends AbstractALU()(p) {
  ...
  val shout_r =
    (Cat(isSub(io.fn) & shin(xLen - 1), shin).asSInt >> shamt)(xLen - 1, 0)
  val shout_l = Reverse(shout_r)
  val shout = Mux(
    io.fn === FN_SR || io.fn === FN_SRA || io.fn === FN_BEXT,
    shout_r,
    0.U
  ) |
  ...
  val cond_out = Option.when(usingConditionalZero)(
    Mux(
      (io.fn === FN_CZEQZ && in2_not_zero) || (io.fn === FN_CZNEZ && !in2_not_zero),
      io.in1,
      0.U
    )
  )
  ...
  // AND, OR, XOR
  val logic = Mux(
    io.fn === FN_XOR || io.fn === FN_OR || io.fn === FN_ORN || io.fn === FN_XNOR,
    in1_xor_in2,
    0.U
  ) |
    Mux(
      io.fn === FN_OR || io.fn === FN_AND || io.fn === FN_ORN || io.fn === FN_ANDN,
      in1_and_in2,
      0.U
    )

  val bext_mask =
    Mux(coreParams.useZbs.B && io.fn === FN_BEXT, 1.U, ~(0.U(xLen.W)))
  ...
  // CLZ, CTZ, CPOP
  val tz_in = MuxLookup((io.dw === DW_32) ## !io.in2(0), 0.U)(
    Seq(
      0.U -> io.in1,
      1.U -> Reverse(io.in1),
      2.U -> 1.U ## io.in1(31, 0),
  ...
  val unary = MuxLookup(io.in2(11, 0), count)(
    Seq(
      0x287.U -> orcb,
  ...
  val rotout =
    Mux(io.fn(0), rotout_r, rotout_l) | Mux(io.fn(0), shout_l, shout_r)

  val out = MuxLookup(io.fn, shift_logic_cond)(
    Seq(
      FN_ADD -> io.adder_out,
      FN_SUB -> io.adder_out
