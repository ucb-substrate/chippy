...
class PMPReg(implicit p: Parameters) extends CoreBundle()(p) {
  ...
  def readAddr = if (pmpGranularity.log2 == PMP.lgAlign) addr
  else {
...
class PMP(implicit p: Parameters) extends PMPReg {
  ...
  private def pow2Match(x: UInt, lgSize: UInt, lgMaxSize: Int) = {
    ...
    if (lgMaxSize <= pmpGranularity.log2) {
      ...
      val msbMatch =
        eval(x >> lgMaxSize, comparand >> lgMaxSize, mask >> lgMaxSize)
      val lsbMatch = eval(
        x(lgMaxSize - 1, 0),
        comparand(lgMaxSize - 1, 0),
        lsbMask(lgMaxSize - 1, 0)
      )
      msbMatch && lsbMatch
    }
  }
  ...
  private def boundMatch(x: UInt, lsbMask: UInt, lgMaxSize: Int) = {
    if (lgMaxSize <= pmpGranularity.log2) {
      ...
      val lsbsLess =
        (x(lgMaxSize - 1, 0) | lsbMask) < comparand(lgMaxSize - 1, 0)
  ...
  private def pow2Homogeneous(x: UInt, pgLevel: UInt) = {
    val maskHomogeneous = pgLevelMap { idxBits =>
      if (idxBits > paddrBits) false.B else mask(idxBits - 1)
    }(pgLevel)
    maskHomogeneous || (pgLevelMap { idxBits =>
      ((x ^ comparand) >> idxBits) =/= 0.U
    }(pgLevel))
  }
  ...
  def homogeneous(x: UInt, pgLevel: UInt, prev: PMP): Bool =
    Mux(
      napot,
      pow2Homogeneous(x, pgLevel),
      !torNotNAPOT || rangeHomogeneous(x, pgLevel, prev)
    )

  // returns whether this matching PMP fully contains the access
  def aligned(x: UInt, lgSize: UInt, lgMaxSize: Int, prev: PMP): Bool = if (
    lgMaxSize <= pmpGranularity.log2
  ) true.B
  else {
    val lsbMask = UIntToOH1(lgSize, lgMaxSize)
    val straddlesLowerBound =
      ((x >> lgMaxSize) ^ (prev.comparand >> lgMaxSize)) === 0.U && (prev
        .comparand(lgMaxSize - 1, 0) & ~x(lgMaxSize - 1, 0)) =/= 0.U
    val straddlesUpperBound =
      ((x >> lgMaxSize) ^ (comparand >> lgMaxSize)) === 0.U && (comparand(
        lgMaxSize - 1,
        0
  ...
  def hit(x: UInt, lgSize: UInt, lgMaxSize: Int, prev: PMP): Bool =
    Mux(
      napot,
      pow2Match(x, lgSize, lgMaxSize),
      torNotNAPOT && rangeMatch(x, lgSize, lgMaxSize, prev)
    )
}
...
class PMPHomogeneityChecker(pmps: Seq[PMP])(implicit p: Parameters) {
  def apply(addr: UInt, pgLevel: UInt): Bool = {
    pmps
      .foldLeft((true.B, 0.U.asTypeOf(new PMP))) { case ((h, prev), pmp) =>
...
class PMPChecker(lgMaxSize: Int)(implicit val p: Parameters)
    extends Module
    ...
    case (prev, (pmp, prevPMP)) =>
      ...
      for (
        (name, idx) <- Seq(
          "no",
          "TOR",
          if (pmpGranularity <= 4) "NA4" else "",
          "NAPOT"
        ).zipWithIndex; if name.nonEmpty
      )
        property.cover(
          pmp.cfg.a === idx.U,
          s"The cfg access is set to ${name} access ",
          "Cover PMP access mode setting"
        )

      property.cover(
        pmp.cfg.l === 0x1.U,
        s"The cfg lock is set to high ",
        "Cover PMP lock mode setting"
      )
