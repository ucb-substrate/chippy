...
/** IO between TLB and PTW
  ...
  */
class TLBPTWIO(implicit p: Parameters)
...
/** Datapath IO between PTW and Core
  ...
  */
class DatapathPTWIO(implicit p: Parameters)
    extends CoreBundle()(p)
    with HasCoreParameters {
  val ptbr = Input(new PTBR())
  val hgatp = Input(new PTBR())
...
class PTE(implicit p: Parameters) extends CoreBundle()(p) {
  ...
  def table(dummy: Int = 0) =
    v && !r && !w && !x && !d && !a && !u && reserved_for_future === 0.U
...
/** L2TLB PTE template
  ...
  */
class L2TLBEntry(nSets: Int)(implicit p: Parameters)
    extends CoreBundle()(p)
    with HasCoreParameters {
  val idxBits = log2Ceil(nSets)
  val tagBits =
    maxSVAddrBits - pgIdxBits - idxBits + (if (usingHypervisor) 1 else 0)
...
/** PTW contains L2TLB, and performs page table walk for high level TLB, and
  ...
  * ==Structure==
  *   - l2tlb : for leaf PTEs
  *     - set-associative (configurable with [[CoreParams.nL2TLBEntries]]and
  ...
  */
class PTW(n: Int)(implicit edge: TLEdgeOut, p: Parameters)
  ...
  withClock(gated_clock) { // entering gated-clock domain
    ...
    val r_hgatp_initial_count =
      pgLevels.U - minPgLevels.U - r_hgatp.additionalPgLevels
    ...
    val (pte, invalid_paddr, invalid_gpa) = {
      ...
      res.ppn := Mux(
        do_both_stages && !stage2,
        tmp.ppn(vpnBits.min(tmp.ppn.getWidth) - 1, 0),
        tmp.ppn(ppnBits.min(tmp.ppn.getWidth) - 1, 0)
      )
      when(tmp.r || tmp.w || tmp.x) {
        ...
        for (i <- 0 until pgLevels - 1)
          when(
            count <= i.U && tmp.ppn(
              (pgLevels - 1 - i) * pgLevelBits - 1,
              (pgLevels - 2 - i) * pgLevelBits
            ) =/= 0.U
          ) { res.v := false.B }
      }
      (
    ...
    val traverse =
      pte.table() && !invalid_paddr && !invalid_gpa && count < (pgLevels - 1).U
    ...
    val pte_addr =
      if (!usingVM) 0.U
        ...
        val vpn_idxs = (0 until pgLevels).map { i =>
          val width = pgLevelBits + (if (i <= pgLevels - minPgLevels)
        ...
        val raw_pte_addr =
          ((r_pte.ppn << pgLevelBits) | vpn_idx) << log2Ceil(xLen / 8)
    ...
    val stage2_pte_cache_addr =
      if (!usingHypervisor) 0.U
        ...
        val raw_s2_pte_cache_addr =
          Cat(aux_pte.ppn, vpn_idx) << log2Ceil(xLen / 8)
        raw_s2_pte_cache_addr(
          vaddrBits.min(raw_s2_pte_cache_addr.getWidth) - 1,
          0
        )
      }
    ...
    def makeFragmentedSuperpagePPN(ppn: UInt): Seq[UInt] = {
      (pgLevels - 1 until 0 by -1).map(i =>
        Cat(
          ppn >> (pgLevelBits * i),
          r_req
            .addr(((pgLevelBits * i) min vpnBits) - 1, 0)
            .padTo(pgLevelBits * i)
        )
      )
    }
    ...
    /** PTECache caches non-leaf PTE
      ...
      */
    def makePTECache(s2: Boolean): (Bool, UInt) = if (
      coreParams.nPTECacheEntries == 0
    ) {
      (false.B, 0.U)
    } else {
      ...
      val tags = Reg(
        Vec(
          ...
          UInt((if (usingHypervisor) 1 + vaddrBits else paddrBits).W)
        )
      )
      ...
      val data = Reg(
        Vec(
          coreParams.nPTECacheEntries,
          UInt((if (usingHypervisor && s2) vpnBits else ppnBits).W)
        )
      )
      val can_hit =
        if (s2)
          ...
          count < (pgLevels - 1).U && Mux(r_req.vstage1, stage2, !r_req.stage2)
      val can_refill =
      ...
      val tag =
        if (s2) Cat(true.B, stage2_pte_cache_addr.padTo(vaddrBits))
          ...
          Cat(
            r_req.vstage1,
            pte_addr.padTo(if (usingHypervisor) vaddrBits else paddrBits)
          )
      ...
      when(
        mem_resp_valid && traverse && can_refill && !hits.orR && !invalidated
      ) {
      ...
      when(
        io.dpath.sfence.valid && (!io.dpath.sfence.bits.rs1 || usingHypervisor.B && io.dpath.sfence.bits.hg)
      ) { valid := 0.U }

      val lcount = if (s2) aux_count else count
      for (i <- 0 until pgLevels - 1) {
        ccover(
          hit && state === s_req && lcount === i.U,
          s"PTE_CACHE_HIT_L$i",
    ...
    val (l2_hit, l2_error, l2_pte, l2_tlb_ram) =
      if (coreParams.nL2TLBEntries == 0)
        ...
        val ram = DescribedSRAM(
          ...
          data = Vec(
            coreParams.nL2TLBWays,
            UInt(code.width(new L2TLBEntry(nL2TLBSets).getWidth).W)
          )
        )

        val g = Reg(Vec(coreParams.nL2TLBWays, UInt(nL2TLBSets.W)))
        val valid =
          RegInit(VecInit(Seq.fill(coreParams.nL2TLBWays)(0.U(nL2TLBSets.W))))
        // use r_req to construct tag
        val (r_tag, r_idx) = Split(
          Cat(r_req.vstage1, r_req.addr(maxSVAddrBits - pgIdxBits - 1, 0)),
          idxBits
        )
          ...
          for (way <- 0 until coreParams.nL2TLBWays) {
            valid(way) :=
              Mux(
                ...
                valid(way) & ~UIntToOH(
                  io.dpath.sfence.bits.addr(idxBits + pgIdxBits - 1, pgIdxBits)
                ),
        ...
        val s0_suitable =
          arb.io.out.bits.bits.vstage1 === arb.io.out.bits.bits.stage2 && !arb.io.out.bits.bits.need_gpa
        val s1_valid = RegNext(s0_valid && s0_suitable && arb.io.out.bits.valid)
        val s2_valid = RegNext(s1_valid)
        // read from tlb idx
        val s1_rdata =
          ram.read(arb.io.out.bits.bits.addr(idxBits - 1, 0), s0_valid)
        val s2_rdata =
        ...
        // decode
        val s2_entry_vec =
          s2_rdata.map(_.uncorrected.asTypeOf(new L2TLBEntry(nL2TLBSets)))
        ...
        for (way <- 0 until coreParams.nL2TLBWays) {
          ccover(
            ...
            s"L2_TLB_HIT_WAY$way",
            s"L2 TLB hit way$way"
          )
        }
    ...
    require(
      !usingHypervisor || pageGranularityPMPs,
      s"hypervisor requires pmpGranularity >= ${1 << pgIdxBits}"
    )
    ...
    val pmaPgLevelHomogeneous = (0 until pgLevels) map { i =>
      ...
      if (pageGranularityPMPs && i == pgLevels - 1) {
        ...
        TLBPageLookup(
          ...
          pgSize,
          xLen / 8
        )(r_pte.ppn << pgIdxBits).homogeneous
      }
    }
    val pmaHomogeneous = pmaPgLevelHomogeneous(count)
    val pmpHomogeneous = new PMPHomogeneityChecker(io.dpath.pmp)
      .apply(r_pte.ppn << pgIdxBits, count)
    ...
    for (i <- 0 until io.requestor.size) {
      ...
      io.requestor(i)
        .resp
        .bits
        .homogeneous := homogeneous || pageGranularityPMPs.B
      io.requestor(i)
        .resp
        .bits
        .fragmented_superpage := resp_fragmented_superpage && pageGranularityPMPs.B
    ...
    switch(state) {
      is(s_ready) {
        when(arb.io.out.fire) {
          val satp_initial_count =
            pgLevels.U - minPgLevels.U - satp.additionalPgLevels
          val vsatp_initial_count =
            pgLevels.U - minPgLevels.U - io.dpath.vsatp.additionalPgLevels
          val hgatp_initial_count =
            pgLevels.U - minPgLevels.U - io.dpath.hgatp.additionalPgLevels
          val aux_ppn = Mux(
          ...
          resp_pf := false.B
          resp_gf := checkInvalidHypervisorGPA(
            io.dpath.hgatp,
            aux_ppn
          ) && arb.io.out.bits.bits.stage2
          resp_hr := true.B
      ...
      is(s_req) {
        when(stage2 && count === r_hgatp_initial_count) {
          gpa_pgoff := Mux(
            aux_count === (pgLevels - 1).U,
            r_req.addr << (xLen / 8).log2,
            stage2_pte_cache_addr
          )
        }
        // pte_cache hit
    ...
    val merged_pte = {
      val superpage_masks = (0 until pgLevels).map(i =>
        ((BigInt(1) << pte.ppn.getWidth) - (BigInt(
          1
        ) << (pgLevels - 1 - i) * pgLevelBits)).U
      )
      val superpage_mask =
        superpage_masks(Mux(stage2_final, max_count, (pgLevels - 1).U))
    ...
    when(mem_resp_valid) {
      ...
      when(traverse) {
        ...
        val gf = (stage2 && !stage2_final && !pte.ur()) || (pte
          .leaf() && pte.reserved_for_future === 0.U && invalid_gpa)
        ...
        when(do_both_stages && !stage2_final && success) {
          ...
          l2_refill := success && count === (pgLevels - 1).U && !r_req.need_gpa &&
            (!r_req.vstage1 && !r_req.stage2 ||
              do_both_stages && aux_count === (pgLevels - 1).U && pte
                .isFullPerm())
          count := max_count
          ...
          val s1_ppns = (0 until pgLevels - 1).map(i =>
            Cat(
              ...
              r_req
                ...
                .padTo((pgLevels - i - 1) * pgLevelBits)
            )
          ) :+ pte.ppn
    ...
    for (i <- 0 until pgLevels) {
      ...
      ccover(
        leaf && pte.v && !invalid_paddr && !invalid_gpa && pte.reserved_for_future === 0.U,
        s"L$i",
        s"successful page-table access, level $i"
      )
      ccover(
        leaf && pte.v && invalid_paddr,
        s"L${i}_BAD_PPN_MSB",
        s"PPN too large, level $i"
      )
      ccover(
        leaf && pte.v && invalid_gpa,
        s"L${i}_BAD_GPA_MSB",
        s"GPA too large, level $i"
      )
      ccover(
        leaf && pte.v && pte.reserved_for_future =/= 0.U,
        s"L${i}_BAD_RSV_MSB",
        s"reserved MSBs set, level $i"
      )
      ccover(
        leaf && !mem_resp_data(0),
        s"L${i}_INVALID_PTE",
        s"page not present, level $i"
      )
      if (i != pgLevels - 1)
        ccover(
          leaf && !pte.v && mem_resp_data(0),
          s"L${i}_BAD_PPN_LSB",
          s"PPN LSBs not zero, level $i"
        )
    }
    ccover(
      mem_resp_valid && count === (pgLevels - 1).U && pte.table(),
      s"TOO_DEEP",
      s"page table too deep"
    )
    ccover(io.mem.s2_nack, "NACK", "D$ nacked page-table access")
    ccover(
      state === s_wait2 && io.mem.s2_xcpt.ae.ld,
      "AE",
      "access exception while walking page table"
    )
  ...
  private def makeHypervisorRootPTE(hgatp: PTBR, vpn: UInt, default: PTE) = {
    ...
    val idxs = (0 to pgLevels - minPgLevels).map(i =>
      (vpn >> (pgLevels - i) * pgLevelBits)
    )
  ...
  private def checkInvalidHypervisorGPA(hgatp: PTBR, vpn: UInt) = {
    val count = pgLevels.U - minPgLevels.U - hgatp.additionalPgLevels
    val idxs = (0 to pgLevels - minPgLevels).map(i =>
      (vpn >> ((pgLevels - i) * pgLevelBits) + maxHypervisorExtraAddrBits)
    )
...
trait CanHavePTWModule extends HasHellaCacheModule {
  ...
  val ptw = Module(
    new PTW(outer.nPTWPorts)(outer.dcache.node.edges.out(0), outer.p)
  )
  ptw.io.mem <> DontCare
  if (outer.usingPTW) {
