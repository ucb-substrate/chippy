...
class Replay(implicit p: Parameters)
    extends HellaCacheReqInternal()(p)
    with HasCoreData

class ReplayInternal(implicit p: Parameters)
    extends HellaCacheReqInternal()(p)
...
class MSHRReqInternal(implicit p: Parameters)
    extends ReplayInternal()(p)
    with HasMissInfo

class WritebackReq(params: TLBundleParameters)(implicit p: Parameters)
...
class IOMSHR(id: Int)(implicit edge: TLEdgeOut, p: Parameters)
  ...
  val s_idle :: s_mem_access :: s_mem_ack :: s_resp_1 :: s_resp_2 :: Nil = Enum(
    5
  )
  ...
  val loadgen =
    new LoadGen(req.size, req.signed, req.addr, grant_word, false.B, wordBytes)
  ...
  val atomics = if (edge.manager.anySupportLogical) {
    MuxLookup(req.cmd, (0.U).asTypeOf(new TLBundleA(edge.bundle)))(
      Seq(
        M_XA_SWAP -> edge
          .Logical(a_source, a_address, a_size, a_data, TLAtomics.SWAP)
          ._2,
        M_XA_XOR -> edge
          .Logical(a_source, a_address, a_size, a_data, TLAtomics.XOR)
          ._2,
        M_XA_OR -> edge
          .Logical(a_source, a_address, a_size, a_data, TLAtomics.OR)
          ._2,
        M_XA_AND -> edge
          .Logical(a_source, a_address, a_size, a_data, TLAtomics.AND)
          ._2,
        M_XA_ADD -> edge
          .Arithmetic(a_source, a_address, a_size, a_data, TLAtomics.ADD)
          ._2,
        M_XA_MIN -> edge
          .Arithmetic(a_source, a_address, a_size, a_data, TLAtomics.MIN)
          ._2,
        M_XA_MAX -> edge
          .Arithmetic(a_source, a_address, a_size, a_data, TLAtomics.MAX)
          ._2,
        M_XA_MINU -> edge
          .Arithmetic(a_source, a_address, a_size, a_data, TLAtomics.MINU)
          ._2,
        M_XA_MAXU -> edge
          .Arithmetic(a_source, a_address, a_size, a_data, TLAtomics.MAXU)
          ._2
      )
    )
  } else {
  ...
  io.mem_access.bits := Mux(
    isAMO(req.cmd),
    atomics,
    Mux(isRead(req.cmd), get, put)
  )
...
class MSHR(id: Int)(implicit edge: TLEdgeOut, p: Parameters)
    extends L1HellaCacheModule()(p) {
  val io = IO(new Bundle {
  ...
  val s_invalid :: s_wb_req :: s_wb_resp :: s_meta_clear :: s_refill_req :: s_refill_resp :: s_meta_write_req :: s_meta_write_resp :: s_drain_rpq :: Nil =
    Enum(9)
  ...
  rpq.io.enq.valid := (io.req_pri_val && io.req_pri_rdy || io.req_sec_val && sec_rdy) && !isPrefetch(
    io.req_bits.cmd
  )
  ...
  io.replay.bits.addr := Cat(
    io.tag,
    req_idx,
    rpq.io.deq.bits.addr(blockOffBits - 1, 0)
  )
...
class MSHRFile(implicit edge: TLEdgeOut, p: Parameters)
    extends L1HellaCacheModule()(p) {
  val io = IO(new Bundle {
  ...
  val cacheable =
    edge.manager.supportsAcquireBFast(io.req.bits.addr, lgCacheBlockBytes.U)

  val sdq_val = RegInit(0.U(cfg.nSDQ.W))
  val sdq_alloc_id = PriorityEncoder(~sdq_val(cfg.nSDQ - 1, 0))
  val sdq_rdy = !sdq_val.andR
  val sdq_enq =
    io.req.valid && io.req.ready && cacheable && isWrite(io.req.bits.cmd)
  ...
  val wb_req_arb = Module(
    new Arbiter(new WritebackReq(edge.bundle), cfg.nMSHRs)
  )
  ...
  val mshrs = (0 until cfg.nMSHRs) map { i =>
    ...
    mshr.io.req_bits.viewAsSupertype(new HellaCacheReqInternal) := io.req.bits
      .viewAsSupertype(new HellaCacheReqInternal)
  ...
  TLArbiter.lowestFromSeq(
    edge,
    io.mem_acquire,
    mshrs.map(_.io.mem_acquire) ++ mmios.map(_.io.mem_access)
  )
  ...
  io.replay.bits.viewAsSupertype(
    new HellaCacheReqInternal
  ) <> replay_arb.io.out.bits.viewAsSupertype(new HellaCacheReqInternal)

  when(io.replay.valid || sdq_enq) {
    sdq_val := sdq_val & ~(UIntToOH(replay_arb.io.out.bits.sdq_id) & Fill(
      cfg.nSDQ,
      free_sdq
    )) |
...
class WritebackUnit(implicit edge: TLEdgeOut, p: Parameters)
    extends L1HellaCacheModule()(p) {
  val io = IO(new Bundle {
  ...
  when(active) {
    ...
    when(r2_data_req_fired) {
      ...
      when(!io.release.ready) {
        ...
        data_req_cnt := data_req_cnt - Mux[UInt](
          (refillCycles > 1).B && r1_data_req_fired,
          2.U,
          1.U
        )
      }
  ...
  io.data_req.bits.addr := (if (refillCycles > 1)
                              Cat(
                                req.idx,
                                data_req_cnt(log2Up(refillCycles) - 1, 0)
                              )
...
class ProbeUnit(implicit edge: TLEdgeOut, p: Parameters)
    extends L1HellaCacheModule()(p) {
  val io = IO(new Bundle {
  ...
  assert(
    !io.rep.valid || !edge.hasData(io.rep.bits),
...
class DataArray(implicit p: Parameters) extends L1HellaCacheModule()(p) {
  ...
  if (doNarrowRead) {
    for (w <- 0 until nWays by rowWords) {
      ...
      for (i <- 0 until resp.size) {
        ...
        when(wway_en.orR && io.write.valid && io.write.bits.wmask(i)) {
          val data = VecInit.fill(rowWords)(
      ...
      for (dw <- 0 until rowWords) {
        val r = VecInit(
          resp.map(_(encDataBits * (dw + 1) - 1, encDataBits * dw))
        )
        val resp_mux =
          if (r.size == 1) r
    ...
    for (w <- 0 until nWays) {
      ...
      io.resp(w) := array
        .read(raddr, io.read.bits.way_en(w) && io.read.valid)
        .asUInt
    }
  }
...
class NonBlockingDCache(staticIdForMetadataUseOnly: Int)(implicit p: Parameters)
    extends HellaCache(staticIdForMetadataUseOnly)(p) {
  override lazy val module = new NonBlockingDCacheModule(this)
}

class NonBlockingDCacheModule(outer: NonBlockingDCache)
    extends HellaCacheModule(outer) {

  require(isPow2(nWays)) // TODO: relax this
  require(dataScratchpadSize == 0)
  require(
    !usingVM || untagBits <= pgIdxBits,
    s"untagBits($untagBits) > pgIdxBits($pgIdxBits)"
  )
  ...
  val s2_valid = RegNext(
    s1_valid_masked && !s1_sfence,
    false.B
  ) && !io.cpu.s2_xcpt.asUInt.orR
  ...
  val dtlb = Module(
    new TLB(false, log2Ceil(coreDataBytes), TLBConfig(nTLBSets, nTLBWays))
  )
  io.ptw <> dtlb.io.ptw
  ...
  when(wb.io.meta_read.valid) {
    s1_req.addr := Cat(
      wb.io.meta_read.bits.tag,
      wb.io.meta_read.bits.idx
    ) << blockOffBits
    s1_req.phys := true.B
  }
  when(prober.io.meta_read.valid) {
    s1_req.addr := Cat(
      prober.io.meta_read.bits.tag,
      prober.io.meta_read.bits.idx
    ) << blockOffBits
    s1_req.phys := true.B
  }
  ...
  val wdata_encoded = (0 until rowWords).map(i =>
    dECC.encode(
      writeArb.io.out.bits.data(coreDataBits * (i + 1) - 1, coreDataBits * i)
    )
  )
  ...
  val s1_tag_eq_way =
    wayMap((w: Int) => meta.io.resp(w).tag === (s1_addr >> untagBits)).asUInt
  val s1_tag_match_way =
  ...
  val s2_hit_state = Mux1H(
    s2_tag_match_way,
    wayMap((w: Int) => RegEnable(meta.io.resp(w).coh, s1_clk_en))
  )
  val (s2_has_permission, _, s2_new_hit_state) =
  ...
  val s2_lrsc_addr_match =
    lrsc_valid && lrsc_addr === (s2_req.addr >> blockOffBits)
  ...
  when(
    s2_valid_masked && !(s2_tag_match && s2_has_permission) && s2_lrsc_addr_match
  ) {
    lrsc_count := 0.U
  }
  ...
  val s2_data_decoded = (0 until rowWords).map(i =>
    dECC.decode(s2_data_muxed(encDataBits * (i + 1) - 1, encDataBits * i))
  )
  ...
  val s2_word_idx =
    if (doNarrowRead) 0.U
    else s2_req.addr(log2Up(rowWords * coreDataBytes) - 1, log2Up(wordBytes))
  val s2_data_correctable =
    s2_data_decoded.map(_.correctable).asUInt(s2_word_idx)

  // store/amo hits
  s3_valid := (s2_valid_masked && s2_hit || s2_replay) && !s2_sc_fail && isWrite(
    s2_req.cmd
  )
  val amoalu = Module(new AMOALU(xLen))
  when(
    (s2_valid || s2_replay) && (isWrite(s2_req.cmd) || s2_data_correctable)
  ) {
    s3_req := s2_req
  ...
  writeArb.io.in(0).bits.wmask := UIntToOH(
    s3_req.addr.extract(rowOffBits - 1, offsetlsb)
  )
  ...
  val s2_repl_meta = Mux1H(
    s2_replaced_way_en,
    wayMap((w: Int) =>
      RegEnable(meta.io.resp(w), s1_clk_en && s1_replaced_way_en(w))
    ).toSeq
  )

  // miss handling
  mshrs.io.req.valid := s2_valid_masked && !s2_hit && (isPrefetch(
    s2_req.cmd
  ) || isRead(s2_req.cmd) || isWrite(s2_req.cmd))
  mshrs.io.req.bits.viewAsSupertype(new Replay) := s2_req.viewAsSupertype(
    new HellaCacheReq
  )
  mshrs.io.req.bits.tag_match := s2_tag_match
  mshrs.io.req.bits.old_meta := Mux(
    s2_tag_match,
    L1Metadata(s2_repl_meta.tag, s2_hit_state),
    s2_repl_meta
  )
  mshrs.io.req.bits.way_en := Mux(
    s2_tag_match,
    s2_tag_match_way,
    s2_replaced_way_en
  )
  ...
  val bypasses = List(
    ...
    (
      r._1 && (s1_addr >> wordOffBits === r._2.addr >> wordOffBits) && isWrite(
        r._2.cmd
      ),
      r._3
    )
  ...
  val s2_data_word_prebypass =
    s2_data_uncorrected >> Cat(s2_word_idx, 0.U(log2Up(coreDataBits).W))
  val s2_data_word =
  ...
  val s1_nack =
    dtlb.io.req.valid && dtlb.io.resp.miss || io.cpu.s2_nack || s1_tlb_req_valid ||
      s1_req.addr(
        idxMSB,
        idxLSB
      ) === prober.io.meta_write.bits.idx && !prober.io.req.ready
  ...
  io.cpu.s2_xcpt := Mux(
    ...
    RegEnable(s1_xcpt, s1_clk_en),
    0.U.asTypeOf(s1_xcpt)
  )
