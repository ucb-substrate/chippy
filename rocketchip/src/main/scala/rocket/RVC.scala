...
class RVCDecoder(x: UInt, xLen: Int, fLen: Int, useAddiForMv: Boolean = false) {
  ...
  def q0 = {
    ...
    def ld =
      inst(Cat(ldImm, rs1p, 3.U(3.W), rs2p, 0x03.U(7.W)), rs2p, rs1p, rs2p)
    def lw =
      inst(Cat(lwImm, rs1p, 2.U(3.W), rs2p, 0x03.U(7.W)), rs2p, rs1p, rs2p)
    def fld =
      inst(Cat(ldImm, rs1p, 3.U(3.W), rs2p, 0x07.U(7.W)), rs2p, rs1p, rs2p)
    def flw = {
      if (xLen == 32)
        inst(Cat(lwImm, rs1p, 2.U(3.W), rs2p, 0x07.U(7.W)), rs2p, rs1p, rs2p)
      else ld
    }
    def unimp = inst(
      Cat(lwImm >> 5, rs2p, rs1p, 2.U(3.W), lwImm(4, 0), 0x3f.U(7.W)),
      rs2p,
      rs1p,
      rs2p
    )
    def sd = inst(
      Cat(ldImm >> 5, rs2p, rs1p, 3.U(3.W), ldImm(4, 0), 0x23.U(7.W)),
      rs2p,
      rs1p,
      rs2p
    )
    def sw = inst(
      Cat(lwImm >> 5, rs2p, rs1p, 2.U(3.W), lwImm(4, 0), 0x23.U(7.W)),
      rs2p,
      rs1p,
      rs2p
    )
    def fsd = inst(
      Cat(ldImm >> 5, rs2p, rs1p, 3.U(3.W), ldImm(4, 0), 0x27.U(7.W)),
      rs2p,
      rs1p,
      rs2p
    )
    def fsw = {
      if (xLen == 32)
        inst(
          Cat(lwImm >> 5, rs2p, rs1p, 2.U(3.W), lwImm(4, 0), 0x27.U(7.W)),
          rs2p,
          rs1p,
          rs2p
        )
  ...
  def q1 = {
    ...
    def jal = {
      if (xLen == 32)
        inst(
          Cat(jImm(20), jImm(10, 1), jImm(11), jImm(19, 12), ra, 0x6f.U(7.W)),
          ra,
          rd,
          rs2p
        )
    ...
    def j = inst(
      Cat(jImm(20), jImm(10, 1), jImm(11), jImm(19, 12), x0, 0x6f.U(7.W)),
      x0,
      rs1p,
      rs2p
    )
    ...
    def arith = {
      ...
      def rtype = {
        val funct =
          Seq(0.U, 4.U, 6.U, 7.U, 0.U, 0.U, 2.U, 3.U)(Cat(x(12), x(6, 5)))
  ...
  def q2 = {
    ...
    def flwsp = {
      if (xLen == 32)
        inst(Cat(lwspImm, sp, 2.U(3.W), rd, 0x07.U(7.W)), rd, sp, rs2)
      else ldsp
    }
    def sdsp = inst(
      Cat(sdspImm >> 5, rs2, sp, 3.U(3.W), sdspImm(4, 0), 0x23.U(7.W)),
      rd,
      sp,
      rs2
    )
    def swsp = inst(
      Cat(swspImm >> 5, rs2, sp, 2.U(3.W), swspImm(4, 0), 0x23.U(7.W)),
      rd,
      sp,
      rs2
    )
    def fsdsp = inst(
      Cat(sdspImm >> 5, rs2, sp, 3.U(3.W), sdspImm(4, 0), 0x27.U(7.W)),
      rd,
      sp,
      rs2
    )
    def fswsp = {
      if (xLen == 32)
        inst(
          Cat(swspImm >> 5, rs2, sp, 2.U(3.W), swspImm(4, 0), 0x27.U(7.W)),
          rd,
          sp,
          rs2
        )
  ...
  def q2_ill = {
    ...
    def fswsp32 =
      if (xLen == 64) false.B else if (fLen >= 32) false.B else true.B
...
class RVCExpander(useAddiForMv: Boolean = false)(implicit val p: Parameters)
    extends Module
