...
trait HasBtbParameters extends HasCoreParameters {
  ...
  val matchBits = btbParams.nMatchBits max log2Ceil(
    p(CacheBlockBytes) * tileParams.icache.get.nSets
  )
  val entries = btbParams.nEntries
  val updatesOutOfOrder = btbParams.updatesOutOfOrder
  val nPages =
    (btbParams.nPages + 1) / 2 * 2 // control logic assumes 2 divides pages
}

abstract class BtbModule(implicit val p: Parameters)
    extends Module
    with HasBtbParameters

abstract class BtbBundle(implicit val p: Parameters)
    extends Bundle
    with HasBtbParameters
...
class BHT(params: BHTParams)(implicit val p: Parameters)
  ...
  def index(addr: UInt, history: UInt) = {
    def hashHistory(hist: UInt) = if (
      params.historyLength == params.historyBits
    ) hist
    else {
      val k = math.sqrt(3) / 2
    ...
    def hashAddr(addr: UInt) = {
      ...
      hi(log2Ceil(params.nEntries) - 1, 0) ^ (hi >> log2Ceil(params.nEntries))(
        1,
        0
      )
    }
    hashAddr(addr) ^ (hashHistory(history) << (log2Up(
      params.nEntries
    ) - params.historyBits))
  }
  ...
  def updateTable(addr: UInt, d: BHTResp, taken: Bool): Unit = {
    ...
    when(!resetting) {
      ...
      wdata := (params.counterLength match {
        case 1 => taken
        case 2 =>
          Cat(taken ^ d.value(0), d.value === 1.U || d.value(1) && taken)
      })
    }
  }
...
class BTB(implicit p: Parameters) extends BtbModule {
  ...
  val pagesMasked = (pageValid.asBools zip pages).map { case (v, p) =>
    Mux(v, p, 0.U)
  }
  ...
  val (updateHit, updateHitAddr) =
    if (updatesOutOfOrder) {
      val updateHits =
        (pageHit << 1)(Mux1H(idxMatch(r_btb_update.bits.pc), idxPages))
      (updateHits.orR, OHToUInt(updateHits))
    } else
      (
        r_btb_update.bits.prediction.entry < entries.U,
  ...
  val idxPageRepl = Cat(pageHit(nPages - 2, 0), pageHit(nPages - 1)) | Mux(
    usePageHit,
    0.U,
    UIntToOH(nextPageRepl)
  )
  ...
  when(r_btb_update.valid) {
    ...
    idxPages(
      waddr
    ) := idxPageUpdate +& 1.U // the +1 corresponds to the <<1 on io.resp.valid
  ...
  io.resp.bits.target := Cat(
    pagesMasked(Mux1H(idxHit, tgtPages)),
  ...
  io.resp.bits.mask := Cat(
    (1.U << ~Mux(io.resp.bits.taken, ~io.resp.bits.bridx, 0.U)) - 1.U,
    1.U
  )
  ...
  if (btbParams.bhtParams.nonEmpty) {
    ...
    when(io.bht_update.valid) {
      when(io.bht_update.bits.branch) {
        bht.updateTable(
          io.bht_update.bits.pc,
          io.bht_update.bits.prediction,
          io.bht_update.bits.taken
        )
