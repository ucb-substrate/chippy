...
case object DTSTimebase
    extends Field[BigInt](
      0
    ) // Clock frequency of clint RTC (use 0 if you don't know it)
...
object DTS {
  ...
  private val nodeChars = (('a' to 'z') ++ ('A' to 'Z') ++ ('0' to '9') ++ Seq(
    ',', '.', '_', '+', '-', '@')).toSet
  def legalNode(x: String): Boolean =
    x == "/" || (!x.isEmpty && x.size < 48 && nodeStartChars.contains(x(0)) && x
      .forall(nodeChars.contains(_)))

  // the DTS spec does not list '-', but uses it everywhere ...
  private val propChars = (('a' to 'z') ++ ('A' to 'Z') ++ ('0' to '9') ++ Seq(
    ',', '.', '_', '+', '?', '#', '-')).toSet
  ...
  private def fmtCell(x: BigInt, cells: Int): Seq[String] = {
    ...
    (0 until cells).reverse map { case i =>
      "0x%x".format((x >> (i * cellbits)) & mask)
    }
  }
  ...
  private def fmtMapping(x: ResourceMapping, cells: Cells): Seq[String] = {
    val ranges = AddressRange.fromSets(x.address)
    ranges.flatMap { case AddressRange(base, size) =>
      fmtCell(base + x.offset, cells.selfAddress) ++ fmtCell(
        base,
        cells.parentAddress
      ) ++ fmtCell(size, cells.selfSize)
    }
  }
  ...
  private def fmtString(x: ResourceString): Seq[String] = {
    require(
      legalString(x.value),
      s"The string '${x.value}' contains chars probably unwise for use in a DTS string"
    )
    Seq("\"" + x.value + "\"")
  }
  ...
  private def fmtMap(
    ...
    val selfAddress =
      x.value.getOrElse("#address-cells", Nil).headOption.flatMap(getInt)
    val selfSize =
      x.value.getOrElse("#size-cells", Nil).headOption.flatMap(getInt)
    ...
    props.flatMap { case (k, seq) =>
      require(
        legalProperty(k),
        s"The string '${k}' is not a legal DTS property name"
      )
          ...
          seq.foreach { r =>
            r match {
              ...
              case _ =>
                require(
                  false,
                  s"The property '${k}' has values of conflicting type: ${seq}"
                )
            }
          }
          Seq(
            ...
            k,
            " = ",
            seq.flatMap(z => helper(z, "", myCells)).mkString(", "),
            ";\n"
          )
        }
      ...
      nodes.flatMap { case (k, Seq(s: ResourceMap)) =>
        ...
        Seq(indent) ++ s.labels
          .map(_ + ": ")
          .filter(_ => !indent.isEmpty) ++ // labels on root not allowed
          Seq(k, " {\n") ++ helper(s, indent + "\t", myCells) ++ Seq(
            indent,
            "};\n"
          )
      }
