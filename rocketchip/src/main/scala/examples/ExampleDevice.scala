...
/** Base class for the ExampleDevice containing its diplomatic+DTS metadata as
  * well as its concrete chisel implementation and register map.
  */
abstract class ExampleDevice(params: ExampleDeviceParams)(implicit
    p: Parameters
    ...
    regmap(
      0 -> Seq(
        RegField(
          ...
          RegFieldDesc(
            "state",
            "State: Example of a R/W Register with description.",
            reset = Some(0)
          )
        )
      ),
...
/** Specialize the generic ExampleDevice to make it attachable to a TileLink
  * interconnect.
  */
class TLExampleDevice(params: ExampleDeviceParams)(implicit p: Parameters)
    extends ExampleDevice(params)
    with HasTLControlRegMap

/** Specialize the generic ExampleDevice to make it attachable to an AXI4
  * interconnect.
  */
...
/** Specialize the generic ExampleDevice to make it attachable to an AHB
  * interconnect.
  */
