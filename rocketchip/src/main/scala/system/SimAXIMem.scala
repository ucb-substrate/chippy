...
/** Memory with AXI port for use in elaboratable test harnesses.
  *
  * Topology: AXIRAM <-< AXI4Buffer <-< AXI4Fragmenter <-< AXI4Xbar <-<
  * AXI4MasterNode
  */
class SimAXIMem(edge: AXI4EdgeParameters, size: BigInt, base: BigInt = 0)(
...
object SimAXIMem {
  def connectMMIO(
      dut: CanHaveMasterAXI4MMIOPort
  )(implicit p: Parameters): Seq[SimAXIMem] = {
    dut.mmio_axi4
      .zip(dut.mmioAXI4Node.in)
      .map { case (io, (_, edge)) =>
        // test harness size capped to 4KB (ignoring p(ExtMem).get.master.size)
        val mmio_mem = LazyModule(
          new SimAXIMem(edge, base = p(ExtBus).get.base, size = 4096)
        )
  ...
  def connectMem(
      dut: CanHaveMasterAXI4MemPort
  )(implicit p: Parameters): Seq[SimAXIMem] = {
    dut.mem_axi4
      .zip(dut.memAXI4Node.in)
      .map { case (io, (_, edge)) =>
        val mem = LazyModule(
          new SimAXIMem(
            edge,
