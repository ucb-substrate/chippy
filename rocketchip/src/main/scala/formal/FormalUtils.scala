  ...
  def apply(
    ...
    val line_info = sourceInfo match {
      case SourceLine(filename, line, col) =>
        s"${filename}_L${line}_C${col}_I${idx}".replace('.', '_')
      case _ => ""
    }
    val proposed_src =
      if (custom_name == "") prop_type.toString + "_" + line_info
      else custom_name
    ...
    if (dir == MonitorDirection.Monitor) {
      when(!cond) {
        printf(
          s"assert:${proposed_src}:${prop_type.toString} ${message + "_" + line_info}"
        )
      }
      ...
      when(!cond) {
        printf(
          s"assume:${proposed_src}:${prop_type.toString} ${message + "_" + line_info}"
        )
      }
      ...
      if (prop_type == PropertyClass.CoverDisableMonitor) {
        ...
        when(cond) {
          printf(
            s"cover:${proposed_src}:${prop_type.toString} ${message + "_" + line_info}"
          )
        }
      }
    }
...
object ResetUtils {
  def inactive_output_override[T <: Data](
      inactive_length: Int
  )(sigs: T, override_assn: (T) => Unit): Unit = {
...
object ScalaUtils {
  def repeatfill[T <: Any](size: Int, base_seq: Seq[T]): Seq[T] = {
    if (size == 1) {
      ...
      base_seq.head +: ScalaUtils.repeatfill(
        size - 1,
        base_seq.tail :+ base_seq.head
      )
    }
  }
}
