...
case class APBToTLNode()(implicit valName: ValName)
    extends MixedAdapterNode(APBImp, TLImp)(
      dFn = { mp =>
        ...
        APBSlavePortParameters(
          slaves = mp.managers.map { m =>
            APBSlaveParameters(
              ...
              nodePath = m.nodePath,
              supportsWrite =
                m.supportsPutPartial.intersect(TransferSizes(1, mp.beatBytes)),
              supportsRead =
                m.supportsGet.intersect(TransferSizes(1, mp.beatBytes))
            )
          },
...
class APBToTL()(implicit p: Parameters) extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) {
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      require(
        beatBytes == in.params.dataBits / 8,
      ...
      val a_legal =
        Mux(
          in.pwrite,
          edgeOut.manager
            .supportsPutPartialSafe(aligned_addr, data_size, Some(beat)),
      ...
      when(out.a.fire) {
        assert(
          in.paddr === out.a.bits.address,
          "Do not expect to have to perform alignment in APB2TL Conversion"
        )
      }
