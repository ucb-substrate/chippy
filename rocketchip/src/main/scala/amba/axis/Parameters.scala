...
object AXISBundleParameters {
  ...
  def union(x: Seq[AXISBundleParameters]) =
...
class AXISEdgeParameters private (
  ...
  require(
    !slave.beatBytes.isEmpty || !master.beatBytes.isEmpty,
    s"Neither master nor slave port specify a bus width (insert an AXISBusBinder between them?) at ${sourceInfo}"
  )
  require(
    slave.beatBytes.isEmpty || master.beatBytes.isEmpty || slave.beatBytes == master.beatBytes,
    s"Master and slave ports specify incompatible bus widths (insert an AXISWidthWidget between them?) at ${sourceInfo}"
  )
  require(
    !slave.reqAligned || master.isAligned,
    s"Slave port requires aligned stream data at ${sourceInfo}"
  )
  require(
    !slave.reqContinuous || master.isContinuous,
    s"Slave port requires continuous stream data at ${sourceInfo}"
  )
