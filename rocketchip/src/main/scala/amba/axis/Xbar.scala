...
class AXISXbar(beatBytes: Int, policy: TLArbiter.Policy = TLArbiter.roundRobin)(
  ...
  val node = AXISNexusNode(
    masterFn = { seq =>
      ...
      seq(0).v1copy(
        ...
        masters = (AXISXbar.mapInputIds(seq) zip seq) flatMap {
          case (range, port) =>
            port.masters map { master =>
              master.v1copy(sourceId = master.sourceId.shift(range.start))
            }
        }
      )
    },
    slaveFn = { seq =>
      seq(0).v1copy(
        beatBytes = Some(beatBytes),
        slaves =
          (AXISXbar.mapOutputIds(seq) zip seq) flatMap { case (range, port) =>
            port.slaves.map { slave =>
              slave.v1copy(destinationId = slave.destinationId + range.start)
            }
          }
      )
    }
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    val wide_bundle =
      AXISBundleParameters.union(io_in.map(_.params) ++ io_out.map(_.params))
    ...
    val request = in.map { i =>
      outputIdRanges.map { o => o.contains(i.bits.dest) }
    }
    val ports = (in zip request) map { case (i, r) => AXISXbar.fanout(i, r) }

    // Arbitrate amongst the sources
    (out zip ports.transpose) map { case (o, i) =>
      AXISXbar.arbitrate(policy)(o, i)
    }
  }
}

object AXISXbar {
  def apply(beatBytes: Int, policy: TLArbiter.Policy = TLArbiter.roundRobin)(
      implicit p: Parameters
  ): AXISNode = {
  ...
  def mapInputIds(ports: Seq[AXISMasterPortParameters]) =
    TLXbar.assignRanges(ports.map(_.endSourceId))
  def mapOutputIds(ports: Seq[AXISSlavePortParameters]) =
    TLXbar.assignRanges(ports.map(_.endDestinationId))

  def arbitrate(
      policy: TLArbiter.Policy
  )(sink: AXISBundle, sources: Seq[AXISBundle]): Unit = {
    if (sources.isEmpty) {
      sink.valid := false.B
      ...
      val readys = VecInit(
        policy(valids.size, Cat(valids.reverse), idle).asBools
      )
      ...
      Connectable.waiveUnmatched(
        sink.bits,
        Mux1H(muxState, sources.map(_.bits))
      ) match {
