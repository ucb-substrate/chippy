...
package object amba {
  ...
  case class AMBAProtField()
      extends BundleField[AMBAProtBundle](
        AMBAProt,
        Output(new AMBAProtBundle),
        x => {
  ...
  case class AMBACorruptField()
      extends BundleField[Bool](AMBACorrupt, Output(Bool()), x => x := false.B)
}
