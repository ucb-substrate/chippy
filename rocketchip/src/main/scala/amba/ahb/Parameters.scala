...
case class AHBSlavePortParameters(
  ...
  val minMaxTransfer =
    slaves.map(_.minMaxTransfer).min // useful for fragmentation
...
object AHBBundleParameters {
  val emptyBundleParams = AHBBundleParameters(
    addrBits = 1,
    dataBits = 8,
    requestFields = Nil,
    responseFields = Nil,
  ...
  def union(x: Seq[AHBBundleParameters]) =
    if (x.isEmpty) emptyBundleParams
    else x.tail.foldLeft(x.head)((x, y) => x.union(y))
