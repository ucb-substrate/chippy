...
case class AHBRegisterNode(
          ...
          Seq(
            AHBSlaveParameters(
              ...
              supportsWrite = TransferSizes(
                1,
                min(
                  address.alignment.toInt,
                  beatBytes * AHBParameters.maxTransfer
                )
              ),
  ...
  def regmap(mapping: RegField.Map*) = {
    ...
    val request =
      ahb.htrans === AHBParameters.TRANS_NONSEQ || ahb.htrans === AHBParameters.TRANS_SEQ
