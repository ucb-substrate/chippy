...
case class AHBToTLNode()(implicit valName: ValName)
    extends MixedAdapterNode(AHBImpSlave, TLImp)(
      dFn = { case mp =>
            ...
            def adjust(x: TransferSizes) = {
              if (x.contains(mp.beatBytes)) {
                TransferSizes(
                  x.min,
                  m.minAlignment
                    .min(mp.beatBytes * AHBParameters.maxTransfer)
                    .toInt
                )
...
class AHBToTL()(implicit p: Parameters) extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) {
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      val a_burst_size = VecInit.tabulate(burst_sizes.length)(
        burst_sizes.map(beats => log2Ceil(beats * beatBytes).U)
      )(in.hburst)
      val a_burst_mask = VecInit.tabulate(burst_sizes.length)(
        burst_sizes.map(beats => (beats * beatBytes - 1).U)
      )(in.hburst)
      ...
      val a_burst_ok =
        in.htrans === AHBParameters.TRANS_NONSEQ && // only start burst on first AHB beat
          ...
          Mux(
            in.hwrite, // target device supports the burst
      ...
      val a_access =
        in.htrans === AHBParameters.TRANS_NONSEQ || in.htrans === AHBParameters.TRANS_SEQ
      ...
      assert(
        !RegNext(in.hresp(0) && !in.hreadyout, false.B) || RegNext(
          in.hrdata
        ) === in.hrdata
      )
      ...
      val hresp =
        d_fail || (out.d.valid && (out.d.bits.denied || out.d.bits.corrupt))
      val hreadyout = Mux(
        d_write,
        (!d_send || out.a.ready) && (!d_last || !d_recv || out.d.valid),
        out.d.valid || !d_recv
      )
