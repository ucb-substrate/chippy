...
class AHBRAM(
      ...
      AHBSlavePortParameters(
        Seq(
          AHBSlaveParameters(
            ...
            supportsRead =
              TransferSizes(1, beatBytes * AHBParameters.maxTransfer),
            supportsWrite =
              TransferSizes(1, beatBytes * AHBParameters.maxTransfer)
          )
        ),
  ...
  class Impl extends LazyModuleImp(this) with HasJustOneSeqMem {
    ...
    val a_address = Cat(
      (mask zip (in.haddr >> log2Ceil(beatBytes)).asBools)
        .filter(_._1)
        .map(_._2)
        .reverse
    )
    ...
    val d_wdata = VecInit.tabulate(beatBytes) { i =>
      in.hwdata(8 * (i + 1) - 1, 8 * i)
    }
    ...
    val muxdata = VecInit(
      (p_mask.asBools zip (p_wdata zip d_rdata))
    ...
    val disable_ahb_fuzzing =
      PlusArg("disable_ahb_fuzzing", default = 0, "1:Disabled 0:Enabled.")(0)

    // Finally, the outputs
    in.hreadyout := Mux(
      disable_ahb_fuzzing,
      true.B, {
        if (fuzzHreadyout) { !d_request || LFSRNoiseMaker(1)(0) }
        else { true.B }
      }
    )
    in.hresp := Mux(
      !d_request || d_legal || !in.hreadyout,
      AHBParameters.RESP_OKAY,
      AHBParameters.RESP_ERROR
    )
