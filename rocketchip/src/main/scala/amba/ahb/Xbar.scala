...
class AHBFanout()(implicit p: Parameters) extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) {
    if (node.edges.in.size >= 1) {
      require(
        node.edges.in.size == 1,
        "AHBFanout does not support multiple masters"
      )
      require(node.edges.out.size > 0, "AHBFanout requires at least one slave")
      node.edges.out.foreach { eo =>
        require(
          eo.slave.lite,
          s"AHBFanout only supports AHB-Lite slaves (${eo.slave.slaves.map(_.name)})"
        )
      }
      ...
      edgesOut.foreach { edge =>
        ...
        require(
          ...
          s"${port.slaves.map(_.name)} ${port.beatBytes} vs ${port0.slaves
              .map(_.name)} ${port0.beatBytes}"
        )
      }
      ...
      val (in, _) = node.in(0)
      val a_sel = VecInit(
        route_addrs.map(seq => seq.map(_.contains(in.haddr)).reduce(_ || _))
      )
