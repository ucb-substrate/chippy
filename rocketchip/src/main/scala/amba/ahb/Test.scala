...
class AHBFuzzNative(aFlow: Boolean, txns: Int)(implicit p: Parameters)
    extends LazyModule {
  ...
  xbar.node := arb.node := TLToAHB(aFlow) := TLDelayer(
    0.1
...
class AHBNativeTest(aFlow: Boolean, txns: Int = 5000, timeout: Int = 500000)(
...
class AHBFuzzMaster(aFlow: Boolean, txns: Int)(implicit p: Parameters)
    extends LazyModule
    with HasFuzzTarget {
  ...
  val model = LazyModule(
    new TLRAMModel("AHBFuzzMaster", ignoreCorruptData = false)
  )
...
class AHBFuzzSlave()(implicit p: Parameters)
...
class AHBFuzzBridge(aFlow: Boolean, txns: Int)(implicit p: Parameters)
    extends LazyModule {
...
class AHBBridgeTest(aFlow: Boolean, txns: Int = 5000, timeout: Int = 500000)(
    implicit p: Parameters
) extends UnitTest(timeout) {
