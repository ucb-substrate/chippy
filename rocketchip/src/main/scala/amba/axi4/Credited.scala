...
class AXI4CreditedBuffer(delay: AXI4CreditedDelay)(implicit p: Parameters)
    extends LazyModule {
...
object AXI4CreditedBuffer {
  def apply(
      delay: AXI4CreditedDelay
  )(implicit p: Parameters): AXI4CreditedAdapterNode = {
    val buffer = LazyModule(new AXI4CreditedBuffer(delay))
    buffer.node
  }
  def apply(delay: CreditedDelay)(implicit
      p: Parameters
  ): AXI4CreditedAdapterNode = apply(AXI4CreditedDelay(delay))
  def apply()(implicit p: Parameters): AXI4CreditedAdapterNode = apply(
    CreditedDelay(1, 1)
  )
}

class AXI4CreditedSource(delay: AXI4CreditedDelay)(implicit p: Parameters)
    extends LazyModule {
...
object AXI4CreditedSource {
  def apply(
      delay: AXI4CreditedDelay
  )(implicit p: Parameters): AXI4CreditedSourceNode = {
    val source = LazyModule(new AXI4CreditedSource(delay))
    source.node
  }
  def apply(delay: CreditedDelay)(implicit
      p: Parameters
  ): AXI4CreditedSourceNode = apply(AXI4CreditedDelay(delay))
  def apply()(implicit p: Parameters): AXI4CreditedSourceNode = apply(
    CreditedDelay(1, 1)
  )
}

class AXI4CreditedSink(delay: AXI4CreditedDelay)(implicit p: Parameters)
    extends LazyModule {
...
object AXI4CreditedSink {
  def apply(
      delay: AXI4CreditedDelay
  )(implicit p: Parameters): AXI4CreditedSinkNode = {
    val sink = LazyModule(new AXI4CreditedSink(delay))
    sink.node
  }
  def apply(delay: CreditedDelay)(implicit
      p: Parameters
  ): AXI4CreditedSinkNode = apply(AXI4CreditedDelay(delay))
  def apply()(implicit p: Parameters): AXI4CreditedSinkNode = apply(
    CreditedDelay(1, 1)
  )
}
...
class AXI4RAMCreditedCrossing(txns: Int, params: CreditedCrossing)(implicit
    p: Parameters
) extends LazyModule {
  ...
  island.crossAXI4In(ram.node) := toaxi.node := TLDelayer(
    0.1
...
class AXI4RAMCreditedCrossingTest(txns: Int = 5000, timeout: Int = 500000)(
    implicit p: Parameters
) extends UnitTest(timeout) {
  val dut_1000 = Module(
    LazyModule(
      new AXI4RAMCreditedCrossing(
        txns,
