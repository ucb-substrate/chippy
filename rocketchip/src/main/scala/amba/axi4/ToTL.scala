...
case class AXI4ToTLNode(wcorrupt: Boolean)(implicit valName: ValName)
    extends MixedAdapterNode(AXI4Imp, TLImp)(
      dFn = { case mp =>
        mp.masters.foreach { m =>
          require(
            m.maxFlight.isDefined,
            "AXI4 must include a transaction maximum per ID to convert to TL"
          )
        }
      ...
      uFn = { mp =>
        AXI4SlavePortParameters(
          slaves = mp.managers.map { m =>
            val maxXfer =
              TransferSizes(1, mp.beatBytes * (1 << AXI4Parameters.lenBits))
            AXI4SlaveParameters(
              address = m.address,
          ...
          requestKeys = (if (wcorrupt) Seq(AMBACorrupt)
                         else Seq()) ++ mp.requestKeys.filter(_ != AMBAProt)
        )
      }
    )
...
/** Convert AXI4 master to TileLink.
  *
  * You can use this adapter to connect external AXI4 masters to TileLink bus
  * topology.
  *
  * Setting wcorrupt=true is insufficient to enable w.user.corrupt. One must
  * additionally list it in the AXI4 master's requestFields.
  *
  * @param wcorrupt
...
class AXI4ToTL(wcorrupt: Boolean)(implicit p: Parameters) extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) {
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      val beatCountBits =
        AXI4Parameters.lenBits + (1 << AXI4Parameters.sizeBits) - 1
      val maxFlight = edgeIn.master.masters.map(_.maxFlight.get).max
      val logFlight = log2Ceil(maxFlight)
      val txnCountBits =
        log2Ceil(maxFlight + 1) // wrap-around must not block b_allow
      ...
      val errorDevs = edgeOut.manager.managers.filter(
        _.nodePath.last.lazyModule.className == "TLError"
      )
      require(
        !errorDevs.isEmpty,
        "There is no TLError reachable from AXI4ToTL. One must be instantiated."
      )
      val errorDev = errorDevs.maxBy(_.maxTransfer)
      val error = errorDev.address.head.base
      require(
        errorDev.supportsPutPartial.contains(edgeOut.manager.maxTransfer),
        s"Error device supports ${errorDev.supportsPutPartial} PutPartial but must support ${edgeOut.manager.maxTransfer}"
      )
      require(
        errorDev.supportsGet.contains(edgeOut.manager.maxTransfer),
      ...
      val r_addr = Mux(
        r_ok,
        in.ar.bits.addr,
        error.U | in.ar.bits.addr(log2Up(beatBytes) - 1, 0)
      )
      ...
      assert(
        !in.ar.valid || r_size1 === UIntToOH1(r_size, beatCountBits)
      ) // because aligned
      ...
      val w_addr = Mux(
        w_ok,
        in.aw.bits.addr,
        error.U | in.aw.bits.addr(log2Up(beatBytes) - 1, 0)
      )
      ...
      assert(
        !in.aw.valid || w_size1 === UIntToOH1(w_size, beatCountBits)
      ) // because aligned
      assert(
        !in.aw.valid || in.aw.bits.len === 0.U || in.aw.bits.size === log2Ceil(
          beatBytes
        ).U
      ) // because aligned
      ...
      w_out.bits :<= edgeOut
        .Put(w_id, w_addr, w_size, in.w.bits.data, in.w.bits.strb)
        ._2
      ...
      val d_resp = Mux(
        out.d.bits.denied || out.d.bits.corrupt,
...
class AXI4BundleRError(params: AXI4BundleParameters)
