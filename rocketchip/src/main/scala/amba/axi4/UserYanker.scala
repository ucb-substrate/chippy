...
/** This adapter prunes all user bit fields of the echo type from request
  * messages, storing them in queues and echoing them back when matching
  * response messages are received.
  *
  * It also optionally rate limits the number of transactions that can be in
  * flight simultaneously per FIFO domain / A[W|R]ID.
  *
  * @param capMaxFlight
  *   is an optional maximum number of transactions that can be in flight per
  *   A[W|R]ID.
  */
class AXI4UserYanker(capMaxFlight: Option[Int] = None)(implicit p: Parameters)
    extends LazyModule {
  val node = AXI4AdapterNode(
    masterFn = { mp =>
      mp.copy(
        masters = mp.masters.map { m =>
          m.copy(maxFlight = (m.maxFlight, capMaxFlight) match {
    ...
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      edgeOut.master.masters.foreach { m =>
        require(
          ...
          "UserYanker needs a flight cap on each ID"
        )
      }

      def queue(id: Int) = {
        val depth = edgeOut.master.masters
          .find(_.id.contains(id))
          .flatMap(_.maxFlight)
          .getOrElse(0)
        if (depth == 0) {
          Wire(
            new QueueIO(BundleMap(echoFields), 1)
          ) // unused ID => undefined value
        } else {
      ...
      assert(
        !out.r.valid || r_valid
      ) // Q must be ready faster than the response
