...
/** AXI4 Crossbar. It connects multiple AXI4 masters to slaves.
  ...
  *   arbitration policy
  * @param maxFlightPerId
  *   maximum inflight transactions per id
  * @param awQueueDepth
  *   queue depth for AW channel
  */
class AXI4Xbar(
  ...
  val node = new AXI4NexusNode(
    masterFn = { seq =>
      seq(0).copy(
        ...
        masters = (AXI4Xbar.mapInputIds(seq) zip seq) flatMap {
          case (range, port) =>
            port.masters map { master =>
              master.copy(id = master.id.shift(range.start))
            }
        }
      )
    },
      ...
      seq(0).copy(
        ...
        slaves = seq.flatMap { port =>
          require(
            ...
            s"Xbar data widths don't match: ${port.slaves
                .map(_.name)} has ${port.beatBytes}B vs ${seq(0).slaves
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    val requestARIO = io_in.map { i =>
      VecInit(outputPorts.map { o => o(i.ar.bits.addr) })
    }
    val requestAWIO = io_in.map { i =>
      VecInit(outputPorts.map { o => o(i.aw.bits.addr) })
    }
    val requestROI = io_out.map { o =>
      inputIdRanges.map { i => i.contains(o.r.bits.id) }
    }
    val requestBOI = io_out.map { o =>
      inputIdRanges.map { i => i.contains(o.b.bits.id) }
    }
    ...
    val requestWIO = awIn.map { q =>
      if (io_out.size > 1) q.io.deq.bits.asBools else Seq(true.B)
    }

    // We need an intermediate size of bundle with the widest possible identifiers
    val wide_bundle =
      AXI4BundleParameters.union(io_in.map(_.params) ++ io_out.map(_.params))
    ...
    for (i <- 0 until in.size) {
      ...
      if (io_out.size > 1) {
        ...
        for (master <- edgesIn(i).master.masters) {
          def idTracker(port: UInt, req_fire: Bool, resp_fire: Bool) = {
            if (master.maxFlight == Some(0)) {
              ...
              val portMatch = if (flight == 1) { true.B }
              else { last === port }
        ...
        in(i).aw.valid := io_in(i).aw.valid && (latched || awIn(
          i
        ).io.enq.ready) && allowAW
        io_in(i).aw.ready := in(i).aw.ready && (latched || awIn(
          i
        ).io.enq.ready) && allowAW
        ...
        in(i).w.valid := io_in(i).w.valid && awIn(
          i
        ).io.deq.valid // depends on awvalid (but not awready)
        io_in(i).w.ready := in(i).w.ready && awIn(i).io.deq.valid
        awIn(i).io.deq.ready := io_in(i).w.valid && io_in(i).w.bits.last && in(
          i
        ).w.ready
      } else {
    ...
    for (i <- 0 until out.size) {
      ...
      if (io_in.size > 1) {
        ...
        io_out(i).aw.valid := out(i).aw.valid && (latched || awOut(
          i
        ).io.enq.ready)
        out(i).aw.ready := io_out(i).aw.ready && (latched || awOut(
          i
        ).io.enq.ready)
        ...
        io_out(i).w.valid := out(i).w.valid && awOut(
          i
        ).io.deq.valid // depends on awvalid (but not awready)
        out(i).w.ready := io_out(i).w.ready && awOut(i).io.deq.valid
        awOut(i).io.deq.ready := out(i).w.valid && out(i).w.bits.last && io_out(
          i
        ).w.ready
      } else {
    ...
    def transpose[T](x: Seq[Seq[T]]) = Seq.tabulate(x(0).size) { i =>
      Seq.tabulate(x.size) { j => x(j)(i) }
    }
    val portsAROI = transpose((in zip requestARIO) map { case (i, r) =>
      AXI4Xbar.fanout(i.ar, r)
    })
    val portsAWOI = transpose((in zip requestAWIO) map { case (i, r) =>
      AXI4Xbar.fanout(i.aw, r)
    })
    val portsWOI = transpose((in zip requestWIO) map { case (i, r) =>
      AXI4Xbar.fanout(i.w, r)
    })
    val portsRIO = transpose((out zip requestROI) map { case (o, r) =>
      AXI4Xbar.fanout(o.r, r)
    })
    val portsBIO = transpose((out zip requestBOI) map { case (o, r) =>
      AXI4Xbar.fanout(o.b, r)
    })
    ...
    for (o <- 0 until out.size) {
      awOut(o).io.enq.bits := // Record who won AW arbitration to select W
        AXI4Arbiter
          .returnWinner(arbitrationPolicy)(out(o).aw, portsAWOI(o): _*)
          .asUInt
...
object AXI4Xbar {
  def apply(
    ...
    val axi4xbar = LazyModule(
      new AXI4Xbar(arbitrationPolicy, maxFlightPerId, awQueueDepth)
    )
    axi4xbar.node
  }

  def mapInputIds(ports: Seq[AXI4MasterPortParameters]) =
    TLXbar.assignRanges(ports.map(_.endId))

  // Replicate an input port to each output port
  def fanout[T <: AXI4BundleBase](
      input: IrrevocableIO[T],
      select: Seq[Bool]
  ) = {
...
object AXI4Arbiter {
  ...
  )(sink: IrrevocableIO[T], sources: IrrevocableIO[T]*): Unit = {
    if (sources.isEmpty) {
      sink.valid := false.B
    } else {
  ...
  def returnWinner[T <: Data](
      policy: TLArbiter.Policy
  )(sink: IrrevocableIO[T], sources: IrrevocableIO[T]*) = {
...
class AXI4XbarFuzzTest(name: String, txns: Int, nMasters: Int, nSlaves: Int)(
    implicit p: Parameters
) extends LazyModule {
  val xbar = AXI4Xbar()
  val slaveSize = 0x1000
  val masterBandSize = slaveSize >> log2Ceil(nMasters)
  def filter(i: Int) = TLFilter.mSelectIntersect(
    AddressSet(i * masterBandSize, ~BigInt(slaveSize - masterBandSize))
  )

  val slaves = Seq.tabulate(nSlaves) { i =>
    LazyModule(new AXI4RAM(AddressSet(slaveSize * i, slaveSize - 1)))
  }
  slaves.foreach { s =>
    (s.node
      := AXI4Fragmenter()
...
class AXI4XbarTest(txns: Int = 5000, timeout: Int = 500000)(implicit
    p: Parameters
) extends UnitTest(timeout) {
  val dut21 = Module(
    LazyModule(new AXI4XbarFuzzTest("Xbar DUT21", txns, 2, 1)).module
  )
  val dut12 = Module(
    LazyModule(new AXI4XbarFuzzTest("Xbar DUT12", txns, 1, 2)).module
  )
  val dut22 = Module(
    LazyModule(new AXI4XbarFuzzTest("Xbar DUT22", txns, 2, 2)).module
  )
