...
class AXI4LiteFuzzRAM(txns: Int)(implicit p: Parameters) extends LazyModule {
  ...
  xbar.node := TLDelayer(0.1) := TLBuffer(BufferParams.flow) := TLDelayer(
    0.2
  ) := model.node := fuzz.node
  ram.node := AXI4UserYanker() := AXI4IdIndexer(0) := TLToAXI4(
    true
  ) := TLFragmenter(4, 16, holdFirstDeny = true) := xbar.node
  gpio.node := AXI4UserYanker() := AXI4IdIndexer(0) := TLToAXI4(
    false
  ) := TLFragmenter(4, 16, holdFirstDeny = true) := xbar.node
...
class AXI4LiteFuzzRAMTest(txns: Int = 5000, timeout: Int = 500000)(implicit
    p: Parameters
...
class AXI4LiteUserBitsFuzzRAM(txns: Int)(implicit p: Parameters)
    extends LazyModule {
  val fuzz = LazyModule(new TLFuzzer(txns))
  val model = LazyModule(new TLRAMModel("AXI4LiteFuzzRAM"))
  val xbar = LazyModule(new TLXbar)
...
class AXI4LiteUserBitsFuzzRAMTest(txns: Int = 5000, timeout: Int = 500000)(
...
class AXI4FullFuzzRAM(txns: Int)(implicit p: Parameters) extends LazyModule {
  ...
  ram.node := AXI4Fragmenter() := AXI4Deinterleaver(16) := TLToAXI4(
    false
  ) := xbar.node
  gpio.node := AXI4Fragmenter() := AXI4Deinterleaver(16) := TLToAXI4(
    true
  ) := xbar.node
...
class AXI4FullFuzzRAMTest(txns: Int = 5000, timeout: Int = 500000)(implicit
    p: Parameters
) extends UnitTest(timeout) {
...
trait HasFuzzTarget {
  ...
  val pattern = Seq(
    AddressSet(0x8, ~0x808), // ie: 0x8-0xf, 0x18-0x1f, ... 0x7f8-0x7ff
    AddressSet(0x900, ~0x900)
  ) // ie: 0x900-0x9ff, 0xb00-0xbff, ... 0xf00-0xfff
}

class AXI4FuzzMaster(txns: Int)(implicit p: Parameters)
    extends LazyModule
    with HasFuzzTarget {
...
class AXI4FuzzSlave()(implicit p: Parameters)
...
class AXI4BridgeTest(txns: Int = 5000, timeout: Int = 500000)(implicit
    p: Parameters
) extends UnitTest(timeout) {
