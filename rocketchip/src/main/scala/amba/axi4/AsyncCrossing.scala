...
/** Source(Master) side for AXI4 crossing clock domain
  ...
  */
class AXI4AsyncCrossingSource(sync: Option[Int])(implicit p: Parameters)
    extends LazyModule {
...
/** Sink(Slave) side for AXI4 crossing clock domain
  ...
  *   async queue params
  */
class AXI4AsyncCrossingSink(params: AsyncQueueParams = AsyncQueueParams())(
...
object AXI4AsyncCrossingSource {
  ...
  def apply(sync: Int)(implicit p: Parameters): AXI4AsyncSourceNode = apply(
    Some(sync)
  )
...
@deprecated(
  "AXI4AsyncCrossing is fragile. Use AXI4AsyncCrossingSource and AXI4AsyncCrossingSink",
  "rocket-chip 1.2"
)
class AXI4AsyncCrossing(params: AsyncQueueParams = AsyncQueueParams())(implicit
    p: Parameters
) extends LazyModule {
...
/** Unit tests for AXI4RAM with Async Crossing
  *
  * topology: AXI4RAM <-< AXI4CrossingSink <-< AXI4CrossingSource <-< TLToAXI4
  * <-< TLRAMModel <-< TLFuzzer
  */
class AXI4RAMAsyncCrossing(txns: Int)(implicit p: Parameters)
    extends LazyModule {
...
class AXI4RAMAsyncCrossingTest(txns: Int = 5000, timeout: Int = 500000)(implicit
    p: Parameters
