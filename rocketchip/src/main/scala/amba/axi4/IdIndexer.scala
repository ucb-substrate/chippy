...
case class AXI4ExtraIdField(width: Int)
    extends SimpleBundleField(AXI4ExtraId)(Output(UInt(width.W)), 0.U)

/** This adapter limits the set of FIFO domain ids used by outbound
  * transactions.
  *
  * Extra AWID and ARID bits from upstream transactions are stored in a User
  * Bits field called AXI4ExtraId, which values are expected to be echoed back
  * to this adapter alongside any downstream response messages, and are then
  * prepended to the RID and BID field to restore the original identifier.
  *
  * @param idBits
  *   is the desired number of A[W|R]ID bits to be used
  */
  ...
  val node = AXI4AdapterNode(
    masterFn = { mp =>
      ...
      mp.masters.foreach { m =>
        for (i <- m.id.start until m.id.end) {
          ...
          masters(j) = accumulated.copy(
            ...
            maxFlight = accumulated.maxFlight.flatMap { o =>
              m.maxFlight.map { n => o + n }
            }
          )
        }
      }
      val finalNameStrings = names.map { n =>
        if (n.isEmpty) "(unused)" else n.toList.mkString(", ")
      }
      ...
      mp.copy(
        ...
        masters = masters
          .zip(finalNameStrings)
          .map { case (m, n) => m.copy(name = n) }
          .toIndexedSeq
      )
    },
    slaveFn = { sp => sp }
