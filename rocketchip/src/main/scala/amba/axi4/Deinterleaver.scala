...
/** This adapter deinterleaves read responses on the R channel.
  *
  * Deinterleaving guarantees that once the first beat of a read response has
  * been accepted by the recipient, all further presented read responses will be
  * from the same burst transaction, until the burst is complete.
  *
  * @param maxReadBytes
  *   is the maximum supported read burst size that this adapter has been
  *   provisioned to support.
  * @param buffer
  *   is the internal buffering to provide in the case where no deinterleaving
  *   is required.
  */
class AXI4Deinterleaver(
    maxReadBytes: Int,
  ...
  require(
    maxReadBytes >= 1,
    s"AXI4Deinterleaver: maxReadBytes must be at least 1, not $maxReadBytes"
  )
  require(
    isPow2(maxReadBytes),
    s"AXI4Deinterleaver: maxReadBytes must be a power of two, not $maxReadBytes"
  )
  ...
  val node = new AXI4AdapterNode(
    ...
    slaveFn = { sp =>
      sp.copy(slaves =
        sp.slaves.map(s =>
          s.copy(
            supportsRead =
              s.supportsRead.intersect(TransferSizes(1, maxReadBytes)),
    ...
    override def circuitIdentity =
      edges.out.map(_.slave).forall(nothingToDeinterleave)
  }
  ...
  class Impl extends LazyModuleImp(this) {
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      if (nothingToDeinterleave(edgeOut.slave)) {
        in.r.asInstanceOf[ReadyValidIO[AXI4BundleR]] :<>= buffer.irrevocable(
          out.r
        )
      } else {
        // We only care to deinterleave ids that are actually in use
        val maxFlightPerId = Seq.tabulate(endId) { i =>
          edgeOut.master.masters
            .find(_.id.contains(i))
            .flatMap(_.maxFlight)
            .getOrElse(0)
        }
        ...
        require(
          deq_OH_bools.size == qs.size,
          s"deq_OH.size != qs.size (${deq_OH_bools.size} vs ${qs.size})"
        )
        ...
        require(
          enq_OH_bools.size == qs.size,
          s"enq_OH.size != qs.size (${enq_OH_bools.size} vs ${qs.size})"
        )
...
object AXI4Deinterleaver {
  def apply(maxReadBytes: Int, buffer: BufferParams = BufferParams.default)(
      implicit p: Parameters
  ): AXI4Node = {
