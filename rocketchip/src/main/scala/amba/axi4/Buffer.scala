...
class AXI4Buffer(
    ...
    extends LazyModule {
  def this(aw: BufferParams, br: BufferParams)(implicit p: Parameters) =
    this(aw, aw, br, aw, br)
  ...
  val node = AXI4AdapterNode(
    ...
    slaveFn = { p =>
      p.copy(minLatency =
        p.minLatency + min(aw.latency, ar.latency) + min(r.latency, b.latency)
      )
    }
  )
...
object AXI4Buffer {
  ...
  def apply(aw: BufferParams, br: BufferParams)(implicit
      p: Parameters
  ): AXI4Node = apply(aw, aw, br, aw, br)
  def apply(
      aw: BufferParams,
      w: BufferParams,
      b: BufferParams,
