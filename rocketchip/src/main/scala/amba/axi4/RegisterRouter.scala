...
case class AXI4RRIdField(width: Int)
    extends SimpleBundleField(AXI4RRId)(Output(UInt((1 max width).W)), 0.U)
...
case class AXI4RegisterNode(
  ...
  def regmap(mapping: RegField.Map*) = {
    ...
    val fields =
      AXI4RRIdField(ar.bits.params.idBits) +: ar.bits.params.echoFields
    val params =
      RegMapperParams(log2Up((address.mask + 1) / beatBytes), beatBytes, fields)
...
/** Mix this trait into a RegisterRouter to be able to attach its register map
  * to an AXI4 bus
  */
