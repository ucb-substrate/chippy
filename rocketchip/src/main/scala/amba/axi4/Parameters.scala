...
/** Parameters for AXI4 slave
  ...
  *   memory region type
  * @param executable
  *   whether processor can execute from this memory
  */
...
) // The device will not interleave responses (R+B)
{
...
object AXI4BundleParameters {
  ...
  def union(x: Seq[AXI4BundleParameters]) =
...
case class AXI4AsyncSlavePortParameters(
    async: AsyncQueueParams,
    base: AXI4SlavePortParameters
)
case class AXI4AsyncMasterPortParameters(base: AXI4MasterPortParameters)

case class AXI4AsyncBundleParameters(
    async: AsyncQueueParams,
    base: AXI4BundleParameters
)
case class AXI4AsyncEdgeParameters(
    master: AXI4AsyncMasterPortParameters,
    slave: AXI4AsyncSlavePortParameters,
    params: Parameters,
    sourceInfo: SourceInfo
) {
  val bundle = AXI4AsyncBundleParameters(
    slave.async,
    AXI4BundleParameters(master.base, slave.base)
  )
}
...
case class AXI4CreditedSlavePortParameters(
    delay: AXI4CreditedDelay,
    base: AXI4SlavePortParameters
)
case class AXI4CreditedMasterPortParameters(
    delay: AXI4CreditedDelay,
    base: AXI4MasterPortParameters
)
