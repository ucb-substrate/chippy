...
class L1BusErrors(implicit p: Parameters)
  ...
  def toErrorList = List(
    ...
    icache.correctable.map(
      (_, "I_CORRECTABLE", "Instruction cache or ITIM correctable ECC error")
    ),
    icache.uncorrectable.map(
      (_, "I_UNCORRECTABLE", "ITIM uncorrectable ECC error")
    ),
    None,
    Some((dcache.bus, "DBUS", "Load/Store/PTW TileLink bus error")),
    dcache.correctable.map(
      (_, "D_CORRECTABLE", "Data cache correctable ECC error")
    ),
    dcache.uncorrectable.map(
      (_, "D_UNCORRECTABLE", "Data cache uncorrectable ECC error")
    )
  )
}
...
class BusErrorUnit[T <: BusErrors](
    t: => T,
    params: BusErrorUnitParams,
    beatBytes: Int
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    val sources_enums = sources_and_desc.zipWithIndex.flatMap { case (s, i) =>
      s.map { e => (BigInt(i) -> (e._2, e._3)) }
    }

    val causeWidth = log2Ceil(sources.lastIndexWhere(_.nonEmpty) + 1)
    val (cause, cause_desc) = DescribedReg(
      UInt(causeWidth.W),
      "cause",
      "Cause of error event",
      reset = Some(0.U(causeWidth.W)),
      volatile = true,
      enumerations = sources_enums.toMap
    )

    val (value, value_desc) = DescribedReg(
      UInt(sources.flatten.map(_.bits.getWidth).max.W),
      "value",
      "Physical address of error event",
      reset = None,
      volatile = true
    )
    ...
    val accrued_desc = sources.zipWithIndex.map { case (s, i) =>
      if (s.nonEmpty)
        RegFieldDesc(s"accrued_$i", "", reset = Some(0), volatile = true)
      else RegFieldDesc.reserved
    }
    ...
    new_value := DontCare
    for (
      (((s, en), acc), i) <- (sources zip enable zip accrued).zipWithIndex;
      if s.nonEmpty
    ) {
      when(s.get.valid) {
        acc := true.B
        ...
        property.cover(
          en,
          s"BusErrorCause_$i",
          s"Core;;BusErrorCause $i covered"
        )
      }
    }
    ...
    def reg(v: Vec[Bool], gn: String, gd: String, d: Seq[RegFieldDesc]) =
      RegFieldGroup(
        gn,
        Some(gd),
        (v zip d).map { case (r, rd) => RegField(1, r, rd) }
      )
    def numberRegs(x: Seq[Seq[RegField]]) = x.zipWithIndex.map { case (f, i) =>
      (i * regWidth / 8) -> f
    }

    val omRegMap = node.regmap(
      numberRegs(
        Seq(
          ...
          reg(
            global_interrupt,
            "plic_interrupt",
            "Platform-level interrupt enable mask",
            global_interrupt_desc
          ),
          reg(accrued, "accrued", "Accrued event mask", accrued_desc),
          reg(
            local_interrupt,
            "local_interrupt",
            "Hart-local interrupt-enable mask",
            local_interrupt_desc
          )
        )
      ): _*
    )
