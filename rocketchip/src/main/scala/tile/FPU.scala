...
class FPUDecoder(implicit p: Parameters) extends FPUModule()(p) {
  ...
  val h: Array[(BitPat, List[BitPat])] =
    Array(
      FLH -> List(Y, Y, N, N, N, X, X, X2, X2, N, N, N, N, N, N, N, N),
  ...
  val f: Array[(BitPat, List[BitPat])] =
    Array(
      FLW -> List(Y, Y, N, N, N, X, X, X2, X2, N, N, N, N, N, N, N, N),
  ...
  val d: Array[(BitPat, List[BitPat])] =
    Array(
      FLD -> List(Y, Y, N, N, N, X, X, X2, X2, N, N, N, N, N, N, N, N),
  ...
  val fcvt_hd: Array[(BitPat, List[BitPat])] =
    Array(
      FCVT_H_D -> List(N, Y, Y, N, N, N, X, D, H, N, N, Y, N, N, N, Y, N),
  ...
  val insns = ((minFLen, fLen) match {
    ...
    case other =>
      throw new Exception(
        s"minFLen = ${minFLen} & fLen = ${fLen} is an unsupported configuration"
      )
...
class IntToFPInput(implicit p: Parameters)
...
class FPInput(implicit p: Parameters)
    extends CoreBundle()(p)
    with HasFPUCtrlSigs {
  val rm = Bits(FPConstants.RM_SZ.W)
  val fmaCmd = Bits(2.W)
...
case class FType(exp: Int, sig: Int) {
  ...
  def ieeeQNaN =
    ((BigInt(1) << (ieeeWidth - 1)) - (BigInt(1) << (sig - 2))).U(ieeeWidth.W)
  def qNaN = ((BigInt(7) << (exp + sig - 3)) + (BigInt(1) << (sig - 2)))
    .U(recodedWidth.W)
...
trait HasFPUParameters {
  ...
  def floatTypes =
    FType.all.filter(t => minFLen <= t.ieeeWidth && t.ieeeWidth <= fLen)
  ...
  def typeTagGroup(t: FType) =
    (if (floatTypes.contains(t)) typeTag(t) else typeTag(maxType)).U
  // typeTag
  ...
  private def isBox(x: UInt, t: FType): Bool =
    x(t.sig + t.exp, t.sig + t.exp - 4).andR
  ...
  def consistent(x: UInt): Bool = {
    def helper(x: UInt, t: FType): Bool = if (typeTag(t) == 0) true.B
      ...
      val curOK = !t.isNaN(x) || x(t.sig + t.exp - 4) === x(
        t.sig - 2,
        prevT.recodedWidth - 1
      ).andR
      prevOK && curOK
    }
    helper(x, maxType)
  ...
  def box(x: UInt, t: FType): UInt = {
    if (t == maxType) {
      ...
      bigger | ((BigInt(1) << maxType.recodedWidth) - (BigInt(
        1
      ) << nt.recodedWidth)).U
    }
  }
  ...
  def sanitizeNaN(x: UInt, t: FType): UInt = {
    if (typeTag(t) == 0) {
      ...
      val maskedNaN =
        x & ~((BigInt(1) << (t.sig - 1)) | (BigInt(1) << (t.sig + t.exp - 4)))
          .U(t.recodedWidth.W)
  ...
  def recode(x: UInt, tag: UInt): UInt = {
    ...
    val boxes = floatTypes.map(t =>
      ((BigInt(1) << maxType.ieeeWidth) - (BigInt(1) << t.ieeeWidth)).U
    )
...
abstract class FPUModule(implicit val p: Parameters)
    extends Module
...
class FPToInt(implicit p: Parameters) extends FPUModule()(p) {
  ...
  val toint_ieee = (floatTypes.map(t =>
    if (t == FType.H)
      Fill(maxType.ieeeWidth / minXLen, ieee(in.in1)(15, 0).sextTo(minXLen))
  ...
  io.out.bits.store := (floatTypes.map(t =>
    Fill(fLen / t.ieeeWidth, ieee(in.in1)(t.ieeeWidth - 1, 0))
  ): Seq[UInt])(tag)
  io.out.bits.toint := ((0 until nIntTypes).map(i =>
    toint((minXLen << i) - 1, 0).sextTo(xLen)
  ): Seq[UInt])(intType)
  io.out.bits.exc := 0.U

  when(in.rm(0)) {
    val classify_out = (floatTypes.map(t =>
      t.classify(maxType.unsafeConvert(in.in1, t))
    ): Seq[UInt])(tag)
  ...
  when(in.wflags) { // feq/flt/fle, fcvt
    toint := (~in.rm & Cat(
      dcmp.io.lt,
      dcmp.io.eq
    )).orR | (toint_ieee >> minXLen << minXLen)
    ...
    when(!in.ren2) { // fcvt
      ...
      for (i <- 0 until nIntTypes - 1) {
        ...
        when(cvtType === i.U) {
          val narrow =
            Module(new hardfloat.RecFNToIN(maxExpWidth, maxSigWidth, w))
          narrow.io.in := in.in1
          narrow.io.roundingMode := in.rm
          narrow.io.signedOut := ~in.typ(0)

          val excSign =
            in.in1(maxExpWidth + maxSigWidth) && !maxType.isNaN(in.in1)
          val excOut = Cat(conv.io.signedOut === excSign, Fill(w - 1, !excSign))
          val invalid =
            conv.io.intExceptionFlags(2) || narrow.io.intExceptionFlags(1)
...
class IntToFP(val latency: Int)(implicit p: Parameters) extends FPUModule()(p) {
  ...
  when(in.bits.wflags) { // fcvt
    ...
    val dataPadded =
      data.init.map(d => Cat(data.last >> d.getWidth, d)) :+ data.last
...
class FPToFP(val latency: Int)(implicit p: Parameters) extends FPUModule()(p) {
  ...
  when(in.bits.wflags) { // fmin/fmax
    ...
    fsgnjMux.data := Mux(
      isNaNOut,
      maxType.qNaN,
      Mux(isLHS, in.bits.in1, in.bits.in2)
    )
  }
  ...
  for (t <- floatTypes.init) {
    when(outTag === typeTag(t).U) {
      mux.data := Cat(
        ...
        maxType.unsafeConvert(fsgnjMux.data, t)
      )
    }
  }
  ...
  when(in.bits.wflags && !in.bits.ren2) { // fcvt
    if (floatTypes.size > 1) {
      ...
      for (outType <- floatTypes.init)
        when(
          outTag === typeTag(outType).U && ((typeTag(
            outType
          ) == 0).B || outTag < inTag)
        ) {
          val narrower = Module(
...
class MulAddRecFNPipe(latency: Int, expWidth: Int, sigWidth: Int)
  ...
  override def desiredName =
    s"MulAddRecFNPipe_l${latency}_e${expWidth}_s${sigWidth}"
  require(latency <= 2)
  ...
  val mulAddRecFNToRaw_preMul = Module(
    new hardfloat.MulAddRecFNToRaw_preMul(expWidth, sigWidth)
  )
  val mulAddRecFNToRaw_postMul = Module(
    new hardfloat.MulAddRecFNToRaw_postMul(expWidth, sigWidth)
  )
  ...
  val roundRawFNToRecFN = Module(
    new hardfloat.RoundRawFNToRecFN(expWidth, sigWidth, 0)
  )
...
class FPUFMAPipe(val latency: Int, val t: FType)(implicit p: Parameters)
    extends FPUModule()(p) {
...
class FPU(cfg: FPUParams)(implicit p: Parameters) extends FPUModule()(p) {
  ...
  val load_wb_typeTag =
    RegEnable(io.ll_resp_type(1, 0) - typeTagWbOffset, io.ll_resp_val)
  ...
  class FPUImpl { // entering gated-clock domain
    ...
    val wb_reg_valid =
      RegNext(mem_reg_valid && (!killm || mem_cp_valid), false.B)
    ...
    val frfWriteBundle =
      Seq.fill(2)(WireInit(new CoreMonitorBundle(xLen, fLen), DontCare))
    ...
    val ex_rm =
      Mux(ex_reg_inst(14, 12) === 7.U, io.fcsr_rm, ex_reg_inst(14, 12))

    def fuInput(minT: Option[FType]): FPInput = {
      val req = Wire(new FPInput)
      val tag = ex_ctrl.typeTagIn
      req.viewAsSupertype(new Bundle with HasFPUCtrlSigs) :#= ex_ctrl
        .viewAsSupertype(new Bundle with HasFPUCtrlSigs)
      req.rm := ex_rm
    ...
    val pipes = List(
      ...
      Pipe(
        sfma,
        sfma.latency,
        (c: FPUCtrlSigs) => c.fma && c.typeTagOut === S,
        sfma.io.out.bits
      )
    ) ++
      (fLen > 32).option({
        ...
        Pipe(
          dfma,
          dfma.latency,
          (c: FPUCtrlSigs) => c.fma && c.typeTagOut === D,
          dfma.io.out.bits
        )
      }) ++
      (minFLen == 16).option({
        ...
        Pipe(
          hfma,
          hfma.latency,
          (c: FPUCtrlSigs) => c.fma && c.typeTagOut === H,
          hfma.io.out.bits
        )
      })
    ...
    def pipeid(c: FPUCtrlSigs) =
      pipes.zipWithIndex.map(p => Mux(p._1.cond(c), p._2.U, 0.U)).reduce(_ | _)
    ...
    val mem_wen =
      mem_reg_valid && (mem_ctrl.fma || mem_ctrl.fastpipe || mem_ctrl.fromint)
    val write_port_busy = RegEnable(
      mem_wen && (memLatencyMask & latencyMask(
        ex_ctrl,
        1
      )).orR || (wen & latencyMask(ex_ctrl, 0)).orR,
      req_valid
    )
    ccover(
      mem_reg_valid && write_port_busy,
      "WB_STRUCTURAL",
    ...
    val wdata = box(
      Mux(
        divSqrt_wen,
        divSqrt_wdata,
        (pipes.map(_.res.data): Seq[UInt])(wbInfo(0).pipeid)
      ),
      wtypeTag
    )
    ...
    assert(
      !io.cp_req.valid || pipes.forall(_.lat == pipes.head.lat).B,
      s"FPU only supports coprocessor if FMA pipes have uniform latency ${pipes
          .map(_.lat)}"
    )
    ...
    io.dec <> id_ctrl
    def useScoreboard(f: ((Pipe, Int)) => Bool) = pipes.zipWithIndex
      .filter(_._1.lat > 3)
      .map(x => f(x))
      .fold(false.B)(_ || _)
    io.sboard_set := wb_reg_valid && !wb_cp_valid && RegNext(
      useScoreboard(
        _._1.cond(mem_ctrl)
      ) || mem_ctrl.div || mem_ctrl.sqrt || mem_ctrl.vec
    )
    io.sboard_clr := !wb_cp_valid && (divSqrt_wen || (wen(0) && useScoreboard(
      x => wbInfo(0).pipeid === x._2.U
    )))
    io.sboard_clra := waddr
    ccover(
      io.sboard_clr && load_wb,
      "DUAL_WRITEBACK",
      "load and FMA writeback on same cycle"
    )
    // we don't currently support round-max-magnitude (rm=4)
    io.illegal_rm := io.inst(14, 12).isOneOf(5.U, 6.U) || io.inst(
      14,
      12
    ) === 7.U && io.fcsr_rm >= 5.U
    ...
    if (cfg.divSqrt) {
      val divSqrt_inValid =
        mem_reg_valid && (mem_ctrl.div || mem_ctrl.sqrt) && !divSqrt_inFlight
      ...
      ccover(
        divSqrt_inFlight && divSqrt_killed,
        "DIV_KILLED",
        "divide killed after issued to divider"
      )
      ccover(
        divSqrt_inFlight && mem_reg_valid && (mem_ctrl.div || mem_ctrl.sqrt),
        "DIV_BUSY",
        "divider structural hazard"
      )
      ccover(
        mem_reg_valid && divSqrt_write_port_busy,
        "DIV_WB_STRUCTURAL",
        "structural hazard on division writeback"
      )
