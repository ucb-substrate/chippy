...
case class RocketTileParams(
  ...
  )(implicit p: Parameters): RocketTile = {
...
class RocketTile private (
  ...
  dtim_adapter.foreach(lm =>
    connectTLSlave(lm.node, lm.node.portParams.head.beatBytes)
  )

  val bus_error_unit = rocketParams.beuAddr map { a =>
    val beu = LazyModule(
      new BusErrorUnit(new L1BusErrors, BusErrorUnitParams(a), xLen / 8)
    )
  ...
  /** A Centralized Trace encoder controller, controlling enable/disable of
    * trace encoder and selecting trace sink
    */
  val trace_encoder_controller = rocketParams.traceParams.map { t =>
    val trace_encoder_controller =
      LazyModule(new TraceEncoderController(t.encoderBaseAddr, xBytes))
  ...
  nDCachePorts += 1 /*core */ + (dtim_adapter.isDefined).toInt + rocketParams.core.vector
    .map(_.useDCache.toInt)
    .getOrElse(0)

  val dtimProperty = dtim_adapter
    .map(d => Map("sifive,dtim" -> d.device.asProperty))
    .getOrElse(Nil)

  val itimProperty =
    frontend.icache.itimProperty.toSeq.flatMap(p => Map("sifive,itim" -> p))
  ...
  val cpuDevice: SimpleDevice =
    new SimpleDevice("cpu", Seq("sifive,rocket0", "riscv")) {
  ...
  override def makeMasterBoundaryBuffers(
      crossing: ClockCrossingType
  )(implicit p: Parameters) = (rocketParams.boundaryBuffers, crossing) match {
...
class RocketTileModuleImp(outer: RocketTile)
  ...
  if (outer.rocketParams.traceParams.isDefined) {
    ...
    val trace_sink_arbiter = Module(
      new TraceSinkArbiter(
        outer.traceSinkIds,
  ...
  if (outer.roccs.size > 0) {
    ...
    core.io.rocc.busy <> (cmdRouter.get.io.busy || outer.roccs
      .map(_.module.io.busy)
      .reduce(_ || _))
    core.io.rocc.interrupt := outer.roccs
      .map(_.module.io.interrupt)
      .reduce(_ || _)
