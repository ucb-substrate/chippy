...
object ClockImp
  ...
  def render(e: ClockEdgeParameters) =
    RenderedEdge(colour = "#00cc00" /* green */ )
}

case class ClockSourceNode(val portParams: Seq[ClockSourceParameters])(implicit
    valName: ValName
) extends SourceNode(ClockImp)(portParams) {
  def fixedClockResources(
      name: String,
      prefix: String = "soc/"
  ): Seq[Option[FixedClockResource]] = portParams.map { p =>
    p.give.map(g => new FixedClockResource(name, g.freqMHz, prefix))
  }
}

case class ClockSinkNode(val portParams: Seq[ClockSinkParameters])(implicit
    valName: ValName
...
case class ClockIdentityNode()(implicit valName: ValName)
    extends IdentityNode(ClockImp)()

case class ClockEphemeralNode()(implicit valName: ValName)
    extends EphemeralNode(ClockImp)()

object ClockNameNode {
  def apply(name: ValName) = ClockIdentityNode()(name)
  def apply(name: Option[String]): ClockIdentityNode = apply(
...
object ClockSinkNode {
  def apply(
      ...
      jitterPS: Double = 300
  )(implicit valName: ValName): ClockSinkNode =
    ClockSinkNode(
      Seq(
        ClockSinkParameters(
          phaseDeg = phaseDeg,
...
object ClockSourceNode {
  def apply(freqMHz: Double, dutyCycle: Double = 50, jitterPS: Double = 300)(
      implicit valName: ValName
  ): ClockSourceNode =
    ClockSourceNode(
      Seq(
...
object ClockGroupImp
  ...
  def render(e: ClockGroupEdgeParameters) =
    RenderedEdge(colour = "#00cc00" /* green */ )
}

case class ClockGroupSourceNode(params: Seq[ClockGroupSourceParameters])(
    implicit valName: ValName
) extends SourceNode(ClockGroupImp)(params)
case class ClockGroupSinkNode(params: Seq[ClockGroupSinkParameters])(implicit
    valName: ValName
...
case class ClockGroupIdentityNode()(implicit valName: ValName)
    extends IdentityNode(ClockGroupImp)()

case class ClockGroupEphemeralNode()(implicit valName: ValName)
    extends EphemeralNode(ClockGroupImp)()
