...
case class IntSinkParameters(nodePath: Seq[BaseNode] = Seq()) {
  val name =
    nodePath.lastOption.map(_.lazyModule.name).getOrElse("disconnected")
}
...
case class IntSourcePortParameters(sources: Seq[IntSourceParameters]) {
  ...
  sources.map(_.range).combinations(2).foreach { case Seq(a, b) =>
    ...
    if (num == 0) Nil
      ...
      Seq.fill(ports)(
        IntSourcePortParameters(
          Seq.tabulate(sources)(idx =>
            IntSourceParameters(
              range = IntRange(idx * num, idx * num + num),
              resources = resources
            )
          )
        )
...
case class IntEdge(
    source: IntSourcePortParameters,
    sink: IntSinkPortParameters,
    params: Parameters,
