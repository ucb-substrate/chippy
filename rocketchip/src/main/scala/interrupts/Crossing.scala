...
class IntSyncCrossingSource(alreadyRegistered: Boolean = false)(implicit
    p: Parameters
) extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    override def desiredName =
      s"IntSyncCrossingSource_n${node.out.size}x${outSize}"
  ...
  class ImplRegistered extends LazyRawModuleImp(this) {
    def outSize = node.out.headOption.map(_._1.sync.size).getOrElse(0)
    override def desiredName =
      s"IntSyncCrossingSource_n${node.out.size}x${outSize}_Registered"
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      out.sync := in
    }
  }
...
object IntSyncCrossingSink {
  @deprecated(
    "IntSyncCrossingSink which used the `sync` parameter to determine crossing type is deprecated. Use IntSyncAsyncCrossingSink, IntSyncRationalCrossingSink, or IntSyncSyncCrossingSink instead for > 1, 1, and 0 sync values respectively",
    "rocket-chip 1.2"
  )
...
class IntSyncAsyncCrossingSink(sync: Int = 3)(implicit p: Parameters)
    extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) {
    override def desiredName =
      s"IntSyncAsyncCrossingSink_n${node.out.size}x${node.out.head._1.size}"
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
...
class IntSyncSyncCrossingSink()(implicit p: Parameters) extends LazyModule {
  ...
  class Impl extends LazyRawModuleImp(this) {
    ...
    override def desiredName =
      s"IntSyncSyncCrossingSink_n${node.out.size}x${outSize}"
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      out := in.sync
    }
  }
...
class IntSyncRationalCrossingSink()(implicit p: Parameters) extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) {
    def outSize = node.out.headOption.map(_._1.size).getOrElse(0)
    override def desiredName =
      s"IntSyncRationalCrossingSink_n${node.out.size}x${outSize}"
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      out := RegNext(in.sync)
    }
  }
