...
object IntImp
  ...
  def edge(
      pd: IntSourcePortParameters,
      pu: IntSinkPortParameters,
      p: Parameters,
      sourceInfo: SourceInfo
  ) = IntEdge(pd, pu, p, sourceInfo)
  def bundle(e: IntEdge) = Vec(e.source.num, Bool())
  def render(e: IntEdge) = RenderedEdge(
    colour = "#0000ff" /* blue */,
    label = e.source.sources.map(_.range.size).sum.toString,
    flipped = true
  )

  override def mixO(
      pd: IntSourcePortParameters,
      node: OutwardNode[IntSourcePortParameters, IntSinkPortParameters, Vec[
        Bool
      ]]
  ): IntSourcePortParameters =
    pd.copy(sources = pd.sources.map { s =>
      s.copy(nodePath = node +: s.nodePath)
    })
  override def mixI(
      pu: IntSinkPortParameters,
      node: InwardNode[IntSourcePortParameters, IntSinkPortParameters, Vec[
        Bool
      ]]
  ): IntSinkPortParameters =
...
case class IntSourceNode(portParams: Seq[IntSourcePortParameters])(implicit
    valName: ValName
) extends SourceNode(IntImp)(portParams)
    with IntFormatNode
case class IntSinkNode(portParams: Seq[IntSinkPortParameters])(implicit
    valName: ValName
) extends SinkNode(IntImp)(portParams)
    with IntFormatNode
case class IntAdapterNode(
    ...
    with IntFormatNode
case class IntIdentityNode()(implicit valName: ValName)
    extends IdentityNode(IntImp)()
    with IntFormatNode
case class IntEphemeralNode()(implicit valName: ValName)
    extends EphemeralNode(IntImp)()
    with IntFormatNode

object IntNameNode {
  def apply(name: ValName) = IntIdentityNode()(name)
  def apply(name: Option[String]): IntIdentityNode = apply(
...
case class IntSyncIdentityNode()(implicit valName: ValName)
    extends IdentityNode(IntSyncImp)()
    with IntFormatNode

object IntSyncNameNode {
  def apply(name: ValName) = IntSyncIdentityNode()(name)
  def apply(name: Option[String]): IntSyncIdentityNode = apply(
    (ValName(name.getOrElse("with_no_name")))
  )
  def apply(name: String): IntSyncIdentityNode = apply(Some(name))
}

case class IntSyncSourceNode(alreadyRegistered: Boolean)(implicit
    valName: ValName
