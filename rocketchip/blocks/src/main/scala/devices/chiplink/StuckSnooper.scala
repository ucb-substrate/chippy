...
class StuckSnooperNode(
    uFn: Seq[TLClientPortParameters] => TLClientPortParameters
  ...
  def resolveStar(
    ...
    require(
      iStars == 0 && oStars == 0,
      "StuckSnooper node does not support :=* or :*="
    )
  ...
  def mapParamsD(
      n: Int,
      p: Seq[TLClientPortParameters]
  ): Seq[TLClientPortParameters] = { Seq(uFn(p)) }
  def mapParamsU(
      n: Int,
      p: Seq[TLManagerPortParameters]
  ): Seq[TLManagerPortParameters] = { p ++ p }
}

class StuckSnooper(uFn: Seq[TLClientPortParameters] => TLClientPortParameters)(
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    def castA(x: TLBundleA) = {
      val ret = WireDefault(out.a.bits); ret <> x; ret
    }
    ...
    if (edgeOut.manager.anySupportAcquireB && edgeOut.client.anySupportProbe) {
      ...
      def castC(x: TLBundleC) = {
        val ret = WireDefault(out.c.bits); ret <> x; ret
      }
      ...
      def castE(x: TLBundleE) = {
        val ret = WireDefault(out.e.bits); ret <> x; ret
      }
...
class TLStuckSnooperTester(txns: Int)(implicit p: Parameters)
    extends LazyModule {
  ...
  mux.node := TLFilter(
    TLFilter.mSelectIntersect(AddressSet(0, ~16))
  ) := model1.node := fuzz1.node
  mux.node := TLFilter(
    TLFilter.mSelectIntersect(AddressSet(16, ~16))
  ) := model2.node := fuzz2.node
...
class TLStuckSnooperTest(txns: Int = 5000, timeout: Int = 500000)(implicit
    p: Parameters
