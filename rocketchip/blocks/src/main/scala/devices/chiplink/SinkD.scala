...
class SinkD(info: ChipLinkInfo) extends Module {
  ...
  val d_grant =
    d.bits.opcode === TLMessages.Grant || d.bits.opcode === TLMessages.GrantData

  when(io.q.fire) {
    switch(state) {
      is(s_header) {
        state := Mux(d_grant, s_sink, Mux(d_hasData, s_data, s_header))
      }
  ...
  val isLastState =
    state === Mux(d_hasData, s_data, Mux(d_grant, s_sink, s_header))
