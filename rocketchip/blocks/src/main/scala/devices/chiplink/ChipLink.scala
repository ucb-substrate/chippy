...
class ChipLink(val params: ChipLinkParams)(implicit p: Parameters)
    extends LazyModule() {

  val device = new SimpleBus("chiplink", Seq("sifive,chiplink"))

  private def maybeManager(
      x: Seq[AddressSet],
      f: Seq[AddressSet] => TLSlaveParameters
  ) =
      ...
      TLMasterPortParameters.v1(
        clients = Seq.tabulate(params.domains) { i =>
          TLMasterParameters.v1(
            ...
            sourceId = IdRange(
              i * params.sourcesPerDomain,
              (i + 1) * params.sourcesPerDomain
            ),
            requestFifo = i > 0,
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    require(
      ...
      s"ChipLink supports at most one caching master, ${edgeIn.client.clients
    ...
    val do_bypass = ResetCatchAndSync(
      clock,
      rx.reset.asBool
    ) || ResetCatchAndSync(clock, tx.reset.asBool)
