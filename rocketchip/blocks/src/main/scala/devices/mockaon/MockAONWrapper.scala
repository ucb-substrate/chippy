...
class MockAONWrapper(w: Int, c: MockAONParams)(implicit p: Parameters)
    extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    val crossing_slave_reset = ResetCatchAndSync(
      lfclk,
...
object IsoZero {
  def apply(iso: Bool, in: UInt): UInt = {
    ...
    val w = in.getWidth
    val isos: List[IsoZero] = List.tabulate(in.getWidth)(x =>
      Module(new IsoZero).suggestName(s"iso_$x")
    )
