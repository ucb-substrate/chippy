...
class WatchdogTimer extends Module with GenericTimer {
  ...
  override protected lazy val countAwake = AsyncResetReg(
    io.regs.cfg.write.running,
    io.regs.cfg.write_running && unlocked
  )(0)
  protected lazy val countEn = {
    val corerstSynchronized = RegNext(RegNext(io.corerst))
    countAlways || (countAwake && !corerstSynchronized)
  }
  override protected lazy val rsten = AsyncResetReg(
    io.regs.cfg.write.sticky,
    io.regs.cfg.write_sticky && unlocked
  )(0.U)
  protected lazy val ip = RegEnable(
    VecInit(Seq(io.regs.cfg.write.ip(0) || elapsed(0))),
  ...
  val tmpStickyDesc = RegFieldDesc(
    "wdogrsten",
  ...
  override protected lazy val cfg_desc =
    DefaultGenericTimerCfgDescs("wdog", ncmp).copy(
