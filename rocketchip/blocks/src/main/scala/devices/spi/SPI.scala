...
case class SPILocated(loc: HierarchicalLocation)
    extends Field[Seq[SPIAttachParams]](Nil)
...
case class SPIAttachParams(
  ...
  def attachTo(where: Attachable)(implicit p: Parameters): TLSPI = where {
    ...
    val spi = spiClockDomainWrapper {
      LazyModule(new TLSPI(tlbus.beatBytes, device))
    }
    ...
    tlbus.coupleTo(s"device_named_$name") { bus =>
      ...
      spiClockDomainWrapper.clockNode := (controlXType match {
        ...
        case _: AsynchronousCrossing =>
          ...
          blockerOpt.map { _.clockNode := spiClockGroup }.getOrElse {
            spiClockGroup
          }
      })
...
case class SPIFlashLocated(loc: HierarchicalLocation)
    extends Field[Seq[SPIFlashAttachParams]](Nil)
...
case class SPIFlashAttachParams(
  ...
  def attachTo(where: Attachable)(implicit p: Parameters): TLSPIFlash = where {
    val name =
      s"qspi_${SPI.nextFlashId()}" // TODO should these be shared with regular SPIs?
    val cbus = where.locateTLBusWrapper(controlWhere)
    val mbus = where.locateTLBusWrapper(dataWhere)
    val qspiClockDomainWrapper = LazyModule(new ClockSinkDomain(take = None))
    val qspi = qspiClockDomainWrapper {
      LazyModule(new TLSPIFlash(cbus.beatBytes, device))
    }
    ...
    cbus.coupleTo(s"device_named_$name") { bus =>
      ...
      qspiClockDomainWrapper.clockNode := (controlXType match {
        ...
        case _: AsynchronousCrossing =>
          ...
          blockerOpt.map { _.clockNode := qspiClockGroup }.getOrElse {
            qspiClockGroup
          }
      })
    ...
    mbus.coupleTo(s"device_named_$name") { bus =>
      val blockerOpt = blockerAddr.map { a =>
        val blocker = LazyModule(
          new TLClockBlocker(
            BasicBusBlockerParams(a + 0x1000, mbus.beatBytes, mbus.beatBytes)
          )
        )
        mbus.coupleTo(s"bus_blocker_for_$name") {
          blocker.controlNode := TLFragmenter(mbus) := _
        }
        blocker
      }
...
object SPI {
  ...
  def makePort(node: BundleBridgeSource[SPIPortIO], name: String)(implicit
      p: Parameters
  ): ModuleValue[SPIPortIO] = {
  ...
  def makeFlashPort(node: BundleBridgeSource[SPIPortIO], name: String)(implicit
      p: Parameters
  ): ModuleValue[SPIPortIO] = {
