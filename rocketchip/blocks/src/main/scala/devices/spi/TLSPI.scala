...
class SPITopModule(c: SPIParamsBase, outer: TLSPIBase)
  ...
  fifo.io.ctrl.fmt
    .viewAsSupertype(new SPIFormat(c) with HasSPILength) := ctrl.fmt
  ...
  val regmapBase = Seq(
    ...
    SPICRs.csdef -> ctrl.cs.dflt.zipWithIndex.map { case (x, i) =>
      RegField(
        1,
        x,
        RegFieldDesc(
          s"csdef$i",
          s"Chip select ${i} default",
          group = Some("csdef"),
    ...
    SPICRs.csmode -> Seq(
      RegField(
        ...
        RegFieldDesc(
          "csmode",
          "Chip select mode",
          reset = Some(SPICSMode.Auto.litValue)
        )
      )
    ),
    SPICRs.dcssck -> Seq(
      ...
      Seq(
        RegField(
          ...
          RegFieldDesc(
            "proto",
            "SPI Protocol",
            reset = Some(SPIProtocol.Single.litValue)
          )
        ),
        RegField(
          SPIEndian.width,
          ...
          RegFieldDesc(
            "endian",
            "SPI Endianness",
            reset = Some(SPIEndian.MSB.litValue)
          )
        ),
        RegField(
          SPIDirection.width,
          ...
          RegFieldDesc(
            "iodir",
            "SPI I/O Direction",
            reset = Some(SPIDirection.Rx.litValue)
          )
        )
      )
    ),
    SPICRs.len -> Seq(
      RegField(
        ...
        RegFieldDesc(
          ...
          reset = Some(math.min(c.frameBits, 8))
        )
      )
    ),
    SPICRs.txfifo -> RegFieldGroup(
      "txdata",
      Some("Transmit data"),
      NonBlockingEnqueue(fifo.io.tx)
    ),
    SPICRs.rxfifo -> RegFieldGroup(
      "rxdata",
      Some("Receive data"),
...
class MMCDevice(spi: Device, maxMHz: Double = 20)
    extends SimpleDevice("mmc", Seq("mmc-spi-slot")) {
