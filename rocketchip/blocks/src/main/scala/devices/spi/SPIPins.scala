...
class SPISignals[T <: Data](private val pingen: () => T, c: SPIParamsBase)
    extends SPIBundle(c) {
...
class SPIPins[T <: Pin](pingen: () => T, c: SPIParamsBase)
    extends SPISignals(pingen, c)
  ...
  def apply[T <: Pin](
    ...
    withClockAndReset(clock, reset) {
      ...
      (pins.dq zip spi.dq).zipWithIndex.foreach { case ((p, s), i) =>
        ...
        p.o.oe := s.oe
        p.o.ie := ~s.oe
        s.i := SynchronizerShiftReg(
          p.i.ival,
          syncStages,
          name = Some(s"spi_dq_${i}_sync")
        )
      }
