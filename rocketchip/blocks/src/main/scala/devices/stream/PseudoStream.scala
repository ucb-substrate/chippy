...
abstract class PseudoStream(busWidthBytes: Int, val params: PseudoStreamParams)(
...
class TLPseudoStream(busWidthBytes: Int, params: PseudoStreamParams)(implicit
    p: Parameters
) extends PseudoStream(busWidthBytes, params)
    with HasTLControlRegMap

case class PseudoStreamLocated(loc: HierarchicalLocation)
    extends Field[Seq[PseudoStreamAttachParams]](Nil)
...
case class PseudoStreamAttachParams(
  ...
  def attachTo(where: Attachable)(implicit p: Parameters): TLPseudoStream =
    where {
      val name = s"stream_${PseudoStream.nextId()}"
      val tlbus = where.locateTLBusWrapper(controlWhere)
      val streamClockDomainWrapper =
        LazyModule(new ClockSinkDomain(take = None))
      val stream = streamClockDomainWrapper {
        LazyModule(new TLPseudoStream(tlbus.beatBytes, device))
      }
      ...
      tlbus.coupleTo(s"device_named_$name") { bus =>
        ...
        streamClockDomainWrapper.clockNode := (controlXType match {
          ...
          case _: AsynchronousCrossing =>
            ...
            blockerOpt.map { _.clockNode := streamClockGroup }.getOrElse {
              streamClockGroup
            }
        })
...
object PseudoStream {
  ...
  def makePort(node: BundleBridgeSource[PseudoStreamPortIO], name: String)(
      implicit p: Parameters
  ): ModuleValue[PseudoStreamPortIO] = {
