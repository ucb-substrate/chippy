...
case class TimerLocated(loc: HierarchicalLocation)
    extends Field[Seq[TimerAttachParams]](Nil)
...
case class TimerAttachParams(
  ...
  def attachTo(where: Attachable)(implicit p: Parameters): Timer = where {
    ...
    val timer = timerClockDomainWrapper {
      LazyModule(new Timer(tlbus.beatBytes, device))
    }
    ...
    tlbus.coupleTo(s"device_named_$name") { bus =>
      ...
      timerClockDomainWrapper.clockNode := (controlXType match {
        ...
        case _: AsynchronousCrossing =>
          ...
          blockerOpt.map { _.clockNode := timerClockGroup }.getOrElse {
            timerClockGroup
          }
      })
