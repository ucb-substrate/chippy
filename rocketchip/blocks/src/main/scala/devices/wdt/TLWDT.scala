...
case class WDTParams(address: BigInt, size: Int = 0x1000, regBytes: Int = 4)
...
abstract class WDT(busWidthBytes: Int, val params: WDTParams)(implicit
    p: Parameters
...
class TLWDT(busWidthBytes: Int, params: WDTParams)(implicit p: Parameters)
    extends WDT(busWidthBytes, params)
    with HasTLControlRegMap

case class WDTLocated(loc: HierarchicalLocation)
    extends Field[Seq[WDTAttachParams]](Nil)
...
case class WDTAttachParams(
  ...
  def attachTo(where: Attachable)(implicit p: Parameters): TLWDT = where {
    ...
    val wdt = wdtClockDomainWrapper {
      LazyModule(new TLWDT(tlbus.beatBytes, device))
    }
    ...
    tlbus.coupleTo(s"device_named_$name") { bus =>
      ...
      wdtClockDomainWrapper.clockNode := (controlXType match {
        ...
        case _: AsynchronousCrossing =>
          ...
          blockerOpt.map { _.clockNode := wdtClockGroup }.getOrElse {
            wdtClockGroup
          }
      })
...
object WDT {
  ...
  def makePort(node: BundleBridgeSource[WDTPortIO], name: String)(implicit
      p: Parameters
  ): ModuleValue[WDTPortIO] = {
