...
abstract class GPIO(busWidthBytes: Int, c: GPIOParams)(implicit p: Parameters)
  ...
  val iofNode =
    c.includeIOF.option(BundleBridgeSource(() => new IOFPortIO(c.width)))
  ...
  lazy val module = new LazyModuleImp(this) {
      ...
      if (c.includeIOF)
        (Seq(
          RegField.rwReg(
            ...
            iofEnReg.io,
            Some(
              RegFieldDesc("iof_en", "HW I/O functon enable", reset = Some(0))
            )
          )
        ))
    ...
    val dsRegsAndDescs = Seq.tabulate(c.dsWidth)(i =>
      Seq(
        RegField(
          ...
          RegFieldDesc(
            s"ds$i",
            s"Pin drive strength $i selection",
            reset = Some(0)
          )
        )
      )
    ...
    val dsRegMap =
      for ((rd, i) <- dsRegsAndDescs.zipWithIndex)
        yield (GPIOCtrlRegs.drive + 4 * i -> Seq(
          RegField(
            c.width,
            dsReg(i),
            RegFieldDesc(
    ...
    val mapping = Seq(
      ...
      GPIOCtrlRegs.rise_ie + dsOffset -> Seq(
        RegField(
          c.width,
          riseIeReg,
      ...
      GPIOCtrlRegs.rise_ip + dsOffset -> Seq(
        RegField.w1ToClear(
          c.width,
          riseIpReg,
          rise,
          Some(
            RegFieldDesc("rise_ip", "Rise interrupt pending", volatile = true)
          )
        )
      ),
      GPIOCtrlRegs.fall_ie + dsOffset -> Seq(
        RegField(
          c.width,
          fallIeReg,
      ...
      GPIOCtrlRegs.fall_ip + dsOffset -> Seq(
        RegField.w1ToClear(
          c.width,
          fallIpReg,
          fall,
          Some(
            RegFieldDesc("fall_ip", "Fall interrupt pending", volatile = true)
          )
        )
      ),
      GPIOCtrlRegs.high_ie + dsOffset -> Seq(
        RegField(
          c.width,
          highIeReg,
      ...
      GPIOCtrlRegs.high_ip + dsOffset -> Seq(
        RegField.w1ToClear(
          c.width,
          highIpReg,
          valueReg,
          Some(
            RegFieldDesc("high_ip", "High interrupt pending", volatile = true)
          )
        )
      ),
      GPIOCtrlRegs.low_ie + dsOffset -> Seq(
        RegField(
          c.width,
          lowIeReg,
      ...
      GPIOCtrlRegs.low_ip + dsOffset -> Seq(
        RegField.w1ToClear(
          c.width,
          lowIpReg,
          ~valueReg,
      ...
      GPIOCtrlRegs.out_xor + dsOffset -> Seq(
        RegField(
          c.width,
          xorReg,
      ...
      GPIOCtrlRegs.passthru_high_ie + dsOffset -> Seq(
        RegField(
          c.width,
          passthruHighIeReg,
          RegFieldDesc(
      ...
      GPIOCtrlRegs.passthru_low_ie + dsOffset -> Seq(
        RegField(
          c.width,
          passthruLowIeReg,
          RegFieldDesc(
...
case class GPIOLocated(loc: HierarchicalLocation)
    extends Field[Seq[GPIOAttachParams]](Nil)
...
case class GPIOAttachParams(
  ...
  def attachTo(where: Attachable)(implicit p: Parameters): TLGPIO = where {
    ...
    val gpio = gpioClockDomainWrapper {
      LazyModule(new TLGPIO(cbus.beatBytes, device))
    }
    ...
    cbus.coupleTo(s"device_named_$name") { bus =>
      val blockerOpt = blockerAddr.map { a =>
        val blocker = LazyModule(
          new TLClockBlocker(
            BasicBusBlockerParams(a, cbus.beatBytes, cbus.beatBytes)
          )
        )
        cbus.coupleTo(s"bus_blocker_for_$name") {
          blocker.controlNode := TLFragmenter(cbus) := _
        }
        blocker
      }
      ...
      gpioClockDomainWrapper.clockNode := (controlXType match {
        ...
        case _: AsynchronousCrossing =>
          ...
          blockerOpt.map { _.clockNode := gpioClockGroup }.getOrElse {
            gpioClockGroup
          }
      })
...
object GPIO {
  ...
  def makePort(node: BundleBridgeSource[GPIOPortIO], name: String)(implicit
      p: Parameters
  ): ModuleValue[GPIOPortIO] = {
  ...
  def loopback(g: GPIOPortIO)(pinA: Int, pinB: Int) {
    require(
      g.pins.length > pinA,
      s"Pin ${pinA} out of range for GPIO port with only ${g.pins.length} pins"
    )
    require(
      g.pins.length > pinB,
      s"Pin ${pinB} out of range for GPIO port with only ${g.pins.length} pins"
    )
    g.pins.foreach { p =>
    ...
    a.i.ival := Mux(
      b.o.oe,
      (b.o.oval | b.o.pue),
      (a.o.pue | (a.o.oe & a.o.oval))
    ) & a.o.ie
    b.i.ival := Mux(
      a.o.oe,
      (a.o.oval | b.o.pue),
      (b.o.pue | (b.o.oe & b.o.oval))
    ) & b.o.ie
  }
}
