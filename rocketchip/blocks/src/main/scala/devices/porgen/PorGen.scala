...
class PorGenModuleImp(outer: PorGen) extends LazyModuleImp(outer) {
  ...
  val ereset_catch =
    ResetCatchAndSync(pin.hfclk, ~port.ereset_n, name = "e_reset_sync")
  // Copied - AsyncDownCounter from federation to sifive-blocks.util package for debounce counter
  val ereset_debouncer = Module(
    new AsyncDownCounter(clock = pin.hfclk, reset = ereset_catch, value = 255)
  )
...
abstract class PorGen(busWidthBytes: Int, val c: PorGenParams)(implicit
    p: Parameters
  ...
  val pin: BundleBridgeSource[PorGenPinIO] =
    BundleBridgeSource(() => new PorGenPinIO(c))
  lazy val module = new PorGenModuleImp(this)
}

class TLPorGen(busWidthBytes: Int, params: PorGenParams)(implicit p: Parameters)
    extends PorGen(busWidthBytes, params)
    with HasTLControlRegMap

case class PorGenLocated(loc: HierarchicalLocation)
    extends Field[Seq[PorGenAttachParams]](Nil)
...
case class PorGenAttachParams(
  ...
  def attachTo(where: Attachable)(implicit p: Parameters): TLPorGen = where {
    ...
    val porgen = porgenClockDomainWrapper {
      LazyModule(new TLPorGen(tlbus.beatBytes, device))
    }
    ...
    tlbus.coupleTo(s"device_named_$name") { bus =>
      ...
      porgenClockDomainWrapper.clockNode := (controlXType match {
        ...
        case _: AsynchronousCrossing =>
          ...
          blockerOpt.map { _.clockNode := porgenClockGroup }.getOrElse {
            porgenClockGroup
          }
      })
...
object PorGen {
  ...
  def makePort(node: BundleBridgeSource[PorGenPortIO], name: String)(implicit
      p: Parameters
  ): ModuleValue[PorGenPortIO] = {
    val porNode = node.makeSink()
    InModuleBody { porNode.makeIO()(ValName(name)) }
  }

  def makePin(node: BundleBridgeSource[PorGenPinIO], name: String)(implicit
      p: Parameters
  ): ModuleValue[PorGenPinIO] = {
    val porNode = node.makeSink()
    InModuleBody { porNode.makeIO()(ValName(name)) }
  }
