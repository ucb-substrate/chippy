...
/** UARTTx module recives TL bus data from Tx fifo in parallel and transmits
  * them to Port(Tx).
  *
  * ==datapass==
  ...
  *
  * ==Structure==
  *   - baud rate divisor counter: generate pulse, the enable signal for data
  *     shift.
  *   - data shift logic: parallel in, serial out
  *
  * @note
  *   Tx fifo transmits TL bus data to Tx module
  */
class UARTTx(c: UARTParams) extends Module {
  ...
  val plusarg_tx =
    PlusArg("uart_tx", 1, "Enable/disable the TX to speed up simulation").orR
  val plusarg_printf =
    PlusArg("uart_tx_printf", 0, "Enable/disable the TX printf").orR
  ...
  when(io.in.fire && plusarg_tx) {
    if (c.includeParity) {
      val includebit9 =
        if (c.dataBits == 9) Mux(io.data8or9.get, false.B, io.in.bits(8))
        else false.B
      val parity = Mux(
        io.enparity.get,
        includebit9 ^ io.in.bits(7, 0).asBools.reduce(_ ^ _) ^ io.parity.get,
        true.B
      )
      val paritywithbit9 =
        if (c.dataBits == 9)
      ...
      counter := Mux1H(
        (0 until c.stopBits).map(i => (io.nstop === i.U) -> (n + i + 1).U)
      ) - (!io.enparity.get).asUInt - io.data8or9.getOrElse(0.U)
      ...
      val bit9 =
        if (c.dataBits == 9) Mux(io.data8or9.get, 1.U(1.W), io.in.bits(8))
        else 1.U(1.W)
      shifter := Cat(bit9, io.in.bits(7, 0), 0.U(1.W))
      counter := Mux1H(
        (0 until c.stopBits).map(i => (io.nstop === i.U) -> (n + i + 1).U)
      ) - io.data8or9.getOrElse(0.U)
    }
  }
  when(busy) {
    prescaler := Mux(
      pulse || io.cts_n.getOrElse(false.B),
      io.div,
      prescaler - 1.U
    )
  }
