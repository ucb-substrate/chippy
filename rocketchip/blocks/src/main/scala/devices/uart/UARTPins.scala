...
class UARTSignals[T <: Data](
    private val pingen: () => T,
    val wire4: Boolean = false
) extends Bundle {
  val rxd = pingen()
  val txd = pingen()
...
object UARTPinsFromPort {
  def apply[T <: Pin](
    ...
    withClockAndReset(clock, reset) {
      ...
      uart.rxd := SyncResetSynchronizerShiftReg(
        rxd_t,
        syncStages,
        init = true.B,
        name = Some("uart_rxd_sync")
      )
      pins.rts_n.foreach { rt => rt.outputPin(uart.rts_n.get) }
      pins.cts_n.foreach { ct =>
        val cts_t = ct.inputPin()
        uart.cts_n.get := SyncResetSynchronizerShiftReg(
          cts_t,
          syncStages,
          init = false.B,
          name = Some("uart_cts_sync")
        )
      }
    }
  }
