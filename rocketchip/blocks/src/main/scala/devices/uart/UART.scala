...
/** UART parameters
  *
  * @param address
  *   uart device TL base address
  * @param dataBits
  *   number of bits in data frame
  * @param stopBits
  *   number of stop bits
  * @param divisorBits
  *   width of baud rate divisor
  * @param oversample
  *   constructs the times of sampling for every data bit
  * @param nSamples
  *   number of reserved Rx sampling result for decide one data bit
  * @param nTxEntries
  *   number of entries in fifo between TL bus and Tx
  * @param nRxEntries
  *   number of entries in fifo between TL bus and Rx
  * @param includeFourWire
  *   additional CTS/RTS ports for flow control
  * @param includeParity
  *   parity support
  * @param includeIndependentParity
  *   Tx and Rx have opposite parity modes
  * @param initBaudRate
  *   initial baud rate
  *
  * @note
  *   baud rate divisor = clk frequency / baud rate. It means the number of clk
  *   period for one data bit. Calculated in [[UARTAttachParams.attachTo()]]
  *
  * @example
  *   To configure a 8N1 UART with features below:
  *   {{{
...
case class UARTParams(
    ...
    includeIndependentParity: Boolean =
      false, // Tx and Rx have opposite parity modes
...
/** UART Module organizes Tx and Rx module with fifo and generates control
  * signals for them according to CSRs and UART parameters.
  *
  * ==Component==
  *   - Tx
  *   - Tx fifo
  ...
  *
  * @param divisorInit:
  *   number of clk period for one data bit
  */
class UART(busWidthBytes: Int, val c: UARTParams, divisorInit: Int = 0)(implicit
    p: Parameters
  ...
  require(
    divisorInit != 0,
    "UART divisor wasn't initialized during instantiation"
  )
  require(
    divisorInit >> c.divisorBits == 0,
    s"UART divisor reg (width $c.divisorBits) not wide enough to hold $divisorInit"
  )
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    port.rts_n.foreach { r =>
      r := Mux(enwire4, !(rxq.io.count < c.nRxEntries.U), tx_busy ^ invpol)
    }
    if (c.includeParity) {
    ...
    val mapping = Seq(
      UARTCtrlRegs.txfifo -> RegFieldGroup(
        "txdata",
        Some("Transmit data"),
        NonBlockingEnqueue(txq.io.enq)
      ),
      UARTCtrlRegs.rxfifo -> RegFieldGroup(
        "rxdata",
        Some("Receive data"),
        NonBlockingDequeue(rxq.io.deq)
      ),
      UARTCtrlRegs.txctrl -> RegFieldGroup(
        "txctrl",
        Some("Serial transmit control"),
        Seq(
          RegField(
            1,
            txen,
      ...
      UARTCtrlRegs.ie -> RegFieldGroup(
        "ie",
        Some("Serial interrupt enable"),
        Seq(
          RegField(
            1,
            ie.txwm,
      ...
      UARTCtrlRegs.ip -> RegFieldGroup(
        "ip",
        Some("Serial interrupt pending"),
        Seq(
          RegField.r(
            1,
            ip.txwm,
    ...
    val optionalparity =
      if (c.includeParity)
        Seq(
          UARTCtrlRegs.parity -> RegFieldGroup(
            "paritygenandcheck",
            Some("Odd/Even Parity Generation/Checking"),
            Seq(
              RegField(
                1,
                enparity,
              ...
              RegField(
                ...
                RegFieldDesc(
                  "errie",
                  "Interrupt on error in parity enable",
                  reset = Some(0)
                )
              )
            )
    ...
    val optionalwire4 =
      if (c.includeFourWire)
        Seq(
          UARTCtrlRegs.wire4 -> RegFieldGroup(
            "wire4",
            Some("Configure Clear-to-send / Request-to-send ports / RS-485"),
            Seq(
              RegField(
                1,
                enwire4,
              ...
              RegField(
                ...
                RegFieldDesc(
                  "invpol",
                  "Invert polarity of RTS in RS-485 mode",
                  reset = Some(0)
                )
              )
            )
    ...
    val optional8or9 =
      if (c.dataBits == 9)
        Seq(
          UARTCtrlRegs.either8or9 -> RegFieldGroup(
            "ConfigurableDataBits",
            Some("Configure number of data bits to be transmitted"),
            Seq(
              RegField(
                1,
                data8or9,
...
class TLUART(busWidthBytes: Int, params: UARTParams, divinit: Int)(implicit
    p: Parameters
) extends UART(busWidthBytes, params, divinit)
    with HasTLControlRegMap

case class UARTLocated(loc: HierarchicalLocation)
    extends Field[Seq[UARTAttachParams]](Nil)
...
case class UARTAttachParams(
  ...
  def attachTo(where: Attachable)(implicit p: Parameters): TLUART = where {
    ...
    val uartClockDomainWrapper =
      LazyModule(new ClockSinkDomain(take = None, name = Some("TLUART")))
    val uart = uartClockDomainWrapper {
      LazyModule(new TLUART(tlbus.beatBytes, device, divinit))
    }
    ...
    tlbus.coupleTo(s"device_named_$name") { bus =>
      val blockerOpt = blockerAddr.map { a =>
        ...
        tlbus.coupleTo(s"bus_blocker_for_$name") {
          blocker.controlNode := TLFragmenter(tlbus, Some("UART_Blocker")) := _
        }
        blocker
      }
      ...
      uartClockDomainWrapper.clockNode := (controlXType match {
        ...
        case _: AsynchronousCrossing =>
          ...
          blockerOpt.map { _.clockNode := uartClockGroup }.getOrElse {
            uartClockGroup
          }
      })
...
object UART {
  ...
  def makePort(node: BundleBridgeSource[UARTPortIO], name: String)(implicit
      p: Parameters
  ): ModuleValue[UARTPortIO] = {
