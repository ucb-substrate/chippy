...
/** UARTRx module recivies serial input from Rx port and transmits them to Rx
  * fifo in parallel
  *
  * ==Datapass==
  ...
  *
  * ==Structure==
  *   - baud rate divisor counter: generate pulse, the enable signal for sample
  *     and data shift
  *   - sample counter: sample happens in middle
  *   - data counter control signals for data shift process
  *   - sample and data shift logic
  *
  * ==State Machine==
  * s_idle: detect start bit, init data_count and sample count, start pulse
  * counter s_data: data reciving
  *
  * @note
  *   Rx fifo transmits Rx data to TL bus
  */
class UARTRx(c: UARTParams) extends Module {
  ...
  val sample_mid =
    (sample_count === ((c.oversampleFactor - c.nSamples + 1) >> 1).U)
  // todo unused
  ...
  val extend =
    (sample_count < remainder) // Pad head: (sample_count > ~remainder)
  ...
  valid := false.B
  io.out.valid := valid
  io.out.bits := (if (c.dataBits == 8) shifter
                  else Mux(io.data8or9.get, Cat(0.U, shifter(8, 1)), shifter))
    ...
    is(s_idle) {
      ...
      when(!io.in) {
        ...
        when(debounce_max) {
          ...
          // init data_count
          data_count := (c.dataBits + 1).U + (if (c.includeParity)
                                                io.enparity.get
                                              else 0.U) - io.data8or9
            .getOrElse(false.B)
            .asUInt
      ...
      when(pulse) {
        ...
        when(sample_mid) {
          if (c.includeParity) {
            ...
            when(parity_bit) {
              io.errorparity.get := (shifter.asBools.reduce(
                _ ^ _
              ) ^ voter ^ io.parity.get)
            }
            when(data_last) {
