...
class MSIMaster(targets: Seq[MSITarget])(implicit p: Parameters)
    extends LazyModule {
  val masterNode = TLClientNode(
    Seq(
      TLMasterPortParameters.v1(
        Seq(TLMasterParameters.v1("MSI Master", sourceId = IdRange(0, 2)))
      )
    )
  )
  ...
  val intNode = IntNexusNode(
    sourceFn = { _ =>
      IntSourcePortParameters(Seq(IntSourceParameters(1, Nil)))
    },
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    val targetMap = targets
      .flatMap { case MSITarget(address, spacing, number) =>
      ...
      .map { addr =>
        ...
        require(
          m.isDefined,
          s"MSIMaster ${name} was pointed at address 0x${addr}%x which does not exist"
        )
        require(
          m.get.supportsPutFull.contains(1),
          s"MSIMaster ${name} requires device ${m.get.name} supportPutFull of 1 byte (${m.get.supportsPutFull})"
        )
        addr.U
      }
    ...
    require(
      interrupts.size <= targetMap.size,
      s"MSIMaster ${name} has more interrupts (${interrupts.size}) than addresses to use (${targetMap.size})"
    )
