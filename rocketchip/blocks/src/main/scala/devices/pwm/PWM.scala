...
class PWMTimer(
  ...
  override protected lazy val oneShot = RegEnable(
    io.regs.cfg.write.running && !countReset,
    false.B,
    (io.regs.cfg.write_running && unlocked) || countReset
  )
  override protected lazy val extra: Vec[Bool] = RegEnable(
    io.regs.cfg.write.extra,
    VecInit.fill(maxcmp) { false.B },
    orR(io.regs.cfg.write_extra) && unlocked
  )
  override protected lazy val center: Vec[Bool] = RegEnable(
    io.regs.cfg.write.center,
    orR(io.regs.cfg.write_center) && unlocked
  )
  override protected lazy val gang: Vec[Bool] =
    RegEnable(io.regs.cfg.write.gang, orR(io.regs.cfg.write_gang) && unlocked)
  override protected lazy val deglitch = RegEnable(
    io.regs.cfg.write.deglitch,
    io.regs.cfg.write_deglitch && unlocked
  )(0)
  override protected lazy val sticky =
    RegEnable(io.regs.cfg.write.sticky, io.regs.cfg.write_sticky && unlocked)(0)
  override protected lazy val ip = {
    ...
    reg := (sel & elapsed) | (~sel & (elapsed | (VecInit.fill(ncmp) {
      doSticky
    } & reg)))
    when(orR(io.regs.cfg.write_ip) && unlocked) {
      reg := io.regs.cfg.write.ip.take(ncmp)
    }
    reg
  }
  ...
  override protected lazy val cfg_desc =
    DefaultGenericTimerCfgDescs(prefix, ncmp).copy(
      extra = Seq.tabulate(ncmp) { i =>
        RegFieldDesc(
          s"${prefix}invert${i}",
          s"Invert Comparator ${i} Output",
          reset = Some(0)
        )
      }
    )
  ...
  io.gpio := ((ipU & ~(gangU & Cat(ipU(0), ipU >> 1))) ^ invert)
    .asTypeOf(io.gpio)
...
abstract class PWM(busWidthBytes: Int, val params: PWMParams)(implicit
    p: Parameters
...
case class PWMLocated(loc: HierarchicalLocation)
    extends Field[Seq[PWMAttachParams]](Nil)
...
case class PWMAttachParams(
  ...
  def attachTo(where: Attachable)(implicit p: Parameters): TLPWM = {
    ...
    val pwm = pwmClockDomainWrapper {
      LazyModule(new TLPWM(tlbus.beatBytes, device))
    }
    ...
    tlbus.coupleTo(s"device_named_$name") { bus =>
      ...
      pwmClockDomainWrapper.clockNode := (controlXType match {
        ...
        case _: AsynchronousCrossing =>
          ...
          blockerOpt.map { _.clockNode := pwmClockGroup }.getOrElse {
            pwmClockGroup
          }
      })
...
object PWM {
  ...
  def makePort(node: BundleBridgeSource[PWMPortIO], name: String)(implicit
      p: Parameters
  ): ModuleValue[PWMPortIO] = {
