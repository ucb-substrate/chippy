...
object DefaultGenericTimerCfgDescs {
  def apply(prefix: String, ncmp: Int): GenericTimerCfgDescs =
    GenericTimerCfgDescs(
      scale = RegFieldDesc(s"${prefix}scale", "Counter scale value."),
      sticky = RegFieldDesc(
        s"${prefix}sticky",
        "Sticky. Disallow clearing of ${prefix}cmpXip bits"
      ),
      zerocmp =
        RegFieldDesc(s"${prefix}zerocmp", "Reset counter to zero after match."),
      deglitch = RegFieldDesc(
        s"${prefix}deglitch",
        "Deglitch - latch ${prefix}cmpXip within same cycle."
      ),
      countAlways = RegFieldDesc(
        s"${prefix}enalways",
      ...
      center = Seq.tabulate(ncmp) { i =>
        RegFieldDesc(s"${prefix}cmp${i}center", s"Comparator ${i} Center")
      },
      extra = Seq.tabulate(ncmp) { i =>
        RegFieldDesc(s"${prefix}extra${i}", s"Comparator ${i} Extra")
      },
      gang = Seq.tabulate(ncmp) { i =>
        RegFieldDesc(
          s"${prefix}gang${i}",
          s"Comparator ${i}/${(i + 1) % ncmp} Gang"
        )
      },
      ip = Seq.tabulate(ncmp) { i =>
        RegFieldDesc(
          s"${prefix}ip${i}",
          s"Interrupt ${i} Pending",
          volatile = true
        )
      }
    )
...
class GenericTimerCfgRegIFC(val ncmp: Int, val maxcmp: Int, val scaleWidth: Int)
    extends Bundle {
    ...
    RegFieldGroup(
      ...
      Seq(
        ...
        RegField(
          ...
          RegWriteFn((v, d) =>
            writeFn(v, d, write.countAlways, write_countAlways)
          ),
          descs.countAlways
        ),
        RegField(
        ...
        ++ Seq.tabulate(ncmp) { i =>
          RegField(
            1,
            RegReadFn(read.ip(i)),
            RegWriteFn((v, d) => writeFn(v, d, write.ip(i), write_ip(i))),
            descs.ip(i)
          )
        }
        ++ (if (ncmp < maxcmp) Seq(RegField(maxcmp - ncmp)) else Nil)
...
trait GenericTimer {
  ...
  protected def countHi_desc: RegFieldDesc = if (countWidth > regWidth)
    RegFieldDesc(s"${prefix}counthi", "High bits of Counter", volatile = true)
  else RegFieldDesc.reserved
  protected def s_desc: RegFieldDesc = RegFieldDesc(
    s"${prefix}s",
  ...
  protected def feed_desc: RegFieldDesc =
    RegFieldDesc(s"${prefix}feed", "Feed register")
  protected def key_desc: RegFieldDesc =
    RegFieldDesc(s"${prefix}key", "Key Register")
  protected def cfg_desc: GenericTimerCfgDescs =
    DefaultGenericTimerCfgDescs(prefix, ncmp)

  val io: GenericTimerIO

  protected val scale =
    RegEnable(io.regs.cfg.write.scale, io.regs.cfg.write_scale && unlocked)
  protected lazy val zerocmp =
    RegEnable(io.regs.cfg.write.zerocmp, io.regs.cfg.write_zerocmp && unlocked)
  protected val cmp =
    io.regs.cmp.map(c => RegEnable(c.write.bits, c.write.valid && unlocked))
  ...
  protected val elapsed = VecInit.tabulate(ncmp) { i =>
    Mux(s(cmpWidth - 1) && center(i), ~s, s) >= cmp(i)
  }
  protected val countReset = feed || (zerocmp && elapsed(0))
  when(countReset) { count := 0.U }
  when(io.regs.countLo.write.valid && unlocked) {
    count := Cat(count >> regWidth, io.regs.countLo.write.bits)
  }
...
object GenericTimer {
  def timerRegMap(
    ...
    val cmpRegs = (t.io.regs.cmp zip t.cmp_desc).zipWithIndex map {
      case ((r, d), i) => (8 + i) -> (r, d)
    }
    val otherRegs =
      for ((i, (r, d)) <- (regs ++ cmpRegs))
        yield (offset + regBytes * i) -> Seq(r.toRegField(Some(d)))
    cfgRegs ++ otherRegs
  }
}
