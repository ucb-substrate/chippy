...
class SourceBRequest(params: InclusiveCacheParameters)
  ...
  if (params.firstLevel) {
    ...
    if (params.clientBits > 1) {
      params.ccover(
        PopCount(remain) > 1.U,
        "SOURCEB_MULTI_PROBE",
        "Had to probe more than one client"
      )
    }
    ...
    params.ccover(
      b.valid && !b.ready,
      "SOURCEB_STALL",
      "Backpressured when issuing a probe"
    )

    val tag =
      Mux(!busy, io.req.bits.tag, RegEnable(io.req.bits.tag, io.req.fire))
    val set =
      Mux(!busy, io.req.bits.set, RegEnable(io.req.bits.set, io.req.fire))
    val param =
      Mux(!busy, io.req.bits.param, RegEnable(io.req.bits.param, io.req.fire))
