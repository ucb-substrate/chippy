...
class PutBufferAEntry(params: InclusiveCacheParameters)
...
class PutBufferPop(params: InclusiveCacheParameters)
    extends InclusiveCacheBundle(params) {
...
class SinkA(params: InclusiveCacheParameters) extends Module {
  ...
  when(a.valid && first && hasData && !req_block && !buf_block) {
    lists_set := freeOH
  }
