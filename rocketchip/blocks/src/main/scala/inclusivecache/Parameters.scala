...
case class InclusiveCacheMicroParameters(
    ...
    memCycles: Int =
      40, // # of L2 clock cycles for a memory round-trip (50ns @ 800MHz)
    portFactor: Int =
      4, // numSubBanks = (widest TL port * portFactor) / writeBytes
    dirReg: Boolean = false,
    innerBuf: InclusiveCachePortParameters =
      InclusiveCachePortParameters.fullC, // or none
    outerBuf: InclusiveCachePortParameters = InclusiveCachePortParameters.full
) // or flowAE
{
...
case class InclusiveCacheParameters(
  ...
  val lastLevel =
    !outer.manager.managers.exists(_.regionType > RegionType.UNCACHED)
  require(lastLevel)
  ...
  val flatAddresses =
    AddressSet.unify(outer.manager.managers.flatMap(_.address))
  val pickMask = AddressDecoder(
  ...
  def bitOffsets(
      x: BigInt,
      offset: Int = 0,
      tail: List[Int] = List.empty[Int]
  ): List[Int] =
    if (x == 0) tail.reverse
    else
      bitOffsets(x >> 1, offset + 1, if ((x & 1) == 1) offset :: tail else tail)
  ...
  def clientBit(source: UInt): UInt = {
    if (clientBitsRaw == 0) {
      ...
      Cat(
        inner.client.clients
          .filter(_.supports.probe)
          .map(_.sourceId.contains(source))
          .reverse
      )
    }
  }
  ...
  def expandAddress(tag: UInt, set: UInt, offset: UInt): UInt = {
    val base =
      Cat(widen(tag, tagBits), widen(set, setBits), widen(offset, offsetBits))
  ...
  def restoreAddress(expanded: UInt): UInt = {
    val missingBits = flatAddresses
      .map { a =>
        (a.widen(pickMask).base, a.widen(~pickMask))
      } // key is the bits to restore on match
      .groupBy(_._1)
      .view
...
object InclusiveCacheParameters {
  ...
  def out_mshrs(
    ...
    max(
      if (micro.dirReg) 3 else 2,
      (micro.memCycles + cache.blockBeats - 1) / cache.blockBeats
    )
  }
  def all_mshrs(
      cache: CacheParameters,
