...
case class ListBufferParameters[T <: Data](
    gen: T,
    queues: Int,
    entries: Int,
    bypass: Boolean
...
class ListBufferPush[T <: Data](params: ListBufferParameters[T])
    extends Bundle {
...
class ListBuffer[T <: Data](params: ListBufferParameters[T]) extends Module {
  override def desiredName =
    s"ListBuffer_${params.gen.typeName}_q${params.queues}_e${params.entries}"
  ...
  io.data := (if (!params.bypass) data.read(pop_head)
              else Mux(!pop_valid, io.push.bits.data, data.read(pop_head)))
  ...
  when(io.pop.fire) {
    ...
    head.write(
      io.pop.bits,
      Mux(
        io.push.fire && push_valid && push_tail === pop_head,
        freeIdx,
        next.read(pop_head)
      )
    )
