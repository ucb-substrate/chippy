...
abstract class BaseNoCTest(
  ...
  configs.foreach { config =>
    it should s"pass test with config ${config.getClass.getName}" in {
      ...
      // TODO: Add these flags once ChiselSim supports Verilator flags or copy Verilator backend code.
      // simulator.addArg(
      //   "-CFLAGS",
      //   Seq("-DNO_VPI", "-fcommon", "-fpermissive").iterator.mkString(" ")
      // )
      // simulator
      //   .addArg(
      //     "-LDFLAGS",
      //     Seq("-Wl,--allow-multiple-definition", "-fcommon").iterator
      //       .mkString(" ")
      //   )
      // simulator.addArg(extraVerilatorFlags)
      simulate(gen(p)) {
        RunUntilFinished.module(1000 * 1000)
      }
    }
  }
}

abstract class NoCTest(configs: Seq[Config])
    extends BaseNoCTest(p => new NoCChiselTester()(p), configs)
abstract class TLNoCTest(configs: Seq[Config])
    extends BaseNoCTest(p => new TLNoCChiselTester()(p), configs)
abstract class AXI4NoCTest(configs: Seq[Config])
    extends BaseNoCTest(p => new AXI4NoCChiselTester()(p), configs)
abstract class EvalNoCTest(configs: Seq[Config])
    extends BaseNoCTest(
      p => new EvalNoCChiselTester()(p),
      configs,
      Seq("../../../src/main/resources/csrc/netrace/netrace.o")
    )
