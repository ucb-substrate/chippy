...
trait PhysicalTopology {
  ...
  /** Plotter from TopologyPlotters.scala. Helps construct diagram of a concrete
    * topology.
    */
...
case class UnidirectionalLine(n: Int, skips: Seq[(Int, Int)] = Nil)
    extends PhysicalTopology {
  val nNodes = n
  def topo(src: Int, dest: Int) =
    dest - src == 1 || ((dest > src) && skips.contains((src, dest)))
...
/** A k-ary n-fly butterfly topology. This network has n stages of nodes; each
  * node connects to k nodes in the next stage.
  *
  * @param kAry
  *   number of channels of a node that connect to the previous and/or next
  *   stage
  * @param nFly
...
case class Butterfly(kAry: Int, nFly: Int) extends PhysicalTopology {
  ...
  def digitsToNum(dig: Seq[Int]) = dig.zipWithIndex.map { case (d, i) =>
    d * pow(kAry, i).toInt
  }.sum
  ...
  val channels = (1 until nFly).map { i =>
    table.map { e =>
      (digitsToNum(e.drop(1)), digitsToNum(e.updated(i, e(0)).drop(1)))
    }
  }
...
/** An dary**height tree topology.
  */
case class BidirectionalTree(val height: Int, val dAry: Int = 2)
    extends PhysicalTopology {
  ...
  def topo(src: Int, dest: Int) = {
    val dstChildOfSrc =
      List.range(1, dAry + 1).map((c: Int) => src * dAry + c).contains(dest)
    val srcChildOfDst =
      List.range(1, dAry + 1).map((c: Int) => dest * dAry + c).contains(src)
...
/** A 2D mesh network with nX * nY nodes. Bidirectional channels exist between
  * nodes that are a Manhattan distance of 1 away from each other. Node i can be
  * thought of as being located at euclidean coordinate (i % nX, i / nX) where
  * nX is as described below.
  *
  * @param nX
  *   maximum x-coordinate of a node
  * @param nY
  *   maximum y-coordinate of a node
  */
case class Mesh2D(nX: Int, nY: Int) extends Mesh2DLikePhysicalTopology {
  def topo(src: Int, dst: Int) = {
    ...
    val (dstX, dstY) = (dst % nX, dst / nX)(
      srcX == dstX && (srcY - dstY).abs == 1
    ) || (srcY == dstY && (srcX - dstX).abs == 1)
  }
}
...
/** A 2D unidirectional torus network with nX * nY nodes.
  ...
  *   maximum y-coordinate of a node
  */
case class UnidirectionalTorus2D(nX: Int, nY: Int)
...
/** A 2D bidirectional torus network with nX * nY nodes.
  ...
  *   maximum y-coordinate of a node
  */
case class BidirectionalTorus2D(nX: Int, nY: Int)
    extends Mesh2DLikePhysicalTopology {
  def topo(src: Int, dst: Int) = {
    val (srcX, srcY) = (src % nX, src / nX)
    val (dstX, dstY) = (dst % nX, dst / nX)
...
case class HierarchicalTopology(
    val base: PhysicalTopology,
    val children: Seq[HierarchicalSubTopology]
) extends PhysicalTopology {
  ...
  def topo(src: Int, dst: Int) = {
    (isBase(src), isBase(dst)) match {
      ...
      case (false, false) => {
        val sid = childId(src)
        val did = childId(dst)
        sid == did && children(sid).topo
          .topo(toChildNode(src), toChildNode(dst))
      }
    }
  }
  // TODO fix
...
case class CustomTopology(n: Int, edgeList: Seq[TopologyEdge])
    extends PhysicalTopology {
  val nNodes = n
  ...
  def topo(src: Int, dst: Int): Boolean =
    edgeList.contains(TopologyEdge(src, dst))
  val plotter = new LinePlotter(this)
}
