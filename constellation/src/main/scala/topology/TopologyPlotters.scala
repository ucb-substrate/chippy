...
class LinePlotter(topo: PhysicalTopology) extends PhysicalTopologyPlotter {
  ...
  def ingress(t: Int, nT: Int, n: Int) =
    (n - 0.5 + (t + 1).toDouble / (nT + 1).toDouble, 0.5)
  def egress(t: Int, nT: Int, n: Int) =
    (n - 0.5 + (t + 1).toDouble / (nT + 1).toDouble, -0.5)
}

class Torus1DPlotter(topo: Torus1DLikeTopology)
    extends PhysicalTopologyPlotter {
  def node(n: Int) = {
...
class ButterflyPlotter(topo: Butterfly) extends PhysicalTopologyPlotter {
  ...
  def ingress(t: Int, nT: Int, n: Int) =
    (-1, n % height - 0.5 + (t + 1).toDouble / (nT + 1).toDouble)
  def egress(t: Int, nT: Int, n: Int) =
    (1 + n / height, n % height - 0.5 + (t + 1).toDouble / (nT + 1).toDouble)
}

class Mesh2DPlotter(topo: Mesh2DLikePhysicalTopology)
    extends PhysicalTopologyPlotter {
  ...
  def node(nodeId: Int) = {
    parent(nodeId) match {
      ...
      case Some(p) =>
        ...
        (
          parentCoords._1 - rowLength / 2 + rowLength * (childIndex / (dAry - 1)),
          parentCoords._2 - 1
        )
    }
  }
...
class TerminalRouterPlotter(topo: TerminalRouter)
    extends PhysicalTopologyPlotter {
