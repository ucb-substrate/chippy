...
class AXI4MasterToNoC(
  ...
  io.flits.aw.bits.egress_id := Mux1H(requestAWIO.zipWithIndex.map {
    case (r, i) =>
  ...
  io.flits.w.bits.egress_id := Mux1H(requestWIO.zipWithIndex.map {
    case (r, i) =>
  ...
  io.flits.ar.bits.egress_id := Mux1H(requestARIO.zipWithIndex.map {
    case (r, i) =>
...
class AXI4SlaveToNoC(
  ...
  val aw_q = Module(
    new Queue(new AXI4BundleAW(wideBundle), awQueueDepth, pipe = true)
  )
  ...
  aw_q.io.enq.valid := out.w.valid && Mux1H(
    out_w_in_id,
    aw_val
  ) && out_w_in_head
  ...
  io.flits.r.bits.egress_id := Mux1H(requestROI.zipWithIndex.map {
    case (r, i) =>
  ...
  io.flits.b.bits.egress_id := Mux1H(requestBOI.zipWithIndex.map {
    case (r, i) =>
...
class AXI4InterconnectInterface(
    edgesIn: Seq[AXI4EdgeParameters],
    edgesOut: Seq[AXI4EdgeParameters]
...
case class AXI4ProtocolParams(
  ...
  val wideBundle =
    AXI4BundleParameters.union(edgesIn.map(_.bundle) ++ edgesOut.map(_.bundle))
  val inputIdRanges = AXI4Xbar.mapInputIds(edgesIn.map(_.master))
  val minPayloadWidth = {
    val b = new AXI4Bundle(wideBundle)
    Seq(b.aw, b.ar, b.w, b.b, b.r).map { t =>
      t.bits match {
  ...
  val flows = (0 until edgeInNodes.size)
    .map { i =>
      (0 until edgeOutNodes.size).map { o =>
        Seq(
          FlowParams(i * 3 + 0, o * 3 + 0 + edgesIn.size * 2, 4), // AW
  ...
  def genIO()(implicit p: Parameters): Data =
    new AXI4InterconnectInterface(edgesIn, edgesOut)
  def interface(
...
class AXI4NoCNode(implicit valName: ValName)
    extends AXI4NexusNode(
      masterFn = { seq =>
        seq(0).copy(
          ...
          masters =
            (AXI4Xbar.mapInputIds(seq) zip seq) flatMap { case (range, port) =>
...
abstract class AXI4NoCModuleImp(outer: LazyModule)
...
class AXI4NoC(params: AXI4NoCParams, name: String = "test")(implicit
    p: Parameters
) extends LazyModule {
  // END: AXI4NoCParams
