...
abstract class TLChannelFromNoC[T <: TLChannel](gen: => T)(implicit
    val p: Parameters
) extends Module
    with TLFieldHelper {
  val flitWidth = minTLPayloadWidth(gen)
  ...
  when(io.flit.fire && io.flit.bits.head) {
    is_const := false.B; const_reg := io.flit.bits.payload
  }
...
trait HasAddressDecoder {
  ...
  lazy val reacheableIO = edgesOut.map { mp =>
    edgeIn.client.clients.exists { c =>
...
class TLAToNoC(
    ...
    extends TLChannelToNoC(new TLBundleA(bundle), edgeIn, slaveToAEgress)(p)
    with HasAddressDecoder {
  has_body := edgeIn.hasData(protocol.bits) || (~protocol.bits.mask =/= 0.U)
  lazy val connectAIO = reacheableIO
  lazy val requestOH = outputPortFn(connectAIO).zipWithIndex.map {
    case (o, j) =>
...
class TLAFromNoC(edgeOut: TLEdge, bundle: TLBundleParameters)(implicit
    p: Parameters
) extends TLChannelFromNoC(new TLBundleA(bundle))(p) {
...
class TLBToNoC(
    ...
    extends TLChannelToNoC(new TLBundleB(bundle), edgeOut, masterToBIngress)(
      p
    ) {
  ...
  lazy val requestOH = inputIdRanges.map { i =>
    i.contains(protocol.bits.source)
  }
  q.io.enq <> io.protocol
}

class TLBFromNoC(edgeIn: TLEdge, bundle: TLBundleParameters, sourceSize: Int)(
    implicit p: Parameters
) extends TLChannelFromNoC(new TLBundleB(bundle))(p) {
  io.protocol <> protocol
...
class TLCToNoC(
    ...
    extends TLChannelToNoC(new TLBundleC(bundle), edgeIn, slaveToCEgress)(p)
    with HasAddressDecoder {
  ...
  lazy val requestOH = outputPortFn(connectCIO).zipWithIndex.map {
    case (o, j) =>
      connectCIO(j).B && (unique(connectCIO) || o(protocol.bits.address))
  }
  q.io.enq <> io.protocol
  q.io.enq.bits.source := io.protocol.bits.source | sourceStart.U
}

class TLCFromNoC(edgeOut: TLEdge, bundle: TLBundleParameters)(implicit
    p: Parameters
) extends TLChannelFromNoC(new TLBundleC(bundle))(p) {
  io.protocol <> protocol
}
...
class TLDToNoC(
    ...
    extends TLChannelToNoC(new TLBundleD(bundle), edgeOut, masterToDIngress)(
      p
    ) {
...
class TLDFromNoC(edgeIn: TLEdge, bundle: TLBundleParameters, sourceSize: Int)(
    implicit p: Parameters
) extends TLChannelFromNoC(new TLBundleD(bundle))(p) {
  io.protocol <> protocol
  io.protocol.bits.source := trim(protocol.bits.source, sourceSize)
}
...
class TLEToNoC(
    ...
    extends TLChannelToNoC(new TLBundleE(bundle), edgeIn, slaveToEEgress)(p) {
  has_body := edgeIn.hasData(protocol.bits)
  lazy val outputIdRanges = TLXbar.mapOutputIds(edgesOut.map(_.manager))
  lazy val requestOH = outputIdRanges.map { o =>
    o.contains(protocol.bits.sink)
  }
  q.io.enq <> io.protocol
}
class TLEFromNoC(edgeOut: TLEdge, bundle: TLBundleParameters, sourceSize: Int)(
    implicit p: Parameters
) extends TLChannelFromNoC(new TLBundleE(bundle))(p) {
  io.protocol <> protocol
