...
trait TLFieldHelper {
  ...
  def getConstFields(b: TLChannel): Seq[Data] = b match {
    case b: TLBundleA =>
      Seq(b.opcode, b.param, b.size, b.source, b.address, b.user, b.echo)
    case b: TLBundleB => Seq(b.opcode, b.param, b.size, b.source, b.address)
    case b: TLBundleC =>
      Seq(b.opcode, b.param, b.size, b.source, b.address, b.user, b.echo)
    case b: TLBundleD =>
      Seq(b.opcode, b.param, b.size, b.source, b.user, b.echo, b.sink, b.denied)
    case b: TLBundleE => Seq(b.sink)
  }
  def minTLPayloadWidth(b: TLChannel): Int =
    Seq(getBodyFields(b), getConstFields(b)).map(_.map(_.getWidth).sum).max
  def minTLPayloadWidth(bs: Seq[TLChannel]): Int =
    bs.map(b => minTLPayloadWidth(b)).max
  def minTLPayloadWidth(b: TLBundle): Int = minTLPayloadWidth(
    Seq(b.a, b.b, b.c, b.d, b.e).map(_.bits)
  )
}
...
class TileLinkInterconnectInterface(
    edgesIn: Seq[TLEdge],
    edgesOut: Seq[TLEdge]
...
trait TileLinkProtocolParams extends ProtocolParams with TLFieldHelper {
  ...
  require(
    edgesIn.size == edgeInNodes.size && edgesOut.size == edgeOutNodes.size
  )
  def wideBundle =
  ...
  def genIO()(implicit p: Parameters): Data =
    new TileLinkInterconnectInterface(edgesIn, edgesOut)
}
...
case class TileLinkABCDEProtocolParams(
  ...
  val ingressNodes =
    (edgeInNodes.map(u => Seq.fill(3)(u)) ++ edgeOutNodes.map(u =>
      Seq.fill(2) { u }
    )).flatten
  val egressNodes =
    (edgeInNodes.map(u => Seq.fill(2)(u)) ++ edgeOutNodes.map(u =>
      Seq.fill(3) { u }
    )).flatten
  val nVirtualNetworks = 5

  val flows = edgesIn.zipWithIndex
    .map { case (edgeIn, ii) =>
      edgesOut.zipWithIndex.map { case (edgeOut, oi) =>
        val reachable = edgeIn.client.clients.exists { c =>
          edgeOut.manager.managers.exists { m =>
        ...
        val probe = edgeIn.client.anySupportProbe && edgeOut.manager.managers
          .exists(_.regionType >= RegionType.TRACKED)
        val release =
          edgeIn.client.anySupportProbe && edgeOut.manager.anySupportAcquireB
        ((if (reachable)
            Some(FlowParams(ii * 3 + 0, oi * 3 + 0 + edgesIn.size * 2, 4))
          else None) ++ // A
          (if (probe)
             Some(FlowParams(oi * 2 + 0 + edgesIn.size * 3, ii * 2 + 0, 3))
           else None) ++ // B
          (if (release)
             Some(FlowParams(ii * 3 + 1, oi * 3 + 1 + edgesIn.size * 2, 2))
           else None) ++ // C
          (if (reachable)
             Some(FlowParams(oi * 2 + 1 + edgesIn.size * 3, ii * 2 + 1, 1))
           else None) ++ // D
          (if (release)
             Some(FlowParams(ii * 3 + 2, oi * 3 + 2 + edgesIn.size * 2, 0))
           else None)) // E
      }
    }
    .flatten
    ...
    protocol match {
      case protocol: TileLinkInterconnectInterface => {
        edgesIn.zipWithIndex.map { case (e, i) =>
          val nif_master = Module(
            new TLMasterToNoC(
              e,
              edgesOut,
              inputIdRanges(i).start,
              inputIdRanges(i).size,
              wideBundle,
        ...
        edgesOut.zipWithIndex.map { case (e, i) =>
          val nif_slave = Module(
            new TLSlaveToNoC(
              e,
              edgesIn,
              outputIdRanges(i).start,
              outputIdRanges(i).size,
              wideBundle,
              (s) => s * 2 + egressOffset,
              minPayloadWidth
            )
...
case class TileLinkACDProtocolParams(
  ...
  val minPayloadWidth = minTLPayloadWidth(
    Seq(genBundle.a, genBundle.c, genBundle.d).map(_.bits)
  )
  val ingressNodes =
    (edgeInNodes.map(u => Seq.fill(2)(u)) ++ edgeOutNodes.map(u =>
      Seq.fill(1) { u }
  ...
  val egressNodes =
    (edgeInNodes.map(u => Seq.fill(1)(u)) ++ edgeOutNodes.map(u =>
      Seq.fill(2) { u }
    )).flatten
    ...
    .map { case (edgeIn, ii) =>
      edgesOut.zipWithIndex.map { case (edgeOut, oi) =>
        ...
        val release =
          edgeIn.client.anySupportProbe && edgeOut.manager.anySupportAcquireB
        ((if (reachable)
            Some(FlowParams(ii * 2 + 0, oi * 2 + 0 + edgesIn.size * 1, 2))
          else None) ++ // A
          (if (release)
             Some(FlowParams(ii * 2 + 1, oi * 2 + 1 + edgesIn.size * 1, 1))
           else None) ++ // C
          (if (reachable)
             Some(FlowParams(oi * 1 + 0 + edgesIn.size * 2, ii * 1 + 0, 0))
           else None)) // D
  ...
  def interface(
    ...
    protocol match {
      case protocol: TileLinkInterconnectInterface => {
        protocol := DontCare
        edgesIn.zipWithIndex.map { case (e, i) =>
          val nif_master_acd = Module(
            new TLMasterACDToNoC(
              e,
              edgesOut,
              inputIdRanges(i).start,
              inputIdRanges(i).size,
        ...
        edgesOut.zipWithIndex.map { case (e, i) =>
          ...
          ingresses(
            i * 1 + 0 + edgesIn.size * 2
          ).flit <> nif_slave_acd.io.flits.d
          nif_slave_acd.io.flits.a <> egresses(
            i * 2 + 0 + edgesIn.size * 1
          ).flit
          nif_slave_acd.io.flits.c <> egresses(
            i * 2 + 1 + edgesIn.size * 1
          ).flit
        }
      }
    }
...
case class TileLinkBEProtocolParams(
  ...
  val minPayloadWidth = minTLPayloadWidth(
    Seq(genBundle.b, genBundle.e).map(_.bits)
  )
  val ingressNodes =
    (edgeInNodes.map(u => Seq.fill(1)(u)) ++ edgeOutNodes.map(u =>
      Seq.fill(1) { u }
    ...
    .map { case (edgeIn, ii) =>
      edgesOut.zipWithIndex.map { case (edgeOut, oi) =>
        ...
        val release =
          edgeIn.client.anySupportProbe && edgeOut.manager.anySupportAcquireB
        ((if (probe)
            Some(FlowParams(oi * 1 + 0 + edgesIn.size * 1, ii * 1 + 0, 1))
          else None) ++ // B
          (if (release)
             Some(FlowParams(ii * 1 + 0, oi * 1 + 0 + edgesIn.size * 1, 0))
           else None)) // E
      }
    }
    .flatten
    ...
    protocol match {
      case protocol: TileLinkInterconnectInterface => {
        ...
        edgesOut.zipWithIndex.map { case (e, i) =>
          ...
          ingresses(
            i * 1 + 0 + edgesIn.size * 1
          ).flit <> nif_slave_be.io.flits.b
      ...
      seq(0).v1copy(
        ...
        managers = seq.flatMap { port =>
          require(
            ...
            s"TLNoC (data widths don't match: ${port.managers
...
abstract class TLNoCModuleImp(outer: LazyModule) extends LazyModuleImp(outer) {
  ...
  val nocName: String
  lazy val inNames =
    nodeMapping.genUniqueName(edgesIn.map(_.master.masters.map(_.name)))
  lazy val outNames =
    nodeMapping.genUniqueName(edgesOut.map(_.slave.slaves.map(_.name)))
  ...
  lazy val module = new TLNoCModuleImp(this) {
    ...
    val noc = Module(
      new ProtocolNoC(
        ProtocolNoCParams(
          params.nocParams
            .copy(hasCtrl = false, nocName = name, inlineNoC = inlineNoC),
          Seq(protocolParams),
          inlineNoC = inlineNoC
        )
      )
...
class TLSplitACDxBENoC(
    params: SplitACDxBETLNoCParams,
    name: String = "test",
    inlineNoC: Boolean = false
)(implicit p: Parameters)
    extends TLNoCLike {
...
class TLGlobalNoC(params: GlobalTLNoCParams, name: String = "test")(implicit
    p: Parameters
) extends TLNoCLike {
