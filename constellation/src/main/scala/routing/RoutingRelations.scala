...
/** Routing and channel allocation policy
  *
  * @param f
  *   function that takes in source channel, next channel, and packet routing
  *   info. Returns True if packet can acquire/proceed to this next channel,
  *   False if not. An example is the bidirectionalLine method; see its
  *   docstring.
  * @param isEscape
  *   function that takes in ChannelRoutingInfo and a virtual network ID.
  *   Returns True if the channel represented by ChannelRoutingInfo is an escape
  *   channel and False if it is not. By default, we ignore the
  *   escape-channel-based deadlock-free properties, and just check for
  *   deadlock-freedom by verifying lack of a cyclic dependency.
  */
// BEGIN: RoutingRelation
abstract class RoutingRelation(topo: PhysicalTopology) {
  // Child classes must implement these
  def rel(
      srcC: ChannelRoutingInfo,
      nxtC: ChannelRoutingInfo,
      flow: FlowRoutingInfo
  ): Boolean
  ...
  private val memoize = new HashMap[
    (ChannelRoutingInfo, ChannelRoutingInfo, FlowRoutingInfo),
    Boolean
  ]()
  def apply(
      srcC: ChannelRoutingInfo,
...
/** Given a deadlock-prone routing relation and a routing relation representing
  * the network's escape channels, returns a routing relation that adds the
  * escape channels to the deadlock-prone relation.
  *
  * @param escapeRouter
  *   routing relation representing the network's escape channels
  * @param normalrouter
  *   deadlock-prone routing relation
  * @param nEscapeChannels
...
object EscapeChannelRouting {
  def apply(
      ...
      nEscapeChannels: Int = 1
  ) = (topo: PhysicalTopology) =>
    new RoutingRelation(topo) {
        ...
        if (srcC.src == -1) {
          if (nxtC.vc >= nEscapeChannels) { // if ingress and jumping into normal channel, use normal relation
            normal(
              srcC,
              nxtC.copy(
                vc = nxtC.vc - nEscapeChannels,
                n_vc = nxtC.n_vc - nEscapeChannels
              ),
              flow
            )
          ...
          normal(
            srcC.copy(
              vc = srcC.vc - nEscapeChannels,
              n_vc = srcC.n_vc - nEscapeChannels
            ),
            nxtC.copy(
              vc = nxtC.vc - nEscapeChannels,
              n_vc = nxtC.n_vc - nEscapeChannels
      ...
      override def getPrio(
          src: ChannelRoutingInfo,
          a: ChannelRoutingInfo,
          flow: FlowRoutingInfo
      ): Int = {
        ...
        val nSrc = if (src.isIngress) {
          ...
          src.copy(
            vc = src.vc - nEscapeChannels,
            n_vc = src.n_vc - nEscapeChannels
          )
        }
        if (aEscape) {
          escape.getPrio(nSrc, a.copy(n_vc = nEscapeChannels), flow) + normal
            .getNPrios(src)
        } else {
          normal.getPrio(
            nSrc,
            a.copy(
              vc = a.vc - nEscapeChannels,
              n_vc = a.n_vc - nEscapeChannels
            ),
            flow
          )
        }
...
object AllLegalRouting {
  def apply() = (topo: PhysicalTopology) =>
    new RoutingRelation(topo) {
      def rel(
          srcC: ChannelRoutingInfo,
          nxtC: ChannelRoutingInfo,
  ...
  def apply() = (topo: PhysicalTopology) =>
    new RoutingRelation(topo) {
      def rel(
        ...
        if (nxtC.src < nxtC.dst) flow.egressNode >= nxtC.dst
        else flow.egressNode <= nxtC.dst
      }
    }
}
...
object ButterflyRouting {
  def apply() = (topo: PhysicalTopology) =>
    topo match {
      case topo: constellation.topology.Butterfly =>
        new RoutingRelation(topo) {
          def rel(
              srcC: ChannelRoutingInfo,
              nxtC: ChannelRoutingInfo,
            ...
            val (nodeX, nodeY) =
              (nxtC.src / topo.height, nxtC.src % topo.height)
            val (dstX, dstY) =
              (flow.egressNode / topo.height, flow.egressNode % topo.height)
            if (dstX <= nodeX) {
              false
              ...
              val dsts =
                (nxtX until topo.nFly - 1).foldRight((0 until topo.height).map {
                  i => Seq(i)
                }) { case (i, l) =>
                  (0 until topo.height).map { s =>
                    topo
                      .channels(i)
                      .filter(_._1 == s)
                      .map { case (_, d) =>
...
object BidirectionalTreeRouting {
  def apply() = (topo: PhysicalTopology) =>
    topo match {
      case topo: constellation.topology.BidirectionalTree =>
        new RoutingRelation(topo) {

          /** Returns a boolean indicating whether src is an ancestor node of
            * dst
            */
          def isAncestor(src: Int, dst: Int): Boolean = {
            if (src == dst) {
              ...
              ((topo.dAry * src + 1) to (topo.dAry * src + topo.dAry))
                .foldLeft(false)((sofar, nextChild) =>
                  sofar || isAncestor(nextChild, dst)
                )
            }
          }
          def rel(
...
object Mesh2DDimensionOrderedRouting {
  def apply(firstDim: Int = 0) = (topo: PhysicalTopology) =>
    topo match {
      case topo: Mesh2DLikePhysicalTopology =>
        new RoutingRelation(topo) {
          def rel(
              srcC: ChannelRoutingInfo,
              nxtC: ChannelRoutingInfo,
            ...
            val (dstX, dstY) =
              (flow.egressNode % topo.nX, flow.egressNode / topo.nX)

            if (firstDim == 0) {
              if (dstX != nodeX) {
                (if (nodeX < nxtX) dstX >= nxtX
                 else dstX <= nxtX) && nxtY == nodeY
              } else {
                (if (nodeY < nxtY) dstY >= nxtY
                 else dstY <= nxtY) && nxtX == nodeX
              }
            } else {
              if (dstY != nodeY) {
                (if (nodeY < nxtY) dstY >= nxtY
      ...
      case topo: Mesh2DLikePhysicalTopology =>
        new RoutingRelation(topo) {
          def rel(
            ...
            val xR =
              (if (nodeX < nxtX) dstX >= nxtX
               else if (nodeX > nxtX) dstX <= nxtX
               else nodeX == nxtX)
            val yR =
              (if (nodeY < nxtY) dstY >= nxtY
               else if (nodeY > nxtY) dstY <= nxtY
               else nodeY == nxtY)
            xR && yR
          }
        }
...
object UnidirectionalTorus2DDatelineRouting {
  def apply() = (topo: PhysicalTopology) =>
    topo match {
      case topo: Mesh2DLikePhysicalTopology =>
        new RoutingRelation(topo) {
          val baseX = UnidirectionalTorus1DDatelineRouting()(
            constellation.topology.UnidirectionalTorus1D(topo.nX)
          )
          val baseY = UnidirectionalTorus1DDatelineRouting()(
            constellation.topology.UnidirectionalTorus1D(topo.nY)
          )
          def rel(
              srcC: ChannelRoutingInfo,
              nxtC: ChannelRoutingInfo,
...
object DimensionOrderedBidirectionalTorus2DDatelineRouting {
  def apply() = (topo: PhysicalTopology) =>
    topo match {
      case topo: Mesh2DLikePhysicalTopology =>
        new RoutingRelation(topo) {
          val baseX = BidirectionalTorus1DShortestRouting()(
            constellation.topology.BidirectionalTorus1D(topo.nX)
          )
          val baseY = BidirectionalTorus1DShortestRouting()(
            constellation.topology.BidirectionalTorus1D(topo.nY)
          )
...
object TerminalRouterRouting {
  def apply(baseRouting: PhysicalTopology => RoutingRelation) =
    (topo: PhysicalTopology) =>
      topo match {
        case topo: constellation.topology.TerminalRouter =>
          new RoutingRelation(topo) {
              ...
              if (srcC.isIngress && topo.isBase(nxtC.dst)) {
                ...
                if (topo.isTerminal(srcC.src)) {
                  base(
                    srcC.copy(
                      src = (-1),
                      dst = srcC.dst % topo.base.nNodes,
                      vc = 0,
                      n_vc = 1
                    ),
                    nxtC.copy(
                      src = nxtC.src % topo.base.nNodes,
                      dst = nxtC.dst % topo.base.nNodes
                    ),
            ...
            override def getNPrios(c: ChannelRoutingInfo): Int = {
              if (topo.isBase(c.dst) && topo.isBase(c.src)) {
                base.getNPrios(
                  c.copy(
                    src = c.src % topo.base.nNodes,
                    dst = c.dst % topo.base.nNodes
                  )
                )
              } else {
                1
...
/** Produces a system with support for n virtual subnetworks. The output routing
  * relation ensures that the virtual subnetworks do not block each other. This
  * is accomplished by allocating a portion of the virtual channels to each
  * subnetwork; all physical resources are shared between virtual subnetworks
  * but virtual channels and buffers are not shared.
  */
object NonblockingVirtualSubnetworksRouting {
  def apply(
      f: PhysicalTopology => RoutingRelation,
      n: Int,
      nDedicatedChannels: Int
  ) = (topo: PhysicalTopology) =>
    new RoutingRelation(topo) {
      ...
      def lower(c: ChannelRoutingInfo) = c.copy(
        ...
        n_vc =
          if (c.isIngress) 1
          else c.n_vc - n * nDedicatedChannels + nDedicatedChannels
      )
      val base = f(topo)
      def rel(
          srcC: ChannelRoutingInfo,
        ...
        val able =
          (nxtC.vc >= n * nDedicatedChannels) || ((nxtC.vc / nDedicatedChannels) == flow.vNetId)
    ...
    new RoutingRelation(topo) {
      ...
      def rel(
        ...
        val r = if (srcC.isIngress && lNxtV >= 0) {
          ...
          base(
            srcC.copy(n_vc = srcC.n_vc - flow.vNetId * nDedicated, vc = lSrcV),
            nxtC.copy(n_vc = nxtC.n_vc - flow.vNetId * nDedicated, vc = lNxtV),
            flow.copy(vNetId = 0)
          )
        }
      ...
      override def isEscape(c: ChannelRoutingInfo, v: Int) = {
        c.vc >= v * nDedicated && base.isEscape(
          c.copy(vc = c.vc - v * nDedicated, n_vc = c.n_vc - v * nDedicated),
          0
        )
      }
    }
...
object HierarchicalRouting {
  def apply(
    ...
    topo match {
      case topo: HierarchicalTopology =>
        new RoutingRelation(topo) {
          val base = baseRouting(topo.base)
          val children = (childRouting zip topo.children).map { case (l, r) =>
            l(r.topo)
          }
          ...
          def rel(
            ...
            (thisBase, flowBase, nextBase) match {
              case (true, true, true) =>
                base(
                  srcC.copy(src =
                    if (srcC.src >= topo.base.nNodes) -1 else srcC.src
                  ),
                  nxtC,
                  flow
                )
              ...
              case (true, false, false) =>
                topo.childId(nxtC.dst) == topo.childId(flow.egressNode)
              ...
              case (false, false, true) =>
                topo.childId(srcC.dst) != topo.childId(flow.egressNode)
          ...
          override def getNPrios(c: ChannelRoutingInfo): Int = {
            if (topo.isBase(c.dst) && topo.isBase(c.src)) {
              ...
              topo.childId(c.src) == topo.childId(c.dst) && !c.isEgress
            ) {
              children(topo.childId(c.src)).getNPrios(
                c.copy(src = if (c.src >= topo.base.nNodes) -1 else c.src)
              )
            } else {
              1
            }
            ...
            (thisBase, nextBase) match {
              case (true, true) =>
                base.getPrio(
                  ...
                  nxtC,
                  flow.copy(egressNode =
                    topo.children(topo.childId(flow.egressNode)).src
                  )
                )
              case (false, false) => {
          ...
          override def isEscape(c: ChannelRoutingInfo, v: Int) = {
            ...
            val dstBase = topo.isBase(c.dst)(srcBase, dstBase) match {
...
/** Pure shortest-path routing strategy with some VC-based load balancing.
  *
  * This routing relation computes single-path shortest routes between all node
  * pairs using BFS and restricts flow transitions to only those that follow the
  * precomputed shortest-path next hop. No deadlock avoidance mechanism is
  * enforced.
  *
  * If multiple virtual channels (VCs) are available, we can assist the
  * PrioritizingVCAllocator by assigning priorities deterministically via a
  * hashing of (flow.ingressNode, flow.egressNode) to help with better VC
  * utilization. However VC transitions are not enforced and per-hop flows are
  * allowed to use any VC as long as the edge is on the shortest path.
  *
  * This routing strategy is best for minimal-hop designs, and scenarios where
  * higher-level deadlock prevention is handled externally such as through
  * EscapeChannelRouting.
  *
  * maxVCs: The number of virtual channels available per physical channel. This
  * must be >= 1. Defaults to 1 (no VC usage).
  */

object ShortestPathRouting {
  def apply(maxVCs: Int = 1) = (topo: PhysicalTopology) =>
    new RoutingRelation(topo) {
      require(
        maxVCs >= 1,
        s"[ShortestPathRouting] maxVCs must be >= 1 (got $maxVCs)"
      )
      ...
      val nextHop: Map[(Int, Int), Int] = {
        val paths =
          for (
            src <- 0 until topo.nNodes; dst <- 0 until topo.nNodes if src != dst
          ) yield {
        ...
        while (queue.nonEmpty) {
          ...
          if (!visited.contains(node)) {
            ...
            adjList(node).foreach { neighbor =>
              queue.enqueue(
                (neighbor, if (path.isEmpty) List(neighbor) else path)
              )
            }
          }
        }
      ...
      def rel(
        ...
        if (srcC.src == -1) {
          ...
          val legal =
            nxtC.src == flow.ingressNode && expected.contains(nxtC.dst)
          return legal
        }
    ...
    topo match {
      case topo: CustomTopology =>
        new RoutingRelation(topo) {
          ...
          val allFlowAssignments: Map[(Int, Int), Int] = allSSPs.map {
            case ((src, dst), path) =>
              val assignedLayer = packed.indexWhere(layer =>
                path.toSet.subsetOf(layer.flatMap(_._2).toSet)
              )
              ((src, dst), assignedLayer)
          }.toMap

          // Build nextHop map per VC (layer)
          val nextHopMap: Map[Int, Map[(Int, Int), Int]] =
            packed.zipWithIndex.map { case (layer, vc) =>
          ...
          def rel(
            ...
            val assignedVC = allFlowAssignments.getOrElse(
              flowKey, {
                return false
              }
            )
            ...
            val flowPath = flowPaths.getOrElse(
              flowKey, {
                return false
              }
            )
            ...
            val vcOk =
              nxtC.vc == assignedVC && (srcC.src == -1 || srcC.vc == assignedVC)
...
/** ShortestPathGeneralizedDatelineRouting implements a deadlock-free routing
  * relation based on identifying cycle-breaking datelines and the presence of
  * virtual channels.
  *
  ...
  *      - Compute the minimum number of virtual channels (VCs) needed to
  *        support all shortest paths without deadlock
  *
  * 2. Recomputes all-pairs shortest paths and annotates each hop with the
  * required VC level. VC level increases only at dateline crossings, and
  * remains constant otherwise. This leads to an enforced acyclic traversal of
...
object ShortestPathGeneralizedDatelineRouting {
  ...
  def apply(
    ...
    new RoutingRelation(topo) {
      ...
      val (shortestPaths, pathVCs)
          : (Map[(Int, Int), List[Int]], Map[(Int, Int), List[Int]]) = {
        val g = new CustomGraph(
          topo.nNodes,
          (0 until topo.nNodes).flatMap(u =>
            (0 until topo.nNodes).collect {
              case v if topo.topo(u, v) => (u, v)
            }
          )
        )
        val sspPaths = g.generateSSPs()
        val pathMap = sspPaths.map { case ((src, dst), edges) =>
          ((src, dst), src +: edges.map(_._2))
        }.toMap
        val vcMap = pathMap.map { case ((src, dst), path) =>
          val vcs = if (useStaticVC0) {
            path
              .sliding(2)
              .scanLeft(0) { case (vc, Seq(u, v)) =>
            ...
            path
              .sliding(2)
              .scanLeft(baseVC) { case (vc, Seq(u, v)) =>
      ...
      println(
        s"[ShortestPathGeneralizedDatelineRouting] Initialized with ${datelineEdges.size} datelines and $maxVC VCs"
      )
      ...
      def rel(
        ...
        val path =
          shortestPaths.getOrElse((flow.ingressNode, flow.egressNode), Nil)
        val vcs = pathVCs.getOrElse((flow.ingressNode, flow.egressNode), Nil)

        if (srcC.src == -1) {
          val validInjectionEdge = path.headOption
            .contains(nxtC.src) && path.lift(1).contains(nxtC.dst)
          ...
          val idx = pathEdges.indexWhere { case Seq(a, b) =>
            a == edge._1 && b == edge._2
          }

          val validEdge = idx != -1
          val expectedVC =
            if (idx >= 0 && idx < vcs.length) vcs(idx + 1) else -1
          val vcValid = nxtC.vc == expectedVC
