...
/** Represents the properties of a packet that are relevant for routing
  * ingressId and egressId uniquely identify a flow, but vnet and dst are used
  * here to simplify the implementation of routingrelations
  *
  * @param ingressId
  *   packet's source ingress point
  * @param egressId
  *   packet's destination egress point
  * @param vNet
  *   virtual subnetwork identifier
  * @param dst
  *   packet's destination node ID
  */
...
class FlowRoutingBundle(implicit val p: Parameters)
    extends Bundle
    with HasNoCParams {
