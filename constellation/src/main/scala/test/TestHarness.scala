...
class InputGen(idx: Int, cParams: IngressChannelParams)(implicit
    val p: Parameters
) extends Module
    with HasNoCParams {
  ...
  val packet_remaining =
    if (p(NoCTesterKey).constPacketSize) maxFlits.U else (LFSR(20) % maxFlits.U)
  val random_flit_delay =
    (LFSR(20) < (inputFlitStallProbability * (1 << 20)).toInt.U)
  val random_packet_delay =
    (LFSR(20) < (inputPacketStallProbability * (1 << 20)).toInt.U)
  ...
  io.out.bits.egress_id := VecInit(
    cParams.possibleFlows.toSeq.map(_.egressId.U)
  )(flow_idx)
...
class NoCTester(
    inputParams: Seq[IngressChannelParams],
    outputParams: Seq[EgressChannelParams]
)(implicit val p: Parameters)
    extends Module
    with HasNoCParams {
  val allPayloadBits =
    (inputParams.map(_.payloadBits) ++ outputParams.map(_.payloadBits)).toSet
  ...
  val io = IO(new Bundle {
    ...
    val from_noc = MixedVec(outputParams.map { u =>
      Flipped(new EgressChannel(u))
    })
    val success = Output(Bool())
  })
  ...
  val enable_print_latency =
    PlusArg("noctest_enable_print", default = 0, width = 1)(0)
  ...
  io.from_noc.zipWithIndex map { case (o, i) =>
    ...
    val flow_ctrs = RegInit(VecInit.fill(outputParams(i).possibleFlows.size) {
      0.U(8.W)
    })
    when(o.flit.fire) {
      val flows = outputParams(i).possibleFlows.toSeq
      val fifo =
        flows.filter(_.fifo).map(_.ingressId.U === o.flit.bits.ingress_id).orR
      val flow_id = flows.zipWithIndex
        .map { case (f, fid) =>
      ...
      assert(
        rob_payload(rob_idx).asUInt === o.flit.bits.payload.asUInt,
        cf"out[${i.toString}] incorrect payload"
      );
      assert(
        o.flit.bits.ingress_id === rob_ingress_id(rob_idx),
        cf"out[${i.toString}] incorrect source"
      )
      assert(
        i.U === rob_egress_id(rob_idx),
        cf"out[${i.toString}] incorrect destination"
      )
      assert(
        rob_flits_returned(rob_idx) < rob_n_flits(rob_idx),
        cf"out[${i.toString}] too many flits returned"
      )
      assert((!packet_valid && o.flit.bits.head) || rob_idx === packet_rob_idx)
      when(fifo && o.flit.bits.tail) {
        assert(
          flow_ctrs(flow_id) === o.flit.bits.payload
            .asTypeOf(new Payload)
            .flow_idx,
      ...
      when(o.flit.bits.head) {
        packet_valid := true.B; packet_rob_idx := rob_idx
      }
  ...
  for (i <- 0 until robSz) {
    when(rob_valids(i)) {
      assert(
        tsc - rob_tscs(i) < (16 << 10).U,
        cf"ROB Entry ${i.toString} took too long"
      )
    }
  }
}

class TestHarness(implicit val p: Parameters)
    extends Module
  ...
  val noc_tester = Module(
    new NoCTester(lazyNoC.allIngressParams, lazyNoC.allEgressParams)(lazyNoC.iP)
  )
