...
case object AXI4NoCTesterKey
    extends Field[AXI4NoCTesterParams](AXI4NoCTesterParams())

class WithAXI4NoCTesterParams(p: AXI4NoCTesterParams)
...
class AXI4NoCTester(implicit p: Parameters) extends LazyModule {
  ...
  val inNodeMapping = ListMap(tParams.inNodeMapping.zipWithIndex.map {
    case (i, j) => s"[$j]" -> i
  }: _*)
  val outNodeMapping = ListMap(tParams.outNodeMapping.zipWithIndex.map {
    case (i, j) => s"[$j]" -> i
  }: _*)
  val nodeMapping = DiplomaticNetworkNodeMapping(inNodeMapping, outNodeMapping)
  ...
  val noc = LazyModule(
    new AXI4NoC(
      AXI4NoCParams(
        nodeMapping,
        p(AXI4NoCTesterKey).nocParams.copy(nocName = "test")
      )
    )
  )
  ...
  val masters = Seq.fill(nMasters) {
    LazyModule(new TLFuzzer(txns, 32, nOrdered = Some(1)))
  }
...
class AXI4TestHarness(implicit val p: Parameters)
    extends Module
    with HasSuccessIO {
