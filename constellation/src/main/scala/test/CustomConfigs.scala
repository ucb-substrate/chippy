    ...
    nDedicated: Int = 3
      ...
      NoCTesterParams(
        NoCParams(
          topology = topology,
          channelParamGen = (a, b) =>
            UserChannelParams(Seq.fill(nSubnets * nDedicated) {
          ...
          flows = Seq
            .tabulate(topology.nNodes, topology.nNodes) { (s, d) =>
              FlowParams(s, d, 0)
            }
            .flatten,
      ...
      TLNoCTesterParams(
        ...
        nocParams = {
          ...
          NoCParams(
            topology = topology,
            channelParamGen = (a, b) =>
              UserChannelParams(Seq.fill(numChannels) {
                UserVirtualChannelParams(4)
              }),
...
object CustomTopologies {
  ...
  val uniRing =
    CustomTopology(n, (0 until n).map(i => TopologyEdge(i, (i + 1) % n)))

  val biRing = CustomTopology(
    n,
    (0 until n).flatMap(i =>
      Seq(
  ...
  val bypass = CustomTopology(
    ...
    ) ++ Seq((0, 5), (5, 0), (1, 4), (4, 1)).map { case (a, b) =>
      TopologyEdge(a, b)
    }
  )

  val crossbar = CustomTopology(
    n,
    (0 until n).flatMap(i =>
      Seq(
        TopologyEdge(i, (i + 1) % n),
    ...
    ) ++ Seq((0, 4), (4, 0), (1, 4), (4, 1), (0, 5), (5, 0), (1, 5), (5, 1))
      .map { case (a, b) =>
        TopologyEdge(a, b)
      }
  )

  val fullMesh = CustomTopology(
    n,
    for {
...
// Unidirectional Ring
class TestConfigUniRingGD
    extends CustomTestConfigBase(
      uniRing,
...
class TLTestConfigUniRingGD
    extends CustomTLTestConfigBase(
      uniRing,
      ShortestPathGeneralizedDatelineRouting(),
      "TLUniRingGD",
      nSubnets = 5,
