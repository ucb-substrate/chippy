...
case class ConstellationSystemBusParams(
  ...
  ): ConstellationSystemBus = {
    val constellation = LazyModule(
      new ConstellationSystemBus(
        sbusParams,
        tlNoCParams,
        loc.name,
        context,
...
class ConstellationSystemBus(
    sbus_params: SystemBusParams,
    noc_params: TLNoCParams,
    name: String,
    context: HasTileLinkLocations,
    inlineNoC: Boolean
)(implicit p: Parameters)
    extends TLBusWrapper(sbus_params, name) {
  private val replicator =
    sbus_params.replication.map(r => LazyModule(new RegionReplicator(r)))
  ...
  private val system_bus_noc = noc_params match {
    case params: GlobalTLNoCParams =>
      context.asInstanceOf[CanHaveGlobalNoC].globalNoCDomain {
        LazyModule(new TLGlobalNoC(params, name))
      }
    case params: SimpleTLNoCParams =>
      LazyModule(new TLNoC(params, name, inlineNoC))
    case params: SplitACDxBETLNoCParams =>
      LazyModule(new TLSplitACDxBENoC(params, name, inlineNoC))
  }

  val inwardNode: TLInwardNode =
    (system_bus_noc.node :=* TLFIFOFixer(TLFIFOFixer.allVolatile)
  ...
  val builtInDevices: BuiltInDevices =
    BuiltInDevices.attach(sbus_params, outwardNode)
}
...
case class ConstellationMemoryBusParams(
  ...
  def instantiate(context: HasTileLinkLocations, loc: Location[TLBusWrapper])(
      implicit p: Parameters
  ): ConstellationMemoryBus = {
    val constellation = LazyModule(
      new ConstellationMemoryBus(
        mbusParams,
        tlNoCParams,
        loc.name,
        context,
...
class ConstellationMemoryBus(
    mbus_params: MemoryBusParams,
    noc_params: TLNoCParams,
    name: String,
    context: HasTileLinkLocations,
    inlineNoC: Boolean
)(implicit p: Parameters)
    extends TLBusWrapper(mbus_params, name) {
  private val replicator =
    mbus_params.replication.map(r => LazyModule(new RegionReplicator(r)))
  val prefixNode = replicator.map { r =>
    r.prefix := addressPrefixNexusNode
    addressPrefixNexusNode
  }
  ...
  val inwardNode: TLInwardNode =
    replicator
      .map(memory_bus_noc.node :*=* TLFIFOFixer(TLFIFOFixer.all) :*=* _.node)
  ...
  val builtInDevices: BuiltInDevices =
    BuiltInDevices.attach(mbus_params, outwardNode)
}
...
case class ConstellationPeripheryBusParams(
  ...
  def instantiate(context: HasTileLinkLocations, loc: Location[TLBusWrapper])(
      implicit p: Parameters
  ): ConstellationPeripheryBus = {
    val constellation = LazyModule(
      new ConstellationPeripheryBus(
        pbusParams,
        tlNoCParams,
        loc.name,
        context,
...
class ConstellationPeripheryBus(
    pbus_params: PeripheryBusParams,
    noc_params: TLNoCParams,
    name: String,
    context: HasTileLinkLocations,
    inlineNoC: Boolean
)(implicit p: Parameters)
    extends TLBusWrapper(pbus_params, name) {

  private val replicator =
    pbus_params.replication.map(r => LazyModule(new RegionReplicator(r)))
  val prefixNode = replicator.map { r =>
    r.prefix := addressPrefixNexusNode
    addressPrefixNexusNode
  }
  ...
  private val node: TLNode = pbus_params.atomics
    .map { pa =>
      val in_xbar = LazyModule(new TLXbar)
      val out_noc = genNoC()
      val fixer_node =
        replicator.map(fixer.node :*= _.node).getOrElse(fixer.node)
      (out_noc.node
        :*= fixer_node
        :*= TLBuffer(pa.buffer)
        :*= (pa.widenBytes
  ...
  val builtInDevices: BuiltInDevices =
    BuiltInDevices.attach(pbus_params, outwardNode)
}
