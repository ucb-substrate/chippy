...
case class NoCParams(
    ...
    channelParamGen: (Int, Int) => UserChannelParams = (_, _) =>
      UserChannelParams(),
...
trait HasNoCParams {
  ...
  def virtualChannelBits = log2Up(
    (nocParams.channelParams.map(_.nVirtualChannels) :+ 0).max
  )
}
...
object InternalNoCParams {
  def apply(nocParams: NoCParams): InternalNoCParams = {
    ...
    val channelParams = Seq
      .tabulate(nNodes, nNodes) { case (i, j) =>
    ...
    for (i <- 0 until nocParams.ingresses.size) {
      for (e <- 0 until nocParams.egresses.size) {
        require(
          nocParams.flows
            .filter(f => f.ingressId == i && f.egressId == e)
            .size <= 1
        )
      }
    }
    ...
    val flows = nocParams.flows.map { f =>
      ...
      FlowRoutingInfo(
        ingressId = f.ingressId,
        egressId = f.egressId,
        vNetId = f.vNetId,
        ingressNode = ingressNode,
        egressNode = egressNode,
        ingressNodeId =
          nocParams.ingresses.take(f.ingressId).count(_.destId == ingressNode),
        egressNodeId =
          nocParams.egresses.take(f.egressId).count(_.srcId == egressNode),
        fifo = f.fifo
      )
    }
    val ingressParams = nocParams.ingresses.zipWithIndex.map {
      case (u, i) => {
    ...
    val egressParams = nocParams.egresses.zipWithIndex.map {
      case (u, e) => {
    ...
    val possibleFlowMap = scala.collection.mutable
      .Map[ChannelRoutingInfo, Set[FlowRoutingInfo]]()
      .withDefaultValue(Set())

    /** Uses ROUTINGREL to check that, for any ingress, all possible flows from
      * said ingress are able to reach all intended egresses.
      *
      * @param vNetId
      *   virtual network id
      * @param routingRel
      *   the routing relation for the network
      */
    def checkConnectivity(vNetId: Int, routingRel: RoutingRelation) = {
      val nextChannelParamMap = (0 until nNodes).map { i =>
        i -> channelParams.filter(_.srcId == i)
      }.toMap
      val tempPossibleFlowMap = scala.collection.mutable
        .Map[ChannelRoutingInfo, Set[FlowRoutingInfo]]()
        .withDefaultValue(Set())
      ...
      ingressParams.filter(_.vNetId == vNetId).map { iP =>
        ...
        iP.possibleFlows.map { flow =>
          val flowPossibleFlows =
            scala.collection.mutable.Set[ChannelRoutingInfo]()
          ...
          while (unexplored.size != 0) {
            ...
            while (stack.size != 0) {
              val head = stack.head
              val atDest =
                head.dst == flow.egressNode || flowPossibleFlows.contains(head)
              if (!atDest) {
                ...
                require(
                  (nexts.toSet & stack.toSet).size == 0,
                  s"$flow, $nexts, $stack"
                )
                require(
                  !(flow.fifo && nexts.map(_.dst).distinct.size != 1),
                  s"FIFO flow $flow at $head routes to multiple possible next-nodes \n $nexts"
                )
                stack = Seq(nexts.head) ++ stack
                unexplored =
                  nexts.tail.filter(n => !unexplored.contains(n)) ++ unexplored
              } else {
    ...
    def checkAcyclic(
        routingRel: RoutingRelation
    ): Option[Seq[ChannelRoutingInfo]] = {
    ...
    if (skipValidationChecks) {
      println(
        s"Constellation WARNING: $nocName skip checking virtual subnet connectivity"
      )
    } else {
      ...
      for (vNetId <- 0 until nVirtualNetworks) {
        ...
        val blockees = (0 until nVirtualNetworks).filter(v =>
          v != vNetId && nocParams.vNetBlocking(vNetId, v) && flows.exists(
            _.vNetId == v
          )
        )
        if (blockees.size > 0)
          println(
            s"Constellation: $nocName Checking if $vNetId can proceed when blocked by ${blockees}"
          )
        ...
        for (b <- blockeeSets) {
          checkConnectivity(
            vNetId,
            new RoutingRelation(nocParams.topology) {
              def rel(
                ...
                val blocked = b
                  .map { v => possibleFlowMap(nxtC).map(_.vNetId == v) }
                  .flatten
                  .fold(false)(_ || _)
                base && !blocked
              }
    ...
    require(
      acyclicPath.isEmpty,
      s"Cyclic path may cause deadlock: ${acyclicPath.get}"
    )

    // Tie off inpossible virtual channels
    // Also set possible nodes for each channel
    val finalChannelParams = channelParams
      .map { cP =>
        cP.copy(
          virtualChannelParams =
            cP.virtualChannelParams.zipWithIndex.map { case (vP, vId) =>
              val traversable =
                possibleFlowMap(cP.channelRoutingInfos(vId)).size != 0
              if (!traversable) {
                println(
                  s"Constellation WARNING: $nocName virtual channel $vId from ${cP.srcId} to ${cP.destId} appears to be untraversable"
                )
              }
              vP.copy(
      ...
      .map(cP => {
        if (!cP.traversable)
          println(
            s"Constellation WARNING: $nocName physical channel from ${cP.srcId} to ${cP.destId} appears to be untraversable."
          )
        cP
      })
