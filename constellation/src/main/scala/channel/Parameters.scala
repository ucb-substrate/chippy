...
case class UserChannelParams(
    virtualChannelParams: Seq[UserVirtualChannelParams] = Seq(
      UserVirtualChannelParams()
    ),
    channelGen: Parameters => ChannelOutwardNode => ChannelOutwardNode = p =>
      u => u,
...
/** Represents an ingress into the network
  *
  * @param destId
  *   node identifier for the network node this ingress connects to
  * @param vNetId
  *   virtual network id
  * @param payloadBits
  *   width of the wire providing the input for the ingress
  */
...
/** Represents an egress from the network
  *
  * @param srcId
  *   node identifier for the network node this egress connects to
  * @param vNetId
  *   virtual network id
  * @param payloadBits
  *   width of the wire coming out of the egress
  */
...
case class ChannelParams(
  ...
  val channelRoutingInfos = (0 until nVirtualChannels).map(i =>
    ChannelRoutingInfo(
      src = srcId,
      vc = i,
      dst = destId,
  ...
  def apply(
    ...
    ChannelParams(
      ...
      virtualChannelParams =
        user.virtualChannelParams.zipWithIndex.map { case (vP, vc) =>
          VirtualChannelParams(
            srcId,
            destId,
            vc,
...
case class IngressChannelParams(
  ...
  val srcId = -1
  val channelRoutingInfos = Seq(
    ChannelRoutingInfo(src = (-1), vc = 0, dst = destId, n_vc = 1)
  )
}
...
case class EgressChannelParams(
  ...
  val destId = -1
  val channelRoutingInfos = Seq(
    ChannelRoutingInfo(src = srcId, vc = 0, dst = (-1), n_vc = 1)
  )
}
