...
class OutputChannelStatus(implicit val p: Parameters)
    extends Bundle
    with HasNoCParams {
  val occupied = Bool()
...
class OutputChannelAlloc(implicit val p: Parameters)
    extends Bundle
    with HasNoCParams {
  val alloc = Bool()
  val flow = new FlowRoutingBundle
...
class OutputUnit(
    ...
    extends AbstractOutputUnit(inParams, ingressParams, cParam)(p) {

  class OutputUnitIO
      extends AbstractOutputUnitIO(inParams, ingressParams, cParam) {
  ...
  val states = Reg(MixedVec(cParam.virtualChannelParams.map { u =>
    new OutputState(u.bufferSize)
  }))
  ...
  (states zip io.allocs).zipWithIndex.map { case ((s, a), i) =>
    if (cParam.virtualChannelParams(i).traversable) {
      when(a.alloc) {
        s.occupied := true.B
        s.flow := a.flow
