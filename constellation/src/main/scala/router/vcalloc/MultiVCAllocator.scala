...
abstract class MultiVCAllocator(vP: VCAllocatorParams)(implicit p: Parameters)
    extends VCAllocator(vP)(p) {
  // Input arbitration
  io.req.foreach(_.ready := false.B)
  val in_allocs = allInParams.zipWithIndex.map { case (iP, i) =>
    val a = Wire(MixedVec(allOutParams.map { u =>
  ...
  val out_allocs = allOutParams.zipWithIndex.map { case (oP, i) =>
    (0 until oP.nVirtualChannels).map { j =>
      Mux(
        io.channel_status(i)(j).available,
        outputAllocPolicy(
  ...
  for (i <- 0 until allOutParams.size) {
    (0 until allOutParams(i).nVirtualChannels).map { j =>
      io.out_allocs(i)(j).alloc := in_allocs
        .map(_(i)(j))
        .orR && io.channel_status(i)(j).available
      io.out_allocs(i)(j).flow := Mux1H(
        out_allocs(i)(j).asUInt,
        io.req.map(_.bits.flow).toSeq
      )
    }
  }
