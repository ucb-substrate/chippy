  ...
  def prioritizing(
    ...
    if (w > 1) {
      val nPrios = (allOutParams ++ allInParams)
        ...
        .flatten
        .map(c => routingRelation.getNPrios(c))
        .max

      case class PrioHelper(
          prio: Int,
          outId: Int,
          outVId: Int,
          inId: Int,
      ...
      val prio_map = (0 until allOutParams.size)
        .map { i =>
          (0 until allOutParams(i).nVirtualChannels).map { j =>
            (0 until allInParams.size).map { m =>
              (0 until allInParams(m).nVirtualChannels).map { n =>
                val flows = allInParams(m) match {
                  case iP: ChannelParams =>
                    iP.virtualChannelParams(n).possibleFlows
                ...
                flows.map { flow =>
                  ...
                  require(
                    prio < nPrios && prio >= 0,
                    s"Invalid $prio not in [0, $nPrios) ${allInParams(m)
                        .channelRoutingInfos(n)} ${allOutParams(i).channelRoutingInfos(j)}"
                  )
      ...
      val in_prio = (0 until allOutParams.size).map { i =>
        (0 until allOutParams(i).nVirtualChannels).map { j =>
          val lookup =
            prio_map.filter(t => t.outId == i && t.outVId == j).map { e =>
              val ref =
                (((e.inVId << addr_bundle.id.getWidth) | e.inId) << addr_bundle.flow.getWidth) | e.flow
                  .asLiteral(flow)
      ...
      val prio_sels = (0 until nPrios).map { p =>
        val sel = Wire(MixedVec(allOutParams.map { u =>
          Vec(u.nVirtualChannels, Bool())
        }))
        (0 until allOutParams.size).map { i =>
          (0 until allOutParams(i).nVirtualChannels).map { j =>
      ...
      sel.asTypeOf(MixedVec(allOutParams.map { u =>
        Vec(u.nVirtualChannels, Bool())
      }))
    } else {
      in
...
class PrioritizingSingleVCAllocator(vP: VCAllocatorParams)(implicit
    p: Parameters
