...
class RouteComputerReq(implicit val p: Parameters)
    extends Bundle
    with HasNoCParams {
...
class RouteComputer(
  ...
  val io = IO(new Bundle {
    val req = MixedVec(allInParams.map { u =>
      Flipped(Decoupled(new RouteComputerReq))
    })
    val resp = MixedVec(allInParams.map { u =>
      Output(new RouteComputerResp(outParams, egressParams))
    })
  })
  ...
  (io.req zip io.resp).zipWithIndex.map { case ((req, resp), i) =>
    ...
    if (outParams.size == 0) {
      ...
      def toUInt(t: (Int, FlowRoutingInfo)): UInt = {
        ...
        val l4 =
          (l3 << req.bits.flow.ingress_node_id.getWidth) | t._2.ingressNodeId
        val l5 = (l4 << req.bits.flow.egress_node.getWidth) | t._2.egressNode
        val l6 =
          (l5 << req.bits.flow.egress_node_id.getWidth) | t._2.egressNodeId
