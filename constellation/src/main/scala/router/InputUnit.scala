...
class AbstractInputUnitIO(
  ...
  val out_credit_available = Input(MixedVec(allOutParams.map { u =>
    Vec(u.nVirtualChannels, Bool())
  }))

  val salloc_req = Vec(
    cParam.destSpeedup,
    Decoupled(new SwitchAllocReq(outParams, egressParams))
  )

  val out =
    Vec(cParam.destSpeedup, Valid(new SwitchBundle(outParams, egressParams)))
...
class InputBuffer(cParam: ChannelParams)(implicit p: Parameters)
    extends Module {
  val nVirtualChannels = cParam.nVirtualChannels
  val io = IO(new Bundle {
    val enq =
      Flipped(Vec(cParam.srcSpeedup, Valid(new Flit(cParam.payloadBits))))
    val deq =
      Vec(cParam.nVirtualChannels, Decoupled(new BaseFlit(cParam.payloadBits)))
  })

  val useOutputQueues = cParam.useOutputQueues
  val delims = if (useOutputQueues) {
    cParam.virtualChannelParams
      .map(u => if (u.traversable) u.bufferSize else 0)
      .scanLeft(0)(_ + _)
  } else {
    ...
    cParam.virtualChannelParams
      .map(u => if (u.traversable) u.bufferSize + 1 else 0)
      .scanLeft(0)(_ + _)
  }
  ...
  if (
    (cParam.srcSpeedup > 1 || cParam.destSpeedup > 1 || fullSize <= 1) || !cParam.unifiedBuffer
  ) {
    require(useOutputQueues)
    val qs = cParam.virtualChannelParams.map(v =>
      Module(new Queue(new BaseFlit(cParam.payloadBits), v.bufferSize))
    )
    ...
    val qs = Seq.fill(nVirtualChannels) {
      Module(new Queue(new BaseFlit(cParam.payloadBits), 1, pipe = true))
    }
    ...
    val direct_to_q = (Mux1H(vc_sel, qs.map(_.io.enq.ready)) && Mux1H(
      vc_sel,
      empty
    )) && useOutputQueues.B
    ...
    if (useOutputQueues) {
      val can_to_q = (0 until nVirtualChannels).map { i =>
        !empty(i) && qs(i).io.enq.ready
      }
...
class InputUnit(
    ...
    extends AbstractInputUnit(cParam, outParams, egressParams)(p) {

  val nVirtualChannels = cParam.nVirtualChannels
  val virtualChannelParams = cParam.virtualChannelParams

  class InputUnitIO
      extends AbstractInputUnitIO(cParam, outParams, egressParams) {
  ...
  val route_arbiter = Module(
    new Arbiter(
      new RouteComputerReq,
      nVirtualChannels
    )
  )
  ...
  for (i <- 0 until cParam.srcSpeedup) {
    when(io.in.flit(i).fire && io.in.flit(i).bits.head) {
      ...
      if (anyFifo) {
        val fifo = cParam.possibleFlows
          .filter(_.fifo)
          .map(_.isFlow(io.in.flit(i).bits.flow))
          .toSeq
          .orR
        states(id).fifo_deps := VecInit(states.zipWithIndex.map { case (s, j) =>
          s.g =/= g_i && s.flow.asUInt === io.in
            .flit(i)
            .bits
            .flow
            .asUInt && j.U =/= id
  ...
  val vcalloc_reqs = Wire(
    Vec(nVirtualChannels, new VCAllocReq(cParam, outParams, egressParams))
  )
  val vcalloc_vals = Wire(Vec(nVirtualChannels, Bool()))
  val vcalloc_filter = PriorityEncoderOH(
    Cat(vcalloc_vals.asUInt, vcalloc_vals.asUInt & ~mask)
  )
  val vcalloc_sel = vcalloc_filter(
    nVirtualChannels - 1,
    0
  ) | (vcalloc_filter >> nVirtualChannels)
  ...
  when(io.router_req.fire) {
    ...
    mask := Mux1H(
      vcalloc_sel,
      (0 until nVirtualChannels).map { w => ~(0.U((w + 1).W)) }
    )
  }
  ...
  states.zipWithIndex.map { case (s, idx) =>
    if (virtualChannelParams(idx).traversable) {
      vcalloc_vals(idx) := s.g === g_v && (if (anyFifo) s.fifo_deps === 0.U
                                           else true.B)
      vcalloc_reqs(idx).in_vc := idx.U
      vcalloc_reqs(idx).vc_sel := s.vc_sel
      vcalloc_reqs(idx).flow := s.flow
      when(vcalloc_vals(idx) && vcalloc_sel(idx) && io.vcalloc_req.ready) {
        s.g := g_a
      }
      if (combineRCVA) {
  ...
  (states zip salloc_arb.io.in).zipWithIndex.map { case ((s, r), i) =>
    if (virtualChannelParams(i).traversable) {
      val credit_available =
        (s.vc_sel.asUInt & io.out_credit_available.asUInt) =/= 0.U
  ...
  io.in.credit_return := salloc_arb.io.out.zipWithIndex
    .map { case (o, i) =>
      Mux(o.fire, salloc_arb.io.chosen_oh(i), 0.U)
    }
    .reduce(_ | _)
  io.in.vc_free := salloc_arb.io.out.zipWithIndex
    .map { case (o, i) =>
      Mux(
        o.fire && Mux1H(
          salloc_arb.io.chosen_oh(i),
          input_buffer.io.deq.map(_.bits.tail)
        ),
        salloc_arb.io.chosen_oh(i),
        0.U
      )
  ...
  for (i <- 0 until cParam.destSpeedup) {
    ...
    when(salloc_arb.io.out(i).fire) {
      ...
      salloc_out.flit.payload := Mux1H(
        salloc_arb.io.chosen_oh(i),
        input_buffer.io.deq.map(_.bits.payload)
      )
      salloc_out.flit.head := Mux1H(
        salloc_arb.io.chosen_oh(i),
        input_buffer.io.deq.map(_.bits.head)
      )
      salloc_out.flit.tail := Mux1H(
        salloc_arb.io.chosen_oh(i),
        input_buffer.io.deq.map(_.bits.tail)
      )
      salloc_out.flit.flow := Mux1H(
        salloc_arb.io.chosen_oh(i),
        states.map(_.flow)
      )
    }.otherwise {
