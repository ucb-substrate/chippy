...
class SwitchBundle(
  ...
  val out_virt_channel = UInt(
    log2Up(allOutParams.map(_.nVirtualChannels).max).W
  )
}
...
class Switch(
  ...
  val io = IO(new Bundle {
    val in = MixedVec(allInParams.map { u =>
      Vec(
        u.destSpeedup,
        Input(Valid(new SwitchBundle(outParams, egressParams)))
      )
    })
    val out = MixedVec(allOutParams.map { u =>
      Vec(u.srcSpeedup, Output(Valid(new Flit(u.payloadBits))))
    })
    val sel = MixedVec(allOutParams.map { o =>
      Vec(
        o.srcSpeedup,
  ...
  for (i <- 0 until nAllOutputs) {
    for (j <- 0 until allOutParams(i).srcSpeedup) {
      ...
      io.out(i)(j).valid := Mux1H(
        sel_flat,
        in_flat.map(_.valid)
      ) && sel_flat =/= 0.U
      io.out(i)(j).bits := Mux1H(sel_flat, in_flat.map(_.bits.flit))
      io.out(i)(j).bits.virt_channel_id := Mux1H(
        sel_flat,
        in_flat.map(_.bits.out_virt_channel)
      )
    }
  }
}
