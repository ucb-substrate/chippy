...
/** Trait describing the actual [[Module]] implementation wrapped by a
  * [[LazyModule]].
  *
  * This is the actual Chisel module that is lazily-evaluated in the second
  * phase of Diplomacy.
  */
sealed trait LazyModuleImpLike extends RawModule {
  ...
  /** instantiate this [[LazyModule]], return [[AutoBundle]] and a unconnected
    * [[Dangle]]s from this module and submodules.
    */
    ...
    val childDangles = wrapper.children.reverse.flatMap { c =>
      ...
      c.cloneProto
        .map { cp =>
          ...
          def assignCloneProtos(bases: Seq[LazyModule], clones: Seq[LazyModule])
              : Unit = {
            require(bases.size == clones.size)
            (bases.zip(clones)).map { case (l, r) =>
              require(
                l.getClass == r.getClass,
                s"Cloned children class mismatch ${l.name} != ${r.name}"
              )
              l.cloneProto = Some(r)
          ...
          val clone =
            CloneModuleAsRecord(cp.module).suggestName(c.suggestedName)
          ...
          val dangles =
            (rawDangles.zip(clonedAuto.elements)).map { case (d, (_, io)) =>
          ...
          val mod =
            try {
              ...
              case e: ChiselException => {
                println(
                  s"Chisel exception caught when instantiating ${c.name} within ${this.name} at ${c.line}"
                )
                throw e
              }
            }
...
/** Actual description of a [[Module]] which can be instantiated by a call to
  * [[LazyModule.module]].
  *
  * @param wrapper
...
class LazyModuleImp(val wrapper: LazyModule)
    extends Module
    with LazyModuleImpLike {

  /** Instantiate hardware of this `Module`. */
  val (auto, dangles) = instantiate()
}

/** Actual description of a [[RawModule]] which can be instantiated by a call to
  * [[LazyModule.module]].
  *
  * @param wrapper
  *   the [[LazyModule]] from which the `.module` call is being made.
  */
class LazyRawModuleImp(val wrapper: LazyModule)
    extends RawModule
    with LazyModuleImpLike {
