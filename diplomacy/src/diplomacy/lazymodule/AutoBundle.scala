...
/** [[AutoBundle]] will construct the [[Bundle]]s for a [[LazyModule]] in
  * [[LazyModuleImpLike.instantiate]],
  *
  * @param elts
  *   is a sequence of data containing for each IO port a tuple of (name, data,
  *   flipped), where name: IO name data: actual data for connection. flipped:
  *   flip or not in [[makeElements]]
  */
private[diplomacy] final class AutoBundle(elts: (String, Data, Boolean)*)
    extends Record {
  ...
  val elements: ListMap[String, Data] = ListMap() ++ elts.zipWithIndex
    ...
    .flatMap {
      ...
      case seq =>
        seq.zipWithIndex.map { case ((key, element, i), j) =>
          i -> (key + "_" + j -> element)
        }
    }
    .toList
