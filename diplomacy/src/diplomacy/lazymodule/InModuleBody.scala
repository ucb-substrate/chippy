...
/** Used to inject code snippets to be evaluated in
  * [[LazyModuleImp.instantiate]] in the current [[LazyModule.scope]].
  *
  * It can be used to create additional hardware outside of the
  * [[LazyModule.children]], connections other than the internal [[BaseNode]]
  * connections, or additional IOs aside from the [[AutoBundle]]
  */
object InModuleBody {
  def apply[T](body: => T): ModuleValue[T] = {
    require(
      LazyModule.scope.isDefined,
      s"InModuleBody invoked outside a LazyModule"
    )
    ...
    val out = new ModuleValue[T] {
      ...
      def getWrappedValue: T = {
        require(
          result.isDefined,
          s"InModuleBody contents were requested before module was evaluated!"
        )
        result.get
      }
    }
