...
object Select {
  ...
  def collectInwardEdges[T](
      node: BaseNode
  )(collect: PartialFunction[InwardEdge[_ <: Data, _], T]): Iterable[T] = {
    node match {
  ...
  def collectOutwardEdges[T](
      node: BaseNode
  )(collect: PartialFunction[OutwardEdge[_ <: Data, _], T]): Iterable[T] = {
    node match {
  ...
  /** Applies the collect function to a [[LazyModule]] and recursively to all of
    * its children.
    */
  def collectDeep[T](
      lmod: LazyModule
  )(collect: PartialFunction[LazyModule, T]): Iterable[T] = {
    collect.lift(lmod) ++
  ...
  /** Applies the collect function to a [[LazyModule]] and its children if the
    ...
    * returns false. e.g. for this hierarchy
    * {{{
    *     A
    *    / \
    *   B   C
