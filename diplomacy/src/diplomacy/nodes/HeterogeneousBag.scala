...
final case class HeterogeneousBag[T <: Data](elts: Seq[T])
    extends Record
  ...
  val elements = ListMap(elts.zipWithIndex.map { case (n, i) =>
    (i.toString, chiselTypeClone(n))
  }: _*)
...
object HeterogeneousBag {
  def fromNode[D <: Data, E](elts: Seq[(D, E)]) = new HeterogeneousBag(
    elts.map(_._1.cloneType)
  )
}
