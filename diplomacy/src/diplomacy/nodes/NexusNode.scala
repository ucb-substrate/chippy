...
/** [[MixedNexusNode]] is used when the number of nodes connecting from either
  * side is unknown (e.g. a Crossbar which also is a protocol adapter).
  *
  * The [[NodeImp]] is different between [[inner]] and [[outer]],
  *
  * @param dFn
  *   Function for mapping the parameters flowing downward into new outward
  *   flowing down parameters.
  * @param uFn
  *   Function for mapping the parameters flowing upward into new inward flowing
  *   up parameters.
  * @param inputRequiresOutput
  *   True if it is required that if there are input connections, there are
  *   output connections (this node can't just be a sink).
  * @param outputRequiresInput
  *   True if it is required that if there are output connections, there are
  *   input connections (this node can't just be a source).
  */
...
/** [[NexusNode]] is a [[MixedNexusNode]], in which the inward and outward side
  * of the node have the same [[NodeImp]] implementation.
  */
class NexusNode[D, U, EO, EI, B <: Data](
    ...
    extends MixedNexusNode[D, U, EI, B, D, U, EO, B](imp, imp)(
      dFn,
      uFn,
      inputRequiresOutput,
      outputRequiresInput
