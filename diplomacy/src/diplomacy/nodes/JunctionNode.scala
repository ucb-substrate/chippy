...
/** A JunctionNode creates multiple parallel arbiters.
  ...
  *   }}}
  *
  * In the above example, only the first two connections have their multiplicity
  * specified. All the other connections include a '*' on the JBar's side, so
  * the JBar decides the multiplicity. Thus, in this example, we get 2x
  * crossbars with 2 masters like this: {slave1, extras.1} <= jbar.1 <=
  * {masters1.1, masters2.1} {slave2, extras.2} <= jbar.2 <= {masters1.2,
  * masters2,2}
  *
  * @example
  *   {{{
  ...
  *   }}}
  *   In the above example, the first connection takes multiplicity (*) from the
  *   right (masters). Supposing masters.node had 3 edges, this would result in
  *   these three arbiters: {slaves1.1, slaves2.1} <= jbar.1 <= { masters.1 }
  *   {slaves1.2, slaves2.2} <= jbar.2 <= { masters.2 } {slaves1.3, slaves2.3}
  *   <= jbar.3 <= { masters.3 }
  */
class MixedJunctionNode[DI, UI, EI, BI <: Data, DO, UO, EO, BO <: Data](
  ...
  protected[diplomacy] def resolveStar(
      iKnown: Int,
      oKnown: Int,
      iStars: Int,
      oStars: Int
...
/** A node type which has a fixed ratio between the number of input edges and
  * output edges.
  *
  ...
  *
  * Suppose you had N banks of L2 and wanted to connect those to two different
  * driver crossbars. In that case you can do this:
  * {{{
  *   l2banks.node :*= jbar.node
  *   jbar.node :*= xbar1.node
  *   jbar.node :*= xbar2.node
  * }}}
  * If the L2 has 4 banks, now there are 4 egress ports on both xbar1 and xbar2
  * and they are arbitrated by the jbar.
  */
class JunctionNode[D, U, EO, EI, B <: Data](
    imp: NodeImp[D, U, EO, EI, B]
)(dFn: Seq[D] => Seq[D], uFn: Seq[U] => Seq[U])(implicit valName: ValName)
