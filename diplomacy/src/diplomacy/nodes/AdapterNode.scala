...
/** [[MixedAdapterNode]] is used to transform between different diplomacy
  * protocols ([[NodeImp]]), without changing the number of edges passing
  * through it.
  *
  * For example, a [[MixedAdapterNode]] is needed for a TL to AXI bridge
  * (interface).
  * {{{
...
/** A node which modifies the parameters flowing through it, but without
  * changing the number of edges or the diplomatic protocol implementation.
  */
class AdapterNode[D, U, EO, EI, B <: Data](
    imp: NodeImp[D, U, EO, EI, B]
)(dFn: D => D, uFn: U => U)(implicit valName: ValName)
