...
/** A node which represents a node in the graph which only has outward edges and
  * no inward edges.
  *
  * A [[SourceNode]] cannot appear left of a `:=`, `:*=`, `:=*, or `:*=*` There
  * are no Mixed [[SourceNode]]s, There are no "Mixed" [[SourceNode]]s because
  * each one only has an outward side.
  */
class SourceNode[D, U, EO, EI, B <: Data](
    imp: NodeImp[D, U, EO, EI, B]
)(po: Seq[D])(implicit valName: ValName)
