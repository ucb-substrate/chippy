...
/** [[InwardNodeImp]] defines the types that describe the inward side of the
  * [[BaseNode]].
  *
  * @tparam DI
  *   The type of the downward-flowing parameters received on the inner side of
  *   the node.
  * @tparam UI
  *   The type of the upward-flowing parameters generated by the inner side of
  *   the node.
  * @tparam EI
  *   The type of the diplomatically-resolved parameters for an Edge connected
  *   to the inner side of the node.
  * @tparam BI
  *   The type of the [[chisel3.Data]] (usually a [[chisel3.Bundle]]) used when
  *   connecting to the inner side of the node, corresponding to the real
  *   hardware interface that is emitted along the graph edge, generally
  *   parameterized by the [[EI]] type.
  */
trait InwardNodeImp[DI, UI, EI, BI <: Data] {

  /** Creates the inward edge parameters by combining the downward-flowing and
    * upward-flowing parameters for edges that connect to the inward side of
    * this [[BaseNode]].
    *
    * It is left up to a user defining a particular protocol implementation to
    * decide how the parameters flowing through the graph in both directions on
    * this Edge are combined into a single representation.
    *
    * @param pd
    ...
    * @param p
    *   A view of [[Parameters]] at the point at which the returned edge is
    *   being bound.
    * @param sourceInfo
...
/** [[OutwardNodeImp]] defines the types that describe the outwards side of the
  * [[BaseNode]].
  *
  * @tparam DO
  *   The type of the downward-flowing parameters generated by the outer side of
  *   the node
  * @tparam UO
  *   Tye type of the upward-flowing parameters received by the outer side of
  *   the node
  * @tparam EO
  *   The type of the diplomatically-resolved parameters for an Edge connected
  *   to the outer side of the node.
  * @tparam BO
  *   The type of the [[chisel3.Data]] (usually a [[chisel3.Bundle]]) used when
  *   connecting to the outer side of the node, corresponding to the real
  *   hardware interface that is emitted along the graph edge, generally
  *   parameterized by the [[EO]] type.
  */
trait OutwardNodeImp[DO, UO, EO, BO <: Data] {

  /** Creates the outward edge parameters by combining the downward-flowing and
    * upward-flowing parameters for edges that connect to the outward side of
    * this [[BaseNode]].
    *
    * It is left up to a user defining a particular protocol implementation to
    * decide how the parameters flowing through the graph in both directions on
...
/** The [[NodeImp]] combines an [[InwardNodeImp]] and an [[OutwardNodeImp]].
  *
  * This allows it to define whether it is a protocol-modifying (bridging) sort
  * of node, or whether it is an adapter type node that just modifies the
  * parameters within a protocol.
  *
  * This class has no members and is solely used for holding type information.
  * Applications of diplomacy should extend [[NodeImp]] with a case object that
  * sets concrete type arguments.
  *
  * @tparam D
...
/** A [[NodeImp]] where the inward and outward edge parameters are of the same
  * type.
  *
  * If, in a given protocol implementation, the parameters visible to the node
  * on the inward side of an edge are the same as the parameters visible to the
  * node on the outward side of an edge, [[SimpleNodeImp]] can be used instead
  * of [[NodeImp]].
  *
  * @tparam D
  *   Type of the downward-flowing parameters of the node.
...
abstract class SimpleNodeImp[D, U, E, B <: Data]
  ...
  /** Creates the edge parameters by combining the downward-flowing and
    * upward-flowing parameters for edges that connect to this node.
    *
    * It is left up to a user defining a particular protocol implementation to
    * decide how the parameters flowing through the graph in both directions are
    * combined into a single representation on an Edge.
    *
    * @param pd
    *   The downward-flowing parameters into the node along the edge.
    * @param pu
  ...
  def edgeO(pd: D, pu: U, p: Parameters, sourceInfo: SourceInfo): E =
    edge(pd, pu, p, sourceInfo)

  def edgeI(pd: D, pu: U, p: Parameters, sourceInfo: SourceInfo): E =
    edge(pd, pu, p, sourceInfo)
...
/** [[RenderedEdge]] can set the color and label of the visualization of the
  * DAG.
  */
