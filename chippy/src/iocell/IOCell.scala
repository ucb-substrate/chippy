...
/** The base IO bundle for an analog signal (typically something with no digital
  * buffers inside) pad: off-chip (external) connection core: internal
  * connection
  */
...
/** The base IO bundle for a signal with runtime-controllable direction pad:
  * off-chip (external) connection i: input to chip logic (output from IO cell)
  * ie: enable signal for i o: output from chip logic (input to IO cell) oe:
  * enable signal for o
  */
...
/** The base IO bundle for a digital output signal pad: off-chip (external)
  * connection o: output from chip logic (input to IO cell) oe: enable signal
  * for o
  */
...
/** The base IO bundle for a digital input signal pad: off-chip (external)
  * connection i: input to chip logic (output from IO cell) ie: enable signal
  * for i
  */
...
trait IOCell extends BaseModule {
  ...
  /** Set IOCell name
    * @param s
    *   Proposed name for the IOCell
    *
    * @return
    *   An inherited IOCell with given the proposed name
    */
...
object IOCell {
  ...
  /** From within a RawModule or MultiIOModule context, generate new module IOs
    * from a given signal and return the new IO and a Seq containing all
    * generated IO cells.
    * @param coreSignal
    *   The signal onto which to add IO cells
    * @param name
    *   An optional name or name prefix to use for naming IO cells
    * @param abstractResetAsAsync
    *   When set, will coerce abstract resets to AsyncReset, and otherwise to
    *   Bool (sync reset)
    * @return
    *   A tuple of (the generated IO data node, a Seq of all generated IO cell
    *   instances)
    */
  def generateIOFromSignal[T <: Data](
    ...
    val padSignal =
      IO(DataMirror.internal.chiselTypeClone[T](coreSignal)).suggestName(name)
    val resetFn = if (abstractResetAsAsync) toAsyncReset else toSyncReset
    val iocells = IOCell.generateFromSignal(
      coreSignal,
      padSignal,
      Some(s"iocell_$name"),
      typeParams,
  ...
  /** Connect two identical signals together by adding IO cells between them and
    * return a Seq containing all generated IO cells.
    * @param coreSignal
    *   The core-side (internal) signal onto which to connect/add IO cells
    * @param padSignal
    *   The pad-side (external) signal onto which to connect IO cells
    * @param name
    *   An optional name or name prefix to use for naming IO cells
    * @return
    *   A Seq of all generated IO cell instances
    */
  ...
  def generateFromSignal[T <: Data, R <: Reset](
    ...
    def genCell[T <: Data](
        castToBool: (T) => Bool,
        castFromBool: (Bool) => T
    )(coreSignal: T, padSignal: T): Seq[IOCell] = {
      DataMirror.directionOf(coreSignal) match {
        ...
        case _ =>
          throw new Exception(
            s"Signal does not have a direction and cannot be matched to an IOCell"
          )
      }
    }
    ...
    (coreSignal, padSignal) match {
      ...
      case (coreSignal: Clock, padSignal: Clock) =>
        genCellForClock(coreSignal, padSignal)
      case (coreSignal: AsyncReset, padSignal: AsyncReset) =>
        genCellForAsyncReset(coreSignal, padSignal)
      case (coreSignal: Bits, padSignal: Bits) => {
        require(
          padSignal.getWidth == coreSignal.getWidth,
          "padSignal and coreSignal must be the same width"
        )
        if (padSignal.getWidth == 0) {
          ...
          DataMirror.directionOf(coreSignal) match {
            case ActualDirection.Input => {
              val iocells = padSignal.asBools.zipWithIndex.map {
                case (sig, i) =>
                  val iocell = typeParams.input()
            ...
            case ActualDirection.Output => {
              val iocells = coreSignal.asBools.zipWithIndex.map {
                case (sig, i) =>
                  val iocell = typeParams.output()
                  // Note that we are relying on chisel deterministically naming this in the index order (which it does)
                  // This has the side-effect of naming index 0 with no _0 suffix, which is how chisel names other signals
            ...
            case _ =>
              throw new Exception(
                "Bits signal does not have a direction and cannot be matched to IOCell(s)"
              )
          }
        }
      }
      case (coreSignal: Reset, padSignal: Reset) =>
        genCellForAbstractReset(coreSignal, padSignal)
      case (coreSignal: Vec[_], padSignal: Vec[_]) => {
        require(
          padSignal.size == coreSignal.size,
          "size of Vec for padSignal and coreSignal must be the same"
        )
        coreSignal.zip(padSignal).zipWithIndex.foldLeft(Seq.empty[IOCell]) {
          case (total, ((core, pad), i)) =>
            val ios = IOCell
              .generateFromSignal(core, pad, name.map(_ + "_" + i), typeParams)
            total ++ ios
        }
      }
      case (coreSignal: Record, padSignal: Record) => {
        coreSignal.elements.foldLeft(Seq.empty[IOCell]) {
          case (total, (eltName, core)) =>
            val pad = padSignal.elements(eltName)
            val ios = IOCell.generateFromSignal(
              core,
              pad,
              name.map(_ + "_" + eltName),
              typeParams
            )
            total ++ ios
        }
