...
/** Base top with periphery devices and ports, and a BOOM + Rocket subsystem
  */
class ChipyardSystem(implicit p: Parameters)
    extends ChipyardSubsystem
...
/** Base top module implementation with periphery devices and ports, and a BOOM
  * + Rocket subsystem
  */
class ChipyardSystemModule(_outer: ChipyardSystem)
    extends ChipyardSubsystemModuleImp(_outer)
...
trait CanHaveMasterTLMemPort { this: BaseSubsystem =>
  ...
  require(
    !(p(ExtTLMem).nonEmpty && p(ExtMem).nonEmpty),
  ...
  private val mbus =
    tlBusWrapperLocationMap.lift(MBUS).getOrElse(locateTLBusWrapper(SBUS))
