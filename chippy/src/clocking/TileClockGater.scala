...
/** This node adds clock gating control registers. If deploying on a platform
  * which does not support clock gating, deasserting the enable flag will
  * generate the registers, preserving the same memory map and behavior, but
  * will not generate any gaters
  */
class TileClockGater(address: BigInt, beatBytes: Int)(implicit
    p: Parameters,
    valName: ValName
) extends LazyModule {
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    val regs = (0 until nSinks).map({ i =>
      ...
      val reg = withReset(sources(i).reset) {
