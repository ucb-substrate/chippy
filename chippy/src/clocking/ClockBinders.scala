...
class WithPLLSelectorDividerClockGenerator(enable: Boolean = true)
    extends OverrideLazyIOBinder({ (system: HasChipyardPRCI) =>
      {
        // Connect the implicit clock
        implicit val p = GetSystemParameters(system)
        val tlbus = system
          .asInstanceOf[BaseSubsystem]
          .locateTLBusWrapper(system.prciParams.slaveWhere)
        val baseAddress = system.prciParams.baseAddress
        val clockDivider = system.prci_ctrl_domain {
          LazyModule(
            new TLClockDivider(
              baseAddress + 0x20000,
              tlbus.beatBytes,
        ...
        val clockSelector = system.prci_ctrl_domain {
          LazyModule(
            new TLClockSelector(
              baseAddress + 0x30000,
              tlbus.beatBytes,
        ...
        val pllCtrl = system.prci_ctrl_domain {
          LazyModule(new FakePLLCtrl(baseAddress + 0x40000, tlbus.beatBytes))
        }

        clockDivider.tlNode := system.prci_ctrl_domain {
          TLFragmenter(tlbus, Some("ClockDivider")) := system.prci_ctrl_bus.get
        }
        clockSelector.tlNode := system.prci_ctrl_domain {
          TLFragmenter(tlbus, Some("ClockSelector")) := system.prci_ctrl_bus.get
        }
        pllCtrl.tlNode := system.prci_ctrl_domain {
          TLFragmenter(tlbus, Some("PLLCtrl")) := system.prci_ctrl_bus.get
        }
        ...
        InModuleBody {
          ...
          val (clock_io, clockIOCell) =
            IOCell.generateIOFromSignal(clock_wire, "clock", p(IOCellKey))
          val (reset_io, resetIOCell) =
            IOCell.generateIOFromSignal(reset_wire, "reset", p(IOCellKey))
          ...
          val fake_pll =
            Module(new ClockSourceAtFreqFromPlusArg("pll_freq_mhz"))
          ...
          (
            Seq(ClockPort(() => clock_io, 100), ResetPort(() => reset_io)),
            clockIOCell ++ resetIOCell
          )
        }
      }
    })
    ...
    extends OverrideLazyIOBinder({ (system: HasChipyardPRCI) =>
      {
        ...
        val clockGroupsSourceNode =
          ClockGroupSourceNode(Seq(ClockGroupSourceParameters()))
        ...
        InModuleBody {
          ...
          val clock_ios =
            (bundle.member.data zip edge.sink.members).map { case (b, m) =>
              require(
                m.take.isDefined,
                s"""Clock ${m.name.get} has no requested frequency
                                     |Clocks: ${edge.sink.members.map(
                    _.name.get
                  )}""".stripMargin
              )
              val freq = m.take.get.freqMHz
              val clock_io =
                IO(Input(Clock())).suggestName(s"clock_${m.name.get}")
              b.clock := clock_io
              b.reset := reset_io
...
class WithSingleClockBroadcastClockGenerator(freqMHz: Int = 100)
    extends OverrideLazyIOBinder({ (system: HasChipyardPRCI) =>
      {
        implicit val p = GetSystemParameters(system)

        val clockGroupsAggregator =
          LazyModule(new ClockGroupAggregator("single_clock"))
        val clockGroupsSourceNode =
          ClockGroupSourceNode(Seq(ClockGroupSourceParameters()))
        ...
        InModuleBody {
          ...
          (
            Seq(ClockPort(() => clock_io, freqMHz), ResetPort(() => reset_io)),
            clockIOCell ++ resetIOCell
          )
        }
      }
...
class WithMultiIOCellsClockGenerator
    extends OverrideLazyIOBinder({ (system: HasChipyardPRCI) =>
      {
        implicit val p = GetSystemParameters(system)
        ...
        InModuleBody {
          ...
          val iosAndIOCells =
            (bundle.member.data zip edge.sink.members).map { case (b, m) =>
              ...
              val freq = m.take.get.freqMHz
              val clock_wire = Wire(Input(Clock()))
              val (clock_io, clockIOCell) = IOCell.generateIOFromSignal(
                clock_wire,
                s"clock_${m.name.get}",
                p(IOCellKey)
              )
          ...
          (
            (clock_ios :+ ResetPort(() => reset_io)),
            clockIOCells ++ resetIOCell
          )
        }
      }
    })

class WithClockTapIOCells
    extends OverrideIOBinder({ (system: CanHaveClockTap) =>
      {
        system.clockTapIO
          .map { tap =>
            val (clock_tap_io, clock_tap_cell) =
              IOCell.generateIOFromSignal(tap.getWrappedValue, "clock_tap")
