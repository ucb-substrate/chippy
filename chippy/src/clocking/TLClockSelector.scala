...
class TLClockSelector(address: BigInt, beatBytes: Int, enable: Boolean = true)(
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    val regs = (0 until sinks.size).map { i =>
      ...
      val reg = withReset(asyncReset) {
        Module(new AsyncResetRegVec(w = log2Ceil(clocks.size), init = 0))
      }
      sel := reg.io.q
      println(
        s"${(address + i * 4).toString(16)}: Clock domain $sinkName clock mux"
      )
