...
/** TODO: figure out how much division is acceptable in our simulators and
  * redefine this.
  */
object FrequencyUtils {

  /** Adds up the squared error between the generated clocks (refClock /
    * [integer] divider) and the requested frequencies.
    *
    * @param refMHz
    *   The candidate reference clock
    * @param desiredFreqMHz
    *   A list of the requested output frequencies
    */
  def squaredError(
      refMHz: Double,
      desiredFreqMHz: List[Double],
      sum: Double = 0.0
  ): Double = desiredFreqMHz match {
    case Nil => sum
    case desired :: xs =>
  ...
  /** Picks a candidate reference frequency by doing a brute-force search over
    * multiples of the fastest requested clock. Choose the smallest multiple
    * that has an RMS error (across all output frequencies) that is: 1) zero or
    * failing that, 2) is within the relativeThreshold of the best or is less
    * than the absoluteThreshold
    *
    * @param requestedOutputs
    *   The desired output frequencies in MHz
    * @param maximumAllowableFreqMHz
    *   The maximum allowable reference in MHz
  ...
  def computeReferenceAsMultipleOfFastestClock(
    ...
    require(
      ...
      s"Fastest Freq $fastestFreq > Max Freq $maximumAllowableFreqMHz"
    )

    val candidateFreqs =
      Seq.tabulate(Math.ceil(maximumAllowableFreqMHz / fastestFreq).toInt)(i =>
        (i + 1) * fastestFreq
      )
    val errorTuples = candidateFreqs.map { f =>
      f -> Math.sqrt(
        squaredError(f, requestedFreqs.toList) / requestedFreqs.size
      )
    }
    val minError = errorTuples.map(_._2).min
    val viableFreqs = errorTuples.collect {
      case (f, error)
          if (error <= minError * relativeThreshold) || (minError > 0 && error < absoluteThreshold) =>
        f
    }
...
class SimplePllConfiguration(
  ...
  val referenceFreqMHz = FrequencyUtils
    .computeReferenceAsMultipleOfFastestClock(
      sinks.flatMap(_.take),
      maximumAllowableFreqMHz
    )
    .freqMHz
  val sinkDividerMap = ListMap((sinks.map({ s =>
    (s, Math.round(referenceFreqMHz / s.take.get.freqMHz).toInt)
  })): _*)
  ...
  def referenceSinkParams(): ClockSinkParameters =
    sinkDividerMap.find(_._2 == 1).get._1
}
