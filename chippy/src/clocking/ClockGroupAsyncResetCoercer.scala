...
class WithAsyncClockGroups(groups: String*)
    extends Config((_, _, _) => { case ClockGroupAsyncResetKey =>
      groups
    })
...
class ClockGroupAsyncResetCoercer()(implicit p: Parameters, valName: ValName)
  ...
  class Impl extends LazyRawModuleImp(this) {
    ...
    (node.out zip node.in).map {
      case ((outBundles, outEdges), (inBundles, _)) =>
        outBundles.member.data
          .zip(inBundles.member.data)
          .zip(outEdges.sink.members)
          .foreach { case ((outBundle, inBundle), outMember) =>
            val groupName = outMember.name.getOrElse("unnamed")
            val makeAsync = asyncResetGroups.contains(groupName)
            if (makeAsync) {
              println(
                s"Coerceing clock group $groupName to use asynchronous reset"
              )
            } else {
              println(s"Leaving clock group $groupName with synchronous reset")
            }
            outBundle.clock :<= inBundle.clock
            outBundle.reset :<= (if (makeAsync) inBundle.reset.asAsyncReset
                                 else inBundle.reset)
          }
    }
  }
